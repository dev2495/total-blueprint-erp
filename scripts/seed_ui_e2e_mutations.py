...
def main():
    ...
    dispatch_rolls = [dispatch_roll]
    for roll_number in range(2, 10):
        dispatch_rolls.append(_create_roll(
            label_prefix=f"{PREFIX}-DSP-ROLL",
            material=film_variant,
            location=dspx_fg or plant_a.fg,
            plant=(dspx_fg.plant if dspx_fg else plant_a.plant),
            grade=None,
            weight_kg=Decimal("3.600") + (Decimal(roll_number) / Decimal("10")),
            width_mm=Decimal("620"),
            thickness_micron=Decimal("12"),
            batch_no=f"{PREFIX}-DSP-{roll_number}-{run_suffix}",
            is_fg=True,
            sales_order_item=dispatch_item,
            template=dispatch_item.template,
            current_step_index=5,
            completed_step_index=5,
            stage_index=5,
            meta_json={"ui_e2e_seed": True, "seed_type": "dispatch_roll_bulk", "run_tag": RUN_TAG, "label_prefix": label("Mutations"), "roll_number": roll_number},
        ))
    ...
    metadata = {
        ...
        "dispatch": {
            ...
            "roll_ids": [str(roll.id) for roll in dispatch_rolls],
