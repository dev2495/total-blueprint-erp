    ...
    def create_sales_order(payload):
        with transaction.atomic():
            ...
            order = SalesOrder.objects.create(
                ...
                ship_to_address=str(payload.get("ship_to_address") or payload.get("address_override") or "").strip(),
                customer_po_reference=str(payload.get("customer_po_reference") or "").strip(),
                payment_terms=str(payload.get("payment_terms") or "").strip(),
                delivery_terms=str(payload.get("delivery_terms") or "").strip(),
                currency=str(payload.get("currency") or "INR").upper(),
                quote_commercial_snapshot=deepcopy(payload.get("quote_commercial_snapshot") or {}),
                quote_cost_snapshot=deepcopy(payload.get("quote_cost_snapshot") or {}),
                quote_acceptance_snapshot=deepcopy(payload.get("quote_acceptance_snapshot") or {}),
            ...
            for raw_item_data in items_data:
                ...
                item = SalesOrderItem.objects.create(
                    ...
                    quote_variant_snapshot=deepcopy(item_data.get("quote_variant_snapshot") or {}),
                    quote_cost_snapshot=deepcopy(item_data.get("quote_cost_snapshot") or {}),
