"""
Unit tests for ART-3: artwork ↔ product master binding enforced at confirm.
...
class ArtworkProductMasterBindingTests(SimpleTestCase):
    @patch("apps.sales.services.order_service.Artwork.objects.filter")
    def test_confirm_rejects_artwork_bound_to_other_product_master(self, mock_filter):
        ...
        self.assertIn("artwork", message)
        self.assertIn("product master", message)
    ...
    def test_confirm_allows_artwork_bound_to_same_product_master(
        ...
        self.assertEqual(call_kwargs["existing_mapping"], {})
