...
class ReconcileStepMaterialActualsTests(SimpleTestCase):
    ...
    @patch("apps.production.services.services_execution.InventoryLocation.objects.filter")
    @patch("apps.production.services.services_execution.JobMaterialRequirement.objects.select_related")
    def test_granule_code_reconciliation_returns_exact_codes_after_partial_consumption(
        self,
        mock_select_related,
        mock_location_filter,
        mock_capture_mode,
        ...
        self.assertEqual(req.consumed_qty, Decimal("3.5000"))
        mock_consume_bulk.assert_not_called()
        self.assertEqual(mock_add_bulk.call_count, 2)
        self.assertEqual(mock_add_bulk.call_args_list[0].kwargs["granule_code_id"], "code-a")
        self.assertEqual(mock_add_bulk.call_args_list[0].kwargs["qty"], Decimal("0.1563"))
        self.assertEqual(mock_add_bulk.call_args_list[1].kwargs["granule_code_id"], "code-b")
        self.assertEqual(mock_add_bulk.call_args_list[1].kwargs["qty"], Decimal("0.3437"))
        self.assertEqual(mock_log_create.call_count, 2)
