...
function readableLoginError(value: unknown): string {
  ...
  if (Array.isArray(value))
    return value.map(readableLoginError).filter(Boolean).join(" ");
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!user || typeof user !== "object") {
        throw new Error(
          "Login succeeded but the user session could not be hydrated.",
        );
      }
      await login(user);
    } catch (err: unknown) {
      ...
      const detail =
        readableLoginError(apiError.response?.data) ||
        readableLoginError(apiError.message);
      setError(
        status && status >= 500
          ? `Server error (${status}). ${detail || ""}`.trim()
        ...
        <section className="auth-panel-wrap">
          <div className="auth-panel">
            ...
            >
              ...
              <label>
                <span className="auth-label-line">
                  ...
                  <a
                    className="auth-link"
                    href="mailto:admin@totalpolyprint.local"
                  >
                    Forgot?
                  </a>
                </span>
            ...
            <div className="auth-policy">
              ...
              Planner, audit, WCM, machine, and dispatch surfaces keep their
              role gates after session bootstrap.
            </div>
          </div>
        </section>
      </div>
