...
export default function CompanyProfilePage() {
    ...
    const setField = <K extends keyof CompanyProfile>(field: K, value: CompanyProfile[K]) => {
    ...
    const mutation = useMutation({
        ...
        onSuccess: (saved) => {
            ...
            toast({
                ...
                description: "Quotation and invoice templates will use the updated details.",
            })
        },
        onError: (err: any) => {
            toast({
                            ...
                            <Tabs defaultValue="identity">
                                <div className="border-b border-slate-200 px-4 pt-3">
                                    <TabsList className="bg-slate-100">
                                        ...
                                        <TabsTrigger value="address">
                                            <MapPin className="mr-1.5 h-3.5 w-3.5" /> Address &amp; Contact
                                        </TabsTrigger>
                                        <TabsTrigger value="tax">
                                            <FileText className="mr-1.5 h-3.5 w-3.5" /> Tax &amp; Statutory
                                        </TabsTrigger>
                                        <TabsTrigger value="bank">
                                            <Landmark className="mr-1.5 h-3.5 w-3.5" /> Bank
                                        </TabsTrigger>
                                        <TabsTrigger value="terms">
                                            <PenSquare className="mr-1.5 h-3.5 w-3.5" /> Terms &amp; Signatory
                                        </TabsTrigger>
                                    </TabsList>
                                </div>
                                ...
                                <div className="p-5">
                                    <TabsContent value="identity" className="m-0 space-y-4">
                                        <Field label="Legal Name" helper="Appears bold on the quotation header and PDF metadata.">
                                            <Input
                                        ...
                                        <Field label="Trading Name" helper="Optional brand or DBA name shown when distinct from legal name.">
                                            <Input
                                        ...
                                        <Field label="Tagline" helper="Italicised line under the legal name on the quotation letterhead.">
                                            <Input
                                        ...
                                        <Field label="Logo Path" helper="Relative to frontend_v2/public/. Default: brand/tpp-logo-pdf.svg">
                                            <Input
                                    ...
                                    <TabsContent value="tax" className="m-0 space-y-4">
                                        <div className="grid gap-4 sm:grid-cols-2">
                                            <Field label="GSTIN" helper="Appears in the letterhead and footer of every quote.">
                                                <Input
                                                    value={profile.gstin ?? ""}
                                    ...
                                    <TabsContent value="bank" className="m-0 space-y-4">
                                        <div className="grid gap-4 sm:grid-cols-2">
                                            ...
                                            <Field label="UPI ID" helper="Optional. Shown on quote PDF if populated.">
                                                <Input
                                    ...
                                    <TabsContent value="terms" className="m-0 space-y-4">
                                        <Field label="Default Payment Terms" helper="Shown in the meta strip on every quotation.">
                                            <Input
                                        ...
                                        <div className="grid gap-4 sm:grid-cols-2">
                                            <Field label="Default Jurisdiction" helper="Used in the standard T&C block (e.g., 'Daman, India').">
                                                <Input
                                                    value={profile.default_jurisdiction ?? ""}
                                                    onChange={(e) => setField("default_jurisdiction", e.target.value)}
                                                    disabled={!canEdit}
                                                />
                                            </Field>
                                            <Field label="Quote Validity (days)" helper="Default expiry for new quotations.">
                                                <Input
                                                    type="number"
                                                    min={1}
                                                    value={profile.quote_validity_days ?? 12}
                                                    onChange={(e) => setField("quote_validity_days", Number(e.target.value) || 1)}
                                                    disabled={!canEdit}
                                                />
                                            </Field>
                                        </div>
                                        <Field label="Standard Terms &amp; Conditions" helper="Optional custom block. Leave blank to use the built-in defaults.">
                                            <Textarea
                                                rows={6}
                                                value={profile.quote_terms_text ?? ""}
                                                onChange={(e) => setField("quote_terms_text", e.target.value)}
                                                disabled={!canEdit}
                                            />
                                        </Field>
                                        <div className="grid gap-4 sm:grid-cols-2">
                                            <Field label="Authorised Signatory Name" helper="Printed under the signature line on outgoing quotes.">
                                                <Input
                        ...
                        <Card className="border-slate-200">
                            ...
                            <CardContent className="p-5">
                                ...
                                <div className="mt-3 flex flex-wrap gap-1.5">
                                    {profile.gstin ? <Badge variant="outline">GSTIN {profile.gstin}</Badge> : null}
                                    {profile.pan ? <Badge variant="outline">PAN {profile.pan}</Badge> : null}
                                ...
                                {profile.authorised_signatory_name ? (
                                    <div className="mt-4 border-t border-dashed border-slate-200 pt-3 text-xs text-content-3">
                                        ...
                                        <div>
                                            {profile.authorised_signatory_role || "Authorised Signatory"}
                                        </div>
                                    </div>
                                ) : null}
                            </CardContent>
                ...
                <div className="fixed inset-x-0 bottom-0 z-30 border-t border-slate-200 bg-white/95 px-6 py-3 backdrop-blur">
                    <div className="mx-auto flex max-w-7xl items-center justify-between gap-3">
                        <div className="text-xs text-slate-500">
                            Changes apply to all quotations and invoices generated from now on.
                        </div>
                        <Button
