...
import { systemUserService, type PermissionCatalogEntry, type Role } from "@/services/system-users"
...
function normalizePermissionList(values: string[]): string[] {
    return Array.from(new Set((values || []).map((value) => normalizePermission(value)).filter(Boolean))).sort()
}

function buildDraftFromMatrix(matrix?: RoleMatrix): Record<string, string[]> {
    const draft: Record<string, string[]> = {}
    Object.entries(matrix || {}).forEach(([roleCode, row]) => {
        draft[roleCode] = normalizePermissionList([...(row?.default_permissions || []), ...(row?.database_permissions || [])])
    })
    return draft
}
...
function permissionTitle(permission: string, entry?: PermissionCatalogEntry): string {
...
export default function RoleMatrixPage() {
    ...
    const [confirmRemoveVisibleOpen, setConfirmRemoveVisibleOpen] = useState(false)
    ...
    const roleCodes = useMemo(() => Object.keys(draftMatrix).sort(), [draftMatrix])
    const selectedRow = selectedRole ? matrixQuery.data?.[selectedRole] : undefined
    ...
    const catalogRows = useMemo(() => catalogQuery.data ?? [], [catalogQuery.data])
    ...
    const allPermissions = useMemo(() => {
        ...
        Object.values(draftMatrix).forEach((values) => values.forEach((permission) => set.add(normalizePermission(permission))))
    ...
    const permissionRows = useMemo(() => {
        return allPermissions.map((permission): PermissionViewRow => {
            ...
            const moduleKey = String(entry?.module || permission.split(".")[0] || "misc").toUpperCase()
            const actionKey = String(entry?.action || permission.split(".")[1] || "").toLowerCase()
            return {
                permission,
                moduleKey,
                actionKey,
    ...
    const moduleOptions = useMemo(() => {
        const values = Array.from(new Set(permissionRows.map((row) => row.moduleKey))).sort()
        return ["ALL", ...values]
    }, [permissionRows])
    ...
    const filteredPermissionRows = useMemo(() => {
        ...
        return permissionRows.filter((row) => {
            ...
            if (moduleFilter !== "ALL" && row.moduleKey !== moduleFilter) return false
            if (showSelectedOnly && !selectedPermissions.has(row.permission)) return false
            if (!query) return true
            return `${row.title} ${row.permission} ${row.moduleKey} ${row.actionKey}`.toLowerCase().includes(query)
        })
    }, [easyMode, moduleFilter, permissionSearch, permissionRows, selectedPermissions, showSelectedOnly])
    ...
    const groupedPermissionRows = useMemo(() => {
        ...
        Object.values(groups).forEach((rows) => rows.sort((left, right) => left.title.localeCompare(right.title)))
        return groups
    }, [filteredPermissionRows])

    const visiblePermissions = useMemo(
        () => filteredPermissionRows.filter((row) => row.assignable).map((row) => row.permission),
        [filteredPermissionRows],
    )

    const visibleNotSelected = useMemo(
        () => visiblePermissions.filter((permission) => !selectedPermissions.has(permission)),
        [visiblePermissions, selectedPermissions],
    )

    const visibleExtraSelected = useMemo(
        () => visiblePermissions.filter((permission) => selectedPermissions.has(permission) && !baselineSet.has(permission)),
    ...
    const readOnlyPresetPermissions = useMemo(
        () =>
            permissionRows
                .filter((row) => row.assignable && (row.actionKey === "view" || row.permission === "users.self_manage"))
                .map((row) => row.permission),
        [permissionRows],
    )

    const floorOpsPresetPermissions = useMemo(
        () => FLOOR_OPS_BUNDLE.filter((permission) => catalogPermissionSet.has(permission)),
        [catalogPermissionSet],
    )

    const baselineCount = baselinePermissions.length
    const selectedCount = selectedPermissions.size
    const extraCount = Array.from(selectedPermissions).filter((permission) => !baselineSet.has(permission)).length
    ...
    const hasUnsavedChanges = useMemo(() => {
        ...
        const allRoles = new Set([...Object.keys(source), ...Object.keys(draftMatrix)])
    ...
    const applyAdditivePreset = (permissions: string[], name: string) => {
        ...
        const validPermissions = permissions.filter((permission) => permissionRows.some((row) => row.permission === permission && row.assignable))
        const additions = validPermissions.filter((permission) => !current.has(permission))
        if (!additions.length) {
            toast({ title: "No change", description: `${name} did not add new permissions.` })
            return
        }
    ...
    const addVisiblePermissions = () => {
        ...
        if (!visibleNotSelected.length) {
            toast({ title: "No change", description: "All visible permissions are already selected." })
            return
        }
        setRolePermissions([...(draftMatrix[selectedRole] || []), ...visibleNotSelected])
        toast({
    ...
    const removeVisibleExtras = () => {
        ...
        if (!visibleExtraSelected.length) {
            toast({ title: "No change", description: "No visible extra permissions to remove." })
            return
        }
        const current = new Set(draftMatrix[selectedRole] || [])
    ...
    const loading = matrixQuery.isLoading || catalogQuery.isLoading || rolesQuery.isLoading
    ...
    return (
        <div className="space-y-6">
            <PageHeader
                ...
                actions={
                    <div className="flex items-center gap-2">
                        ...
                        <Button variant="outline" onClick={() => syncDefaultsMutation.mutate()} disabled={syncDefaultsMutation.isPending}>
                            <RefreshCw className="mr-2 h-4 w-4" />
                            {syncDefaultsMutation.isPending ? "Syncing..." : "Sync Defaults"}
                        </Button>
                        <Button onClick={() => saveMutation.mutate()} disabled={saveMutation.isPending || !selectedRole || !hasUnsavedChanges}>
            ...
            {loading ? (
                <Card>
                    <CardContent className="py-10 text-center text-slate-500">Loading role matrix...</CardContent>
                </Card>
            ) : (
                <div className="grid gap-6 lg:grid-cols-[340px_1fr]">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Pick a role, apply safe presets, and fine-tune access.</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            ...
                            {isSystemRole ? (
                                <div className="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
                                    <div className="flex items-center justify-between gap-3">
                                        ...
                                        <Switch checked={unlockSystemRoles} onCheckedChange={setUnlockSystemRoles} />
                                    </div>
                                    <p className="mt-2">OWNER/ADMIN/SUPER_ADMIN require explicit unlock before any edit.</p>
                                </div>
                            ) : null}
                            ...
                            <div className="space-y-2 rounded-md border bg-slate-50 p-3">
                                ...
                                <Button
                                    ...
                                    disabled={!canEdit}
                                    onClick={() => applyAdditivePreset(readOnlyPresetPermissions, "Read-only preset")}
                                >
                                ...
                                <Button
                                    ...
                                    className="w-full justify-start"
                                    disabled={!canEdit}
                                    onClick={() => applyAdditivePreset(floorOpsPresetPermissions, "Floor ops preset")}
                                >
                            ...
                            {!selectedRole ? (
                                ...
                                <>
                                            ...
                                            <SelectContent>
                                                {moduleOptions.map((moduleKey) => (
                                                    <SelectItem key={moduleKey} value={moduleKey}>
                                                        {moduleKey === "ALL" ? "All modules" : humanize(moduleKey)}
                                                    </SelectItem>
                                                ))}
                                            </SelectContent>
                                        </Select>
                                    ...
                                    <div className="flex flex-wrap items-center gap-2 text-xs">
                                        <Badge variant="outline">Visible: {filteredPermissionRows.length}</Badge>
                                        <Badge variant="outline">Can add: {visibleNotSelected.length}</Badge>
                                        <Badge variant="outline">Visible extras: {visibleExtraSelected.length}</Badge>
                                    </div>

                                    <div className="flex flex-wrap items-center gap-2">
                                        <Button type="button" variant="outline" disabled={!canEdit || !visibleNotSelected.length} onClick={addVisiblePermissions}>
                                            Add visible
                                        </Button>
                                        <Button
                                            type="button"
                                        ...
                                        {Object.entries(groupedPermissionRows).map(([moduleKey, rows]) => (
                                            <div key={moduleKey} className="mb-6 last:mb-0">
                                                ...
                                                <div className="grid gap-2 md:grid-cols-2">
                                                    {rows.map((row) => {
                                                        const checked = selectedPermissions.has(row.permission)
            ...
            <AlertDialog open={confirmResetOpen} onOpenChange={setConfirmResetOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            This will remove all custom additions for <strong>{selectedRole}</strong> and restore default baseline permissions.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
            ...
            <AlertDialog open={confirmRemoveVisibleOpen} onOpenChange={setConfirmRemoveVisibleOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>Remove visible extras?</AlertDialogTitle>
                        <AlertDialogDescription>
                            This removes {visibleExtraSelected.length} visible extra permission
                            {visibleExtraSelected.length === 1 ? "" : "s"} while keeping baseline intact.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel>Cancel</AlertDialogCancel>
