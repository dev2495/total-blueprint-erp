...
import { NotificationService, type NotificationRule, type RoleVisibilitySignoff } from "@/services/notifications"
...
export default function GovernancePage() {
    ...
    const [activeTab, setActiveTab] = useState<"rules" | "signoffs" | "profiles" | "visibility">("rules")

    const roleCode = (user?.entitlements?.role || user?.role_info?.code || "").toUpperCase()
    ...
    const [signoffForm, setSignoffForm] = useState<SignoffFormState>(EMPTY_SIGNOFF_FORM)
    ...
    const saveRuleMutation = useMutation({
        ...
        onSuccess: async () => {
            await queryClient.invalidateQueries({ queryKey: ["governance-notification-rules"] })
            toast({
                title: "Rule saved",
    ...
    const saveSignoffMutation = useMutation({
        ...
        onSuccess: async () => {
            await Promise.all([
                queryClient.invalidateQueries({ queryKey: ["governance-role-signoffs"] }),
                queryClient.invalidateQueries({ queryKey: ["governance-role-visibility"] }),
                queryClient.invalidateQueries({ queryKey: ["governance-permission-audit"] }),
            ])
            toast({
        ...
        onError: (error: unknown) => {
            const message = error instanceof Error ? error.message : "Failed to upsert signoff row"
            toast({
    ...
    const bootstrapMutation = useMutation({
        ...
        onError: (error: unknown) => {
            const message = error instanceof Error ? error.message : "Failed to bootstrap signoff rows"
            toast({
    ...
    useEffect(() => {
        ...
        const requested = String(new URLSearchParams(window.location.search).get("tab") || "").toLowerCase().trim()
        if (requested === "signoffs") setActiveTab("signoffs")
        else if (requested === "visibility") setActiveTab("visibility")
        else if (requested === "profiles" || requested === "profile-approvals") setActiveTab("profiles")
    ...
    const saveSignoff = () => {
        if (!signoffForm.role_code || !signoffForm.module_key.trim() || !signoffForm.action_key.trim()) {
            toast({
        ...
        <div className="p-6 lg:p-8 space-y-6">
            <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                <div className="space-y-1">
                    ...
                    <p className="text-xs text-slate-500">
                        Manage routing rules, role signoffs, and visibility revalidation without calling APIs manually.
                    </p>
                </div>
            ...
            <div className="grid gap-4 md:grid-cols-4">
                <Card>
                    ...
                    <CardContent>
                        <div className="text-2xl font-black">{rulesQuery.data?.length || 0}</div>
                        <p className="text-xs text-slate-500">Configured event routing rows</p>
                    </CardContent>
                </Card>
                <Card>
                    <CardHeader className="pb-2">
                    ...
                    <CardContent>
                        <div className="text-2xl font-black">{(rulesQuery.data || []).filter((r) => r.active).length}</div>
                        <p className="text-xs text-slate-500">Currently dispatching events</p>
                    </CardContent>
                </Card>
                <Card>
                    <CardHeader className="pb-2">
                    ...
                    <CardContent>
                        ...
                        <p className="text-xs text-slate-500">Approved module/action entries</p>
                    </CardContent>
                </Card>
                <Card>
                    <CardHeader className="pb-2">
                    ...
                    <CardContent>
                        ...
                        <p className="text-xs text-slate-500">Rows awaiting department approval</p>
                    </CardContent>
                </Card>
            </div>
            ...
            <Alert>
                ...
                <AlertDescription>
                    Create/update notification routing rules, bootstrap signoff matrix rows from role permissions, approve or revoke role visibility tuples, review profile update requests, and inspect recent permission audit events.
                </AlertDescription>
            </Alert>
                            ...
                            <CardContent className="space-y-4">
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        ...
                                        onChange={(e) => setRuleForm((prev) => ({ ...prev, event_key: e.target.value }))}
                                    />
                                </div>
                                <div className="space-y-2">
                                                    ...
                                                    onClick={() =>
                                                        setRuleForm((prev) => ({
                                                            ...
                                                            target_roles: selected
                                                                ? prev.target_roles.filter((role) => role !== code)
                                ...
                                <div className="grid gap-3 md:grid-cols-2">
                                    <div className="space-y-2">
                                        ...
                                        <Select
                                            ...
                                            onValueChange={(value) =>
                                                setRuleForm((prev) => ({ ...prev, priority: value as RuleFormState["priority"] }))
                                            }
                                        >
                                            <SelectTrigger>
                                ...
                                <div className="flex items-center justify-between rounded-md border p-3">
                                    <div>
                                        ...
                                        <p className="text-xs text-slate-500">Inactive rules are not used for routing.</p>
                                    </div>
                                    <Switch
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        ...
                                        onChange={(e) =>
                                            setRuleForm((prev) => ({ ...prev, email_subject_template: e.target.value }))
                                        }
                                    />
                                </div>
                                <div className="space-y-2">
                                    ...
                                    <Textarea
                                        ...
                                        onChange={(e) =>
                                            setRuleForm((prev) => ({ ...prev, email_body_template: e.target.value }))
                                        }
                                    />
                                </div>
                                <div className="flex items-center gap-2">
                                    <Button onClick={saveRule} disabled={saveRuleMutation.isPending || isBusy}>
                                        <Save className="h-4 w-4 mr-2" />
                                        {saveRuleMutation.isPending ? "Saving..." : editingRuleEventKey ? "Update rule" : "Create rule"}
                                    </Button>
                                    <Button
                                        variant="outline"
                                        onClick={() => {
                                        ...
                                        {(rulesQuery.data || []).map((rule: NotificationRule) => (
                                            <TableRow key={rule.id}>
                                                <TableCell>
                                                    ...
                                                    <div className="text-xs text-slate-500">{rule.priority}</div>
                                                </TableCell>
                                                    ...
                                                    <Button
                                                        ...
                                                        onClick={() => {
                                                            setRuleForm({
                                                                ...
                                                                active: rule.active,
                                                                escalation_minutes: Number(rule.escalation_minutes || 0),
                            ...
                            <CardContent className="space-y-4">
                                <div className="grid gap-3 md:grid-cols-2">
                                    <div className="space-y-2">
                                        ...
                                        <Select
                                            ...
                                            onValueChange={(value) =>
                                                setSignoffForm((prev) => ({ ...prev, role_code: value.toUpperCase() }))
                                            }
                                        >
                                            <SelectTrigger>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <Input
                                            ...
                                            onChange={(e) =>
                                                setSignoffForm((prev) => ({ ...prev, action_key: e.target.value }))
                                            }
                                        />
                                    </div>
                                </div>
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        ...
                                        onChange={(e) =>
                                            setSignoffForm((prev) => ({ ...prev, module_key: e.target.value }))
                                        }
                                    />
                                </div>
                                <div className="flex items-center justify-between rounded-md border p-3">
                                    <div>
                                        ...
                                        <p className="text-xs text-slate-500">Controls module/action visibility status.</p>
                                    </div>
                                    <Switch
                                        checked={signoffForm.approved}
                                        onCheckedChange={(checked) => setSignoffForm((prev) => ({ ...prev, approved: checked }))}
                                    />
                                </div>
                                <div className="space-y-2">
                                ...
                                <div className="flex items-center gap-2">
                                    <Button onClick={saveSignoff} disabled={saveSignoffMutation.isPending}>
                                        <Save className="h-4 w-4 mr-2" />
                                        {saveSignoffMutation.isPending ? "Saving..." : editingSignoffId ? "Update signoff" : "Create signoff"}
                                    </Button>
                                    <Button
                                        variant="outline"
                                        onClick={() => {
                                    ...
                                    >
                                        {bootstrapMutation.isPending ? "Bootstrapping..." : "Bootstrap pending rows"}
                                    </Button>
                                </div>
                            </CardContent>
                        </Card>
                        ...
                        <Card>
                            ...
                            <CardContent className="p-0">
                                <Table>
                                    ...
                                    <TableBody>
                                        {(signoffsQuery.data || []).map((row: RoleVisibilitySignoff) => (
                                            <TableRow key={row.id}>
                                                <TableCell className="font-medium">{row.role_code}</TableCell>
                                                <TableCell>
                                                    <code className="text-xs">
                                                        {row.module_key}.{row.action_key}
                                                    </code>
                                                    <div className="text-xs text-slate-500">{formatTs(row.approved_at)}</div>
                                                </TableCell>
                                                <TableCell>
                                                    {row.approved ? (
                                        ...
                                        {!signoffsQuery.data?.length && (
                                            <TableRow>
                                                <TableCell colSpan={4} className="text-center text-slate-500 py-8">
                                                    No signoff rows yet. Use the bootstrap action to generate baseline entries.
                                                </TableCell>
                                            </TableRow>
                                        )}
                                    </TableBody>
                    ...
                    <div className="grid gap-4 lg:grid-cols-2">
                        <Card>
                            <CardHeader className="flex flex-row items-center justify-between">
                                <CardTitle className="text-base">Role Visibility Revalidation</CardTitle>
                                <Button variant="outline" size="sm" onClick={() => void visibilityQuery.refetch()}>
                                    <RefreshCw className="h-4 w-4 mr-2" />
                                    Revalidate
                                </Button>
                            </CardHeader>
                        ...
                        <Card>
                            <CardHeader className="flex flex-row items-center justify-between">
                                <CardTitle className="text-base">Recent Permission Audit</CardTitle>
                                ...
                                <Table>
                                    ...
                                    <TableBody>
                                        {(auditQuery.data || []).slice(0, 30).map((row) => (
                                            <TableRow key={row.id}>
                                                <TableCell className="text-xs">{formatTs(row.created_at)}</TableCell>
                                                <TableCell>
                                                    <Badge variant="outline">{row.action}</Badge>
                                                </TableCell>
                                                <TableCell>{row.user || "—"}</TableCell>
                                                <TableCell className="max-w-[260px] truncate text-xs text-slate-500">{row.path || "—"}</TableCell>
                                            </TableRow>
                                        ))}
