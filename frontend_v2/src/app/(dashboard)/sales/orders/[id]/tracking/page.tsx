...
function formatDate(value?: string | null) {
  ...
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return value;
  return formatDisplayDate(date, value);
}
...
function codedLabel(
  ...
  if (label && tag && label.toUpperCase() !== tag.toUpperCase())
    return `${label} · ${tag}`;
...
function statusTone(status: string) {
  ...
  if (normalized === "EXECUTING")
    return "border-danger-border bg-danger-bg text-danger-fg";
  if (normalized === "RELEASED" || normalized === "PLANNED")
    return "border-info-border bg-info-bg text-primary";
  if (normalized === "PACKING_READY")
    return "border-info-border bg-info-bg text-primary";
  if (normalized === "IN_TRANSIT" || normalized === "DISPATCH_READY")
...
export default function OrderTrackingPage() {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#f4f6fb]">
        <div className="text-center">
          <Loader2 className="mx-auto h-8 w-8 animate-spin text-content-3" />
          <div className="mt-3 text-sm font-medium text-content-3">
  ...
  if (error || !data || data.error) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#f4f6fb] p-6">
        <Card className="w-full max-w-xl rounded-3xl border border-danger-border bg-surface-1">
  ...
  const completedJobs = Array.isArray(data.completed_jobs)
    ? data.completed_jobs
    : [];
  ...
  const auditTimeline = Array.isArray(data.audit_timeline)
    ? data.audit_timeline
    : [];
  const materialRows = Array.isArray(data.material_audit?.materials)
    ? data.material_audit?.materials
    : [];
  const interplantLinks = Array.isArray(data.interplant_links)
    ? data.interplant_links
    : [];
  ...
  return (
    <div
      className="min-h-screen bg-[#f4f6fb] p-4 lg:p-6"
      data-testid="sales-order-tracking-page"
    >
      <div className="mx-auto flex max-w-[1680px] flex-col gap-5">
        <section className="rounded-3xl border border-line bg-surface-1 p-5 shadow-[0_14px_38px_-28px_rgba(15,23,42,0.32)]">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div className="space-y-3">
              <Button
                ...
                size="sm"
                onClick={() => router.back()}
                className="h-auto px-0 text-content-3 hover:text-content-1"
              >
              ...
              <div>
                ...
                <div className="mt-2 flex flex-wrap gap-3 text-sm text-content-3">
                  <span>{data.customer}</span>
                  <span>Delivery {formatDate(data.delivery_date)}</span>
                  <span>
                    {safeNumber(header?.totals?.ordered_kg).toFixed(2)} kg
                  </span>
                  <span>
                    {header?.totals?.ordered_pcs != null
                      ? `${safeNumber(header.totals.ordered_pcs)} pcs`
                      : "— pcs"}
                  </span>
            ...
            <div className="min-w-[320px] space-y-3 rounded-2xl border border-line bg-surface-2 p-4">
              <div className="flex items-center justify-between">
                <div className="text-[11px] font-black uppercase tracking-[0.14em] text-content-3">
                  Fulfillment truth
                </div>
                <div className="text-right">
                  <div className="text-xl font-black text-primary">
                    {safeNumber(progress.completion_percentage).toFixed(0)}%
                  </div>
              ...
              <ProgressRow
                label="Produced"
                value={safeNumber(progress.produced)}
                total={safeNumber(progress.ordered)}
                percent={progressBars.producedPct}
                tone="bg-primary"
              />
              <ProgressRow
                label="Packed"
                value={safeNumber(progress.packed)}
                total={safeNumber(progress.ordered)}
                percent={progressBars.packedPct}
                tone="bg-info-fg"
              />
              <ProgressRow
                label="Dispatched"
                value={safeNumber(progress.dispatched)}
                total={safeNumber(progress.ordered)}
                percent={progressBars.dispatchedPct}
                tone="bg-success-fg"
              />
              <div className="text-[11px] text-content-3">
                Refresh{" "}
                {freshness?.generated_at
                  ? formatTs(freshness.generated_at)
                  : "live"}
              </div>
            </div>
          </div>
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
          <KpiCard
            icon={Package}
            title="Ordered"
            value={`${safeNumber(kpis.ordered_kg).toFixed(2)} kg`}
            tone="blue"
          />
          <KpiCard
            icon={Factory}
          ...
          <KpiCard
            icon={Layers3}
            title="WIP Output"
            value={`${safeNumber(kpis.wip_output_kg ?? kpis.output_roll_kg).toFixed(2)} kg`}
            tone="amber"
          />
          <KpiCard
            icon={Warehouse}
            title="FG Ready"
            value={`${safeNumber(kpis.fg_kg).toFixed(2)} kg`}
            tone="emerald"
          />
          <KpiCard
            icon={Truck}
          ...
          <KpiCard
            icon={CheckCircle2}
            title="Jobs Closed"
            value={`${safeNumber(kpis.completed_jobs)}`}
            tone="slate"
          />
        </section>

        <Tabs defaultValue="jobs" className="space-y-4">
          <TabsList className="w-full justify-start overflow-x-auto rounded-2xl border border-line bg-surface-1 p-1">
                ...
                {activeJobs.length ? (
                  <div className="space-y-3">
                    {activeJobs.map((job: any) => (
                      <JobCard
                        key={job.job_id || job.id || job.job_number}
                        job={job}
                        mode="active"
                      />
                    ))}
          ...
          <TabsContent value="inventory" className="space-y-4">
            <SectionCard
              title={`Line truth (${lineItems.length})`}
              icon={Package}
            >
              <DataGrid
            ...
            <SectionCard
              title={`Inventory lineage (${wipLineage.length})`}
              icon={Layers3}
            >
              <DataGrid
          ...
          <TabsContent value="dispatch" className="space-y-4">
            ...
            <SectionCard
              title={`Inter-plant movement (${interplantLinks.length})`}
              icon={Truck}
            >
              <DataGrid
          ...
          <TabsContent value="audit" className="space-y-4">
            <SectionCard
              title={`Audit timeline (${auditTimeline.length})`}
              icon={ShieldCheck}
            >
                ...
                {auditTimeline.length ? (
                  auditTimeline
                    ...
                    .map((event: any, index: number) => (
                      <div
                        key={`${event.reference || event.entity_id || index}`}
                      ...
                      >
                        ...
                        <div className="mt-1 flex flex-wrap gap-3 text-[11px] text-content-3">
                          ...
                          {event.delta_qty_kg != null ? (
                            <span>
                              {safeNumber(event.delta_qty_kg).toFixed(3)} kg
                            </span>
                          ) : null}
                        </div>
                      </div>
            ...
            <SectionCard
              title={`Material audit (${materialRows.length})`}
              icon={Layers3}
            >
              <DataGrid
          ...
          <TabsContent value="sku" className="space-y-4">
            <SectionCard
              title={`SKU item status (${lineItems.length})`}
              icon={Package}
            >
                  ...
                  lineItems.map((line: any) => (
                    ...
                    >
                      ...
                      <div className="mt-2 flex flex-wrap gap-2">
                        ...
                        {line.roll_form ? (
                          <span className="rounded-full border border-success-border bg-success-bg px-2 py-0.5 text-[10px] font-bold text-success-fg">
                            {line.roll_form}
                          </span>
                        ) : null}
                        {line.printing_enabled ? (
                          <span className="rounded-full border border-info-border bg-info-bg px-2 py-0.5 text-[10px] font-bold text-info-fg">
                            {line.printing_type || "PRINT"}
                          </span>
                        ) : null}
                      </div>
...
function ProgressRow({
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-center justify-between text-[11px] font-semibold text-content-3">
        <span>{label}</span>
        <span>
          {value.toFixed(2)} / {total.toFixed(2)} kg
        </span>
      </div>
...
function KpiCard({
  ...
  return (
    <Card className="rounded-2xl border border-line bg-surface-1">
      <CardContent className="p-4">
        <div className={cn("mb-3 inline-flex rounded-xl p-2.5", toneClass)}>
          <Icon className="h-4 w-4" />
        </div>
        <div className="text-[11px] font-black uppercase tracking-[0.15em] text-content-4">
          {title}
        </div>
        <div className="mt-1 text-lg font-black text-content-1">{value}</div>
      </CardContent>
    </Card>
  );
}
...
function SectionCard({
  ...
  return (
    <Card className="rounded-3xl border border-line bg-surface-1">
    ...
    <div className="rounded-2xl border border-line bg-surface-2 p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-1 text-[11px] font-semibold text-content-3">
            {[job.step_name || "Process step", job.process_code]
              .filter(Boolean)
              .join(" · ")}
          </div>
        </div>
        <div className="grid grid-cols-2 gap-2 text-right text-[11px] font-semibold text-content-3">
      ...
      <div className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Metric
          label="Machine"
          value={codedLabel(
            job.machine || job.assigned_machine,
            job.machine_code || job.assigned_machine_code,
            "Unassigned",
          )}
        ...
        <Metric
          label="Operator"
          value={identityLabel(job.operator, job.operator_username, "Pending")}
          icon={<UserRound className="h-3.5 w-3.5" />}
        />
        <Metric
          label={mode === "completed" ? "Closed by" : "Assigned by"}
          value={
            mode === "completed"
              ? identityLabel(job.closed_by, job.closed_by_username)
              : identityLabel(job.assigned_by, null)
          }
      ...
      <div className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Metric
          label={mode === "completed" ? "Closed" : "Assigned"}
          value={formatTs(
            mode === "completed" ? job.closed_at : job.assigned_at,
          )}
        />
        <Metric
          label="Variance"
          value={`${safeNumber(job.variance_kg).toFixed(3)} kg`}
        />
      </div>

      {job.force_reason ? (
        <div className="mt-3 rounded-xl border border-warning-border bg-warning-bg px-3 py-2 text-xs text-warning-fg">
          Force-close reason: {job.force_reason}
        </div>
      ) : null}

      <div className="mt-3 rounded-xl border border-line bg-surface-1 p-3">
        <div className="mb-2 flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.14em] text-content-3">
        ...
        {logs.length ? (
          <div className="space-y-2">
            {logs.map((log: any, index: number) => (
              <div
                key={`${job.job_id || job.job_number}-${index}`}
                className="flex flex-wrap items-center justify-between gap-2 text-[11px]"
              >
                <div className="font-semibold text-content-2">
                  {log.type} · {safeNumber(log.qty).toFixed(3)}{" "}
                  {log.uom || "KG"} · {log.actor || "system"}
                </div>
                <div className="text-content-3">{formatTs(log.timestamp)}</div>
              </div>
            ))}
          </div>
        ) : (
...
function Metric({
  ...
  return (
    <div className="rounded-xl border border-line bg-surface-1 px-3 py-2">
      <div className="flex items-center gap-1 text-[10px] font-black uppercase tracking-[0.13em] text-content-4">
        {icon}
        {label}
      </div>
        ...
        <thead className="bg-surface-2">
          <tr>
            {columns.map((column) => (
              <th
                key={column}
                className="px-4 py-3 text-[10px] font-black uppercase tracking-[0.14em] text-content-3"
              >
                {column.replace(/_/g, " ")}
              </th>
            ))}
          ...
          {rows.map((row, index) => (
            <tr key={index}>
              {columns.map((column) => (
                <td
                  key={column}
                  className="px-4 py-3 text-sm font-medium text-content-2"
                >
                  {row[column] || "—"}
                </td>
              ))}
...
function EmptyState({ label }: { label: string }) {
  return (
    <div className="rounded-2xl border border-dashed border-line bg-surface-2 px-4 py-8 text-center text-sm text-content-3">
      {label}
    </div>
  );
}
