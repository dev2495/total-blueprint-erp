        ...
        <div className="p-8 lg:p-12 space-y-10 bg-[#f8fafc] min-h-screen">
            ...
            <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
                <div className="space-y-4">
                    ...
                    <div className="space-y-1">
                        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-50 border border-indigo-100 text-indigo-600 text-[10px] font-black uppercase tracking-widest shadow-sm">
                            <Zap className="h-3 w-3 fill-indigo-600" /> Protocol ID: {order.order_number}
                        </div>
                ...
                <div className="flex flex-col items-end gap-3">
                    ...
                    {order.status === 'DRAFT' && (
                        <Button
                            className="bg-indigo-600 hover:bg-indigo-700 text-white font-black uppercase text-xs tracking-wider px-8 h-12 rounded-xl shadow-xl shadow-indigo-100"
            ...
            <Tabs defaultValue="overview" className="space-y-8">
                <TabsList className="bg-white p-1.5 rounded-2xl border-2 border-slate-100 h-auto flex gap-1 shadow-sm w-fit">
                    <TabsTrigger value="overview" className="rounded-xl px-6 py-2.5 text-[11px] font-black uppercase tracking-wider data-[state=active]:bg-indigo-600 data-[state=active]:text-white transition-all">
                        Overview
                    </TabsTrigger>
                    <TabsTrigger value="items" className="rounded-xl px-6 py-2.5 text-[11px] font-black uppercase tracking-wider data-[state=active]:bg-indigo-600 data-[state=active]:text-white transition-all">
                        Line Items ({order.items?.length || 0})
                    </TabsTrigger>
                    <TabsTrigger value="specs" className="rounded-xl px-6 py-2.5 text-[11px] font-black uppercase tracking-wider data-[state=active]:bg-indigo-600 data-[state=active]:text-white transition-all">
                        Technical Specs
                    </TabsTrigger>
                    <TabsTrigger value="tracking" className="rounded-xl px-6 py-2.5 text-[11px] font-black uppercase tracking-wider data-[state=active]:bg-indigo-600 data-[state=active]:text-white transition-all">
                        Flow Tracking
                    </TabsTrigger>
                    <TabsTrigger value="documents" className="rounded-xl px-6 py-2.5 text-[11px] font-black uppercase tracking-wider data-[state=active]:bg-indigo-600 data-[state=active]:text-white transition-all">
                        Documents
                    </TabsTrigger>
                </TabsList>
                    ...
                    <Card className="border-none shadow-premium rounded-[2rem] bg-white overflow-hidden">
                        <CardHeader className="p-8 pb-4">
                            <CardTitle className="text-xl font-black flex items-center gap-2 italic">
                                <FileText className="h-5 w-5 text-indigo-600" /> Protocol Summary
                            </CardTitle>
                        </CardHeader>
                            ...
                            <Card key={idx} className="border-none shadow-premium rounded-[2rem] bg-white group hover:bg-slate-50 transition-all duration-300">
                                <CardContent className="p-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
                                    <div className="flex items-center gap-6">
                                        <div className="h-16 w-16 rounded-2xl bg-indigo-50 flex items-center justify-center text-indigo-600 shadow-inner group-hover:bg-white transition-colors">
                                            <Package className="h-8 w-8" />
                                        </div>
                                        <div>
                                            <h4 className="text-xl font-black text-slate-900 group-hover:text-indigo-600 transition-colors uppercase italic">{item.template_name || 'SKU PROTOCOL'}</h4>
                                        ...
                                        {item.bom_snapshot?.components?.map((component: any, cIdx: number) => (
                                            <div key={cIdx} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100">
                                                <div className="flex items-center gap-3">
                                                    <span className="h-6 w-6 rounded-full bg-white flex items-center justify-center text-[10px] font-black text-indigo-600 border border-indigo-100">
                                                        {cIdx + 1}
                                                    </span>
                                                    <span className="text-sm font-bold text-slate-700">{component.material_name}</span>
                                                </div>
                                                <span className="text-xs font-black text-indigo-600 italic">{component.thickness_micron || 30}µ</span>
                                            </div>
                                        )) || (
                ...
                <TabsContent value="tracking" className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <Card className="border-none shadow-premium rounded-[3rem] bg-slate-900 p-12 overflow-hidden relative group">
                        <div className="relative z-10 flex flex-col items-center text-center space-y-6">
                            <div className="h-20 w-20 rounded-3xl bg-indigo-500/20 flex items-center justify-center text-indigo-400 group-hover:scale-110 transition-transform duration-500">
                            ...
                            <div className="space-y-4">
                                ...
                                <p className="text-slate-400 font-medium max-w-lg mx-auto leading-relaxed">
                                    ...
                                    and logistical milestones for Protocol <span className="text-indigo-400 font-black">{order.order_number}</span>.
                                </p>
                            </div>
                            <div className="flex flex-col sm:flex-row gap-4">
                                <Button
                                    className="bg-indigo-600 text-white hover:bg-indigo-700 font-black uppercase text-xs tracking-[0.2em] px-10 h-16 rounded-2xl shadow-2xl transition-all active-scale"
                ...
                <TabsContent value="documents" className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {['Sales Protocol', 'Technical Drawing', 'Quality Certificate'].map((doc, idx) => (
                            <Card key={idx} className="border-none shadow-premium rounded-[2rem] bg-white p-6 flex items-center justify-between group hover:border-2 hover:border-indigo-100 transition-all">
                                <div className="flex items-center gap-4">
