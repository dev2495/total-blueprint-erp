...
export default function InventoryHistoryPage() {
  ...
  const loadSnapshots = async () => {
    try {
      setLoading(true);
      const data = await observabilityApi.getSnapshots(undefined, {
        limit: 180,
        days: 180,
      });
      setSnapshots(data);
  ...
  const plantRows = latestSnapshots.map((snapshot) => ({
    ...
    total:
      Number(snapshot.total_bulk_kg || 0) + Number(snapshot.total_roll_kg || 0),
    wip: snapshot.total_wip_kg,
    reserved: snapshot.reserved_roll_kg,
  }));

  const totalStockKg = latest
    ? Number(latest.total_bulk_kg || 0) + Number(latest.total_roll_kg || 0)
    : 0;
  const previousStockKg = previous
    ? Number(previous.total_bulk_kg || 0) + Number(previous.total_roll_kg || 0)
    : 0;
  const stockDeltaPct =
    previousStockKg > 0
      ? ((totalStockKg - previousStockKg) / previousStockKg) * 100
      : 0;
  const wipSharePct =
    totalStockKg > 0 && latest
      ? (Number(latest.total_wip_kg || 0) / totalStockKg) * 100
      : 0;
  const reserveSharePct =
    totalStockKg > 0 && latest
      ? (Number(latest.reserved_roll_kg || 0) / totalStockKg) * 100
      : 0;
      ...
      <div className="space-y-6">
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <SummaryStatCard
            ...
            subLabel={
              latest
                ? `${formatKg(latest.total_wip_kg)} still in process`
                : "No WIP yet"
            }
            icon={Layers}
          ...
          <SummaryStatCard
            ...
            subLabel={
              latest
                ? `${formatKg(latest.reserved_roll_kg)} reserved rolls`
                : "No reserve load"
            }
            icon={ShieldAlert}
        ...
        <div className="grid gap-6 xl:grid-cols-[1.7fr_1fr]">
          <Card className="overflow-hidden rounded-[2rem] border border-surface-1/70 bg-surface-1/85 shadow-[0_24px_70px_-45px_rgba(15,23,42,0.45)] backdrop-blur-xl">
            <CardHeader className="border-b border-line bg-surface-1/70">
              ...
              <p className="text-sm text-content-3">
                Bulk, roll, FG, and WIP stock across the captured history
                window.
              </p>
            </CardHeader>
          ...
          <Card className="overflow-hidden rounded-[2rem] border border-surface-1/70 bg-surface-1/85 shadow-[0_24px_70px_-45px_rgba(15,23,42,0.45)] backdrop-blur-xl">
            <CardHeader className="border-b border-line bg-surface-1/70">
              ...
              <p className="text-sm text-content-3">
                Latest stock snapshot by plant so planner and store teams can
                see where weight is concentrated.
              </p>
            </CardHeader>
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-xs text-content-3">
                          {formatFullDate(snapshot.created_at)}
                        </div>
                      </div>
                      <SemanticBadge
                        kind="severity"
                        value={
                          Number(snapshot.reserved_roll_kg || 0) > 0
                            ? "MEDIUM"
                            : "LOW"
                        }
                        label={
          ...
          <CardHeader className="border-b border-line bg-surface-1/70">
            <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="text-sm text-content-3">
                  Latest snapshots first, with plant, reserve load, scrap, and
                  FG/WIP balance.
                </p>
              </div>
              <Button
                ...
                .map((snapshot) => (
                  ...
                  >
                    <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                      ...
                      <div className="flex flex-wrap gap-2">
                        <SemanticBadge
                          kind="severity"
                          value={
                            Number(snapshot.total_wip_kg || 0) > 0
                              ? "MEDIUM"
                              : "LOW"
                          }
                          label={`WIP ${formatKg(snapshot.total_wip_kg)}`}
                        />
                        <SemanticBadge
                          kind="severity"
                          value={
                          ...
                          label={`Reserved ${formatKg(snapshot.reserved_roll_kg)}`}
                        />
                      </div>
                    </div>
                    <div className="mt-4 grid gap-3 md:grid-cols-5">
                      <MetricTile
                        label="Bulk"
                        value={formatKg(snapshot.total_bulk_kg)}
                      />
                      <MetricTile
                        label="Rolls"
                        value={formatKg(snapshot.total_roll_kg)}
                      />
                      <MetricTile
                        label="FG"
                        value={formatKg(snapshot.total_fg_kg)}
                      />
                      <MetricTile
                        label="WIP"
                        value={formatKg(snapshot.total_wip_kg)}
                      />
                      <MetricTile
                        label="Scrap"
                        value={formatKg(snapshot.scrap_kg)}
                      />
                    </div>
                  </div>
                ))
