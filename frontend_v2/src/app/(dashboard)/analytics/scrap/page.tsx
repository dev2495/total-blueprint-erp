...
import { AlertTriangle, Factory, RefreshCcw, Scissors, Trash2, Wrench } from "lucide-react";
...
export default function ScrapAnalyticsPage() {
    ...
    return (
        <div className="p-6 space-y-6 min-h-screen bg-slate-50/40">
            ...
            <Card>
                ...
                <CardContent>
                    {isLoading ? (
                        ...
                        <div className="text-sm text-slate-500">No scrap events for selected filters.</div>
                    ) : (
                        <div className="space-y-2">
                            {(data?.recent_events || []).slice(0, 60).map((ev: any, idx: number) => (
                                <div key={`scrap-ev-${idx}`} className="grid grid-cols-1 md:grid-cols-6 gap-2 text-sm border rounded-md p-2">
                                    <div className="md:col-span-2 text-slate-500">{ev.timestamp ? new Date(ev.timestamp).toLocaleString() : "—"}</div>
                                    <div>
                                        <Badge variant="outline">{ev.stream || "—"}</Badge>
                                    </div>
                                    <div>{ev.reason || ev.event_type || "—"}</div>
                                    <div className="font-semibold">{toKg(ev.quantity_kg)} kg</div>
                                    <div className="text-slate-500 truncate">{ev.job_number || ev.material_name || ev.location_name || "—"}</div>
                                </div>
                            ))}
                        </div>
                    )}
                ...
                {loading ? (
                    ...
                    <div className="space-y-2">
                        {rows.slice(0, 12).map((r, idx) => (
                            <div key={`${title}-${idx}`} className="flex items-center justify-between text-sm border rounded-md px-3 py-2">
