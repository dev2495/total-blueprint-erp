...
export default function ProcessRatesPage() {
  ...
  const createMutation = useMutation({
    mutationFn: (data: Partial<ProcessCostRate>) =>
      costingService.createProcessRate(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["process-rates"] });
      ...
      toast({
        ...
        description: "New operational rate has been added to the engine.",
      });
    },
    onError: (err: any) => {
      toast({
        ...
        description:
          err.response?.data?.error || "Error creating rate override.",
      });
    },
  });
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-1">
          ...
          <p className="text-content-3 font-medium text-sm flex items-center gap-2">
            Configure baseline hourly costs for each factory stage{" "}
            <ChevronRight className="h-3 w-3" /> Machine & Process Level
          </p>
        </div>
          ...
          <DialogContent className="max-w-md bg-surface-1 rounded-[2rem] border-none shadow-2xl p-0 overflow-hidden">
            <DialogHeader className="bg-surface-3 p-8 text-white">
              ...
              <p className="text-[10px] text-content-4 font-bold uppercase tracking-widest mt-1">
                Define hourly spending for process/machine
              </p>
            </DialogHeader>
            <div className="p-8 space-y-6">
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Select
                    onValueChange={(val) =>
                      setNewRate({ ...newRate, process: val })
                    }
                  >
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    onValueChange={(val) =>
                      setNewRate({ ...newRate, machine: val })
                    }
                  >
                    <SelectTrigger className="h-11 rounded-xl border-line bg-surface-2 font-bold">
            ...
            <DialogFooter className="bg-surface-2 p-6">
              <Button
                onClick={() => {
                ...
                >
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          queryClient.setQueryData(
                            ...
                            updatedRates.map((r) =>
                              r.id === rate.id
                                ? { ...r, power_cost_per_hour: e.target.value }
                                : r,
                            ),
                          );
                        }}
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          queryClient.setQueryData(
                            ...
                            updatedRates.map((r) =>
                              r.id === rate.id
                                ? { ...r, labor_cost_per_hour: e.target.value }
                                : r,
                            ),
                          );
                        }}
                  ...
                  <TableCell className="text-right px-8">
                    <Button
                      ...
                      className="h-9 w-9 p-0 rounded-xl hover:bg-success-bg hover:text-success-fg border border-transparent hover:border-success-border"
                    >
        ...
        {[
          {
            ...
            desc: "Electricity rates for machinery (₹/hr). Typically drives 30-50% of conversion costs.",
          },
          {
            ...
            desc: "Weighted average salary per operator divided by shift hours. Includes benefits & ESIC.",
          },
          {
            title: "Fixed Overheads",
            icon: Building,
            color: "text-content-4",
            bg: "bg-surface-2",
            border: "border-line",
            desc: "Amortized plant rent, maintenance, and administrative allocation. Updated quarterly.",
          },
        ].map((item, i) => (
          <Card
            key={i}
            className={cn(
              "border-none shadow-md rounded-[2.5rem] p-8",
              item.bg,
              "border",
              item.border,
            )}
