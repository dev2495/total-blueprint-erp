...
import {
    Loader2, RefreshCw, Layers, ShoppingCart, Activity, ShieldCheck,
    ChevronRight, ArrowLeft, Target, TrendingUp, DollarSign
} from "lucide-react"
...
export default function OrderProfitabilityPage() {
    ...
    const { data: cost, isLoading } = useQuery({
        ...
        queryFn: () => costingService.getOrderCosts().then(list => list.find(o => o.sales_order_item === id))
    })
        ...
        return (
            <div className="flex h-[80vh] flex-col items-center justify-center bg-[#f8fafc] text-center space-y-6">
                ...
                <div className="space-y-2">
                    <h2 className="text-xl font-black text-slate-900 uppercase italic">Cost Data Not Found</h2>
                    <p className="text-sm text-slate-500 font-bold uppercase tracking-widest">Pricing engine needs a manual trigger for this specific order item</p>
                </div>
                <Button
        ...
        <div className="p-6 lg:p-10 space-y-10 bg-[#f8fafc] min-h-screen">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                ...
                <Button
                    ...
                    disabled={calculateMutation.isPending}
                    variant="outline"
                    className="h-12 px-6 rounded-xl border-slate-200 font-black uppercase text-xs tracking-widest active:scale-95 transition-all bg-white"
                >
                    {calculateMutation.isPending ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <RefreshCw className="h-4 w-4 mr-2" />}
                    Refresh Deep-Dive
                </Button>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                <Card className="lg:col-span-1 border-none bg-slate-900 text-white rounded-[2.5rem] p-8 flex flex-col justify-between shadow-premium overflow-hidden relative">
                    <div className="absolute top-0 right-0 p-8 opacity-10">
                        <Target className="h-32 w-32" />
                    </div>
                    <div>
                        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mb-2 italic">Net Profit Margin</p>
                        <h2 className="text-6xl font-black tracking-tighter italic">{Number(cost.margin_percent).toFixed(1)}%</h2>
                        <div className="mt-4 inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 text-[10px] font-black uppercase tracking-widest">
                            {Number(cost.margin_percent) > 15 ? "Healthy Profit" : "Low Margin Alert"}
                        </div>
                    </div>
                    <div className="mt-12 pt-8 border-t border-white/10">
                        <div className="flex justify-between items-end">
                            <span className="text-[10px] font-black uppercase text-slate-500 italic">Margin Value</span>
                            <span className="text-2xl font-black tabular-nums">₹{Math.round(Number(cost.margin_value)).toLocaleString()}</span>
                        </div>
                    </div>
                </Card>
                ...
                <div className="lg:col-span-3 grid grid-cols-1 md:grid-cols-3 gap-6">
                    {[
                        ...
                        <Card key={i} className="border-none shadow-soft rounded-[2rem] p-8 bg-white group hover:shadow-xl transition-all">
                            <div className={cn("p-3 rounded-2xl bg-slate-50 w-fit mb-6 transition-transform group-hover:scale-110", kpi.color)}>
                                <kpi.icon className="h-6 w-6" />
                            </div>
                            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-1">{kpi.label}</p>
                            <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tighter tabular-nums">{kpi.value}</h3>
                            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-tight mt-4 italic opacity-0 group-hover:opacity-100 transition-opacity">{kpi.desc}</p>
                        </Card>
                    ))}

                    {/* Progress Bar Visualization */}
                    <Card className="md:col-span-3 border-none shadow-soft rounded-[2.5rem] p-10 bg-white">
                        <div className="flex items-center justify-between mb-8">
                            <div className="flex flex-col">
                                <span className="text-xs font-black uppercase tracking-[0.2em] text-slate-900">Total Lifecycle Cost Impact</span>
                                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">₹{Math.round(Number(cost.total_cost)).toLocaleString()} Total Spending</span>
                            </div>
                            <div className="flex items-center gap-6">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[10px] font-black uppercase text-slate-500">Conversion ({(Number(cost.conversion_cost) / Number(cost.total_cost) * 100).toFixed(0)}%)</span>
                                </div>
                            </div>
                        </div>
                ...
                <Card className="border-none shadow-premium rounded-[2.5rem] overflow-hidden bg-white">
                    ...
                    <CardContent className="p-0 h-[400px] overflow-auto scrollbar-hide">
                        <div className="p-8 flex flex-col items-center justify-center h-full text-center opacity-30 select-none">
                            ...
                            <p className="font-black text-[10px] uppercase tracking-widest">Physics-Integrated BOM Breakdown</p>
                            <p className="text-[9px] font-bold mt-2 leading-relaxed">Detailed material cost tracing is active.<br />Rates snapshot used from run date.</p>
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card className="border-none shadow-premium rounded-[2.5rem] overflow-hidden bg-white">
                    ...
                    <CardContent className="p-0 h-[400px] overflow-auto scrollbar-hide">
                        <div className="p-8 flex flex-col items-center justify-center h-full text-center opacity-30 select-none">
                            ...
                            <p className="font-black text-[10px] uppercase tracking-widest">Routing Rule Execution Impact</p>
                            <p className="text-[9px] font-bold mt-2 leading-relaxed">Stage-wise conversion cost tracing.<br />Power, labor, and overhead allocation logic.</p>
                        </div>
                    </CardContent>
                </Card>
            </div>
