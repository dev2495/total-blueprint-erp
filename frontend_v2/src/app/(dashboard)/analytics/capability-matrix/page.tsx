...
const SECTION_TONE = {
    SUPPORTED_NOW: "border-success-border bg-emerald-50/60",
    CONFIG_ONLY: "border-warning-border bg-amber-50/60",
    NEW_LOGIC_REQUIRED: "border-danger-border bg-rose-50/60",
} as const;
        ...
        <div className="min-h-screen bg-slate-50/50 p-6 space-y-6">
            <Card className="border-slate-200 shadow-sm">
                <CardContent className="flex items-start justify-between gap-4 p-6">
                    ...
                    <Badge variant="outline" className="bg-surface-1 text-content-3 border-slate-200">Version {data.version}</Badge>
                </CardContent>
            </Card>
            ...
            {(Array.isArray(data.sections) ? data.sections : []).map((section, sectionIndex) => {
                ...
                const tone = SECTION_TONE[sectionStatus as keyof typeof SECTION_TONE] || "border-slate-200 bg-surface-1";
                return (
                                ...
                                return (
                                    <Card key={`${sectionStatus}-${family}-${entryIndex}`} className="border-slate-200 shadow-sm">
                                        ...
                                        <CardContent className="space-y-4 text-sm">
                                            <div>
                                                <div className="text-[11px] font-black uppercase tracking-[0.18em] text-content-4">What it means</div>
                                                <p className="mt-1 text-slate-700">{safeToken(entry?.what_it_means, "Capability summary unavailable.")}</p>
                                            </div>
                                            <div>
                                                <div className="text-[11px] font-black uppercase tracking-[0.18em] text-content-4">Why</div>
                                                <p className="mt-1 text-slate-700">{safeToken(entry?.why, "Reasoning not provided.")}</p>
                                            </div>
                                            <div>
                                                <div className="text-[11px] font-black uppercase tracking-[0.18em] text-content-4">Examples</div>
                                            ...
                                            <div className="grid gap-4 md:grid-cols-2">
                                                <div>
                                                    <div className="text-[11px] font-black uppercase tracking-[0.18em] text-content-4">Needs only config</div>
                                                ...
                                                <div>
                                                    <div className="text-[11px] font-black uppercase tracking-[0.18em] text-content-4">Needs code / module</div>
                                                    <ul className="mt-2 space-y-1 text-slate-700">
                                            ...
                                            <div>
                                                <div className="text-[11px] font-black uppercase tracking-[0.18em] text-content-4">Limits</div>
                                                <ul className="mt-2 space-y-1 text-slate-700">
