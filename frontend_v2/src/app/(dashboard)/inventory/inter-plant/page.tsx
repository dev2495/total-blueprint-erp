...
export default function InterPlantPage() {
  ...
  const stats = useMemo(() => {
    ...
    const inTransit = challanList.filter(
  ...
  const drafts = useMemo(
    () => challanList.filter((c) => c.status === "DRAFT"),
    [challanList],
  );
  const inTransit = useMemo(
    () => challanList.filter((c) => c.status === "IN_TRANSIT"),
    [challanList],
  );
  const received = useMemo(
    () => challanList.filter((c) => c.status === "RECEIVED"),
    [challanList],
  );
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">
            Transfer materials between factory locations.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
        <Card>
          <CardHeader className="py-3">
            <CardTitle className="text-xs uppercase text-content-3">
              Total Challans
            </CardTitle>
          </CardHeader>
          <CardContent>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
          <LifecycleColumn
            ...
            icon={
              <Circle className="h-4 w-4 text-warning-fg fill-warning-fg" />
            }
            stage="DRAFT"
            rows={drafts}
            emptyText="No draft challans."
            statusClass="border-warning-border bg-warning-bg"
          ...
          <LifecycleColumn
            ...
            statusClass="border-info-border bg-info-bg"
            getStatusBadgeClass={getStatusBadgeClass}
            onPrint={printChallan}
          />
          <LifecycleColumn
            ...
            statusClass="border-success-border bg-success-bg"
            getStatusBadgeClass={getStatusBadgeClass}
            onPrint={printChallan}
          />
        </div>
  ...
  return (
    <Card className={statusClass}>
      ...
      <CardContent className="space-y-3 max-h-[68vh] overflow-y-auto">
        {rows.length === 0 ? (
          <div className="text-xs text-content-3 border border-dashed rounded-md p-4 text-center">
            {emptyText}
          </div>
        ) : (
          rows.map((challan) => (
            <Card key={challan.id} className="border border-line shadow-none">
              <CardHeader className="py-3">
                <div className="flex items-center justify-between gap-2">
                  <div className="font-mono text-xs font-bold">
                    {(challan as any).dc_no ||
                      `${challan.id.substring(0, 8)}...`}
                  </div>
                  <div className="flex items-center gap-1">
                    {(challan as any).is_system_generated && (
                      <Badge
                        variant="outline"
                        className="text-[10px] border-info-border text-primary"
                      >
                        AUTO
                      </Badge>
                    )}
                    <Badge
                      variant={
                        challan.status === "RECEIVED" ? "default" : "outline"
                      }
              ...
              <CardContent className="pt-0 space-y-3">
                {(() => {
                  ...
                  const outputLines = Number(
                    summary?.output_lines ??
                      Math.max(
                        rollLines - Number(summary?.remainder_lines ?? 0),
                        0,
                      ),
                  );
                  ...
                  const remainderOut = Number(
                    summary?.remainder_dispatched_kg ?? 0,
                  );
                  const remainderIn = Number(
                    summary?.remainder_received_kg ?? 0,
                  );
                  const dispatchedTotal = Number(
                    summary?.dispatched_total_kg ?? 0,
                  );
                  ...
                  return (
                    <>
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <Badge
                          variant="outline"
                          className="text-[10px] border-info-border text-primary bg-info-bg"
                        >
                          OUT {outputLines}
                        </Badge>
                        <Badge
                        ...
                        <Badge
                          variant="secondary"
                          className="text-[10px] bg-surface-2 text-content-2"
                        >
                          Out {dispatchedTotal.toFixed(3)} kg
                        </Badge>
                        <Badge
                          variant="secondary"
                          className="text-[10px] bg-success-bg text-success-fg"
                        >
                      ...
                      {(outputLines > 0 || remainderLines > 0) && (
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[10px]">
                          <div className="rounded-md border border-info-border bg-info-bg px-2 py-1.5 text-primary">
                            Output: {outputOut.toFixed(3)} out /{" "}
                            {outputIn.toFixed(3)} in kg
                          </div>
                          <div className="rounded-md border border-warning-border bg-warning-bg px-2 py-1.5 text-warning-fg">
                            Remainder: {remainderOut.toFixed(3)} out /{" "}
                            {remainderIn.toFixed(3)} in kg
                          </div>
                        </div>
                      )}
                    </>
                ...
                {(challan.vehicle_no ||
                  ...
                  <div className="text-[11px] text-content-3">
                    Vehicle: {challan.vehicle_no || "—"} | Driver:{" "}
                    {challan.driver_name || "—"} | Transporter:{" "}
                    {challan.transporter_name || "—"}
                  </div>
                )}
                {Array.isArray(challan.item_preview) &&
                  challan.item_preview.length > 0 && (
                    <div className="rounded-md border border-line bg-surface-2 p-2 space-y-1">
                      {challan.item_preview.map((line, idx) => (
                        <div
                          key={`${challan.id}-line-${idx}`}
                          className="text-[11px] text-content-2 flex items-center justify-between gap-2"
                        >
                          ...
                          <span className="font-semibold whitespace-nowrap">
                            {Number(line.dispatched_qty_kg || 0).toFixed(3)} kg
                          </span>
                        </div>
                      ))}
                    </div>
                  )}
                <div className="grid grid-cols-3 gap-2 text-[10px] text-content-3">
                  <div>
                    <div className="font-semibold uppercase tracking-wide text-content-4">
                      Created
                    </div>
                    <div>
                      {new Date(
                        challan.created_at || Date.now(),
                      ).toLocaleString()}
                    </div>
                  </div>
                  <div>
                ...
                <div className="flex items-center justify-end gap-2">
                  ...
                  {stage === "DRAFT" && (
                    <DispatchChallanDialog challan={challan} />
                  )}
                  {stage === "IN_TRANSIT" && (
                    <ReceiveChallanDialog challan={challan} />
                  )}
                  {stage === "RECEIVED" && (
                    <Badge className="bg-success-bg text-success-fg border-success-border">
                      Closed
                    </Badge>
                  )}
                </div>
...
function CreateChallanDialog() {
  ...
  const destinationLocationOptions = useMemo(
    () =>
      ((destinationLocations || []) as any[]).filter(
        (l: any) => l.is_active && l.type !== "IN_TRANSIT",
      ),
    [destinationLocations],
  );
  const rawAvailableRolls = ((rolls || []) as any[]).filter(
    (r: any) => r.status === "AVAILABLE",
  );
  const rawAvailableBulk = ((bulkStock || []) as any[]).filter(
    (b: any) => Number(b.qty_kg || 0) > 0,
  );
  const sourceLocationOptions = useMemo(() => {
    ...
    pool.forEach((item: any) => {
      ...
      grouped.set(locationId, {
        id: locationId,
        name: String(
          base?.name || item.location_name || item.location || "Location",
        ),
  ...
  const availableRolls = useMemo(() => {
    return rawAvailableRolls.filter((r: any) => {
      ...
      const locationOk =
        !sourceLocationFilter || locationId === sourceLocationFilter;
      const materialOk =
        materialFilter === "ALL" || materialName === materialFilter;
  ...
  useEffect(() => {
    ...
    if (!toPlant) {
      const candidate = rows.find(
        (p: any) => String(p.id) !== String(rows[0].id),
      );
  ...
  useEffect(() => {
    ...
    if (
      ...
      !destinationLocationOptions.some(
        (loc: any) => String(loc.id) === String(destinationLocationId),
      )
    ) {
  ...
  useEffect(() => {
    ...
    if (
      ...
      !sourceLocationOptions.some(
        (loc: any) => String(loc.id) === String(sourceLocationFilter),
      )
    ) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button data-testid="interplant-new-transfer">
      ...
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>
            One flow for DC lifecycle. Pick plants, select material lines, then
            create draft or dispatch immediately.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              ...
              >
                <SelectTrigger data-testid="interplant-from-plant">
            ...
            <div className="space-y-1">
              ...
              <Select value={toPlant} onValueChange={setToPlant}>
                <SelectTrigger data-testid="interplant-to-plant">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              ...
              <Select
                value={transferMode}
                onValueChange={(v: "ROLL" | "BULK") => setTransferMode(v)}
              >
                <SelectTrigger data-testid="interplant-transfer-mode">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="flex items-end pb-2">
              <label className="flex items-center gap-2 text-sm">
                <Checkbox
                  checked={dispatchNow}
                  onCheckedChange={(v) => setDispatchNow(Boolean(v))}
                />
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="space-y-1">
              ...
              >
                <SelectTrigger data-testid="interplant-material-filter">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-1">
              ...
              <Select
                ...
                onValueChange={setSourceLocationFilter}
              >
                <SelectTrigger data-testid="interplant-source-location">
            ...
            <div className="space-y-1">
              ...
              >
                <SelectTrigger data-testid="interplant-destination-location">
              ...
              <CardContent className="space-y-2 max-h-52 overflow-y-auto">
                {availableRolls.length === 0 && (
                  <p className="text-sm text-muted-foreground">
                    No available rolls at selected source plant.
                  </p>
                )}
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="text-sm">
                          ...
                          <div className="text-xs text-muted-foreground">
                            {r.material_name} • {r.weight_kg} kg •{" "}
                            {r.location_name}
                          </div>
                        </div>
                      </div>
            ...
            <Card>
              <CardHeader className="py-3">
                <CardTitle className="text-sm">
                  Available Bulk (select qty by location)
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-2 max-h-52 overflow-y-auto">
                {availableBulk.length === 0 && (
                  <p className="text-sm text-muted-foreground">
                    No bulk stock at selected source plant.
                  </p>
                )}
                {availableBulk.map((b: any) => {
                  ...
                  return (
                    <div
                      key={key}
                      className="grid grid-cols-12 gap-2 border rounded-md px-2 py-2 items-center"
                    >
                      <div className="col-span-8">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {b.location_name} • Available{" "}
                          {Number(b.qty_kg || 0).toFixed(3)} kg
                        </div>
                      </div>
...
function DispatchChallanDialog({ challan }: { challan: DeliveryChallan }) {
  ...
  const availableBulk = (bulkStock || []).filter(
    (b: any) => Number(b.qty_kg || 0) > 0,
  );
  const selectedBulkItems = useMemo(() => {
    return availableBulk
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button
          variant="outline"
          size="sm"
          data-testid={`interplant-dispatch-trigger-${challan.id}`}
        >
          Dispatch
        </Button>
      </DialogTrigger>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Select rolls and bulk lines to move into source IN_TRANSIT.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          <Card>
          ...
          <p className="text-xs text-muted-foreground">
            Selected: {selectedRolls.length} roll(s), {selectedBulkItems.length}{" "}
            bulk line(s)
          </p>
          ...
          <Button
            ...
            disabled={
              (selectedRolls.length === 0 && selectedBulkItems.length === 0) ||
              mutation.isPending
            }
          >
            Confirm Dispatch
...
function ReceiveChallanDialog({ challan }: { challan: DeliveryChallan }) {
  ...
  const transitLocation = useMemo(
    () =>
      (fromLocations || []).find(
        (l: any) => String(l.code).toUpperCase() === "IN_TRANSIT",
      ),
    [fromLocations],
  );
  ...
  const { data: transitRolls } = useQuery({
    ...
    queryFn: () =>
      inventoryService.getRollStock(challan.from_plant, transitLocation!.id),
  ...
  const { data: transitBulk } = useQuery({
    ...
    queryFn: () =>
      inventoryService.getBulkStock({
        plant: challan.from_plant,
        location: transitLocation!.id,
      }),
    enabled: open && Boolean(transitLocation?.id),
  });
  ...
  useEffect(() => {
    ...
    if (
      !targetLocation ||
      !receiveLocationOptions.some(
        (loc: any) => String(loc.id) === String(targetLocation),
      )
    ) {
  ...
  const mutation = useMutation({
    mutationFn: () =>
      inventoryService.receiveChallan(challan.id, {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button
          size="sm"
          data-testid={`interplant-receive-trigger-${challan.id}`}
        >
          Receive
        </Button>
      </DialogTrigger>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Select destination location and lines to receive.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <Select value={targetLocation} onValueChange={setTargetLocation}>
              <SelectTrigger
                data-testid={`interplant-receive-location-${challan.id}`}
              >
            ...
            <CardContent className="space-y-2 max-h-40 overflow-y-auto">
              {!transitLocation && (
                <div className="text-sm text-muted-foreground">
                  Source transit location not found.
                </div>
              )}
              {transitLocation &&
                ((transitRolls || []) as any[]).length === 0 && (
                  <div className="text-sm text-muted-foreground">
                    No rolls in transit.
                  </div>
                )}
                ...
                return (
                  ...
                  >
                    <div className="col-span-8">
                      ...
                      <div className="text-xs text-muted-foreground">
                        Available {Number(b.qty_kg || 0).toFixed(3)} kg
                      </div>
                    </div>
                    <div className="col-span-4">
                      <Input
          ...
          <Button
            ...
            disabled={
              !targetLocation ||
              mutation.isPending ||
              (selectedRollIds.length === 0 && selectedBulkItems.length === 0)
            }
          >
            Confirm Receipt
          </Button>
