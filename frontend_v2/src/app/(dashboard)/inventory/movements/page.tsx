...
export default function RollMovementsPage() {
  ...
  const reasonRows = useMemo(() => {
    ...
    return Array.from(counts.entries())
      .map(([reason, count]) => ({
        reason,
        count,
        color: getSemanticMeta("dispatchStatus", reason, humanizeToken(reason))
          .chartColor,
      }))
  ...
  const productionMoves = filteredMovements.filter(
    (movement) => movement.reason === "PRODUCTION",
  ).length;
  const dispatchMoves = filteredMovements.filter(
    (movement) => movement.reason === "DISPATCH",
  ).length;
  const movementPlants = new Set(
    filteredMovements
      .map((movement) => movement.to_location_name.split(" ")[0])
      .filter(Boolean),
  );
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm font-medium text-content-3">
                Trace physical location moves across GRN, production, WIP
                transfer, inter-plant, jobwork, and dispatch without digging
                through raw transaction tables first.
              </p>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            <div className="relative min-w-[280px]">
              ...
              <Input
                ...
                className="h-12 rounded-2xl border-line bg-surface-1/90 pl-10 shadow-sm"
              />
            </div>
            <Button
              variant="outline"
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <SummaryStatCard
          label="Visible moves"
          value={filteredMovements.length.toLocaleString()}
          subLabel="Current movement rows in view"
          icon={History}
          toneClassName="bg-info-bg text-primary"
        />
        <SummaryStatCard
          label="Production moves"
          value={productionMoves.toLocaleString()}
          subLabel="Moves posted by production outputs"
          icon={Package}
          toneClassName="bg-info-bg text-primary"
        />
        <SummaryStatCard
          label="Dispatch moves"
          value={dispatchMoves.toLocaleString()}
          subLabel="Movements headed to shipment"
          icon={Truck}
          toneClassName="bg-warning-bg text-warning-fg"
        />
        <SummaryStatCard
          label="Route spread"
          value={movementPlants.size.toLocaleString()}
          subLabel={
            newestMove
      ...
      <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <Card className="overflow-hidden rounded-[2rem] border border-surface-1/70 bg-surface-1/85 shadow-[0_24px_70px_-45px_rgba(15,23,42,0.45)] backdrop-blur-xl">
          <CardHeader className="border-b border-line bg-surface-1/70">
            ...
            <p className="text-sm text-content-3">
              Most common reasons driving roll movement in the current view.
            </p>
          </CardHeader>
            ...
            <ChartSurface className="min-h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <XAxis
                    dataKey="reason"
                    tickFormatter={(value) => humanizeToken(String(value))}
                    tickLine={false}
                    axisLine={false}
                  ...
                  <Tooltip
                    formatter={(value: number | string | undefined) =>
                      `${Number(value || 0)} moves`
                    }
                    labelFormatter={(label) => humanizeToken(String(label))}
                  />
        ...
        <Card className="overflow-hidden rounded-[2rem] border border-surface-1/70 bg-surface-1/85 shadow-[0_24px_70px_-45px_rgba(15,23,42,0.45)] backdrop-blur-xl">
          <CardHeader className="border-b border-line bg-surface-1/70">
            ...
            <p className="text-sm text-content-3">
              Latest physical transitions with roll, location, and operator
              context.
            </p>
          </CardHeader>
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="mt-1 flex items-center gap-2 text-xs font-medium text-content-3">
                  ...
                  <SemanticBadge
                    ...
                    label={humanizeToken(movement.reason)}
                  />
                </div>
                ...
                {movement.moved_by_name ? (
                  <div className="mt-2 flex items-center gap-2 text-xs font-medium text-content-3">
      ...
      <Card className="overflow-hidden rounded-[2rem] border border-surface-1/70 bg-surface-1/85 shadow-[0_24px_70px_-45px_rgba(15,23,42,0.45)] backdrop-blur-xl">
        <CardHeader className="border-b border-line bg-surface-1/70">
          ...
          <p className="text-sm text-content-3">
            Audit-first list after the summary view, so store and dispatch teams
            can still inspect every move exactly.
          </p>
        </CardHeader>
        <CardContent className="p-0">
          {filteredMovements.length === 0 ? (
            <div className="grid min-h-[240px] place-items-center p-10 text-center text-content-3">
            ...
            <div className="divide-y divide-line">
              {filteredMovements.map((movement) => (
                <div
                  key={movement.id}
                  className="flex flex-col gap-4 p-5 transition-colors hover:bg-info-bg xl:flex-row xl:items-center xl:justify-between"
                >
                  <div className="flex items-start gap-4">
                    <div className="rounded-2xl border border-info-border bg-info-bg p-3 text-primary">
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    />
                    {movement.job_no ? (
                      <div className="rounded-full border border-line bg-surface-2 px-3 py-1 text-xs font-semibold text-content-3">
                        {movement.job_no}
                      </div>
                    ) : null}
