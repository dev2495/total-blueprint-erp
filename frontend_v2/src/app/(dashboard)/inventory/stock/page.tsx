        ...
        <div className="p-8 lg:p-12 space-y-10 bg-[#f8fafc] min-h-screen">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div className="space-y-1">
                    ...
                    <h1 className="text-4xl font-black tracking-tight text-slate-900 flex items-center gap-3">
                        ...
                        <span className="text-indigo-600 italic">Stock</span>
                    </h1>
                    <p className="text-slate-500 font-medium text-sm flex items-center gap-2">
                        Real-time audit of global enterprise physical holdings <Activity className="h-3.5 w-3.5 text-indigo-400" />
                    </p>
                </div>
                <div className="flex items-center gap-3">
                    <Select value={selectedPlant} onValueChange={setSelectedPlant}>
                        <SelectTrigger className="h-12 w-[240px] rounded-xl border-slate-200 bg-white font-black uppercase text-[10px] tracking-widest text-slate-600 shadow-sm transition-all focus:ring-2 focus:ring-indigo-100">
            ...
            <div className="flex flex-col md:flex-row items-center gap-4 bg-white/50 backdrop-blur-md p-4 rounded-[2rem] border border-white shadow-sm">
                ...
                <Button variant="outline" className="h-12 px-6 rounded-2xl bg-white border-slate-200 font-black uppercase text-[10px] tracking-widest text-slate-600 hover:bg-slate-50 active-scale">
                    <Filter className="h-4 w-4 mr-2 text-indigo-500" /> Advanced Filter
                </Button>
            </div>
            ...
            <Tabs defaultValue="rolls" className="w-full space-y-8">
                <div className="flex items-center justify-between pb-2 border-b border-slate-100">
                    <TabsList className="bg-slate-100/50 p-1 rounded-2xl border border-slate-200/50">
                        <TabsTrigger value="rolls" className="rounded-xl px-8 py-2.5 text-[10px] font-black uppercase tracking-widest data-[state=active]:bg-white data-[state=active]:text-indigo-600 data-[state=active]:shadow-premium transition-all">
                            Kinetic Rolls ({filteredRolls.length})
                        </TabsTrigger>
                        <TabsTrigger value="bulk" className="rounded-xl px-8 py-2.5 text-[10px] font-black uppercase tracking-widest data-[state=active]:bg-white data-[state=active]:text-indigo-600 data-[state=active]:shadow-premium transition-all">
                                            ...
                                            {filteredRolls.map((roll) => (
                                                <TableRow key={roll.id} className="group border-slate-50/50 hover:bg-slate-50/50 transition-colors">
                                                    <TableCell className="px-6 py-4">
                                                        <span className="font-black text-slate-900 text-sm tracking-tight font-mono group-hover:text-indigo-600 transition-colors">{roll.label_id}</span>
                                                    </TableCell>
                                                    ...
                                                    <TableCell className="py-4"><span className="text-sm font-black text-indigo-600 italic">{roll.weight_kg?.toFixed(2)}<span className="text-[10px] ml-1">KG</span></span></TableCell>
                                                    <TableCell className="py-4">
                                        ...
                                        <TableBody>
                                            {filteredBulk.map((item, idx) => (
                                                <TableRow key={idx} className="group border-slate-50/50 hover:bg-slate-50/50 transition-colors">
                                                    <TableCell className="px-6 py-4">
                                                        <div className="flex flex-col">
                                                            <span className="text-sm font-black text-slate-700 uppercase tracking-tight group-hover:text-indigo-600 transition-colors">{item.material_name}</span>
                                                    ...
                                                    <TableCell className="py-4 text-right">
                                                        <span className="text-sm font-black text-indigo-600 italic tracking-tighter">{Number(item.quantity ?? item.qty_kg ?? 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                                                    </TableCell>
                                                    <TableCell className="py-4">
