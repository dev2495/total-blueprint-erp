...
export default function LedgerPage() {
  ...
  const { data: ledger, isLoading } = useQuery({
    ...
    queryFn: () =>
      inventoryService.getLedger(
        typeFilter === "ALL" ? {} : { tx_type: typeFilter },
      ),
  });
  ...
  const getTypeColor = (type: string) => {
    ...
    switch (type) {
      ...
      case "DISPATCH":
        return cn(base, "text-warning-fg bg-warning-bg border-warning-border");
      case "CONSUMPTION":
        return cn(base, "text-danger-fg bg-danger-bg border-danger-border");
      case "RECEIVE":
        return cn(base, "text-success-fg bg-success-bg border-success-border");
      case "TRANSFER":
        return cn(base, "text-primary bg-info-bg border-info-border");
      default:
        return cn(base, "text-content-3 bg-surface-2 border-line");
    }
  };
  ...
  const formatQuantity = (entry: any) => {
    ...
    const qty =
      typeof entry.quantity === "number" ? entry.quantity.toFixed(3) : "0.000";
    return {
      text: `${isPositive ? "+" : "-"} ${qty}`,
      color: isPositive ? "text-success-fg" : "text-warning-fg",
    };
  };
    ...
    <div className="p-6 lg:p-8 space-y-8 bg-[#f8fafc] min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
          <Button
            variant="outline"
            className="h-11 rounded-xl border-line text-content-3 font-bold text-xs uppercase tracking-wide hover:bg-surface-1 hover:text-primary shadow-sm active-scale"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          ...
          },
          {
            label: "Inward Flow",
            value: (ledger || []).filter(
              (e: any) => e.tx_type === "GRN" || e.tx_type === "RECEIVE",
            ).length,
            icon: ArrowDownLeft,
          ...
          },
          {
            ...
            value: (ledger || []).filter(
              (e: any) =>
                e.tx_type === "DISPATCH" || e.tx_type === "CONSUMPTION",
            ).length,
            icon: ArrowUpRight,
          ...
          <Card
            key={i}
            className="border-none shadow-premium rounded-2xl bg-surface-1/70 backdrop-blur-md hover:-translate-y-1 transition-all duration-300"
          >
            ...
            <div className="flex items-center gap-3">
              <div className="relative">
                ...
                <Input
                  ...
                  className="pl-10 h-10 w-[250px] rounded-xl border-line bg-surface-1 font-bold text-xs shadow-sm focus:border-primary transition-all"
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                />
              </div>
              <Select value={typeFilter} onValueChange={setTypeFilter}>
                <SelectTrigger className="w-[180px] h-10 rounded-xl border-line bg-surface-1 font-bold text-xs uppercase tracking-wide shadow-sm">
                ...
                <SelectContent className="rounded-xl border-none shadow-xl">
                  ...
                  <SelectItem
                    value="GRN"
                    className="font-bold text-xs text-success-fg"
                  >
                    Inward (GRN)
                  </SelectItem>
                  <SelectItem
                    value="DISPATCH"
                    className="font-bold text-xs text-warning-fg"
                  >
                    Outward (Dispatch)
                  </SelectItem>
                  <SelectItem
                  ...
                  <SelectItem
                    value="CONSUMPTION"
                    className="font-bold text-xs text-danger-fg"
                  >
                    Consumption
                  </SelectItem>
                  <SelectItem
