...
const FALLBACK_OPERATIONS: StockFormOperation[] = [
  {
    ...
    from_stock_form: "OPEN_WEB",
    to_stock_form: "OPEN_WEB",
    requires_child_widths: true,
  },
  {
    code: "OPEN_TUBE_ONE_WEB",
    label: "Open tube to one sheet",
    from_stock_form: "LAYFLAT_TUBE",
    to_stock_form: "OPEN_WEB",
  },
  {
    code: "OPEN_TUBE_TWO_WEBS",
  ...
  },
  {
    ...
    label: "Fold open web",
    from_stock_form: "OPEN_WEB",
    to_stock_form: "FOLDED_WEB",
  },
  {
    code: "UNFOLD_FOLDED_WEB",
    label: "Unfold to open web",
    from_stock_form: "FOLDED_WEB",
    to_stock_form: "OPEN_WEB",
  },
];
...
function operationIcon(code: string) {
  ...
  if (code === "OPEN_TUBE_ONE_WEB" || code === "OPEN_TUBE_TWO_WEBS")
    return <UnfoldVertical className="h-5 w-5" />;
  if (code === "FOLD_OPEN_WEB")
    return <SplitSquareHorizontal className="h-5 w-5" />;
...
function PreviewRows({
  ...
  const preview = useMemo(() => {
    ...
    if (parentWidth <= 0 || parentWeight <= 0)
      errors.push("Parent roll needs positive width and weight.");
    ...
    switch (operation.code) {
      ...
      case "SLIT_OPEN_WEB": {
        ...
        const consumedWidth =
          ...
          trimMm * childWidths.length;
        if (consumedWidth > parentWidth)
          errors.push("Child widths plus trim exceed parent width.");
        childWidths.forEach((width, index) =>
          rows.push({
            label: `Child ${index + 1}`,
            width,
            weight: width * kgPerMm,
            form: "OPEN_WEB",
          }),
        );
  ...
  if (!roll || !operation) {
    return (
      <div className="rounded-2xl border border-dashed border-line bg-surface-2 p-6 text-sm text-content-3">
        Pick an operation and roll to preview the physical children.
      </div>
    );
  }
    ...
    <div className="space-y-3">
      ...
      <div className="overflow-hidden rounded-2xl border border-line bg-surface-1">
        ...
        {preview.rows.map((row) => (
          <div
            ...
            className="grid grid-cols-[1.2fr_1fr_1fr_1fr] gap-3 border-b border-line px-4 py-3 text-sm last:border-b-0"
          >
            <div className="font-bold text-content-1">{row.label}</div>
            <div>
              <Badge
                variant="outline"
                className="border-info-border bg-info-bg text-primary"
              >
                {stockFormLabel(row.form)}
              </Badge>
            </div>
        ...
        {preview.remainder ? (
          <div className="grid grid-cols-[1.2fr_1fr_1fr_1fr] gap-3 border-t border-warning-border bg-warning-bg px-4 py-3 text-sm">
            <div className="font-bold text-warning-fg">
              {preview.remainder.label}
            </div>
            <div>
              <Badge
                variant="outline"
              ...
              >
                {stockFormLabel(preview.remainder.form)}
              </Badge>
            </div>
            <div className="font-mono font-bold">
              {fmt(preview.remainder.width)} mm
            </div>
            <div className="font-mono font-bold">
              {fmt(preview.remainder.weight, 3)} kg
            </div>
          </div>
        ) : null}
      </div>
      <div className="flex flex-wrap gap-2 text-xs font-bold text-content-3">
        <Badge variant="secondary">
          Parent {fmt(numberOrZero(roll.width_mm))} mm ·{" "}
          {fmt(numberOrZero(roll.weight_kg), 3)} kg
        </Badge>
        <Badge variant="secondary">
          Scrap {fmt(preview.scrapWidth)} mm · {fmt(preview.scrapKg, 3)} kg
        </Badge>
      </div>
    </div>
  );
...
export default function StockConversionsPage() {
  ...
  const operations = operationsQuery.data?.length
    ? operationsQuery.data
    : FALLBACK_OPERATIONS;
  const activeOperation =
    operations.find((operation) => operation.code === operationCode) ||
    operations[0];
    ...
    return rolls
      .filter(
        (roll) =>
          ...
          String(activeOperation?.from_stock_form || "OPEN_WEB").toUpperCase(),
      )
      .filter((roll) => {
  ...
  const selectedRoll =
    compatibleRolls.find((roll) => roll.id === selectedRollId) ||
    rolls.find((roll) => roll.id === selectedRollId) ||
    null;
  const childWidths = useMemo(
    () => parseWidths(childWidthsRaw),
    [childWidthsRaw],
  );
  ...
  const convertMutation = useMutation({
    mutationFn: () =>
      inventoryService.convertStockForm(selectedRollId, {
        operation: activeOperation.code,
        child_widths_mm: activeOperation.requires_child_widths
          ? childWidths
          : [],
        trim_mm: parsedTrim,
        reason,
  ...
  const canSubmit = Boolean(
    selectedRollId &&
      ...
      (!activeOperation.requires_child_widths || childWidths.length > 0),
  );
    ...
    <div className="min-h-screen bg-gradient-to-b from-surface-2 via-white to-surface-2 px-4 py-4 sm:px-6">
      <div className="mx-auto flex max-w-[1800px] flex-col gap-5">
          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-3xl text-sm text-info-border">
                Convert physical roll form without changing route templates:
                slit open-web jumbos, open tube to sheet, split tube into two
                webs, fold sheet, or unfold folded stock. Every post preserves
                weight and roll genealogy.
              </p>
            </div>
        ...
        <section className="grid gap-4 xl:grid-cols-[360px_1fr]">
          <Card className="rounded-3xl border-line shadow-sm">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-lg">
                <Repeat className="h-5 w-5 text-primary" /> Operation
              </CardTitle>
            </CardHeader>
              ...
              {operations.map((operation) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full rounded-2xl border p-4 text-left transition ${active ? "border-primary bg-info-bg shadow-sm" : "border-line bg-surface-1 hover:border-info-border hover:bg-surface-2"}`}
                  >
                    <div className="flex items-start gap-3">
                      <div
                        className={`rounded-xl p-2 ${active ? "bg-primary text-white" : "bg-surface-2 text-content-3"}`}
                      >
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="mt-1 flex flex-wrap items-center gap-1 text-xs text-content-3">
                          <Badge variant="outline">
                            {stockFormLabel(operation.from_stock_form)}
                          </Badge>
                          <ArrowRight className="h-3 w-3" />
                          <Badge variant="outline">
                            {stockFormLabel(operation.to_stock_form)}
                          </Badge>
                        </div>
                      </div>
                    </div>
          ...
          <div className="grid gap-4 2xl:grid-cols-[1.1fr_0.9fr]">
            <Card className="rounded-3xl border-line shadow-sm">
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-lg">
                  <Layers className="h-5 w-5 text-success-fg" /> Pick parent
                  roll
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="relative">
                  ...
                  <Input
                    ...
                    placeholder="Search roll, material, grade, location…"
                ...
                <div className="max-h-[540px] overflow-auto rounded-2xl border border-line">
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="text-xs font-semibold text-content-3">
                          {roll.location_name || roll.plant_name || "-"}
                        </div>
                      </button>
                    );
                  })}
                  {!compatibleRolls.length ? (
                    <div className="p-8 text-center text-sm text-content-3">
                      No available{" "}
                      {stockFormLabel(activeOperation?.from_stock_form)} rolls
                      match this filter.
                    </div>
                  ) : null}
                </div>
            ...
            <Card className="rounded-3xl border-line shadow-sm">
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-lg">
                  <CheckCircle2 className="h-5 w-5 text-success-fg" /> Preview
                  and post
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                {activeOperation?.requires_child_widths ? (
                  <div className="grid gap-3 sm:grid-cols-[1fr_150px]">
                    <div>
                      ...
                      <Input
                        value={childWidthsRaw}
                        onChange={(event) =>
                          setChildWidthsRaw(event.target.value)
                        }
                        placeholder="500, 500, 300"
                        className="mt-1 h-11 rounded-2xl font-mono"
                      />
                    </div>
                    <div>
                      <Label className="text-[11px] font-black uppercase tracking-[0.16em] text-content-3">
                      ...
                      <Input
                        value={trimMm}
                        onChange={(event) => setTrimMm(event.target.value)}
                        className="mt-1 h-11 rounded-2xl font-mono"
                      />
                    </div>
                  </div>
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    placeholder="Example: Open tube for side-seal job; slit jumbo for exact WCM pick."
                    className="mt-1 min-h-[84px] rounded-2xl"
                  />
                </div>
                <PreviewRows
                  roll={selectedRoll}
