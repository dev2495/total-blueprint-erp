...
export default function VendorsPage() {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: any }) =>
      masterDataService.updateVendor(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["vendors"] });
      ...
      >
        <AlertDialogContent className="rounded-[2rem] border-none shadow-2xl">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently remove <strong>{itemToDelete?.name}</strong>{" "}
              from the master registry.
              <br />
              Validation checks will fail for any existing orders linked to this
              entity.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel className="rounded-xl font-bold uppercase text-xs tracking-wider">
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
              onClick={() => {
              ...
              className="rounded-xl bg-danger-solid hover:bg-danger-solid font-bold uppercase text-xs tracking-wider"
            >
              Delete Entity
            </AlertDialogAction>
          </AlertDialogFooter>
