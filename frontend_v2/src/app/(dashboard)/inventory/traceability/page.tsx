...
function statusTone(status?: string | null): "default" | "secondary" | "outline" {
...
function NodeCard({ node, isChild = false, isLast = false }: { node: GenealogyNode; isChild?: boolean; isLast?: boolean }) {
                ...
                <div className="flex items-start justify-between gap-3 mb-2">
                    <div className="min-w-0 flex items-start gap-3">
                        {!isChild ? (
                            ...
                            <div className="bg-slate-100 text-slate-500 w-8 h-8 rounded-full flex items-center justify-center shrink-0">
                    ...
                    <div className="flex flex-col items-end gap-1.5 shrink-0">
                        <Badge variant="outline" className={cn("text-[10px] font-black tracking-wider uppercase", roleTone((node as any).roll_role))}>
                            {(node as any).roll_role || "ROLL"}
                        </Badge>
                        <Badge variant={statusTone(node.status)} className="text-[10px] font-black tracking-wider uppercase">
                            {node.status || "—"}
                        </Badge>
                    </div>
                </div>
            ...
            {hasChildren && (
                <div className="relative mt-4 pl-8">
                    {node.children.map((child, idx) => (
                        <NodeCard key={child.id} node={child as GenealogyNode} isChild={true} isLast={idx === node.children.length - 1} />
                    ))}
                </div>
            )}
        </div>
...
export default function RollTraceabilityPage() {
    ...
    const childrenCount = useMemo(() => {
        function count(node?: GenealogyNode): number {
            ...
            return node.children.length + node.children.reduce((sum, child) => sum + count(child as GenealogyNode), 0);
        }
    ...
    return (
        <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,#eff6ff,#f8fafc_45%,#eef2ff)] p-6">
            <div className="max-w-[1600px] mx-auto space-y-6">
                <Card className="rounded-[2rem] border border-white/60 bg-white/70 backdrop-blur-xl shadow-xl">
                    <CardContent className="p-6 space-y-4">
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                            ...
                            >
                                <SelectTrigger className="h-12 w-[380px] rounded-2xl font-semibold">
                                    <SelectValue placeholder={rollsQuery.isLoading ? "Loading rolls..." : "Select roll from latest 200"} />
                                </SelectTrigger>
                                <SelectContent>
                                    {rollOptions.map((roll) => (
                                        <SelectItem key={roll.id} value={roll.id}>
                                            {roll.label_id} • {Number(roll.weight_kg || 0).toFixed(3)} kg • {roll.stage_name || "—"} • {roll.grade_name || "No grade"}
                                        </SelectItem>
                                    ))}
                                </SelectContent>
                            </Select>
                            <div className="relative flex-1">
                                ...
                                <Input
                                    ...
                                    onKeyDown={(e) => {
                                        if (e.key === "Enter" && query.trim()) traceMutation.mutate(query.trim());
                                    }}
                                />
                            </div>
                            <Button
                        ...
                        {errorText && <div className="text-sm font-semibold text-rose-600">{errorText}</div>}
                    </CardContent>
                </Card>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                                    ...
                                    <div className="rounded-2xl bg-slate-50 border border-slate-100 p-3">
                                        ...
                                        <div className="mt-1 flex items-center gap-2">
                                            ...
                                            <Badge variant={statusTone(currentRoll.status)} className="text-[10px] font-bold uppercase">
                                                {currentRoll.status}
                                            </Badge>
                                        </div>
                                    </div>
                        ...
                        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                            <Card className="rounded-[2rem] border border-white/60 bg-white/80">
                                ...
                                <CardContent className="space-y-3 max-h-[700px] overflow-auto pr-2">
                                    ...
                                    {(!genealogy.trees || genealogy.trees.length === 0) && (
                                        <div className="text-sm text-slate-500 font-semibold py-6 text-center">No tree data found.</div>
                                    )}
                                </CardContent>
                            </Card>
                            ...
                            <Card className="rounded-[2rem] border border-white/60 bg-white/80">
                                ...
                                <CardContent className="space-y-2 max-h-[700px] overflow-auto pr-2">
                                    {timeline.length === 0 && (
                                        <div className="text-sm text-slate-500 font-semibold py-6 text-center">No timeline events recorded.</div>
                                    )}
                                    {timeline.map((event: any, index: number) => (
                                        <div key={`${event.type}-${event.timestamp}-${index}`} className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
                                            <div className="flex items-center justify-between gap-2">
                                                <Badge variant="outline" className="text-[10px] font-bold uppercase">
                                                    {event.type || "EVENT"}
                                                </Badge>
                                                <span className="text-[11px] text-slate-500 font-semibold">{fmtDate(event.timestamp)}</span>
                                            </div>
                                            {event.type === "MOVEMENT" ? (
                                                <div className="mt-2 text-sm font-semibold text-slate-700 flex items-center gap-2">
                        ...
                        <Card className="rounded-[2rem] border border-white/60 bg-white/80">
                            ...
                            <CardContent className="space-y-2">
                                {(result?.recent_movements || []).length === 0 ? (
                                    <div className="text-sm text-slate-500 font-semibold">No movement events found.</div>
                                ) : (
                                    (result?.recent_movements || []).map((m, idx) => (
                                        <div key={`move-${idx}`} className="rounded-xl border border-slate-200 p-3 bg-slate-50/80">
                                            ...
                                            <div className="text-xs text-slate-500 mt-1">
                                                {m.reason || "—"} {m.reason_note ? `• ${m.reason_note}` : ""}
                                            </div>
                                        </div>
                                    ))
                                )}
