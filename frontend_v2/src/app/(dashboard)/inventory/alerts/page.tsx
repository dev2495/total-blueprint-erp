...
export default function AlertsCenterPage() {
  ...
  const [filterResolved, setFilterResolved] = useState<boolean | undefined>(
    false,
  );
  ...
  const severityRows = ["CRITICAL", "HIGH", "MEDIUM", "LOW"].map(
    (severity) => ({
      severity,
      count: filteredAlerts.filter(
        (alert) => String(alert.severity).toUpperCase() === severity,
      ).length,
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <section className="relative overflow-hidden rounded-[2rem] border border-surface-1/70 bg-surface-1/85 p-6 shadow-[0_30px_80px_-42px_rgba(15,23,42,0.45)] backdrop-blur-xl">
        <div className="pointer-events-none absolute inset-x-0 top-0 h-32 bg-gradient-to-r from-danger-solid via-warning-fg to-info-fg" />
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm font-medium text-content-3">
                One place to triage stock anomalies, close alert loops, and see
                which plants or materials are carrying unresolved risk.
              </p>
            </div>
          </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <SummaryStatCard
          label="Open alerts"
          value={openAlerts.length.toLocaleString()}
          subLabel="Alerts still needing action"
          icon={Siren}
        ...
        <SummaryStatCard
          ...
          value={resolvedAlerts.length.toLocaleString()}
          subLabel="Closed alerts within the current filter"
          icon={CheckCircle2}
        ...
        <SummaryStatCard
          ...
          subLabel={`${filteredAlerts.length.toLocaleString()} alert rows visible`}
          icon={TimerReset}
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <Card className="overflow-hidden rounded-[2rem] border border-surface-1/70 bg-surface-1/85 shadow-[0_24px_70px_-45px_rgba(15,23,42,0.45)] backdrop-blur-xl">
          <CardHeader className="border-b border-line bg-surface-1/70">
            ...
            <p className="text-sm text-content-3">
              If critical or high bars stay elevated, the store and planning
        ...
        <Card className="overflow-hidden rounded-[2rem] border border-surface-1/70 bg-surface-1/85 shadow-[0_24px_70px_-45px_rgba(15,23,42,0.45)] backdrop-blur-xl">
          <CardHeader className="border-b border-line bg-surface-1/70">
            ...
            <p className="text-sm text-content-3">
              Most frequent alert categories in the current filter window.
            </p>
          </CardHeader>
          <CardContent className="space-y-3 p-6">
            {alertTypeRows.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-line bg-surface-2 p-8 text-center text-sm text-content-3">
      ...
      <Card className="overflow-hidden rounded-[2rem] border border-surface-1/70 bg-surface-1/85 shadow-[0_24px_70px_-45px_rgba(15,23,42,0.45)] backdrop-blur-xl">
        <CardHeader className="border-b border-line bg-surface-1/70">
          ...
          <p className="text-sm text-content-3">
            Current anomalies with severity, location context, and clear resolve
            actions.
          </p>
        </CardHeader>
          ...
          {filteredAlerts.length === 0 ? (
            <div className="rounded-[1.5rem] border border-dashed border-success-border bg-success-bg p-10 text-center">
              ...
              <div className="mt-1 text-sm text-content-3">
                The current filter window is clean. Switch to All if you want to
                review historical closes.
              </div>
            </div>
          ) : (
            filteredAlerts.map((alert) => (
              <div
                key={alert.id}
                className={`rounded-[1.5rem] border p-5 transition-colors ${alert.resolved ? "border-line bg-surface-2" : "border-line bg-surface-1 hover:bg-info-bg"}`}
              >
                    ...
                    <div>
                      ...
                      {alert.resolved && (
                        <div className="mt-3 rounded-xl border border-success-border bg-success-bg px-3 py-2 text-xs text-success-fg">
                          ...
                          {alert.resolved_at
                            ? ` on ${formatDate(alert.resolved_at)}`
                            : ""}
      ...
      <Dialog open={resolveDialogOpen} onOpenChange={setResolveDialogOpen}>
        <DialogContent className="rounded-[1.75rem] border-surface-1/70 bg-surface-1/95 shadow-[0_24px_70px_-45px_rgba(15,23,42,0.45)] backdrop-blur-xl">
          <DialogHeader>
          ...
          <div className="space-y-4">
            <div className="rounded-2xl border border-line bg-surface-2 px-4 py-3">
              ...
              <div className="mt-1 text-sm font-bold text-content-1">
                {selectedAlert?.type_display || "Inventory alert"}
              </div>
              <div className="mt-2 text-sm text-content-3">
                {selectedAlert?.message || "Choose an alert from the queue."}
              </div>
            </div>
