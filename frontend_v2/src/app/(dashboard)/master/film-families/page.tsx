...
export default function FilmFamiliesPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Success",
        description: "Film Family created successfully.",
      });
  ...
  const updateMutation = useMutation({
    ...
    mutationFn: ({ id, data }: { id: string; data: any }) =>
      filmFamilyService.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["film-families"] });
      toast({
        title: "Success",
        description: "Film Family updated successfully.",
      });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Success",
        description: "Film Family deleted successfully.",
      });
    },
    onError: (error: Error) => {
      toast({
  ...
  const filteredFamilies = (families || []).filter((family) => {
    ...
    return (
      ...
      String(family.commercial_family_name || "")
        .toLowerCase()
        .includes(query)
    );
  });
  ...
  const avgDensity = (families || []).length
    ? (
        (families || []).reduce(
          (sum, family) => sum + Number(family.density_gcm3 || 0),
          0,
      ...
      stats={[
        {
          ...
          subLabel: "Base material structures",
          icon: Layers,
          toneClassName: "bg-info-bg text-primary",
        },
        {
          label: "Visible",
          value: filteredFamilies.length,
          subLabel: "Matching current search",
          icon: Tag,
          toneClassName: "bg-surface-2 text-content-2",
        },
        {
          label: "Linked aliases",
          value: (families || []).filter((family) =>
            Boolean(
              (family as any).commercial_family ||
                (family as any).commercial_family_name,
            ),
          ).length,
          subLabel: "Business family naming attached",
          icon: Tag,
          toneClassName: "bg-info-bg text-primary",
        },
        {
          label: "Avg density",
          value: `${avgDensity} g/cc`,
          subLabel: "Used by the physics layer",
          icon: FlaskConical,
              ...
              onSubmit={(data) =>
                createMutation.mutate({
                  ...
                  commercial_family:
                    ...
                    data.commercial_family === "__NONE__"
                      ? null
                      : data.commercial_family,
                })
              }
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              film family and may affect related variants.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
