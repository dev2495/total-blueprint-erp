...
export const getColumns = ({
          ...
          <span
            ...
            style={
              valid
                ...
                : {
                    background:
                      "repeating-linear-gradient(45deg,#fee2e2 0,#fee2e2 4px,#fff 4px,#fff 8px)",
                  }
            }
          />
          <span
            className={valid ? "font-mono text-content-2" : "text-danger-fg"}
          >
            {valid ? hex.toUpperCase() : "missing"}
          </span>
        </div>
      );
    },
  },
  {
    ...
    header: "Description",
    cell: ({ row }) => (
      <div className="text-content-3 text-xs">{row.getValue("name")}</div>
    ),
  },
  {
    id: "actions",
