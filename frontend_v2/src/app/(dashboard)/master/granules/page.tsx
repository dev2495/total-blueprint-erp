              ...
              <FormItem className="flex flex-row items-start space-x-3 space-y-0">
                <FormControl>
                  <input
                    ...
                    className="mt-1 h-4 w-4 accent-success-fg"
                  />
                </FormControl>
                <div className="space-y-1 leading-none">
                  <FormLabel className="text-content-2">
                    Sellable as trading good
                  </FormLabel>
                  <div className="text-[11px] text-content-3">
                    Enable to make this granule available in Trade Orders.
                  </div>
                </div>
              </FormItem>
            )}
    ...
    <Form {...form}>
      <form
        onSubmit={form.handleSubmit(
          (values: z.infer<typeof qualityCodeSchema>) =>
            onSubmit({
              ...values,
...
export default function GranulesPage() {
  ...
  const [selectedGranuleId, setSelectedGranuleId] = useState<string | null>(
    null,
  );
  const [editingCode, setEditingCode] = useState<GranuleQualityCode | null>(
    null,
  );
  const [codeToDelete, setCodeToDelete] = useState<GranuleQualityCode | null>(
    null,
  );
  ...
  const updateMutation = useMutation({
    ...
    }) => masterDataService.updateGranule(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["granules"] });
  ...
  const updateCodeMutation = useMutation({
    mutationFn: ({
      ...
      id: string;
      data: z.infer<typeof qualityCodeSchema>;
    }) =>
  ...
  const selectedGranule =
    (granules || []).find((item) => item.id === selectedGranuleId) || null;
  const totalQualityCodes = (granules || []).reduce(
    (sum, item) =>
      sum + (item.quality_code_count || item.quality_codes?.length || 0),
    0,
  );
  const activeQualityCodes = (granules || []).reduce(
    (sum, item) =>
      sum +
      (item.quality_codes || []).filter((code) => code.status === "ACTIVE")
        .length,
    0,
  );
      ...
      stats={[
        {
          ...
          subLabel: "Raw material masters",
          icon: Package,
          toneClassName: "bg-success-bg text-success-fg",
        },
        {
          label: "Visible",
          value: filteredGranules.length,
          subLabel: "Matching current search",
          icon: Tag,
          toneClassName: "bg-surface-2 text-content-2",
        },
        {
          label: "Granule masters",
          value: new Set((granules || []).map((item) => item.code)).size,
          subLabel: "Master identifiers",
          icon: Tag,
          toneClassName: "bg-info-bg text-primary",
        },
        {
          label: "Granule codes",
          value: totalQualityCodes,
          subLabel: `${activeQualityCodes} active code options`,
          icon: Tags,
              ...
              <div className="max-h-[360px] overflow-y-auto rounded-2xl border border-line bg-surface-1">
                ...
                {(selectedGranule.quality_codes || []).length ? (
                  (selectedGranule.quality_codes || []).map((code) => (
                    <div
                      key={code.id}
                      className="grid grid-cols-[1.4fr_120px_1.6fr_120px] gap-3 border-b border-line px-4 py-3 text-sm last:border-b-0"
                    >
                  ...
                  <div className="px-4 py-10 text-center text-sm text-content-3">
                    No granule codes exist for this granule yet. Add the codes
                    your inward team and WCM operators need for split reporting.
                  </div>
                )}
              </div>
            </div>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This removes <strong>{codeToDelete?.code}</strong> from the
              granule code registry. Historical GRN, inventory, and consumption
              records will continue to show past transactions, but new inward
              and WCM issue selections will stop using this code.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
