...
export default function TradingGoodsListPage() {
  ...
  const stockSum = goods.reduce(
    (s, g) => s + Number(g.current_stock_qty || 0),
    0,
  );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-success-bg via-white to-surface-2 px-4 py-4 sm:px-6">
      <GradientHero
        ...
        chips={[
          ...
          },
          {
            ...
            label: "Stock units",
            value: String(Math.round(stockSum)),
            tone: "violet",
          },
        ]}
        actions={
          <Link href="/master/trading-goods/new">
            <Button className="bg-surface-1 text-success-fg hover:bg-surface-1/90">
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-3 text-[12px] text-content-3">
          ...
          <span>
            Showing <b className="text-content-1">{filtered.length}</b> of{" "}
            {goods.length}
          </span>
        </div>
      </div>
...
function EmptyState({ empty }: { empty: boolean }) {
  return (
    <div className="flex flex-col items-center gap-3 rounded-3xl border border-line bg-surface-1 p-10 text-center">
      <div className="grid h-14 w-14 place-items-center rounded-2xl bg-success-bg text-success-fg">
      ...
      <div>
        <div className="text-sm font-bold text-content-2">
          {empty
            ...
            : "No trading goods match your search"}
        </div>
      ...
      {empty ? (
        <Link href="/master/trading-goods/new">
          <Button className="bg-success-fg text-white hover:bg-success-fg">
            <Plus className="mr-1.5 h-4 w-4" /> Add first trading good
          </Button>
        </Link>
      ) : null}
    </div>
...
function TradingGoodCard({ g }: { g: TradingGood }) {
  return (
    <Link href={`/master/trading-goods/${g.id}`} className="group">
      <article className="rounded-3xl bg-surface-1 p-5 shadow-[0_20px_60px_-30px_rgba(15,23,42,0.25)] ring-1 ring-line transition hover:ring-success-border">
        <header className="flex items-start justify-between gap-2">
          <div>
            <div className="font-mono text-[12px] font-bold text-success-fg">
              {g.code}
            </div>
            <h3 className="mt-0.5 text-sm font-bold text-content-1 group-hover:text-success-fg">
              {g.name}
            </h3>
            {g.description ? (
              <p className="mt-1 line-clamp-2 text-[11px] text-content-3">
                {g.description}
              </p>
            ) : null}
          </div>
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          <Badge
            variant="outline"
            className="border-success-border bg-success-bg text-[10px] font-bold text-success-fg"
          >
            {TRADE_TYPE_LABELS[g.trade_type || "OTHER"] ||
              g.trade_type ||
              "Other"}
          </Badge>
          <Badge
            variant="outline"
            className="border-line bg-surface-1 text-[10px] font-bold text-content-3"
          >
        ...
        <dl className="mt-4 grid grid-cols-3 gap-2 text-center">
          <Stat
            label="Stock"
            value={`${Number(g.current_stock_qty || 0).toLocaleString()}`}
            hint={g.base_uom}
          />
          <Stat
            label="Sale ₹"
            value={
              g.default_sale_rate != null
                ? Number(g.default_sale_rate).toLocaleString()
                : "—"
            }
          />
          <Stat
            label="GST %"
            value={String(g.default_gst_pct ?? 0)}
