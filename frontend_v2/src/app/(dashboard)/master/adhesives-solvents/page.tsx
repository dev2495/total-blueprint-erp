...
export default function AdhesivesSolventsPage() {
  ...
  return (
    <MasterRegistryShell
      ...
      stats={[
        {
          label: "Chemical masters",
          value: items.length,
          subLabel: "System-managed records",
          icon: FlaskConical,
    ...
    >
      <Card className="border-none shadow-premium rounded-[1.75rem] bg-surface-1/90">
        <CardHeader className="border-b border-line">
          ...
          <CardDescription className="text-xs text-content-3">
            Operators inward only against the fixed adhesive or solvent master.
            Vendor, location, quantity, cost, and reference remain editable
            during GRN.
          </CardDescription>
        </CardHeader>
          ...
          {items.map((item) => (
            ...
            >
              ...
              <div className="mt-4 text-xs text-content-3">
                This master is consumed automatically by production truth and is
                no longer user-expandable from this screen.
              </div>
            </div>
          ))}
        </CardContent>
