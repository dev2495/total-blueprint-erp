...
const vendorSchema = z.object({
    ...
    turnaround_hours: z.number().min(1, "Turnaround must be at least 1 hour").default(48),
    qc_required: z.boolean().default(true),
...
export function VendorForm({ initialData, onSubmit, isLoading }: Props) {
    ...
    const form = useForm<VendorFormInput, any, VendorFormValues>({
        ...
        defaultValues: {
            ...
            turnaround_hours: Number(initialData?.turnaround_hours || 48),
            qc_required: initialData?.qc_required ?? true,
        ...
        <Form {...form}>
            <form
                onSubmit={form.handleSubmit((values) =>
                    onSubmit({
                        ...values,
                        jobwork_capabilities: parseCsv(values.jobwork_capabilities_text),
                        jobwork_plants: parseCsv(values.jobwork_plants_text),
                    } as any)
                )}
                className="space-y-4"
            >
                ...
                <div className="grid grid-cols-2 gap-4">
                            ...
                            <FormItem>
                                ...
                                <FormControl>
                                    <Input
                                        ...
                                        onChange={(e) => field.onChange(Number(e.target.value || 48))}
                                    />
                                </FormControl>
                                <FormMessage />
                            </FormItem>
                    ...
                    <FormField
                        ...
                        name="qc_required"
                        render={({ field }) => (
                            <FormItem className="flex items-center justify-between rounded-xl border border-slate-200 px-3 py-2.5 mt-6">
                                <div>
                                    <Label className="text-xs font-bold text-slate-500 uppercase">QC Required on Return</Label>
                                    <p className="text-xs text-slate-400 mt-1">If enabled, return receives stay blocked until QC confirmation.</p>
                                </div>
                                <FormControl>
                ...
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem>
                            ...
                            <FormControl>
                                <Input placeholder="PRINTING, LAMINATION, SLITTING" className="text-sm font-mono" {...field} />
                            </FormControl>
                            <p className="text-xs text-slate-400">Comma-separated process codes. Keep empty for generic compatibility.</p>
                            <FormMessage />
                        </FormItem>
                    )}
                />
                ...
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem>
                            ...
                            <FormControl>
                                <Input placeholder="PLANT_A, PLANT_B" className="text-sm font-mono" {...field} />
                            </FormControl>
                            <p className="text-xs text-slate-400">Comma-separated plant IDs/codes. Keep empty for all plants.</p>
                            <FormMessage />
                        </FormItem>
                    )}
                />
