...
export default function VendorsPage() {
  ...
  const updateMutation = useMutation({
    ...
    mutationFn: ({ id, data }: { id: string; data: any }) =>
      inventoryService.updateVendor(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["vendors"] });
      ...
      stats={[
        {
          ...
          subLabel: "Suppliers and job workers",
          icon: Truck,
          toneClassName: "bg-info-bg text-primary",
        },
        {
          label: "Active",
          value: (vendors || []).filter((vendor) => vendor.status === "ACTIVE")
            .length,
          subLabel: "Ready for purchase or jobwork",
          icon: ShieldCheck,
        ...
        },
        {
          ...
          value: (vendors || []).filter((vendor) =>
            ["JOBWORK", "BOTH"].includes(vendor.type),
          ).length,
          subLabel: "Can receive outsourced work",
          icon: Factory,
          toneClassName: "bg-info-bg text-info-fg",
        },
        {
          label: "QC required",
          value: (vendors || []).filter((vendor) => Boolean(vendor.qc_required))
            .length,
          subLabel: "Extra incoming quality checks",
          icon: ShieldCheck,
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              vendor record. Ensure no active stock blocks are linked to this
              vendor.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
