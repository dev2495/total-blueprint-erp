...
export default function MasterDataPage() {
  ...
  const { data: tradingGoods } = useQuery({
    queryKey: ["master-trading-goods"],
  ...
  const masters = [
    {
      ...
      description:
        "Business-friendly naming groups used across stock explorer, planner, and reports",
      color: "text-primary",
      bg: "bg-info-bg",
    },
    {
    ...
    },
    {
      ...
      description:
        "All packing SKUs · inner-pouch, gunny, sheet, tape, label, tag · build in-house masters from here",
      color: "text-primary",
      bg: "bg-info-bg",
    },
    {
      ...
      description:
        "Ready pouches, ready rolls, and other resale stock sold directly through trade orders",
      color: "text-success-fg",
      bg: "bg-success-bg",
    },
    {
      ...
      count: count(pod),
      description:
        "All POD SKUs · in-house POD masters + variants link back here for stock tracking",
      color: "text-warm",
      bg: "bg-warm",
    },
  ];
  ...
  const materialCount =
    (count(filmFamilies) ?? 0) +
    (count(filmVariants) ?? 0) +
    (count(inks) ?? 0) +
    (count(adhesives) ?? 0) +
    (count(granules) ?? 0) +
    (count(tradingGoods) ?? 0);
    ...
    <MasterRegistryShell
      ...
      stats={[
        ...
        },
        {
          ...
          subLabel: "Film, ink, adhesive, granule",
          icon: Layers,
          toneClassName: "bg-info-bg text-primary",
        },
        {
          ...
          subLabel: "Customers and vendors",
          icon: Users,
            ...
            <Card className="cursor-pointer border-none shadow-premium hover:shadow-premium-hover hover:-translate-y-1 transition-all duration-300 h-full rounded-[1.5rem] group relative overflow-hidden bg-surface-1">
              ...
              <CardContent className="p-5 sm:p-6 flex flex-col h-full">
                <div className="flex items-start justify-between mb-5">
                  <div
                    className={`p-3.5 rounded-2xl ${master.bg} ${master.color} transition-colors duration-300`}
                  >
                    <master.icon className="h-5 w-5" />
                  </div>
                  <div className="h-7 w-7 rounded-full bg-surface-2 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300">
                    <ArrowRight className="h-3.5 w-3.5 text-content-4" />
                  </div>
                </div>

                <div className="mt-auto">
                  <h3 className="font-bold text-base sm:text-lg text-content-1 mb-1.5 group-hover:text-primary transition-colors">
                    {master.title}
                  </h3>
                  <p className="text-[13px] text-content-3 font-medium leading-relaxed mb-4">
                    {master.description}
                  </p>
                  <div className="flex items-center justify-between gap-2 text-xs font-bold text-content-4 uppercase tracking-wider">
                    {master.count !== null ? (
                      <span className="bg-surface-2 px-2.5 py-1 rounded-lg text-content-3 text-[11px] font-bold tabular-nums">
                        {master.count} Records
                      </span>
                    ) : master.status === "error" ? (
                      <span className="bg-danger-bg px-2.5 py-1 rounded-lg text-danger-fg text-[11px] font-bold">
                        Access error
                      </span>
                    ) : (
                      <span className="flex items-center gap-1.5 bg-surface-2 px-2.5 py-1 rounded-lg text-content-4 text-[11px]">
