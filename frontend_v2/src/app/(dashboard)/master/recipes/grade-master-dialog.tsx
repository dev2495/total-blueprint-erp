...
export function GradeMasterDialog() {
    ...
    const updateMutation = useMutation({
        mutationFn: ({ id, name }: { id: string, name: string }) => recipeService.updateGrade(id, { name }),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["recipe-grades"] })
                ...
                <div className="space-y-4 py-4">
                    ...
                    <div className="border rounded-md divide-y max-h-[300px] overflow-y-auto">
                        {isLoading ? (
                            ...
                            <div key={grade.id} className="p-3 flex items-center justify-between group">
                                {editingGrade?.id === grade.id ? (
                                    <div className="flex flex-1 gap-2">
                                        ...
                                        <Button size="sm" variant="ghost" className="h-8 text-neutral-500" onClick={() => setEditingGrade(null)}>
                                            Cancel
                                        </Button>
                                    </div>
                                ) : (
                                    <>
                                        <span className="font-medium text-slate-700">{grade.name}</span>
                                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => setEditingGrade(grade)}>
