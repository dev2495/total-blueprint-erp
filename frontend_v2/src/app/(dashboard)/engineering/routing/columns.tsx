...
export const getColumns = ({ onEdit, onDelete }: ColumnsProps): ColumnDef<RoutingRule>[] => [
    {
        accessorKey: "name",
        header: "Rule Name",
            ...
            <div className="flex flex-wrap gap-1 max-w-[300px]">
                {(row.getValue("ordered_processes") as string[])?.map((code, index) => (
                    <div key={index} className="flex items-center gap-1">
                        <StatusBadge status={code} variant="outline" className="text-[10px] px-1" />
                        {index < (row.getValue("ordered_processes") as string[]).length - 1 && (
