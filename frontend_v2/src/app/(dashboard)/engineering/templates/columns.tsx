...
export const getColumns = ({ onEdit, onDelete, onApprove, onMakeLive }: ColumnsProps): ColumnDef<TemplateBlueprint>[] => [
    {
        accessorKey: "name",
    ...
    },
    {
        accessorKey: "fg_type",
        header: "Type",
        cell: ({ row }) => <StatusBadge status={row.getValue("fg_type")} variant="outline" />,
    },
    {
        accessorKey: "status",
        header: "Status",
        ...
        cell: ({ row }) => (
            <div className="flex items-center gap-2">
                {row.original.routing_rule_name ? (
                    <StatusBadge status={row.original.routing_rule_name} variant="outline" />
                ) : (
                    <span className="text-xs text-muted-foreground italic">No routing</span>
                )}
            </div>
        ),
    },
