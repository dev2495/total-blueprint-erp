...
function ToolAssetForm({
  ...
  const [storageLocation, setStorageLocation] = useState(
    initial?.storage_location || "__NONE__",
  );
  ...
  const filteredLocations = locations.filter(
    (row: any) =>
      ...
      String(row.type || "").toUpperCase() === "TOOLING",
  );
    ...
    <form
      ...
      onSubmit={(e) => {
        ...
        onSubmit({
          ...
          status,
          storage_location:
            storageLocation === "__NONE__" ? null : storageLocation,
          rack_code: rackCode,
          slot_code: slotCode,
          vendor: vendor === "__NONE__" ? null : vendor,
          service_due_at: serviceDueAt || null,
          notes,
        });
      }}
    >
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <Select value={storageLocation} onValueChange={setStorageLocation}>
            <SelectTrigger>
              <SelectValue placeholder="Tool room" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="__NONE__">No tooling room assigned</SelectItem>
              {filteredLocations.map((row: any) => (
                <SelectItem key={row.id} value={row.id}>
                  {row.name}
                </SelectItem>
              ))}
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <Input
            type="date"
            value={serviceDueAt || ""}
            onChange={(e) => setServiceDueAt(e.target.value)}
          />
        </div>
        <div>
          <Label>Notes</Label>
...
export default function ToolingPage() {
  ...
  const { data: tools = [] } = useQuery({
    queryKey: ["tool-assets"],
    queryFn: () => engineeringService.getToolAssets(),
  });
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return tools;
    return tools.filter((row: ToolAsset) =>
      [
        row.code,
        row.name,
        row.asset_type,
  ...
  const createMutation = useMutation({
    mutationFn: engineeringService.createToolAsset,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["tool-assets"] });
      setOpen(false);
      toast({ title: "Tool asset created" });
    },
    onError: (err: any) =>
      toast({
        title: "Create failed",
  ...
  const updateMutation = useMutation({
    mutationFn: ({
      ...
      id: string;
      payload: Partial<ToolAsset>;
    }) => engineeringService.updateToolAsset(id, payload),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["tool-assets"] });
      setEditing(null);
      toast({ title: "Tool asset updated" });
    },
    onError: (err: any) =>
      toast({
        title: "Update failed",
  ...
  const deleteMutation = useMutation({
    mutationFn: engineeringService.deleteToolAsset,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["tool-assets"] });
      toast({ title: "Tool asset deleted" });
    },
    onError: (err: any) =>
      toast({
        title: "Delete failed",
  ...
  return (
      ...
      actions={
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="max-w-3xl">
            <DialogHeader>
              <DialogTitle>Create Tool Asset</DialogTitle>
            </DialogHeader>
            <ToolAssetForm
              plants={plants}
              locations={locations}
              vendors={vendors}
              onSubmit={(payload: any) => createMutation.mutate(payload)}
    ...
    >
            ...
            <CardContent className="space-y-4 p-5">
              ...
              <div className="flex flex-wrap gap-2">
                <SemanticBadge
                  kind="severity"
                  value="INFO"
                  label={row.asset_type.replaceAll("_", " ")}
                />
                <SemanticBadge
                  kind="severity"
                  value="LOW"
                  label={`${row.plant_name || "No plant"}`}
                />
              </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <div className="mt-1 font-bold text-content-1">
                      {row.location_name || "Not assigned"}
                    </div>
                  </div>
                  <div>
                    <div className="text-[10px] font-black uppercase tracking-[0.16em] text-content-4">
      ...
      >
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            <DialogTitle>Edit Tool Asset</DialogTitle>
          </DialogHeader>
          {editing ? (
            <ToolAssetForm
              initial={editing}
