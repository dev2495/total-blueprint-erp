...
export default function SalesDashboard() {
    ...
    const customerDistribution = Array.isArray(stats?.customer_distribution) ? stats.customer_distribution : []
    ...
    const forecast = stats?.forecast || { percentage: 0, status_text: "No target set" }
    ...
    return (
        <div className="space-y-5 pb-8">
            <section className="overflow-hidden rounded-[2rem] border border-slate-200/70 bg-[linear-gradient(135deg,#1e1b4b_0%,#4338ca_46%,#6366f1_100%)] px-5 py-5 text-white shadow-[0_24px_80px_-36px_rgba(49,46,129,0.55)] md:px-6">
                <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
                    ...
                    <div className="flex flex-wrap gap-2">
                        ...
                        <Button asChild variant="outline" className="h-11 rounded-full border-white/20 bg-white/10 px-5 text-sm font-black text-white hover:bg-white/15 hover:text-white">
            ...
            <section className="grid gap-5 xl:grid-cols-[1.15fr_0.85fr]">
                    ...
                    {trendData.length ? (
                        <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
                            ...
                            <div className="grid gap-3">
                                <InsetStat label="Latest order count" value={compactValue(trendData[trendData.length - 1]?.orders ?? 0)} hint="Orders created on the latest visible day" />
                                <InsetStat label="Peak daily weight" value={`${Math.max(...trendData.map((row: any) => Number(row.weight || 0))).toLocaleString("en-IN", { maximumFractionDigits: 1 })} KG`} hint="Highest visible day in the 30-day window" />
                                <InsetStat label="Forecast" value={`${Number(forecast.percentage || 0)}%`} hint={compactValue(forecast.status_text)} />
                            </div>
                        </div>
                    ) : (
                ...
                <PanelCard title="Customer and Status Mix" description="Where load is concentrated and which order states are dominant.">
                    <div className="grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
                            ...
                            {customerDistribution.length ? (
                                <>
                                    <div className="h-[210px]">
                                        <ResponsiveContainer width="100%" height="100%">
                                            <PieChart>
                                                <Pie data={customerDistribution} dataKey="value" nameKey="name" innerRadius={54} outerRadius={86} paddingAngle={3}>
                                                    {customerDistribution.map((_: any, index: number) => (
                                                        <Cell key={`customer-${index}`} fill={CHART_COLORS[index % CHART_COLORS.length]} />
                                                    ))}
                                                </Pie>
                                                <Tooltip />
                                            </PieChart>
                                    ...
                                    <div className="mt-2 space-y-2">
                                        {customerDistribution.slice(0, 4).map((row: any, index: number) => (
                                            <div key={row.name} className="flex items-center justify-between gap-3 text-xs font-semibold text-slate-600">
                                                ...
                                                <span>{Number(row.value || 0).toLocaleString("en-IN", { maximumFractionDigits: 1 })} KG</span>
                                            </div>
                                        ))}
                                    </div>
                                </>
                            ) : (
                                <EmptyState text="Customer mix will appear here once live sales volume is visible." compact />
                            )}
                        </div>
                        ...
                        <div className="rounded-[1.2rem] border border-slate-200 bg-slate-50/80 p-4">
                            {statusBreakdown.length ? (
                                ...
                                <EmptyState text="Status breakdown is not available yet." compact />
                            )}
                        </div>
                    </div>
                </PanelCard>
            ...
            <section className="grid gap-5 xl:grid-cols-[1.05fr_0.95fr]">
                ...
                >
                    <div className="space-y-3">
                        {recentOrders.length ? recentOrders.slice(0, 8).map((order: any, index: number) => (
                            <div key={`${order.id}-${index}`} className="flex flex-col gap-3 rounded-[1.2rem] border border-slate-200 bg-slate-50/80 p-4 md:flex-row md:items-center md:justify-between">
                                <div className="min-w-0">
                                    <div className="text-sm font-black text-slate-900">{order.order_number || order.id}</div>
                                    <div className="mt-1 text-xs font-semibold text-slate-500">{order.customer}</div>
                                </div>
                ...
                <div className="space-y-5">
                    <PanelCard title="Commercial Watchlist" description="Only actionable queue pressure and exceptions are shown here.">
                        <div className="space-y-3">
                            {alerts.length ? alerts.map((alert: any, index: number) => (
                                <div key={`${alert.type}-${index}`} className="flex items-start gap-3 rounded-[1.2rem] border border-slate-200 bg-slate-50/80 p-4">
                                    ...
                                    <div className="min-w-0 flex-1">
                                        ...
                                        <div className="mt-1 text-xs font-semibold leading-5 text-slate-500">{alert.description}</div>
                                    </div>
                                </div>
                            )) : (
                                <EmptyState text="No commercial alerts are open right now." compact />
                            )}
                        </div>
                    </PanelCard>
            ...
            <CardHeader className="space-y-2 pb-2">
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                    <div className="min-w-0">
                        ...
                        <div className="mt-1 text-sm font-medium leading-6 text-slate-500">{description}</div>
                    </div>
...
function EmptyState({ text, compact = false }: { text: string; compact?: boolean }) {
    return (
        <div className={cn(
            "rounded-[1.2rem] border border-dashed border-slate-200 bg-slate-50 text-center text-sm font-semibold text-slate-500",
