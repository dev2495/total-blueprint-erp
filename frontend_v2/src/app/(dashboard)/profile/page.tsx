...
export default function ProfilePage() {
  ...
  const submitProfileMutation = useMutation({
    mutationFn: (changes: Record<string, string>) =>
      systemUserService.createProfileChangeRequest(changes),
    onSuccess: async () => {
  ...
  const cancelMutation = useMutation({
    mutationFn: (requestId: string) =>
      systemUserService.cancelProfileChangeRequest(requestId),
    onSuccess: async () => {
      await queryClient.invalidateQueries({
        queryKey: ["my-profile-change-requests"],
      });
  ...
  const changePasswordMutation = useMutation({
    mutationFn: () =>
      systemUserService.changePassword(
  ...
  const profileCompleteness = useMemo(() => {
    const required = [
      normalize(draft.first_name),
      normalize(draft.last_name),
      normalize(draft.email),
    ];
  ...
  const submitProfileRequest = () => {
    ...
    const changedEntries = PROFILE_FIELDS.reduce<Record<string, string>>(
      (acc, field) => {
    ...
    if (!Object.keys(changedEntries).length) {
      toast({
        ...
        description: "Update at least one profile field before submitting.",
      });
      return;
    }
    if (!normalize(draft.email)) {
      toast({
        ...
        description: "Email is mandatory for all users.",
        variant: "destructive",
      });
      return;
    }
  ...
  const submitPasswordChange = () => {
    if (
      ...
      toast({
        ...
        description: "Fill current, new, and confirm password.",
        variant: "destructive",
      });
      return;
    }
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm text-content-3">
            Manage your personal details, security settings, and approval
            requests.
          </p>
        </div>
        <div className="flex items-center gap-2">
          {profileCompleteness.complete ? (
            ...
            <Badge variant="secondary">
              {profileCompleteness.done}/{profileCompleteness.total} required
              fields complete
            </Badge>
          )}
        ...
        <TabsContent value="profile">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Changes to profile fields go through admin approval before they
                become active.
              </CardDescription>
            </CardHeader>
        ...
        <TabsContent value="requests">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Track approval status for your submitted profile updates.
              </CardDescription>
            </CardHeader>
        ...
        <TabsContent value="security">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Update your account password instantly after current-password
                verification.
              </CardDescription>
            </CardHeader>
            <CardContent className="grid gap-4 md:grid-cols-2">
