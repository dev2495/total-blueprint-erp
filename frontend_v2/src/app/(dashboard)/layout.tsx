        ...
        return (
            <div className="min-h-screen w-full overflow-x-hidden erp-canvas">
                <div className="relative z-10 flex min-h-screen w-full flex-col px-3 pb-6 pt-4 sm:px-4 sm:pb-8 lg:px-6 lg:pb-10 xl:px-8">
                    <div className="h-20 rounded-3xl border border-white/80 bg-white/75 shadow-premium backdrop-blur-xl" />
                    <div className="mt-5 flex flex-1 gap-5">
                        <div className="hidden h-full w-12 rounded-full border border-white/70 bg-white/70 shadow-sm backdrop-blur-xl lg:block" />
                        <div className="min-w-0 flex-1 space-y-5">
                            <div className="h-14 rounded-3xl border border-white/70 bg-white/75 shadow-sm backdrop-blur-xl" />
                            <div className="h-44 rounded-3xl border border-white/70 bg-white/75 shadow-sm backdrop-blur-xl" />
                            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                                {Array.from({ length: 6 }).map((_, index) => (
                                    <div
                                        key={`dashboard-shell-skeleton-${index}`}
                                        className="h-36 rounded-2xl border border-white/70 bg-white/75 shadow-sm backdrop-blur-xl"
                                    />
                                ))}
                            </div>
                        </div>
