  ...
  const form = useForm({
    ...
    defaultValues: {
      ...
      include_in_official_reports:
        initialData?.include_in_official_reports ?? true,
      ...
      authorized_signatory_name:
        initialData?.legal_profile?.authorized_signatory_name || "",
      authorized_signatory_designation:
        initialData?.legal_profile?.authorized_signatory_designation || "",
      ...
      >
        <div className="space-y-4 overflow-y-auto pr-1">
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  ...
                  >
                    <SelectTrigger className="h-12 rounded-2xl border-line bg-surface-1">
                ...
                <div className="text-xs text-content-3">
                  Fallback group used only when work center, machine, and
                  template-step assignments do not override it.
                </div>
                <FormMessage />
              </FormItem>
            )}
            ...
            render={({ field }) => (
              <FormItem className="flex items-center justify-between rounded-xl border border-line bg-surface-2 px-4 py-3">
                <div>
                  ...
                  <div className="text-xs text-content-3">
                    Show this plant in official daily stock and production
                    packs.
                  </div>
                </div>
                <FormControl>
          ...
          <div className="border-t pt-2">
            <div className="mb-3 text-sm font-semibold text-content-2">
              Legal Profile (for Official DC)
            </div>
...
export default function PlantsPage() {
  ...
  const buildPlantPayload = (data: z.infer<typeof formSchema>) => ({
    ...
    name: data.name,
    default_cost_absorption_group:
      data.default_cost_absorption_group === "NONE"
        ? null
        : data.default_cost_absorption_group || null,
    ...
    legal_profile: {
      ...
      authorized_signatory_name:
        data.authorized_signatory_name || "Authorized Signatory",
      authorized_signatory_designation:
    ...
    onSuccess: async (createdPlant) => {
      queryClient.setQueryData<Plant[]>(["plants"], (current = []) => {
        const withoutExisting = current.filter(
          (plant) => plant.id !== createdPlant.id,
        );
  ...
  const updateMutation = useMutation({
    mutationFn: ({
      ...
      id: string;
      data: ReturnType<typeof buildPlantPayload>;
    }) => factoryService.updatePlant(id, data),
    onSuccess: () => {
          ...
          <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
            ...
            <DialogContent data-testid="plants-dialog" className="sm:max-w-2xl">
              ...
              <PlantForm
                ...
                onSubmit={(data) =>
                  createMutation.mutate(buildPlantPayload(data))
                }
            ...
            <CardContent className="pt-6">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-semibold text-lg text-content-1">
                      {plant.name}
                    </h3>
                    <div className="flex items-center text-xs text-content-3 mt-0.5">
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                      <Badge
                        variant={
                          plant.include_in_official_reports
                            ? "default"
                            : "outline"
                        }
                        className={
                      ...
                      >
                        {plant.include_in_official_reports
                          ? "Official report scope"
                          : "Internal-only scope"}
                      </Badge>
                      {plant.legal_profile?.legal_name ? (
                        <Badge
                          variant="outline"
                          className="text-[10px] border-success-border text-success-fg"
                        >
                        ...
                        <Badge
                          variant="outline"
                          className="text-[10px] border-warning-border text-warning-fg"
                        >
                      ...
                      >
                        {plant.default_cost_absorption_group_code ||
                          "No cost group"}
                      </Badge>
                    </div>
                  </div>
              ...
              <div className="grid grid-cols-3 gap-2 mt-4 pt-4 border-t border-line">
                <div className="text-center">
                  <div className="text-xs text-content-3 mb-1">Locations</div>
                  <Badge variant="secondary" className="bg-surface-2">
                    {plant.location_count || 0}
                  </Badge>
                </div>
                <div className="text-center border-l border-line">
                  <div className="text-xs text-content-3 mb-1">W/C</div>
                  <Badge variant="secondary" className="bg-surface-2">
                    {plant.work_center_count || 0}
                  </Badge>
                </div>
                <div className="text-center border-l border-line">
                  <div className="text-xs text-content-3 mb-1">Machines</div>
                  <Badge variant="secondary" className="bg-surface-2">
                    {plant.machine_count || 0}
                  </Badge>
                </div>
              </div>
            </CardContent>
