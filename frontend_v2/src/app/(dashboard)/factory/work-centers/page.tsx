  ...
  const form = useForm({
    ...
    defaultValues: {
      ...
      default_cost_absorption_group:
        initialData?.default_cost_absorption_group || "NONE",
    },
  });
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <div className="text-[0.8rem] text-muted-foreground">
                Used when machines in this work center do not override the
                costing group directly.
              </div>
              <FormMessage />
            </FormItem>
          )}
          ...
          render={() => (
            <FormItem>
              <div className="mb-4">
                ...
                <div className="text-[0.8rem] text-muted-foreground">
                  Select the processes this work center is capable of
                  performing.
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4 rounded-lg border p-4 max-h-[200px] overflow-y-auto bg-surface-2">
                {allProcesses.length > 0 ? (
                      ...
                      render={({ field }) => {
                        return (
                          ...
                          >
                            ...
                            <FormLabel className="text-sm font-medium cursor-pointer text-content-2">
                              {process.name}
                            </FormLabel>
                          </FormItem>
                        );
                  ...
                  <div className="text-sm text-content-4 col-span-2 text-center py-4">
...
export default function WorkCentersPage() {
  ...
  const createMutation = useMutation({
    mutationFn: (data: z.infer<typeof formSchema>) =>
      factoryService.createWorkCenter(data),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    ...
    }) => factoryService.updateWorkCenter(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["work-centers"] });
    ...
    <FactoryPageLayout
      ...
      actions={
        <div className="flex items-center gap-2">
          <div className="flex bg-surface-2 p-1 rounded-lg mr-2">
            <Button
              variant="ghost"
              size="sm"
              className={cn(
                "h-7 px-2",
                viewMode === "GRID" && "bg-surface-1 shadow-sm",
              )}
            ...
            <Button
              ...
              className={cn(
                "h-7 px-2",
                viewMode === "LIST" && "bg-surface-1 shadow-sm",
              )}
    ...
    >
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="w-full lg:w-64 flex-shrink-0">
          <div className="bg-surface-1 rounded-2xl shadow-sm border border-line p-4 sticky top-24">
            <h3 className="font-semibold text-sm text-content-1 mb-4 px-2">
              Plants
            </h3>
            <div className="space-y-1">
              ...
              >
                ...
                All Plants
                <span className="ml-auto text-xs opacity-60 bg-surface-1/50 px-1.5 py-0.5 rounded-full border border-info-border">
              ...
              {plants?.map((plant) => (
                ...
                >
                  ...
                  {plant.name}
                  <span className="ml-auto text-xs opacity-60 bg-surface-2 px-1.5 py-0.5 rounded-full">
                    {workCenters?.filter((wc) => wc.plant === plant.id)
                      .length || 0}
                  </span>
                </Button>
              ))}
          ...
          {viewMode === "GRID" ? (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {filteredWorkCenters.map((wc) => {
                ...
                const wcProcesses =
                  processes?.filter((p) => wc.processes?.includes(p.id)) || [];
                  ...
                  >
                    ...
                    <CardContent className="pt-6">
                      <div className="flex items-center gap-3 mb-4">
                        <div className="p-2 bg-info-bg text-primary rounded-lg">
                      ...
                      <div className="space-y-2">
                        <div className="text-xs font-medium text-content-3 uppercase tracking-wider">
                          Capabilities
                        </div>
                        <div className="flex flex-wrap gap-1.5">
                          {wcProcesses.length > 0 ? (
                            wcProcesses.slice(0, 3).map((p) => (
                              <Badge
                                key={p.id}
                                variant="secondary"
                                className="bg-surface-2 text-content-3 text-[10px] hover:bg-line"
                              >
                                {p.name}
                              </Badge>
                            ))
                          ...
                          {wcProcesses.length > 3 && (
                            <Badge
                              variant="secondary"
                              className="bg-surface-2 text-content-3 text-[10px]"
                            >
                              +{wcProcesses.length - 3}
                            </Badge>
                          )}
                      ...
                      <div className="mt-4 rounded-2xl border border-line bg-surface-2 px-3 py-3">
                        ...
                        <div className="mt-1 text-sm font-semibold text-content-2">
                          {wc.default_cost_absorption_group_code ||
            ...
            <div className="bg-surface-1 rounded-2xl shadow-sm border border-line overflow-hidden">
              <table className="w-full text-sm text-left">
                <thead className="bg-surface-2 text-content-3 font-medium">
                  <tr>
                ...
                <tbody className="divide-y divide-line">
                  {filteredWorkCenters.map((wc) => {
                    ...
                    const wcProcesses =
                      ...
                      [];
                    return (
                      <tr key={wc.id} className="hover:bg-surface-2">
                        <td className="p-4 font-mono text-content-3">
                          {wc.code}
                        </td>
                        <td className="p-4 font-medium text-content-1">
                          {wc.name}
                        </td>
                        <td className="p-4 text-content-3">{plant?.name}</td>
                        <td className="p-4 text-content-3">
                          {wc.default_cost_absorption_group_code ||
                            "Plant default"}
                        </td>
                        <td className="p-4">
