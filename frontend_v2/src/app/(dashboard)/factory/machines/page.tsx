...
function summarizeMachineErrors(
  errors: Record<string, { message?: string } | undefined>,
) {
  return Object.entries(errors)
    .map(
      ([field, value]) =>
        `${field.replace(/_/g, " ")}: ${value?.message || "Invalid value"}`,
    )
    .join(" | ");
}

function pickMachineFieldErrors(
  error: unknown,
): Partial<Record<keyof MachineFormValues, string>> {
  const fieldErrors = extractApiErrorMap(error);
  const allowed = new Set<keyof MachineFormValues>([
    "code",
    "name",
    "work_center",
...
function MachineForm({
  ...
  useEffect(() => {
    ...
    Object.entries(apiFieldErrors || {}).forEach(([field, message]) => {
      if (!message) return;
      form.setError(field as keyof MachineFormValues, {
        type: "server",
        message,
      });
    });
  ...
  const duplicateMachine = existingMachines.find(
    (machine) =>
          ...
          (data) => {
            ...
            if (duplicateMachine) {
              form.setError("code", {
                ...
                message:
                  duplicateMessage ||
                  "Machine code already exists in this work center.",
              });
              onInvalid?.(
                duplicateMessage ||
                  "Machine code already exists in this work center.",
            ...
            <AlertDescription className="space-y-3">
              ...
              {onOpenExisting ? (
                <Button
                  ...
                  variant="outline"
                  size="sm"
                  onClick={() => onOpenExisting(duplicateMachine)}
                >
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <div className="text-[0.8rem] text-muted-foreground">
                Machine assignment wins first in costing precedence. Leave empty
                to inherit from the work center.
              </div>
              <FormMessage />
            </FormItem>
          )}
...
const getStatusColor = (status: string) => {
  switch (status) {
    case "ACTIVE":
      return "bg-success-bg text-success-fg hover:bg-success-bg border-success-border";
    case "DOWN":
...
export default function MachinesPage() {
  ...
  const updateMutation = useMutation({
    ...
    }) => factoryService.updateMachine(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["machines"] });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (err: unknown) => {
      toast({
        title: "Machine delete failed",
        description: describeApiError(err, "Machine could not be deleted."),
        variant: "destructive",
      });
    },
  });
  ...
  const sortedMachines = [...(machines || [])].sort((left, right) => {
    const codeCompare = normalizeMachineCode(left.code).localeCompare(
      normalizeMachineCode(right.code),
    );
    if (codeCompare !== 0) return codeCompare;
    const workCenterCompare = String(left.work_center_name || "").localeCompare(
      String(right.work_center_name || ""),
    );
          ...
          return (
            ...
            >
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 bg-surface-2 border-b border-line">
                <Badge
                  variant="outline"
                  className={`bg-surface-1 border text-xs font-semibold px-2 py-0.5 rounded-full ${getStatusColor(machine.status)}`}
                >
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <h3 className="mt-2 font-semibold text-lg text-content-1 leading-tight">
                        {machine.name}
                      </h3>
                      ...
                      <div className="mt-2">
                        ...
                        >
                          {machine.cost_absorption_group_code ||
                            wc?.default_cost_absorption_group_code ||
                            "Inherited from work center"}
                        </Badge>
                      </div>
                    </div>
                  </div>
          ...
          {editingItem && (
            <MachineForm
              ...
              onInvalid={(message) => {
                setEditSubmitError(message);
                toast({
                  title: "Cannot update machine yet",
                  description: message,
                  variant: "destructive",
                });
              }}
