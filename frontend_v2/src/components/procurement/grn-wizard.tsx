  ...
  React.useEffect(() => {
    ...
    const openLines = po.items
      ...
      .map((it) => ({
        ...
        width_mm:
          it.expected_width_mm != null ? String(it.expected_width_mm) : "",
        thickness_micron:
          it.expected_thickness_micron != null
            ? String(it.expected_thickness_micron)
            : "",
        notes: "",
        rejection_reason: "",
      }));
      ...
      const payload = {
        ...
        lines: lines
          ...
          .map((l) => ({
            ...
            ...(l.material_category === "FILM_VARIANT" ||
            l.material_category === "POD"
              ? {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-surface-2 via-white to-surface-2 px-4 py-4 sm:px-6">
      <div className="mx-auto max-w-5xl space-y-5">
        ...
        {/* Stepper */}
        <div className="flex items-center justify-between rounded-xl border border-line bg-surface-1 px-4 py-3">
          {STEPS.map((s, idx) => (
            <div key={s.key} className="flex items-center gap-2">
              ...
              <div
                className={`text-xs ${idx === stepIdx ? "font-semibold text-brand-navy-500" : "text-content-3"}`}
              >
                {s.label}
              </div>
              {idx < STEPS.length - 1 && (
        ...
        {step === "vehicle" && (
          <div className="grid gap-4 rounded-xl border border-line bg-surface-1 p-5 shadow-sm sm:grid-cols-2">
        ...
        {step === "lines" && (
          <div className="overflow-hidden rounded-xl border border-line bg-surface-1 shadow-sm">
            <table className="w-full text-sm">
              ...
              <tbody>
                {lines.map((l, idx) => (
                  <tr key={l.po_item_id} className="border-t border-line">
                    ...
                    <td className="px-3 py-2 text-right tabular-nums text-content-3">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setLines((prev) =>
                              prev.map((p, i) =>
                                i === idx
                                  ? { ...p, thickness_micron: e.target.value }
                                  : p,
                              ),
                            )
                          }
