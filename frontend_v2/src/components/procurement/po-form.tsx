...
export function PurchaseOrderForm({ mode, initial }: Props) {
  ...
  const [paymentTerms, setPaymentTerms] = React.useState(
    initial?.payment_terms ?? "Net 30",
  );
  const [freightTerms, setFreightTerms] = React.useState(
    initial?.freight_terms ?? "",
  );
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      return procurementService.update(
        initial!.id,
        payload as Partial<PurchaseOrder>,
      );
    },
    onSuccess: (po) => {
  ...
  const canSave =
    ...
    !!plant &&
    items.length > 0 &&
    items.every((i) => i.material && parseFloat(i.qty_ordered) > 0);
      ...
      <div className="mx-auto max-w-6xl space-y-5">
        <div className="rounded-2xl border border-line bg-gradient-to-r from-brand-navy-500 to-brand-blue-500 p-5 text-white">
          <h1 className="text-xl font-semibold">
            {mode === "create"
              ? "New Purchase Order"
              : `Edit PO ${initial?.code ?? ""}`}
          </h1>
          <p className="mt-1 text-sm text-white/80">
            Create a draft PO. Save first; then send to vendor from the PO
            detail page.
          </p>
        </div>
        ...
        {/* Line items */}
        <div className="rounded-xl border border-line bg-surface-1 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-line p-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                size="sm"
                onClick={() =>
                  setItems((prev) => [...prev, emptyLine(prev.length + 1)])
                }
              >
          ...
          {(materials?.length ?? 0) > 0 &&
            (pickerSearch.length > 0 || materials!.length <= 6) && (
              <div className="border-b border-line bg-surface-2 p-2">
                <div className="text-[10px] uppercase text-content-3">
                  Suggestions
                </div>
                <div className="mt-1 flex flex-wrap gap-1">
                  {materials!.slice(0, 10).map((m) => (
                    <button
                      ...
                      onClick={() => {
                        setItems((prev) => [
                          ...prev,
                          {
                            ...
                            rate_per_uom: String(
                              (m as unknown as { last_purchase_rate?: number })
                      ...
                      className="rounded-full border border-line bg-surface-1 px-2 py-1 text-xs hover:border-brand-blue-500 hover:text-brand-blue-500"
                    >
                      {m.code} · {m.name}
                    </button>
                  ))}
              ...
              {items.map((it, idx) => {
                ...
                return (
                  <tr key={idx} className="border-t border-line align-top">
                    <td className="px-2 py-2 text-content-3">{idx + 1}</td>
                    <td className="px-2 py-2">
                      <div className="text-xs text-content-2">
                        {it.material_label || "(pick a material)"}
                      </div>
                      <Input
                    ...
                    <td className="px-2 py-2">
                      <Input
                        value={it.uom}
                        onChange={(e) =>
                          updateItem(setItems, idx, { uom: e.target.value })
                        }
                        className="h-8"
                      />
                    </td>
                    ...
                    <td className="px-2 py-2">
                      <Input
                        ...
                        value={it.gst_pct}
                        onChange={(e) =>
                          updateItem(setItems, idx, { gst_pct: e.target.value })
                        }
                        className="h-8 text-right"
                      />
                    </td>
                    ...
                    <td className="px-2 py-2 text-right">
                      <button
                        className="text-content-4 hover:text-danger-fg"
                        onClick={() =>
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          ...
          <div className="rounded-xl border border-line bg-surface-1 p-3 shadow-sm">
            ...
            <div className="flex justify-between text-sm">
              <span className="text-content-3">GST</span>
              <span className="tabular-nums">{formatINR(totals.gst)}</span>
            </div>
            <div className="mt-2 flex justify-between border-t border-line pt-2 text-base font-semibold text-brand-navy-500">
              <span>Grand Total</span>
              <span className="tabular-nums">
                INR {formatINR(totals.grand)}
              </span>
            </div>
          </div>
        </div>
