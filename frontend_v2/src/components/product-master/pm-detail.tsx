...
function layerRole(row: any, index: number): string {
  return String(
    row?.role || row?.layer_role || row?.layer || `layer-${index + 1}`,
  ).trim();
}
...
function normalizedMasterLayers(master: ProductMaster): any[] {
  const rows = Array.isArray((master as any)?.layer_template)
    ? (master as any).layer_template
    : [];
  return rows.map((row: any, index: number) =>
    normalizeLayerDisplay(row, index),
  );
}
...
export function PmDetailV37({ productId }: Props) {
    ...
    onSuccess: async (updated) => {
      ...
      toast({
        ...
        description: updated.active
          ...
          : "It remains reachable from Disabled / audit for historical orders.",
      });
    },
    onError: (err: any) => {
      toast({
  ...
  const _earlyIsProductionMaster =
    _earlyMasterKind === "PACKAGING" || _earlyMasterKind === "POD";
  const _earlyHiddenTabs: Set<TabKey> = _earlyIsProductionMaster
    ? new Set<TabKey>(["overlays"])
    : new Set<TabKey>();
  React.useEffect(() => {
  ...
  if (masterLoading) {
    return (
      <div className="flex h-[60vh] items-center justify-center text-sm text-content-3">
        <Loader2 className="mr-2 h-5 w-5 animate-spin text-content-4" /> Loading
        product master…
      </div>
    );
  }
    ...
    return (
      <div className="rounded-2xl border border-danger-border bg-danger-bg p-5 text-sm text-danger-fg">
        ...
        <div className="mt-1 text-xs">
          {describeApiError(masterError, "Check that the ID is correct.")}
        </div>
        <Button
          onClick={() => router.push("/master/products")}
          className="mt-3 rounded-xl bg-danger-solid hover:bg-danger-solid text-white"
          size="sm"
        >
          Back to list
        </Button>
  ...
  const templateName =
    ...
    templates.find((t: any) => t.id === master.template)?.name ||
    master.template_name ||
    "—";
  const counts = {
...
function TopBar({
  ...
  return (
    <header className="relative flex flex-wrap items-center justify-between gap-3 overflow-hidden rounded-2xl border border-order-border bg-gradient-to-r from-white via-order-bg to-order-bg px-5 py-3 shadow-sm ring-1 ring-surface-1/40 backdrop-blur">
      ...
      <div className="relative flex items-center gap-3 min-w-0 pl-2">
        <Link
          ...
          className="flex h-9 w-9 items-center justify-center rounded-xl bg-surface-1/70 text-order-fg ring-1 ring-order-border hover:bg-surface-1 hover:text-order-fg hover:ring-order-border transition"
          title="Back to list"
        >
      ...
      <div className="relative flex items-center gap-2 flex-wrap">
        ...
        >
          <span
            className={cn(
              ...
              master.active ? "bg-success-fg animate-pulse" : "bg-danger-solid",
            )}
          />
        ...
        <button
          onClick={onClone}
          className="inline-flex h-9 items-center gap-1.5 rounded-xl bg-surface-1/80 px-3 text-[11px] font-bold text-content-2 ring-1 ring-line hover:bg-surface-1 hover:ring-order-border hover:text-order-fg transition"
        >
        ...
        <button
          ...
          disabled={isToggling}
          className={cn(
            "inline-flex h-9 items-center gap-1.5 rounded-xl bg-surface-1/80 px-3 text-[11px] font-bold ring-1 transition disabled:opacity-60",
            master.active
              ? "text-warning-fg ring-warning-border hover:bg-surface-1 hover:ring-warning-border"
              : "text-success-fg ring-success-border hover:bg-surface-1 hover:ring-success-border",
          )}
        >
          {isToggling ? (
...
const TAB_TONES: Record<
  TabKey,
  { active: string; hover: string; chip: string }
> = {
  overview: {
    active:
      "bg-gradient-to-r from-order-fg via-order-fg to-order-fg text-white shadow-md ring-1 ring-order-border",
    hover: "text-content-3 hover:bg-order-bg hover:text-order-fg",
    chip: "bg-surface-1/25 text-white",
  },
  layer: {
    active:
      "bg-gradient-to-r from-primary to-info-fg text-white shadow-md ring-1 ring-info-border",
    hover: "text-content-3 hover:bg-info-bg hover:text-primary",
    chip: "bg-surface-1/25 text-white",
  },
  variants: {
    active:
      "bg-gradient-to-r from-order-fg to-purple-600 text-white shadow-md ring-1 ring-order-border",
    hover: "text-content-3 hover:bg-order-bg hover:text-order-fg",
    chip: "bg-surface-1/25 text-white",
  },
  sizes: {
    active:
      "bg-gradient-to-r from-success-fg to-info-fg text-white shadow-md ring-1 ring-success-border",
    hover: "text-content-3 hover:bg-success-bg hover:text-success-fg",
    chip: "bg-surface-1/25 text-white",
  },
  overlays: {
    active:
      "bg-gradient-to-r from-warning-fg to-warm text-white shadow-md ring-1 ring-warning-border",
    hover: "text-content-3 hover:bg-warning-bg hover:text-warning-fg",
    chip: "bg-surface-1/25 text-white",
  },
  artworks: {
    active:
      "bg-gradient-to-r from-order-fg to-danger-solid text-white shadow-md ring-1 ring-order-border",
    hover: "text-content-3 hover:bg-order-bg hover:text-order-fg",
    chip: "bg-surface-1/25 text-white",
  },
  audit: {
    active:
      "bg-gradient-to-r from-surface-3 to-surface-3 text-white shadow-md ring-1 ring-line-strong",
    hover: "text-content-3 hover:bg-surface-2 hover:text-content-1",
    chip: "bg-surface-1/25 text-white",
  },
};
...
function Tabs({
  ...
  hiddenTabs?: Set<TabKey>;
}) {
  const visible = TABS.filter((t) => !hiddenTabs?.has(t.id));
  return (
    <div className="rounded-2xl border border-line bg-surface-1/80 p-1.5 shadow-sm backdrop-blur overflow-x-auto ring-1 ring-surface-1/40">
...
function OverviewTab({
  ...
  overlays: CustomerProductOverlay[];
  routeInfo: any;
  templateName: string;
  artworks: Artwork[];
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="grid grid-cols-1 gap-3 xl:grid-cols-[minmax(0,1fr)_360px]">
        <div className="relative overflow-hidden rounded-3xl border border-order-border bg-gradient-to-br from-order-bg via-white to-order-bg px-6 py-5 shadow-lg ring-1 ring-surface-1/40">
              ...
              <FactCell
                ...
                value={
                  <span className="font-bold text-order-fg">
                    {master.default_reporting_group} · {master.product_kind}
                  </span>
                }
              />
              <FactCell
                ...
                tone="emerald"
                value={
                  <span className="font-bold text-success-fg capitalize">
                    {master.reusable_policy.toLowerCase()}
                  </span>
                }
              />
              <FactCell
                ...
                tone="fuchsia"
                value={
                  <span className="font-bold text-order-fg truncate">
                    {templateName}
                  </span>
                }
              />
        ...
        <div className="grid grid-cols-2 gap-2.5">
          <KpiTile
            ...
            sub={`${variants.filter((v) => v.active).length} active`}
            icon="🌀"
            tone="violet"
          />
          <KpiTile
            ...
            sub={
              sizes.length
                ? `${sizes[0].code} → ${sizes[sizes.length - 1].code}`
                : "—"
            }
            icon="📐"
            tone="emerald"
          ...
          <KpiTile
            ...
            sub={`${overlays.filter((o) => o.active).length} active`}
            icon="👥"
            tone="amber"
          />
          <KpiTile
        ...
        <div className="space-y-4">
          ...
          {(() => {
            ...
            if (isProductionMaster) {
              return (
                <ProductionCatalogMapCard master={master} variants={variants} />
              );
            }
            return (
              <PackagingContractCard
                master={master}
                packagingMaterials={packagingMaterials}
                podVariants={podVariants}
              />
...
const FACT_TONES: Record<string, { bg: string; ring: string; label: string }> =
  {
    indigo: {
      bg: "bg-gradient-to-br from-white to-order-bg",
    ...
    emerald: {
      bg: "bg-gradient-to-br from-white to-success-bg",
...
function FactCell({
  ...
  return (
    <div
      className={cn(
        "rounded-xl px-3 py-2 ring-1 shadow-sm backdrop-blur",
        t.bg,
        t.ring,
      )}
    >
...
> = {
  ...
  fuchsia: {
    ...
    value: "text-order-fg",
    sub: "text-order-fg",
    stripe: "bg-gradient-to-r from-order-fg to-danger-solid",
  },
  indigo: {
    ...
    >
      ...
      <div className="flex items-start justify-between">
        ...
        {icon ? (
          <span
            className={cn(
              "flex h-7 w-7 items-center justify-center rounded-xl text-sm",
              t.iconBg,
            )}
          >
            {icon}
      ...
      {sub ? (
        <div className={cn("text-[10px] font-bold truncate", t.sub)}>{sub}</div>
      ) : null}
    </div>
  );
}
  ...
  return (
    <section className="rounded-2xl border border-line bg-surface-1 shadow-sm overflow-hidden">
      <header className="flex items-center justify-between border-b border-line px-5 py-3">
        <div className="flex items-center gap-2">
          <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-info-bg text-primary">
          ...
          <div>
            ...
            <div className="text-[10px] text-content-3">
              {layers.length} layer{layers.length === 1 ? "" : "s"} · identity
              fixed · thickness/grade configurable per variant
            </div>
          </div>
        </div>
      </header>
      {layers.length === 0 ? (
        <div className="p-6 text-center text-xs text-content-4 italic">
          No layers defined yet.
        </div>
      ) : (
        <div className="p-5 space-y-2">
          {layers.map((l: any, i: number) => {
            const gradeOptions: string[] = Array.isArray(l.grade_options)
              ? l.grade_options
              : [];
            const variable =
              gradeOptions.length > 1 || l.thickness_apportion === "variable";
            return (
              <div
                key={i}
                className={cn(
              ...
              >
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span
                    className={cn(
                      "font-mono text-[11px]",
                      variable ? "text-success-fg" : "text-content-3",
                    )}
                  >
                ...
                <div className="grid grid-cols-3 gap-2 text-[11px]">
                  ...
                  <FieldSlot
                    label={gradeOptions.length > 1 ? "Grade options" : "Grade"}
                  >
    ...
    <section className="rounded-2xl border border-line bg-surface-1 shadow-sm overflow-hidden">
      <header className="flex items-center justify-between border-b border-line px-5 py-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="font-display text-sm font-bold text-content-1">
              Adhesive & solvent defaults
            </h3>
            <div className="text-[10px] text-content-3">
              Fixed master chemistry · one selected material each · GSM included
              in live BOM
            </div>
          </div>
        </div>
...
function VariantAxesCard({
  ...
  const axes = (master.variant_axes || []).filter((a) => {
    ...
    if (isProductionMaster && PRODUCTION_MASTER_CATALOG_AXES.has(key))
      return false;
    if (
      key === "packaging_outer" ||
      (key === "packaging_inner" && kind !== "POUCH")
    )
      return false;
    ...
    <section className="rounded-2xl border border-line bg-surface-1 shadow-sm overflow-hidden">
      <header className="flex items-center justify-between border-b border-line px-5 py-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="font-display text-sm font-bold text-content-1">
              Variant axes · {axes.length} axes
            </h3>
            <div className="text-[10px] text-content-3">
              {isProductionMaster
                ? "These axes define produced variants; catalog SKU is linked manually after variant creation"
                : "Each axis becomes one dropdown sales/planner sees while configuring"}
            </div>
          </div>
        </div>
      </header>
...
function resolveAxisValues(
      ...
      ? explicit
          .map((o) => {
            if (typeof o === "string" || typeof o === "number")
              return String(o);
            return String(
              o?.code || o?.material_code || o?.value || o?.id || "",
            );
          })
          .filter(Boolean)
  ...
  if (axisName === "size") {
    return sizes
      ...
      .map((s) => ({
        code: s.code,
        label: `${s.code} · ${s.width_mm}×${s.height_mm || 0}`,
      }));
  }
  ...
  if (axisName === "layer_thicknesses") {
    ...
    return Array.from(set)
      .sort((a, b) => a - b)
      .map((t) => ({ code: `${t}`, label: `${t}μ` }));
  }
  if (axisName === "layer_grades") {
    const set = new Set<string>();
    for (const l of master.layer_template || []) {
      const opts: string[] = Array.isArray((l as any).grade_options)
        ? (l as any).grade_options
        : [];
  ...
  if (catalogSource) {
    ...
    if (catalogSource === "packaging_material") {
      ...
      return packagingMaterials
        ...
        .filter(
          (m) =>
            !explicitAllowed.size || explicitAllowed.has(normalizeCode(m.code)),
        )
        .filter((m) => packagingMaterialAllowedForAxis(m, axis, master))
        .filter((m) => {
          const kind = String(m.packaging_kind || "").toUpperCase();
          return Array.isArray(kindFilters)
            ? !kindFilters.length || kindFilters.includes(kind)
            : !kindFilters || kind === kindFilters;
        })
        .map((m) => ({
          code: m.code,
          label: `${m.code}${m.name && m.name !== m.code ? ` · ${m.name}` : ""}`,
        }));
    }
    if (catalogSource === "pod_sku_variant") {
      return podVariants
        ...
        .filter(
          (v: any) =>
            !explicitAllowed.size || explicitAllowed.has(normalizeCode(v.code)),
        )
        .map((v: any) => ({
          code: v.code,
          label: `${v.code}${v.name && v.name !== v.code ? ` · ${v.name}` : ""}`,
        }));
    }
    if (catalogSource === "addon") {
      return addons
        ...
        .filter(
          (a: any) =>
            !explicitAllowed.size || explicitAllowed.has(normalizeCode(a.code)),
        )
        .map((a: any) => ({
          code: a.code,
          label: `${a.code}${a.name && a.name !== a.code ? ` · ${a.name}` : ""}`,
        }));
    }
  }
...
function AxisCard({
  ...
  const accent = isCatalog
    ? "border-order-border bg-order-bg"
    : isSize
      ? "border-info-border bg-info-bg"
      : "border-line bg-surface-2";
  const eyebrowTone = isCatalog
    ? "text-order-fg"
    : isSize
      ? "text-primary"
      : "text-content-2";
  const chipTone = isCatalog
    ...
    : isSize
      ...
      : "bg-order-bg text-order-fg ring-order-border";
  const allowed = resolveAxisValues(
    axis,
    master,
    sizes,
    packagingMaterials,
  ...
  return (
    <div className={cn("rounded-xl border p-3", accent)}>
      <div className="flex items-center justify-between mb-1">
        <div className="font-bold text-sm text-content-1">
          {axis.label || axis.axis}
          <span
            className={cn("text-[10px] font-bold uppercase ml-1", eyebrowTone)}
          >
            {prettyAxisType(axis)}
          </span>
        </div>
      ...
      {description ? (
        <div
          className={cn(
            "text-[11px]",
            isCatalog ? "text-order-fg" : "text-content-3",
          )}
        >
          {description}
        </div>
      ...
      {axis.default_value ? (
        <div className="mt-1 text-[10px] font-mono font-bold text-success-fg">
          default · {axis.default_value}
        </div>
      ) : null}
      {axis.qty_per_pcs ? (
        <div className="mt-1 text-[10px] font-mono text-content-3">
          qty · {axis.qty_per_pcs} per pc
        </div>
      ) : null}
      ...
      {visible.length > 0 ? (
        <div className="mt-2 flex flex-wrap gap-1">
          {visible.map((opt) => (
            <span
              key={opt.code}
              className={cn(
                "rounded px-1.5 py-0.5 text-[10px] font-mono font-bold ring-1",
                chipTone,
              )}
              title={opt.label}
            >
        ...
        <div className="mt-2 text-[10px] italic text-content-4">
          No allowed values configured yet.
        </div>
      ) : null}
      {isCatalog && allowed.length > 0 ? (
        <div className="mt-2 text-[10px] font-bold text-order-fg">
          {allowed.length} allowed · auto-demand on shortage
        </div>
      ) : null}
    </div>
  );
...
function axisDescription(axis: VariantAxisDef): string {
  ...
  if (name === "size")
    return "Drives W×H + gusset. Values come from the Sizes sub-section.";
  if (name === "layer_thicknesses")
    return "Per-layer thickness. Fixed for fixed layers, picked per variant for variable layers.";
  if (name === "layer_grades")
    return "Per-layer grade. Picked from the layer's grade options.";
  if (name === "layer_widths")
    return "Per-layer roll width override. Defaults to auto-calculated width.";
  if (name === "addons")
    return "Multi-select addons applied per piece (e.g. zipper, valve, spout).";
  if (name === "artwork_mode")
    return "How artwork is chosen — auto, customer overlay, or deferred.";
...
function packagingMaterialAllowedForAxis(
...
function overlayPackagingLabel(axis: VariantAxisDef, master: ProductMaster) {
  ...
  if (role === "outer")
    return normalizeCode(master.product_kind) === "ROLL"
      ? "Default roll sheet"
      : "Default outer / sheet";
  if (role === "other") return "Default EOD / other packing";
  if (normalizeCode(master.product_kind) === "ROLL")
    return "Default roll sheet";
...
function buildOverlayPackingRecipe(
  ...
  packagingAxes.forEach((axis) => {
    ...
    const material = packagingMaterials.find(
      (row) => normalizeCode(row.code) === normalizeCode(code),
    );
...
function overlayPackagingRole(
  axis: VariantAxisDef,
  material: PackagingMaterial | undefined,
  master: ProductMaster,
) {
  ...
  if (
    axisRole === "outer" ||
    ["GONNY", "SHEET", "OUTER_BAG", "BOX"].includes(kind)
  )
    return kind === "SHEET" && product === "ROLL"
      ? "ROLL_DISPATCH"
      : "FINAL_GUNNY";
  return "EXTRA";
}

function overlayPackagingLine(
  material: PackagingMaterial | undefined,
  code: string,
  role: string,
) {
  ...
  const kind = packagingKind(material);
  const pcsPerPack = Number(
    defaults.pcs_per_pack || defaults.pcs_per_carton || 0,
  );
  ...
  return {
    ...
    basis,
    qty: Number(
      defaults.qty || defaults.target_qty || (basis === "PER_ROLL" ? 1 : 0),
    ),
    uom: material?.base_uom || "PCS",
    pcs_per_pack: pcsPerPack || undefined,
    kg_per_pack: kgPerPack || undefined,
    inners_per_outer:
      Number(defaults.inners_per_outer || defaults.inners_per_gunny || 0) ||
      undefined,
    packaging_kind: material?.packaging_kind,
...
function RoutePreviewCard({
  ...
  const toneFor = (name?: string, code?: string) => {
    ...
    return (
      STEP_TONE[key] ||
      Object.entries(STEP_TONE).find(([k]) => key.includes(k))?.[1] ||
      STEP_TONE.DEFAULT
    );
  };
  return (
    <section className="rounded-2xl border border-line bg-surface-1 shadow-sm overflow-hidden">
      <header className="flex items-center justify-between border-b border-line px-5 py-3">
        <div className="flex items-center gap-2">
          <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-order-fg to-order-fg text-white shadow-sm">
            ⇆
          </span>
          <div>
            <h3 className="font-display text-sm font-bold text-content-1">
              Live route ·{" "}
              {templateName || master.template_name || "live template"}
            </h3>
            <div className="text-[10px] text-content-3">
              Process names come from the live template · highlights the
              artwork-bearing step
            </div>
          </div>
        </div>
      ...
      <div className="p-5">
        {steps.length === 0 ? (
          <div className="rounded-xl border border-dashed border-line bg-surface-2 p-4 text-center text-xs text-content-3">
            No route steps published yet — link a LIVE template to surface the
            production route.
          </div>
        ) : (
            ...
            {steps.map((s, i) => {
              ...
              return (
                <React.Fragment key={`route-${s.index}-${i}`}>
                  ...
                  <div
                    className={cn(
                      "flex h-14 min-w-[120px] flex-col items-start justify-center rounded-xl px-3 ring-1",
                      tone.bg,
                      tone.ring,
                    )}
                  >
                    ...
                    <div className="mt-0.5 text-[10px] font-mono text-content-3">
                      {s.transition || "—"}
                    </div>
                  </div>
                </React.Fragment>
...
function RecentVariantsCard({
  ...
  const totalStockQty = isPackOrPod
    ? variants.reduce(
        (sum, v) => sum + Number(v.inventory_link?.stock_qty || 0),
        0,
      )
    : 0;
  const linkedVariants = isPackOrPod
    ? variants.filter((v) => v.inventory_link).length
    : 0;
  return (
    <section className="rounded-2xl border border-line bg-surface-1 shadow-sm overflow-hidden">
      <header className="flex items-center justify-between border-b border-line px-5 py-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="font-display text-sm font-bold text-content-1">
              Recent variants · {Math.min(recent.length, variants.length)} of{" "}
              {variants.length} shown
            </h3>
            <div className="text-[10px] text-content-3">
              Open Variants tab for matrix / cards / table views
            </div>
          </div>
        </div>
        {isPackOrPod ? (
          <div className="flex items-center gap-2 text-[10px]">
            <span className="rounded-full bg-success-bg px-2 py-0.5 font-bold text-success-fg ring-1 ring-success-border">
              {linkedVariants}/{variants.length} linked
            </span>
            <span className="rounded-full bg-order-bg px-2 py-0.5 font-bold text-order-fg ring-1 ring-order-border">
      ...
      {recent.length === 0 ? (
        <div className="p-6 text-center text-xs text-content-4 italic">
          No variants yet — they appear as sales/planner configures axis tuples.
        </div>
      ) : (
...
function VariantPeekCard({
  ...
  const axisEntries = Object.entries(variant.axis_values || {})
    ...
    .slice(0, 3);
  const unitG = Number(
    (variant as any).geometry_snapshot?.unit_weight_g ||
      (variant as any).unit_weight_g ||
      0,
  );
  const layers = (variant as any).layer_snapshot;
  const matCount = Array.isArray(layers)
    ? layers.length
  ...
  const isPackOrPod = kind === "PACKAGING" || kind === "POD";
  return (
    <div className="rounded-xl border border-line bg-surface-1 p-3 hover:border-order-border hover:shadow-md">
      <div className="flex items-center justify-between">
        <span className="font-mono text-[11px] font-bold text-order-fg truncate">
          {variant.code}
        </span>
        <span
          className={cn(
      ...
      <div className="mt-1 flex flex-wrap gap-1">
        {axisEntries.map(([k, vv]) => {
          const val =
            typeof vv === "object"
              ? (vv as any).code || (vv as any).value || JSON.stringify(vv)
              : String(vv);
          return (
            <span
              key={k}
              className="rounded bg-info-bg text-primary ring-1 ring-info-border px-1.5 py-0.5 text-[10px] font-mono"
            >
              {val}
            </span>
          );
      ...
      <div className="mt-2 font-mono text-[10px] text-content-2">
        {unitG > 0 ? (
          <>
            <span className="font-bold">{unitG}g</span> per pouch
          </>
        ) : (
          "—"
        )}
        <span className="ml-2 text-content-3">
          · {matCount} layer{matCount === 1 ? "" : "s"}
        </span>
      </div>
      {/* ── Inventory linkage · PACKAGING + POD masters only ──── */}
      {isPackOrPod ? (
        <VariantInventoryLinkPanel
          variant={variant}
          master={master}
          link={link}
        />
 ...
 */
function VariantInventoryLinkPanel({
  variant,
  master,
  link,
}: {
  ...
  const fixedFgType = String(
    (master.fixed_attributes as any)?.fg_type || "",
  ).toUpperCase();
  const expectedPackagingKind =
    kind === "PACKAGING"
      ? String(master.packaging_kind || "").toUpperCase() ||
        (fixedFgType === "ROLL" ? "SHEET" : "INNER_POUCH")
      : "";
  ...
  const { data: pool = [] } = useQuery({
    ...
    queryFn: () =>
      expectedCategory === "PACKAGING"
        ...
        : masterDataService
            .getPodSkuVariants({ active: true })
            .then((rows: PodSkuVariant[]) =>
              rows.map((row) => {
  ...
  const filtered = (pool as any[])
    .filter((m) => {
      if (
        String(m.status || m.material_status || "ACTIVE").toUpperCase() ===
        "INACTIVE"
      )
        return false;
      ...
      if (expectedCategory === "PACKAGING") {
        ...
        if (expectedPackagingKind && rowKind !== expectedPackagingKind)
          return false;
      }
  ...
  const linkMut = useMutation({
    mutationFn: (payload: {
      ...
      productMasterService.linkVariantInventory(
        master.id,
        variant.id,
        payload.inventoryMaterialId,
        payload.podSkuVariantId,
      ),
    onSuccess: () => {
      toast({ title: "Catalog link updated", description: variant.code });
      queryClient.invalidateQueries({
        queryKey: ["product-master-variants", master.id],
      });
      setPickerOpen(false);
    },
    onError: (err: any) => {
  ...
  return (
    <>
      {link ? (
        ...
        <div className="mt-2 flex items-center justify-between gap-2 rounded-lg bg-warning-bg px-2 py-1.5 ring-1 ring-warning-border text-[10px] text-warning-fg">
          <span>
            <span className="font-bold">No catalog link.</span> Pick an existing
            fixed SKU.
          </span>
          <button
            type="button"
        ...
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription className="text-[11px]">
              Pick an unlinked fixed SKU in{" "}
              <strong>
                {expectedCategory === "PACKAGING"
                  ? "/master/packaging"
                  : "/master/pod"}
              </strong>
              , or keep the current link.{" "}
              {expectedPackagingKind
                ? `This master only accepts ${expectedPackagingKind.replaceAll("_", " ")} rows. `
                : ""}
              No SKU is created automatically.
            </DialogDescription>
          </DialogHeader>
          <Input
            value={picker}
          ...
          <div className="max-h-[300px] overflow-y-auto space-y-1">
            {filtered.length === 0 ? (
              <div className="rounded-xl border border-dashed border-line bg-surface-2 p-3 text-center text-xs text-content-3">
              ...
              filtered.map((m: any) => (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    const podSkuVariantId = m.pod_sku_variant_id
                      ? String(m.pod_sku_variant_id)
                      : null;
                    linkMut.mutate({
                      inventoryMaterialId: podSkuVariantId
                        ? null
                        : String(m.id),
                      podSkuVariantId,
                    });
                  ...
                  className={cn(
                    "flex w-full items-center justify-between gap-2 rounded-lg border px-3 py-2 text-left hover:border-success-border hover:bg-success-bg",
                    link?.id === m.id
                      ? "border-success-border bg-success-bg ring-1 ring-success-border"
                ...
                >
                  ...
                  <div className="text-right text-[10px] text-content-3">
                    {m.packaging_kind || m.pod_sku_code || m.pod_type || ""}
                    <br />
...
function RecentActivityCard({
  ...
  items.push({
    icon: "+",
    bg: "bg-success-bg text-success-fg",
    title: `${master.name} created`,
    sub: master.created_at ? formatRelative(master.created_at) : "—",
  });
  if (master.updated_at && master.updated_at !== master.created_at) {
    items.push({
      icon: "✎",
      bg: "bg-warning-bg text-warning-fg",
      title: "Master updated",
      sub: formatRelative(master.updated_at),
    });
  }
  if (routeInfo?.route_steps?.length) {
    items.push({
      icon: "↻",
      bg: "bg-surface-2 text-content-2",
      title: `Route · ${routeInfo.route_steps.length} steps`,
      sub: master.template_name || "live template",
    });
  }
  return (
        ...
        {items.map((it, i) => (
          <li key={i} className="flex gap-2">
            ...
            <div>
              ...
              <div className="text-[10px] text-content-3">{it.sub}</div>
            </div>
          </li>
        ))}
      </ol>
      ...
      <div className="p-4 space-y-2 text-[11px]">
        {!isProductionMaster ? (
          <Link
            ...
            className="flex items-center justify-between rounded-lg bg-surface-2 hover:bg-surface-2 px-3 py-2 font-bold text-content-2"
          >
          ...
          <Link
            ...
            className="flex items-center justify-between rounded-lg bg-gradient-to-r from-order-bg to-order-bg hover:from-order-bg hover:to-order-bg px-3 py-2 font-bold text-order-fg ring-1 ring-order-border"
          >
            Open catalog ·{" "}
            {kind === "PACKAGING" ? "/master/packaging" : "/master/pod"}{" "}
        ...
        >
          {isProductionMaster
            ? "Launch in-house production"
            : "Launch planner stock"}{" "}
          <ArrowRight className="h-3.5 w-3.5" />
        </Link>
        <Link
          href="/engineering/routing"
          className="flex items-center justify-between rounded-lg bg-surface-2 hover:bg-surface-2 px-3 py-2 font-bold text-content-2"
        >
          Open Routing studio <ArrowRight className="h-3.5 w-3.5" />
        </Link>
        <button className="flex w-full items-center justify-between rounded-lg bg-surface-2 hover:bg-surface-2 px-3 py-2 font-bold text-content-2">
          <span className="flex items-center gap-2">
            <Printer className="h-3.5 w-3.5" /> Print spec sheet
          </span>
        </button>
      </div>
    </section>
    ...
    <section className="rounded-2xl border border-line bg-surface-1 shadow-sm">
      ...
      <div className="p-4 space-y-2 text-[11px]">
        ...
        <Row
          ...
          value={`${variants.filter((v) => v.active).length} / ${variants.length}`}
        />
        <Row
          label="Invariant"
          value={
            master.invariant_signature ? (
              <span className="font-mono text-[10px] text-content-2 truncate">
                {master.invariant_signature.slice(0, 12)}…
              </span>
            ) : (
              "—"
            )
...
function Row({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="flex items-center justify-between gap-2">
      ...
      <span className="font-bold text-content-2 truncate text-right">
        {value}
      </span>
    </div>
  );
...
function LayerTab({ master }: { master: ProductMaster }) {
  return (
    <div className="space-y-4">
      ...
      />
      <LayerTemplateCard master={master} />
      <div className="relative overflow-hidden rounded-2xl border border-info-border bg-gradient-to-br from-info-bg via-white to-info-bg p-5 text-[12px] text-primary shadow-sm">
        <div
          aria-hidden
          className="pointer-events-none absolute -top-12 -right-12 h-40 w-40 rounded-full bg-info-bg blur-3xl"
        />
        <div className="relative flex items-start gap-3">
          <span className="flex h-9 w-9 flex-none items-center justify-center rounded-xl bg-gradient-to-br from-primary to-info-fg text-white shadow-md">
          ...
          <div>
            ...
            <p className="leading-relaxed text-primary">
              Layer identity (film) is fixed. Thickness is fixed unless declared
              variable. Grade options apply only to extruded layers; purchased
              films skip grade input. Per-layer values for variable layers are
              picked while creating an order.
            </p>
          </div>
        </div>
      </div>
...
function TabBanner({
  ...
  return (
    <section
      className={cn(
        "relative overflow-hidden rounded-2xl border px-5 py-4 shadow-sm ring-1 ring-surface-1/40",
        t.wrap,
      )}
    >
      <div
      ...
      <div className="relative flex items-start justify-between gap-3 pl-2">
        <div className="flex items-start gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            {subtitle ? (
              <div className={cn("mt-0.5 text-[11px]", t.subtitle)}>
                {subtitle}
              </div>
            ) : null}
          </div>
...
> {
  ...
  componentDidCatch(error: any) {
    if (typeof console !== "undefined")
      console.error("MaterialBreakdown error:", error);
  }
  render() {
...
function MaterialBreakdownSection({
  master,
  variants,
  sizes,
}: {
  ...
  const isPackOrPod = kind === "PACKAGING" || kind === "POD";
  return (
    <section className="rounded-2xl border border-success-border bg-gradient-to-br from-white via-success-bg to-success-bg shadow-sm overflow-hidden ring-1 ring-surface-1/40">
      <header className="flex items-center justify-between border-b border-success-border px-5 py-3">
        <div className="flex items-center gap-2">
          <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-success-bg text-success-fg">
            📊
          </span>
          <div>
            <h3 className="font-display text-sm font-bold text-content-1">
              Material breakdown · top {top.length} variant
              {top.length === 1 ? "" : "s"}
            </h3>
            <p className="text-[10px] text-content-3">
              Per-variant layer table · pouch math · roll math · live stock chip
            </p>
          </div>
        </div>
...
function MaterialBreakdownCard({
  ...
  const layers: any[] =
    Array.isArray(layerSnapshot) && layerSnapshot.length
      ? layerSnapshot.map((row: any, index: number) =>
          normalizeLayerDisplay(row, index),
        )
  ...
  const unitG = Number(
    geom?.unit_weight_g || (variant as any)?.unit_weight_g || 0,
  );
  const sizeRow =
    safeSizes.find((s) => String(axisValues?.size || "") === s?.code) ||
    safeSizes[0];
  ...
  const expectedJumbos =
    unitG > 0 ? Math.max(1, Math.ceil(standardOrderKg / 50)) : 0;
  const layerToneList = [
    "bg-order-bg text-order-fg border-order-border",
    "bg-order-bg text-order-fg border-order-border",
  ...
  return (
    <div className="rounded-2xl border border-line bg-surface-1 shadow-sm overflow-hidden">
      <header className="flex items-center justify-between border-b border-line bg-surface-2 px-4 py-2.5">
        ...
        <div className="flex items-center gap-1 text-[10px] text-content-3">
          <span>{totalThickness} µ total</span>
          {unitG > 0 ? (
            <>
              <span>·</span>
              <span>{unitG.toFixed(2)} g/pouch</span>
            </>
          ) : null}
        </div>
      </header>
      ...
      <div className="px-4 py-3">
        ...
        <div className="space-y-1.5">
          {layers.map((l, idx) => {
            ...
            const widthPct =
              totalThickness > 0
                ? Math.max(8, (thk / totalThickness) * 100)
                : 0;
            return (
              <div
                key={idx}
              ...
              >
                <span className="font-mono text-[10px] font-bold">
                  L{l.layer_index || idx + 1}
                </span>
                <span className="font-mono text-[10px]">
                  {l.film_variant_code || l.role || "?"}
                </span>
                <span className="text-[10px] opacity-70">{l.role}</span>
                {l.grade ? (
                  <span className="text-[10px] opacity-70">· {l.grade}</span>
                ) : null}
                <div className="ml-auto flex items-center gap-2">
                  <div className="h-1 w-16 overflow-hidden rounded-full bg-surface-1/70">
                    <div
      ...
      <div className="border-t border-line bg-surface-2 px-4 py-3">
        ...
        <div className="flex items-center gap-3">
          <div className="flex h-16 w-16 items-center justify-center rounded-xl border-2 border-dashed border-line-strong bg-surface-1">
            {widthMm && heightMm ? (
              <div
                className="bg-gradient-to-br from-order-bg to-order-bg ring-1 ring-order-border"
                style={{
          ...
          <div className="grid flex-1 grid-cols-2 gap-1 text-[10px]">
            <div className="rounded bg-surface-1 px-2 py-1 ring-1 ring-line">
              <div className="font-bold text-content-2">W × H</div>
              <div className="font-mono">
                {widthMm || "—"} × {heightMm || "—"} mm
              </div>
            </div>
            <div className="rounded bg-surface-1 px-2 py-1 ring-1 ring-line">
              <div className="font-bold text-content-2">Gusset</div>
              <div className="font-mono">{gussetMm || "—"} mm</div>
            </div>
            <div className="rounded bg-surface-1 px-2 py-1 ring-1 ring-line">
              <div className="font-bold text-content-2">Roll width</div>
              <div className="font-mono">{rollWidth || "—"} mm</div>
            </div>
            <div className="rounded bg-surface-1 px-2 py-1 ring-1 ring-line">
              <div className="font-bold text-content-2">
                Jumbos / {standardOrderKg}kg
              </div>
      ...
      {showLink ? (
        <div className="border-t border-line bg-success-bg px-4 py-2.5">
          <div className="flex items-center justify-between text-[11px]">
            <div className="flex items-center gap-2">
              ...
              {link ? (
                <span className="font-mono text-[11px] font-bold text-success-fg">
                  {link.code}
                </span>
              ) : (
            ...
            {link?.stock_qty != null ? (
              <span className="rounded-full bg-success-bg px-2 py-0.5 text-[10px] font-bold text-success-fg">
        ...
        <div className="border-t border-line bg-order-bg px-4 py-2.5">
          <div className="text-[11px] text-order-fg">
            <span className="font-bold">Auto-generated</span> on order confirm —
            InventoryMaterial code derived from variant
          </div>
        </div>
      )}
    </div>
...
function SizesTab({
  ...
  const isRoll =
    master.product_kind === "ROLL" || master.product_kind === "POD";
  return (
    <div className="space-y-4">
      <TabBanner
        tone="emerald"
        ...
        subtitle={
          isRoll
            ? "Roll width + stock form + trim"
            : "W × H + gusset + pouch style + film-area width"
        }
        icon={<span className="text-lg">📐</span>}
      />
      <section className="rounded-2xl border border-success-border bg-surface-1 shadow-sm overflow-hidden ring-1 ring-surface-1/40">
        {sizes.length === 0 ? (
          <div className="p-10 text-center">
            <div className="mx-auto h-12 w-12 rounded-2xl bg-gradient-to-br from-success-bg to-success-bg flex items-center justify-center text-2xl">
              📐
            </div>
            ...
            <div className="mt-1 text-xs text-content-3">
              Add sizes from PM Edit to surface them here.
            </div>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="min-w-full text-xs">
              ...
              <tbody className="divide-y divide-success-border">
                {sizes.map((s: any, idx: number) => (
                  <tr
                    key={s.id || s.code}
                    className={cn(
                      "transition-colors hover:bg-success-bg",
                  ...
                  >
                    ...
                    <td className="px-4 py-2.5 text-right font-mono font-bold text-content-1">
                      {s.width_mm}
                    </td>
                    <td className="px-4 py-2.5 text-right font-mono font-bold text-content-1">
                      {s.height_mm || (isRoll ? "—" : 0)}
                    </td>
                    <td className="px-4 py-2.5 text-right font-mono text-content-2">
                      {s.gusset_mm || "—"}
                    </td>
                    <td className="px-4 py-2.5 text-right font-mono text-content-2">
                      {s.roll_width_mm || (
                        <span className="italic text-content-4">auto</span>
                      )}
                    </td>
                    <td className="px-4 py-2.5 text-[10px]">
                      <span className="rounded-md bg-gradient-to-r from-success-bg to-info-bg px-2 py-0.5 font-bold text-success-fg ring-1 ring-success-border">
                        {isRoll ? s.roll_form || "—" : s.pouch_style || "—"}
                      </span>
                    </td>
                      ...
                      >
                        <span
                          className={cn(
                            ...
                            s.active ? "bg-success-fg" : "bg-danger-solid",
                          )}
                        />
...
function OverlaysTab({
  ...
  const [selected, setSelected] = React.useState<CustomerProductOverlay | null>(
    null,
  );
  ...
  const overlayVariant = React.useMemo<ProductVariant | null>(() => {
    ...
    if (selected.size_variant_code) {
      const v = variants.find((x) =>
        axisValueMatch(x, "size", selected.size_variant_code!),
      );
      if (v) return v;
    }
      ...
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-[minmax(0,1fr)_380px]">
        <section className="rounded-2xl border border-warning-border bg-surface-1 shadow-sm overflow-hidden ring-1 ring-surface-1/40">
          <header className="flex items-center justify-between border-b border-warning-border bg-gradient-to-r from-warning-bg via-white to-warm px-5 py-2.5">
            ...
            <button
              onClick={onAdd}
              className="inline-flex h-9 items-center gap-1.5 rounded-lg bg-gradient-to-r from-warning-fg to-warm px-3 text-[11px] font-bold text-white shadow-sm hover:shadow-md transition"
            >
              <Plus className="h-3.5 w-3.5" /> Add overlay
            </button>
          </header>
          {overlays.length === 0 ? (
            <div className="p-8 text-center">
              <Users className="mx-auto h-8 w-8 text-content-4" />
              <div className="mt-2 text-sm font-semibold text-content-2">
                No customer overlays yet
              </div>
              <div className="mt-1 text-xs text-content-3 max-w-md mx-auto">
                Add a customer overlay to pre-fill artwork, packing or size
                defaults when sales picks this customer.
              </div>
              <button
                onClick={onAdd}
                className="mt-4 inline-flex items-center gap-1.5 rounded-lg bg-primary px-3 py-1.5 text-[11px] font-bold text-white hover:bg-primary"
              >
                <Plus className="h-3.5 w-3.5" /> Add overlay
              </button>
            </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <div className="font-bold text-sm text-content-1 truncate">
                    ...
                    <div className="mt-2 flex flex-wrap gap-1 text-[10px]">
                      {o.default_price_basis ? (
                        <span className="rounded bg-surface-2 px-1.5 py-0.5 font-bold text-content-2">
                          basis · {o.default_price_basis}
                        </span>
                      ) : null}
                      {o.moq_kg ? (
                        <span className="rounded bg-warning-bg text-warning-fg ring-1 ring-warning-border px-1.5 py-0.5 font-bold">
                          MOQ · {o.moq_kg} kg
                        </span>
                      ) : null}
                      {o.default_artwork_design_code ? (
                        <span className="rounded bg-order-bg text-order-fg ring-1 ring-order-border px-1.5 py-0.5 font-bold">
                          art · {o.default_artwork_design_code}
                        </span>
                      ) : null}
                      {o.size_variant_code ? (
                        <span className="rounded bg-info-bg text-primary ring-1 ring-info-border px-1.5 py-0.5 font-bold">
                          size · {o.size_variant_code}
                        </span>
                      ) : null}
                    </div>
                    {o.notes ? (
                      <div className="mt-2 text-[10px] text-content-3 truncate">
                        {o.notes}
                      </div>
                    ) : null}
                  </button>
...
function axisValueMatch(
  ...
  const code =
    typeof got === "object"
      ? (got as any).code || (got as any).value || ""
      : String(got);
...
function ArtworksTab({
  ...
  if (!master.fixed_attributes?.print_capable) {
    return (
      <div className="relative overflow-hidden rounded-2xl border border-line bg-gradient-to-br from-surface-2 via-white to-surface-2 p-10 text-center shadow-sm">
        <div
          aria-hidden
          className="pointer-events-none absolute -top-16 -right-16 h-40 w-40 rounded-full bg-line blur-3xl"
        />
        <Palette className="relative mx-auto h-10 w-10 text-content-4" />
        <div className="relative mt-3 font-display text-sm font-bold text-content-2">
          Not print-capable
        </div>
        <p className="relative mt-1 text-xs text-content-3 max-w-md mx-auto">
          Enable printing on the master to surface approved artworks.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="rounded-2xl border border-order-border bg-surface-1 shadow-sm overflow-hidden ring-1 ring-surface-1/40">
        {artworks.length === 0 ? (
          <div className="p-10 text-center">
            <div className="mx-auto h-12 w-12 rounded-2xl bg-gradient-to-br from-order-bg to-danger-bg flex items-center justify-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 p-5">
            {artworks.map((a) => (
              <div
                key={a.id}
                className="group relative overflow-hidden rounded-2xl border border-order-border bg-gradient-to-br from-white via-order-bg to-danger-bg p-4 shadow-sm transition hover:shadow-lg hover:border-order-border hover:-translate-y-0.5"
              >
                <div
                  aria-hidden
                ...
                <div className="relative mt-1.5 text-sm font-bold text-content-2 truncate">
                  {a.name || "—"}
                </div>
                <div className="relative mt-2 flex flex-wrap gap-1">
                  ...
                  {(a as any).substrate_mode ? (
                    <span className="rounded-md bg-danger-bg px-1.5 py-0.5 text-[10px] font-bold text-danger-fg ring-1 ring-danger-border">
                      {(a as any).substrate_mode}
                    </span>
                  ) : null}
                </div>
...
function AuditPlaceholder({ masterId }: { masterId: string }) {
  return (
    <div className="space-y-4">
      ...
      />
      <section className="relative overflow-hidden rounded-2xl border border-line bg-gradient-to-br from-surface-2 via-white to-order-bg p-10 text-center shadow-sm ring-1 ring-surface-1/40">
        <div
          aria-hidden
          className="pointer-events-none absolute -top-16 -left-16 h-48 w-48 rounded-full bg-order-bg blur-3xl"
        />
        <div
          aria-hidden
          className="pointer-events-none absolute -bottom-16 -right-16 h-48 w-48 rounded-full bg-line blur-3xl"
        />
        <div className="relative">
          ...
          <p className="mt-1 text-xs text-content-3 max-w-md mx-auto leading-relaxed">
            All master/variant/overlay changes for this product master are
            logged in the system audit centre with before/after diffs, actor and
            timestamp.
          </p>
          <Link
 ...
 */
function ProductionCatalogMapCard({
  master,
  variants,
}: {
  master: ProductMaster;
  ...
  const linked = variants.filter(
    (v) => v.inventory_link && v.inventory_link.id,
  );
  const totalStock = linked.reduce(
    (s, v) => s + Number(v.inventory_link?.stock_qty || 0),
    0,
  );
  ...
  for (const v of linked) {
    ...
    uomCounts[u] =
      (uomCounts[u] || 0) + Number(v.inventory_link?.stock_qty || 0);
  }
  const stockSummary =
    Object.entries(uomCounts)
      .map(
        ([uom, qty]) =>
          `${Number(qty).toLocaleString("en-IN", { maximumFractionDigits: 0 })} ${uom}`,
      )
      .join(" · ") || "—";
  const sample = linked.slice(0, 4);
  return (
    <section className="rounded-2xl border border-order-border bg-surface-1 shadow-sm overflow-hidden">
      <header className="flex items-center justify-between border-b border-order-border bg-gradient-to-r from-order-bg via-white to-order-bg px-5 py-3">
        <div className="flex items-center gap-2">
          <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-order-fg to-order-fg text-white shadow-sm">
            <Database className="h-4 w-4" />
          </span>
          <div>
            <h3 className="font-display text-sm font-bold text-content-1">
              Catalog map · variant ↔ SKU
            </h3>
            <div className="text-[10px] text-content-3">
              Every variant of this {kind} master must be manually linked to one
              fixed catalog row in{" "}
              <Link
                href={catalogHref}
              ...
              — that row is the stock, purchase, and consumption identity.
            </div>
          </div>
        </div>
        <Link
      ...
      <div className="p-5 space-y-3">
        ...
        {sample.length > 0 ? (
            ...
            {sample.map((v) => (
              ...
              >
                ...
                <div className="min-w-0 text-right">
                  ...
                  <div className="text-[10px] font-mono text-content-3">
                    {Number(v.inventory_link?.stock_qty || 0).toLocaleString(
                      "en-IN",
                      { maximumFractionDigits: 0 },
                    )}{" "}
                    {v.inventory_link?.base_uom || ""}
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="rounded-xl border border-dashed border-line bg-surface-2 px-3 py-2.5 text-[11px] text-content-3">
            No variants yet. Create the production variants first, then link
            each one to an existing fixed SKU in{" "}
            <Link
              href={catalogHref}
              className="font-bold text-order-fg underline-offset-2 hover:underline"
            >
          ...
          <div className="rounded-xl border border-dashed border-warning-border bg-warning-bg px-3 py-2.5 text-[11px] text-warning-fg">
            Variants exist but are not linked to catalog SKUs yet. Use the
            variant card&apos;s Link SKU action and choose an existing unlinked
            row.
          </div>
        )}

        <div className="rounded-xl bg-surface-2 px-3 py-2 text-[10px] text-content-3">
          <strong>How it works:</strong> The Product Master variant is the
          in-house production contract. The catalog SKU is created and
          maintained separately in{" "}
          <code className="font-mono bg-surface-1 px-1 py-0.5 rounded ring-1 ring-line">
            {catalogHref}
          </code>
          . Manual linking joins the two; production runs and consumption use
          the linked catalog row.
        </div>
      </div>
    </section>
  );
...
function PackagingContractCard({
  master,
  packagingMaterials,
  podVariants,
}: {
  ...
  const podAxis = variantAxes.find((a) => {
    const k = String(a.axis).toUpperCase();
    return (
      a.master_data_source === "pod_sku_variant" ||
      ["POD", "POD_VARIANT", "POD_SKU_VARIANT"].includes(k)
    );
  });
  ...
  return (
    <section className="rounded-2xl border border-success-border bg-surface-1 shadow-sm overflow-hidden">
      <header className="flex items-center justify-between border-b border-success-border bg-gradient-to-r from-success-bg via-white to-success-bg px-5 py-3">
        <div className="flex items-center gap-2">
          <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-success-fg to-success-bg0 text-white shadow-sm">
            <Package className="h-4 w-4" />
          </span>
          <div>
            <h3 className="font-display text-sm font-bold text-content-1">
              Inner pouch &amp; POD · sales-pickable
            </h3>
            <div className="text-[10px] text-content-3">
              All outer packing (gunny / sheet / tape / label / tag) is tagged
              per order at EOD — not on this master.
            </div>
          </div>
        </div>
        <div className="flex items-center gap-1.5">
      ...
      <div className="p-5 space-y-3">
            ...
            <div className="rounded-xl border border-warning-border bg-gradient-to-br from-warning-bg via-white to-warm px-3 py-2.5 ring-1 ring-warning-border">
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="text-[10px] font-bold text-warning-fg">
                  ...
                  {(innerAxis as any)?.auto_demand_in_house
                    ? " · auto-demand"
                    : ""}
                  {" · "}
              ...
              <div className="mt-1.5 flex flex-wrap gap-1">
                {innerCodes.map((code) => {
                  const material = packagingMaterials.find(
                    (m: any) => m.code === code || m.id === code,
                  );
                  return (
                    <span
                      key={code}
                      className="inline-flex items-center gap-1 rounded-md bg-surface-1 px-1.5 py-0.5 text-[10px] font-mono font-bold text-warning-fg ring-1 ring-warning-border"
                    >
                      {code}
              ...
              <div className="mt-1.5 text-[10px] text-warning-fg">
                Sales picks 1 per order · BOM = ceil(total_pouches /
                pcs_per_inner) · customer overlay can override pcs_per_inner.
              </div>
            </div>
          ) : (
            <div className="rounded-xl border border-dashed border-warning-border bg-warning-bg px-3 py-2 text-[11px] text-warning-fg">
              Inner pouch axis not configured on this POUCH master. Open{" "}
              <strong>Edit → Sales-pickable menu</strong> to allow inner-pouch
              SKUs.
            </div>
          )
        ) : null}
        ...
        {podCodes.length > 0 ? (
          <div className="rounded-xl border border-order-border bg-gradient-to-br from-order-bg via-white to-danger-bg px-3 py-2.5 ring-1 ring-order-border">
            ...
            <div className="mt-1.5 flex flex-wrap gap-1">
              {podCodes.map((code) => {
                const meta = podVariants.find(
                  (v: any) => v.code === code || v.id === code,
                );
                return (
                  <span
                    key={code}
                    className="inline-flex items-center gap-1 rounded-md bg-surface-1 px-1.5 py-0.5 text-[10px] font-mono font-bold text-order-fg ring-1 ring-order-border"
                  >
                    {code}
        ...
        <div className="rounded-xl border border-line bg-gradient-to-br from-surface-2 via-white to-surface-2 px-3 py-2.5">
          <div className="flex items-start gap-2">
            <span className="flex h-7 w-7 flex-none items-center justify-center rounded-lg bg-gradient-to-br from-surface-2 to-surface-3 text-white">
            ...
            <div className="min-w-0">
              ...
              <div className="mt-0.5 text-[11px] text-content-3">
                Gunny / sheet / tape / label / tag are not on the master. Each
        ...
        {kind !== "POUCH" && podCodes.length === 0 ? (
          <div className="rounded-xl border border-dashed border-line bg-surface-2 p-3 text-center text-[11px] text-content-3">
...
function AddonsContractCard({
  ...
  const hasAny = addonAxes.some(
    (a) => axisAllowedCodes(a).length > 0 || a.required,
  );
  return (
    <section className="rounded-2xl border border-line bg-surface-1 shadow-sm overflow-hidden">
      <header className="flex items-center justify-between border-b border-line px-5 py-3">
        <div className="flex items-center gap-2">
          <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-warning-bg text-warning-fg">
          ...
          <div>
            ...
            <div className="text-[10px] text-content-3">
              Optional per-piece add-ons (zipper, valve, spout, etc.) sales can
              attach to a line
            </div>
          </div>
        </div>
        <span className="rounded-full bg-warning-bg px-2 py-0.5 text-[10px] font-bold text-warning-fg ring-1 ring-warning-border">
          {addonAxes.some((a) => a.required)
            ? "required axis"
            : "optional axis"}
        </span>
      </header>
      <div className="p-5 space-y-2">
        {!hasAny ? (
          <div className="rounded-xl border border-dashed border-line bg-surface-2 p-4 text-center text-xs text-content-3">
            Addons axis declared but no allowed codes yet. Add allowed catalog
            codes on the edit page.
          </div>
        ) : null}
        {addonAxes.map((axis) => {
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="text-[10px] font-bold text-warning-fg">
                  {axis.required
                    ? "required · sales must pick"
                    : "optional · sales may skip"}
              ...
              <div className="mt-1 flex flex-wrap gap-1">
                {codes.length > 0 ? (
                  codes.map((code) => {
                    const meta = addons.find(
                      (a: any) => a.code === code || a.id === code,
                    );
                    return (
                      <span
                        key={code}
                  ...
                  <span className="text-[10px] italic text-warning-fg">
                    No allowed codes locked — all active catalog add-ons are
                    pickable.
                  </span>
                )}
              </div>
...
function CreateOverlayDialog({
  ...
  const [packingDefaults, setPackingDefaults] = React.useState<
  ...
  const [pcsPerInnerOverride, setPcsPerInnerOverride] =
  ...
  const catalogAxes = (master.variant_axes || []).filter((a) =>
    axisCatalogSource(a),
  );
  ...
  const podAxis = catalogAxes.find(
    (a) =>
      axisCatalogSource(a) === "pod_sku_variant" &&
      ["POD_VARIANT", "POD", "POD_REF", "POD_SKU_VARIANT"].includes(
        normalizeCode(a.axis),
      ),
  );
  ...
  const podAllowed = podAxis
    ...
    : [];
  const addonsAllowed = addonsAxis
    ? resolveAxisValues(
        addonsAxis,
        master,
        sizes,
  ...
  const handledAxisKeys = new Set([
    ...
    "artwork_mode",
    "packaging_outer",
    "packaging", // dropped from PM model
  ]);
  packagingAxes.forEach((axis) => handledAxisKeys.add(String(axis.axis)));
  const otherAxes = (master.variant_axes || []).filter(
    (a) => !handledAxisKeys.has(String(a.axis)),
  );
  ...
  const perLayerAxes = otherAxes.filter(
    (a) => a.type === "per_layer_number" || a.type === "per_layer_enum",
  );
  // Genuine "other" axes the overlay can override — anything left after
  // dropping handled + per-layer.
  const overrideAxes = otherAxes.filter(
    (a) => a.type !== "per_layer_number" && a.type !== "per_layer_enum",
  );
  ...
  const createMutation = useMutation({
    mutationFn: () => {
      const packingAxisValues = Object.fromEntries(
        Object.entries(packingDefaults).filter(([, v]) => v && v.length),
      );
      const defaultPackingRecipe = buildOverlayPackingRecipe(
        packingAxisValues,
        packagingMaterials,
        master,
        packagingAxes,
      ...
      if (
        overridePcs > 0 &&
        defaultPackingRecipe?.primary_inner_pack?.enabled
      ) {
        defaultPackingRecipe.primary_inner_pack.pcs_per_pack = overridePcs;
        if (Array.isArray(defaultPackingRecipe.packaging_lines)) {
          defaultPackingRecipe.packaging_lines =
            defaultPackingRecipe.packaging_lines.map((ln: any) =>
              ln?.role === "PRIMARY_INNER"
                ? { ...ln, pcs_per_pack: overridePcs }
                : ln,
            );
        }
      }
      return productMasterService.createOverlay(master.id, {
        ...
        default_packing_recipe: Object.keys(defaultPackingRecipe).length
          ? defaultPackingRecipe
          : undefined,
        ...
        axis_values: {
          ...
          ...Object.fromEntries(
            Object.entries(overrideValues).filter(([, v]) => v && v.length),
          ),
        },
      } as any);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["product-master-overlays", master.id],
      });
      queryClient.invalidateQueries({
    ...
    for (const axis of master.variant_axes || []) {
      ...
      if (normalized === "LAYER_THICKNESSES") {
        values[key] = Object.fromEntries(
          (master.layer_template || []).map((row: any, idx: number) => [
            String(idx + 1),
        ...
        const gradeEntries = (master.layer_template || [])
          .map(
            (row: any, idx: number) =>
              [
                String(idx + 1),
                String(row.default_grade || "").trim(),
              ] as const,
          )
          .filter(
            ([, grade]) => grade && (!allowed.size || allowed.has(grade)),
          );
      ...
      const suggested = String(
        (axis as any).default_value || allowed[0]?.code || "",
      );
  ...
  const previewAxesReady = React.useMemo(() => {
    return (master.variant_axes || []).every(
      (axis) =>
        !axis.required ||
    ...
    queryFn: () =>
      productMasterService.previewBom({
        ...
        printing: master.fixed_attributes?.print_capable
          ? {
              enabled: true,
              artwork_id: defaultArtwork || undefined,
              defer_artwork_to_planner: !defaultArtwork,
            }
          : { enabled: false },
      }),
    enabled:
      open && !!master.id && !!previewAxisValues.size && previewAxesReady,
    staleTime: 0,
    retry: false,
  });
  ...
  return (
    ...
    >
      <DialogContent className="max-w-6xl max-h-[92vh] rounded-2xl p-0 overflow-hidden flex flex-col">
        {/* Gradient header */}
        <div className="flex-none border-b border-order-border bg-gradient-to-r from-order-bg via-white to-success-bg px-6 py-4">
          <DialogHeader>
            <div className="flex items-center gap-3">
              <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-order-fg to-success-fg text-white shadow-sm">
              ...
              <div className="min-w-0 flex-1">
                ...
                <DialogTitle className="font-display text-lg font-bold text-content-1">
                  {customerMeta
                    ? `Defaults for ${customerMeta.name}`
                    : "Add customer overlay"}
                </DialogTitle>
                <DialogDescription className="text-[11px] text-content-3 mt-0.5">
                  Customer-specific item code, price basis, axis defaults &amp;
                  packing. The live BOM rail on the right shows the exact math
                  sales gets when this customer orders this master.
                </DialogDescription>
              </div>
            </div>
          </DialogHeader>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_440px] flex-1 min-h-0 overflow-hidden">
          ...
          <div className="overflow-y-auto px-6 py-5 space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setCustomerItemCode(e.target.value.toUpperCase())
                    }
                ...
                <div>
                  ...
                  <Select
                    value={priceBasis}
                    onValueChange={(v) => setPriceBasis(v as "KG" | "PCS")}
                  >
                    <SelectTrigger className="mt-1 h-10 rounded-xl">
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div>
                  ...
                  <Input
                    ...
                    placeholder="Optional · drives sample size in preview"
                    className="mt-1 h-10 rounded-xl font-mono"
                  />
                </div>
                {customerMeta ? (
                  <div className="rounded-xl border border-line bg-surface-2 px-3 py-2 flex items-center gap-2">
                    <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-order-bg text-order-fg font-bold text-xs">
                      {customerInitials}
                    </span>
                    <div className="min-w-0">
                      <div className="text-[11px] font-bold text-content-1 truncate">
                        {customerMeta.name}
                      </div>
                      <div className="font-mono text-[10px] text-content-3 truncate">
                        {customerMeta.code || "—"}
                        {customerMeta.gst_no
                          ? ` · GST ${customerMeta.gst_no}`
                          : ""}
                      </div>
                    </div>
                  </div>
            ...
            <OverlaySection
              eyebrow="2 · Customer order defaults"
              title="What sales auto-picks for this customer"
            >
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <Label className="text-[10px] font-bold uppercase tracking-wider text-content-3">
                  ...
                  <Select
                    value={defaultSizeCode || "__none"}
                    onValueChange={(v) =>
                      setDefaultSizeCode(v === "__none" ? "" : v)
                    }
                  >
                    <SelectTrigger className="mt-1 h-10 rounded-xl">
                      <SelectValue placeholder="No size lock" />
                    </SelectTrigger>
                    <SelectContent>
                ...
                {podAllowed.length > 0 ? (
                  <div>
                    ...
                    <Select
                      value={defaultPod || "__none"}
                      onValueChange={(v) =>
                        setDefaultPod(v === "__none" ? "" : v)
                      }
                    >
                      <SelectTrigger className="mt-1 h-10 rounded-xl">
                ...
                <div className="mt-3 rounded-2xl border border-warning-border bg-gradient-to-br from-warning-bg via-white to-warm p-3 shadow-sm">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-[10px] text-content-3">
                      Outer / sheet / tape / label / tag → packing yard EOD
                      ticks
                    </span>
                  </div>
                  {packagingAxes.map((axis) => {
                    ...
                    const pickedMaterial = packagingMaterials.find(
                      (m) => normalizeCode(m.code) === normalizeCode(value),
                    );
                    const masterPcsPerInner = Number(
                      (pickedMaterial?.packaging_defaults_json as any)
                        ?.pcs_per_pack ??
                        (pickedMaterial?.packaging_defaults_json as any)
                          ?.pcs_per_inner ??
                        0,
                      ...
                      <div key={key} className="space-y-2">
                        <div>
                          ...
                          <div className="mt-1 text-[10px] text-content-3">
                            {allowed.length} allowed from this master
                          </div>
                        </div>
                        {value ? (
                          <div className="rounded-xl border border-warning-border bg-surface-1 px-3 py-2">
                            ...
                            <div className="mt-1.5 grid grid-cols-[minmax(0,1fr)_auto] gap-2 items-center">
                              <Input
                                ...
                                placeholder={
                                  masterPcsPerInner > 0
                                    ? `e.g. ${masterPcsPerInner}`
                                    : "e.g. 50"
                                }
                                value={pcsPerInnerOverride}
                                onChange={(e) =>
                                  setPcsPerInnerOverride(e.target.value)
                                }
                            ...
                            <div className="mt-1 text-[10px] text-content-3">
                              BOM uses{" "}
                              <strong>
                                ceil(total_pouches / pcs_per_inner)
                              </strong>
                              . Leave blank to inherit the master default.
                            </div>
                          </div>
                        ) : null}
                      </div>
              ...
              {addonsAxis ? (
                <div className="mt-3">
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <span className="text-[10px] text-content-3">
                      {addonsAllowed.length} allowed ·{" "}
                      {addonsAxis.required ? "required" : "optional"} · sales
                      picks any number per order
                    </span>
                  </div>
                  {addonsAllowed.length > 0 ? (
                    ...
                    <div className="rounded-xl border border-dashed border-line bg-surface-2 px-3 py-2 text-[11px] text-content-3">
                      No allowed add-ons curated for this master yet. Add
                      allowed codes in PM Edit Section 7.
                    </div>
                  )}
                </div>
              ) : null}
                        ...
                        <div key={key}>
                          ...
                          {allowed.length > 0 ? (
                            ...
                            <Input
                              value={value}
                              onChange={(e) =>
                                patchOverride(key, e.target.value)
                              }
              ...
              {perLayerAxes.length > 0 ? (
                <div className="mt-3 rounded-xl border border-line bg-surface-2 px-3 py-2 text-[10px]">
                  ...
                  <span className="ml-2 text-content-3">
                    {perLayerAxes.map((a) => a.label || a.axis).join(", ")} —
                    picked per layer on each order. Master defaults apply unless
                    customer overrides on the line.
                  </span>
                </div>
              ) : null}
            </OverlaySection>
            ...
            {master.fixed_attributes?.print_capable ? (
              <OverlaySection
                eyebrow="3 · Artwork"
                title="Customer-facing approved artwork"
              >
                <Label className="text-[10px] font-bold uppercase tracking-wider text-content-3">
                  Default approved artwork
                </Label>
                <Select
                  value={defaultArtwork || "__none"}
                  onValueChange={(v) =>
                    setDefaultArtwork(v === "__none" ? "" : v)
                  }
                >
                  <SelectTrigger className="mt-1 h-10 rounded-xl">
                    <SelectValue placeholder="No default · sales must pick on each order" />
                  </SelectTrigger>
                  <SelectContent>
                ...
                <div className="mt-1 text-[10px] text-content-3">
                  ...
                  {master.fixed_attributes?.artwork_required ? (
                    ...
                    <span className="font-bold text-warning-fg">
                      artwork OPTIONAL · warning-print run when not picked
                    </span>
                  )}
                </div>
              </OverlaySection>
            ...
            >
              <Textarea
                ...
                placeholder="Optional · visible only to staff"
        ...
        <div className="flex-none border-t border-line bg-surface-1 px-6 py-3 flex items-center justify-between gap-3">
          <div className="text-[11px] text-content-3">
            {customer ? (
              ...
              <span className="text-warning-fg font-bold">
                Pick a customer to enable save
              </span>
            )}
          </div>
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              className="rounded-xl bg-gradient-to-r from-order-fg to-success-fg text-white shadow-md hover:shadow-lg"
            >
  ...
  return (
    <section className="rounded-2xl border border-line bg-surface-1 shadow-sm overflow-hidden">
      <header className="border-b border-line bg-gradient-to-r from-surface-2 via-white to-white px-4 py-2.5 flex items-center justify-between">
        ...
        {optional ? (
          <span className="rounded-full bg-surface-2 px-2 py-0.5 text-[10px] font-bold text-content-3 ring-1 ring-line">
            optional
          </span>
        ) : null}
      </header>
