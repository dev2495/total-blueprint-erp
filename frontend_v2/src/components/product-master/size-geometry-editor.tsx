...
export function SizeGeometryEditor({
  row,
  kind,
  packagingKind,
  fixedFgType,
  ...
  const currentMultipliers =
    geometryConfig.multipliers && typeof geometryConfig.multipliers === "object"
  ...
  const styleValue = (ROLL_FORMS as readonly string[]).includes(
  ...
  const patchAdjustment = (index: number, patch: Record<string, unknown>) => {
    const next = adjustments.map((item, idx) =>
  ...
  return (
    <div
      ...
      data-testid={`size-geometry-editor-${outputKind.toLowerCase()}`}
    ...
    >
      {isPouchShaped ? (
        <PouchStyleBinding
          row={row}
          onPatch={onPatch}
          fallbackTargetWidthMm={geometry.fallbackRollWidthMm}
        />
      ) : null}
      {outputMissing ? (
        <div className="rounded-xl border border-warning-border bg-gradient-to-r from-warning-bg via-white to-warm px-3 py-2 text-[11px] text-warning-fg">
          <span className="font-bold">Pick a packing output first.</span> Go to{" "}
          <em>Identity → Packing sub-type</em> in section 1 and choose{" "}
          <strong>Inner pouch</strong> or <strong>Sheet / Roll</strong>. Size
          fields are hidden until the physical output is known.
        </div>
      ) : null}
      {isPouchShaped ? (
        <div
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
            ...
            <EditField label="UOM">
              <Select
                value={row.qty_uom || "KG"}
                onValueChange={(v) =>
                  onPatch({ qty_uom: v as "KG" | "PCS" | "METER" })
                }
              >
            ...
            <EditField label="Sort order">
              <Input
                ...
                onChange={(e) =>
                  onPatch({ sort_order: Number(e.target.value) })
                }
              />
            </EditField>
      ...
      {isRollOutput ? (
        <>
          ...
          >
            ...
            <EditField
              label={isRollOutput ? "Roll width (mm)" : "Final width (mm)"}
            >
              <Input
                type="number"
                className="h-8 rounded-lg text-right text-xs"
                value={row.width_mm}
                onChange={(e) => onPatch({ width_mm: Number(e.target.value) })}
              />
            </EditField>
            <EditField label="Trim loss (mm)">
              <Input
                type="number"
                className="h-8 rounded-lg text-right text-xs"
                value={row.trim_loss_mm ?? 10}
                onChange={(e) =>
                  onPatch({ trim_loss_mm: Number(e.target.value) })
                }
              />
            </EditField>
            <EditField label="Trim affects">
              <Select
                value={(row.trim_apply_to || "WIDTH") as string}
                onValueChange={(v) =>
                  onPatch({ trim_apply_to: v as DimensionImpact })
                }
              >
                <SelectTrigger className="h-8 rounded-lg text-xs">
                  <SelectValue />
            ...
            <EditField label="Roll form">
              <Select
                key={`${productKind}-${row.id || row.code}-${styleValue}`}
            ...
            <EditField label="Auto roll width">
              <Metric
                value={geometry.fallbackRollWidthMm}
                suffix="mm"
                tone="emerald"
              />
            </EditField>
            <EditField
              label={isRollOutput ? "Effective roll width" : "Effective W x H"}
              span={2}
            >
              <div
                className={cn(
                  "grid gap-2",
                  isRollOutput ? "grid-cols-1" : "grid-cols-2",
                )}
              >
                <Metric
                  value={geometry.effectiveWidthMm}
                  suffix="mm W"
                  tone="blue"
                />
                {!isRollOutput ? (
                  <Metric
                    value={geometry.effectiveHeightMm}
                    suffix="mm H"
                    tone="blue"
                  />
                ) : null}
          ...
          <div className="rounded-xl border border-line bg-surface-2 p-3">
            <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
              <div>
                ...
                <p className="mt-0.5 text-xs text-content-3">
                  Add exact allowances to width, height, or both without
                  changing the product code model.
                </p>
              </div>
              <Button
                type="button"
            ...
            {adjustments.length === 0 ? (
              <div className="rounded-lg border border-dashed border-line bg-surface-1 px-3 py-2 text-xs font-medium text-content-3">
              ...
              <div className="space-y-2">
                {adjustments.map((adjustment, index) => (
                  <div
                    key={index}
                    className="grid grid-cols-[1fr_96px_140px_32px] gap-2"
                  >
                    <Input
                      className="h-8 rounded-lg text-xs"
                      value={String(adjustment.name || "")}
                      onChange={(e) =>
                        patchAdjustment(index, { name: e.target.value })
                      }
                    />
                    <Input
                      type="number"
                    ...
                    />
                    <Select
                      value={String(adjustment.impact || "WIDTH")}
                      onValueChange={(v) =>
                        patchAdjustment(index, { impact: v })
                      }
                    >
                      <SelectTrigger className="h-8 rounded-lg text-xs">
                        <SelectValue />
                      ...
                      <SelectContent>
                        {DIMENSIONS.filter((item) => item.value !== "NONE").map(
                          (item) => (
                            <SelectItem key={item.value} value={item.value}>
                              {item.label}
                            </SelectItem>
                    ...
                    <button
                      ...
                      className="inline-flex h-8 items-center justify-center rounded-lg text-danger-fg hover:bg-danger-bg"
                    >
...
function Metric({
  ...
  return (
    <div
      className={cn(
        "flex h-8 items-center justify-end rounded-lg px-2 font-mono text-xs font-bold ring-1",
        styles,
      )}
    >
