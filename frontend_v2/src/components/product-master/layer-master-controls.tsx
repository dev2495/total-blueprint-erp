...
const STANDARD_MICRONS = [
  8, 10, 12, 15, 18, 20, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70, 75, 80, 90,
  100, 120, 150,
];
...
export function filmForLayer(layer: LayerTemplateRow, films: Material[]) {
  ...
  return films.find(
    (film) =>
      String(film.id || "").toLowerCase() === filmId ||
      String(film.code || "").toLowerCase() === code,
  );
}
...
function parseThicknessFromText(text: string) {
  const match = String(text || "").match(
    /(?:^|[-\s])(\d+(?:\.\d+)?)\s*(?:micron|microns|um|u|µ|μ)?(?:$|[-\s])/i,
  );
  return match ? Number(match[1]) : null;
}

export function recipeThicknessOptions(
...
export function thicknessOptionsForLayer(
  ...
  if (explicit.length)
    return uniqueNumbers([...explicit, layer.thickness_micron]);
  ...
  if (fromRecipes.length)
    return uniqueNumbers([...fromRecipes, layer.thickness_micron]);
  return uniqueNumbers([layer.thickness_micron]);
}

export function gradeOptionsForLayer(
  layer: LayerTemplateRow,
  films: Material[],
  grades: RecipeGrade[],
  recipes: ExtrusionRecipe[],
  ...
  const defaults = layer.default_grade
    ...
    : [];
  const explicit = (layer.grade_options || [])
    .map((g) => String(g).trim())
    .filter(Boolean);
  const activeGradeMasterNames = grades
    ...
    .filter(Boolean);
  if (activeGradeMasterNames.length)
    return Array.from(
  ...
  if (film?.id) {
    const recipeGrades = recipes
      ...
      .filter(Boolean);
    if (recipeGrades.length)
      return Array.from(new Set([...defaults, ...recipeGrades]));
  }
  return defaults;
}
...
export function sanitizeLayerForFilm(
  ...
  if (!next.thickness_micron && thicknessOptions.length)
    next.thickness_micron = thicknessOptions[0];
  ...
  if (gradeOptions.length) {
    if (!next.default_grade || !gradeOptions.includes(next.default_grade))
      next.default_grade = gradeOptions[0];
    next.grade_apportion = next.grade_apportion || "fixed";
    next.grade_options =
      next.grade_apportion === "variable" ? gradeOptions : [next.default_grade];
  }
  return next;
}
  ...
  if (!options.length) {
    return (
      <input
        ...
        value={current || ""}
        onChange={(event) =>
          onChange({ thickness_micron: Number(event.target.value) })
        }
        placeholder="Micron"
        className={cn(
          "h-9 w-full rounded-xl border border-line bg-surface-1 px-3 text-xs font-semibold text-content-2 shadow-sm focus:border-primary focus:outline-none",
          className,
        )}
      />
    );
  ...
  return (
    <div className={cn("space-y-1.5", className)}>
      <input
        ...
        onChange={(event) =>
          onChange({
            thickness_micron: Number(event.target.value),
            thickness_options: options,
          })
        }
        placeholder="Micron"
        className="h-9 w-full rounded-xl border border-line bg-surface-1 px-3 text-xs font-semibold text-content-2 shadow-sm focus:border-primary focus:outline-none"
      />
      <Select
        value={options.includes(current) ? String(current) : undefined}
        onValueChange={(value) =>
          onChange({
            thickness_micron: Number(value),
            thickness_options: options,
          })
        }
      >
        <SelectTrigger className="h-8 rounded-xl border-line bg-surface-2 text-[11px]">
      ...
      <div className="text-[10px] font-semibold text-content-3">
        Presets guide entry; any valid micron can be typed. Recipe range selects
        extrusion BOM.
      </div>
    </div>
  );
...
export function LayerDefaultGradeSelect({
  ...
  if (isPurchasedOnlyFilm(film)) {
    return (
      <div
        className={cn(
          "rounded-lg border border-line bg-surface-2 px-3 py-2 text-xs font-semibold text-content-3",
          className,
        )}
      >
  ...
  if (!options.length) {
    return (
      <div
        className={cn(
          "rounded-lg border border-warning-border bg-warning-bg px-3 py-2 text-xs font-semibold text-warning-fg",
          className,
        )}
      >
    ...
    <Select
      ...
      onValueChange={(value) => {
        const selected = Array.from(
          new Set([value, ...(layer.grade_options || [])]),
        );
  ...
  if (!options.length) {
    return (
      <div className="text-[11px] font-semibold text-warning-fg">
        No active grade choices found for this film.
      </div>
    );
  }
  return (
...
export function LayerGradeBadge({
  layer,
  films,
}: {
  layer: LayerTemplateRow;
  ...
  const film = filmForLayer(layer, films);
  if (isPurchasedOnlyFilm(film))
    return <Badge variant="secondary">Purchased film: no grade</Badge>;
  if (layer.default_grade)
    return <Badge variant="outline">Default {layer.default_grade}</Badge>;
