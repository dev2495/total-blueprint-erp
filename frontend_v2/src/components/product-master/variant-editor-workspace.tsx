...
export function VariantEditorWorkspace({
  productId,
  mode,
}: VariantEditorWorkspaceProps) {
  ...
  const [layerThicknesses, setLayerThicknesses] = React.useState<
  ...
  const [catalogValues, setCatalogValues] = React.useState<
  ...
  React.useEffect(() => {
    ...
    master.layer_template.forEach((row, i) => {
      ...
      if (row.default_input_roll_width_mm)
        w[i + 1] = row.default_input_roll_width_mm;
    });
  ...
  React.useEffect(() => {
    ...
    (master.variant_axes || []).forEach((a: any) => {
      if (
        a.master_data_source &&
        a.default_value &&
        !catalogValues[String(a.axis)]
      ) {
        next[String(a.axis)] = a.default_value;
      }
    });
    if (Object.keys(next).length)
      setCatalogValues((prev) => ({ ...next, ...prev }));
  ...
  const variantCode = React.useMemo(() => {
    ...
    if (artworkMode && artworkMode !== "DEFER")
      parts.push(artworkMode.replace(/-/g, ""));
  ...
  const totalThickness = React.useMemo(
    () =>
      Object.values(layerThicknesses).reduce((s, v) => s + (Number(v) || 0), 0),
    [layerThicknesses],
  );
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      const axisValues: Record<string, any> = {
        ...
        ...(artworkMode && artworkMode !== "DEFER"
          ? { artwork_mode: artworkMode }
          : { artwork_mode: "DEFER" }),
        ...catalogValues,
      };
      try {
        return await productMasterService.findOrCreateVariant(productId, {
  ...
  const requiredAxes = (master.variant_axes || []).filter(
    (a: VariantAxisDef) => a.required,
  );
  const catalogAxes = (master.variant_axes || []).filter(
    (a: VariantAxisDef) => a.master_data_source,
  );
  const activeOverlay = overlays.find(
    (o: any) => (o.axis_values?.size || sizeCode) === sizeCode,
  );
  ...
  return (
    <div className="space-y-5 pb-24">
      ...
      {/* ─── Hero context strip ─── */}
      <section className="overflow-hidden rounded-2xl border border-order-border bg-gradient-to-br from-order-bg via-white to-order-bg shadow-md ring-1 ring-order-border">
        <div className="grid grid-cols-1 gap-4 px-5 py-4 lg:grid-cols-[200px_1fr_280px]">
          <div className="flex flex-col items-center justify-center">
            ...
            <div className="mt-2 mono text-[10px] text-content-3">
              {master.code}
            </div>
          </div>
          <div>
            ...
            <p className="mt-1 text-sm text-content-3">
              {master.variant_axes.length} axes available.{" "}
              {requiredAxes.length > 0 && (
                <span>
                  Required:{" "}
                  <span className="font-bold text-danger-fg">
                    {requiredAxes.map((a) => a.axis).join(", ")}
                  </span>
                  .
                </span>
              )}
        ...
        <div className="space-y-4">
                            ...
                            return (
                              <button
                                ...
                                className={cn(
                                  ...
                                  active
                                    ? "border-danger-border bg-danger-bg ring-2 ring-danger-border"
                                    : "border-line bg-surface-1 hover:border-info-border hover:shadow-md",
                                )}
                              >
                                <div className="flex items-center justify-between">
                                  <span className="mono font-bold text-content-1 text-xs">
                                    {s.code}
                                  </span>
                                  {s.standard_qty && (
          ...
          <div className="rounded-2xl border border-line bg-surface-1 shadow-sm">
            ...
            <div className="space-y-4 px-4 py-4">
              ...
              <AxisBlock
                name="layers"
                label="Per-layer thicknesses · grades · widths"
              >
                <div className="overflow-hidden rounded-xl border border-line">
                  <table className="min-w-full divide-y divide-line text-xs">
                    <thead className="bg-surface-2 text-content-3">
                      <tr>
                      ...
                      {master.layer_template.map((l, i) => {
                        ...
                        return (
                          <tr key={idx}>
                            ...
                            <td className="px-3 py-2">
                              <span className="mono text-[11px] font-bold text-content-2">
                                {l.film_variant_code}
                              </span>
                            </td>
                            <td className="px-3 py-2">
                              <Input
                                value={layerThicknesses[idx] ?? ""}
                                onChange={(e) =>
                                  setLayerThicknesses({
                                    ...layerThicknesses,
                                ...
                                className="w-16 rounded-md border-line bg-surface-1 px-2 py-1 mono text-xs"
                              />
                            </td>
                            <td className="px-3 py-2">
                              {grades.length > 0 ? (
                                <Select
                                  value={grade}
                                  onValueChange={(v) =>
                                    setLayerGrades({ ...layerGrades, [idx]: v })
                                  }
                                >
                                  <SelectTrigger className="h-8 rounded-md border-line mono text-xs">
                            ...
                            <td className="px-3 py-2">
                              <Input
                                ...
                                className="w-20 rounded-md border-line bg-surface-1 px-2 py-1 mono text-xs"
                              />
                            </td>
                          </tr>
                        );
                    ...
                    <tfoot className="bg-surface-2">
                      <tr>
                        ...
                        <td className="px-3 py-2 mono font-bold text-primary">
                          {totalThickness} μ
                        </td>
                        <td
                          colSpan={2}
                          className="px-3 py-2 text-[10px] text-content-4 italic"
                        >
                          computed from sum
                        </td>
                      </tr>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {[
                      ...
                      return (
                        ...
                        >
                          <div className="text-xs font-bold text-content-1">
                            {m}
                          </div>
                        </button>
                      );
          ...
          {overlays.length > 0 && (
            <div className="rounded-2xl border border-warning-border bg-gradient-to-br from-warning-bg to-white shadow-sm">
              <div className="border-b border-warning-border bg-warning-bg px-4 py-2 flex items-center justify-between">
                ...
                <span className="text-[10px] text-content-3">
                  From customer overlays
                </span>
              </div>
              <div className="px-4 py-3 overflow-x-auto">
                <table className="min-w-full text-xs">
                  <thead className="text-content-3">
                    <tr>
                      <th className="px-2 py-1 text-left font-bold uppercase tracking-wider">
                        Customer
                      </th>
                      <th className="px-2 py-1 text-right">Price basis</th>
                      <th className="px-2 py-1 text-right">Active</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-line">
                    {overlays.slice(0, 5).map((o: any) => (
                      <tr key={o.id} className="hover:bg-warning-bg">
                        <td className="px-2 py-1.5 font-bold text-content-1">
        ...
        <aside className="space-y-4 lg:sticky lg:top-4 lg:self-start lg:max-h-[calc(100vh-2rem)] lg:overflow-y-auto">
          ...
          <div className="overflow-hidden rounded-2xl border border-info-border bg-surface-1 shadow-md ring-1 ring-info-border">
            <div className="bg-gradient-to-br from-primary via-order-fg to-order-fg px-4 py-3 text-white">
              ...
              <div className="text-sm font-bold">
                Per 1,000 KG of finished pouches
              </div>
            </div>
            <div className="px-4 py-3">
                ...
                {Object.entries(catalogValues)
                  ...
                  .map(([axis, code]) => (
                    <li key={axis} className="flex items-center gap-1.5">
                      ...
                      <span className="ml-auto mono text-[10px] text-success-fg">
                        live
                      </span>
                    </li>
                  ))}
                {Object.values(catalogValues).filter(Boolean).length === 0 && (
                  <li className="text-[10px] text-content-4 italic">
                    No catalog axes set
                  </li>
                )}
              </ul>
              ...
              <ul className="mt-1.5 space-y-1 text-[11px]">
                {master.layer_template.map((l, i) => {
                  ...
                  const w =
                    layerWidths[idx] || l.default_input_roll_width_mm || 1050;
                  const tone =
                    i === 0
                      ? "sky"
                  ...
                  return (
                    <li key={idx} className="flex items-center gap-1.5">
                      ...
                      <span className="mono text-content-2 truncate flex-1">
                        {l.film_variant_code} · {t}μ · {w}mm
                      </span>
                      <span className="ml-auto mono text-[10px] text-content-2">
                        ≈ {kg} KG
                      </span>
                    </li>
                  );
              ...
              <div className="mt-2 rounded-lg border border-warning-border bg-warning-bg p-2 text-[10px]">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-warning-fg uppercase tracking-wider">
                    Yield
                  </span>
                  <span className="mono font-bold text-warning-fg">
                    ~94% (6% waste)
                  </span>
                </div>
              </div>
          ...
          <div className="rounded-2xl border border-success-border bg-gradient-to-br from-success-bg to-white p-4 shadow-md ring-1 ring-success-border">
            ...
            <ul className="mt-3 space-y-1 text-[11px]">
              <li className="flex items-center justify-between rounded-lg bg-surface-1 px-2.5 py-1.5 ring-1 ring-success-border">
                <span className="rounded-md bg-success-bg px-2 py-0.5 mono font-bold text-success-fg">
                  Pool D
                </span>
                <span className="text-content-3">
                  Laminate · matching artwork
                </span>
                <span className="ml-auto mono font-bold text-success-fg">
                  est.
                </span>
              </li>
            </ul>
            <div className="mt-1.5 text-[10px] text-success-fg">
              <span className="font-bold">Backend</span> will compute exact
              stock fulfillment on save based on invariant signature match.
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-line bg-surface-1 p-3 shadow-sm">
            ...
            <div className="mt-2 flex flex-col gap-1.5">
              <Button
                ...
                className="gap-1.5 rounded-xl bg-gradient-to-r from-order-fg to-order-fg text-white shadow-md hover:shadow-lg"
              >
              ...
              <Button
                ...
                className="rounded-xl border-info-border bg-info-bg text-primary shadow-sm hover:bg-info-bg"
              >
            ...
            {!validation.ok && (
              <div className="mt-2 flex items-start gap-1.5 rounded-lg bg-danger-bg px-2 py-1.5 text-[11px] text-danger-fg ring-1 ring-danger-border">
        ...
        <div className="mx-auto flex max-w-screen-2xl flex-wrap items-center justify-between gap-3">
          <div className="flex flex-wrap items-center gap-2 text-xs">
            ...
            {validation.ok ? (
              ...
              <span className="rounded-full bg-danger-bg px-2.5 py-0.5 font-bold text-danger-fg ring-1 ring-danger-border">
                {validation.errors[0]}
              </span>
            )}
...
function CatalogAxisField({
  ...
  >({
    ...
    queryFn: async () => {
      const respectAllowed = (code: string) =>
        allowedCodes.size === 0 || allowedCodes.has(String(code).toUpperCase());
      if (source === "pod_sku_variant") {
        ...
        return dedupeByCode(
          list
            .filter((p: PodSkuVariant) => respectAllowed(p.code))
            .map((p: PodSkuVariant) => ({
              id: p.id,
              code: p.code,
      ...
      if (source === "addon") {
        ...
        return dedupeByCode(
          list
            ...
            .filter((a: any) => respectAllowed(a.code))
            .map((a: any) => ({
              id: a.id,
              code: a.code,
              label: a.name || a.code,
  ...
  return (
    <AxisBlock name={String(axis.axis)} label={axis.label || ""}>
      <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-2">
        {options.map((o) => {
          ...
          return (
            ...
            >
              ...
              <div className="mt-0.5 text-[10px] text-content-3 line-clamp-1">
                {o.label}
              </div>
            </button>
          );
        ...
        {options.length === 0 && (
          <div className="col-span-full rounded-lg bg-surface-2 px-3 py-2 text-[11px] text-content-3">
            No catalog options found.
          </div>
        )}
      </div>
      {axis.qty_formula && (
        <div className="mt-2 text-[10px] text-content-3">
          qty:{" "}
          <span className="mono font-bold text-content-2">
            {axis.qty_formula}
          </span>
        </div>
      )}
      {axis.auto_demand_in_house && (
        <div className="mt-1 text-[10px] text-success-fg">
          Auto-demand fires if stock short.
        </div>
      )}
    </AxisBlock>
  );
