...
import { Boxes, ChevronDown, Columns3, Download, LayoutGrid, Rows3, Search, Star } from "lucide-react"
import { useQuery } from "@tanstack/react-query"

import { cn } from "@/lib/utils"
import { Input } from "@/components/ui/input"
import { productMasterService, type VariantAxisDef, type ProductVariant, type PreviewBomResult, type ProductMaster } from "@/services/product-master"
...
function axisValueOf(row: ProductVariant, axis: string): string {
    ...
    if (typeof v === "object") {
        const candidate = (v as any).code || (v as any).value || (v as any).label || ""
...
export function VariantsMatrixV37({ productMasterId, rows, axes, renderCards, master, routeSteps, templateName }: VariantsMatrixV37Props) {
    ...
    const persistViews = React.useCallback((next: SavedView[]) => {
        setSavedViews(next)
        try { localStorage.setItem(`pm-views:${productMasterId}`, JSON.stringify(next)) } catch {/* ignore */}
    }, [productMasterId])
    ...
    const usableAxes = React.useMemo(() => {
        const scored = axes.map((a) => ({ a, count: uniqueAxisValues(rows, String(a.axis)).length }))
        return scored.filter((s) => s.count > 0).sort((p, q) => q.count - p.count).map((s) => s.a)
    }, [axes, rows])
    const [xAxis, setXAxis] = React.useState<string>(() => String(usableAxes[0]?.axis || ""))
    const [yAxis, setYAxis] = React.useState<string>(() => String(usableAxes[1]?.axis || usableAxes[0]?.axis || ""))
    ...
    React.useEffect(() => {
        ...
        if (!list.includes(yAxis)) {
            ...
            const alt = list.find((k) => k !== (list.includes(xAxis) ? xAxis : list[0])) || list[0]
            setYAxis(alt)
        }
    ...
    const filteredRows = React.useMemo(() => {
        ...
        return rows.filter((r) => {
            if (q) {
                const hay = [
                    r.code,
                    ...Object.values(r.axis_values || {}).map((v) => (typeof v === "object" ? (v as any).code || (v as any).value : String(v))),
    ...
    function exportCsv() {
        const cols = ["code", ...axes.map((a) => String(a.axis)), "unit_weight_g", "active"]
    ...
    return (
        <div className="space-y-3">
            ...
            <div className="flex flex-wrap items-center gap-2 text-[11px]">
                <span className="text-[10px] font-black uppercase tracking-[0.22em] text-slate-500">Views</span>
                <button
                ...
                {filters.length ? (
                    <>
                        <span className="ml-3 text-[10px] font-black uppercase tracking-[0.22em] text-slate-500">Filters</span>
                        {filters.map((f) => (
                            <button
                                key={f.axis}
                                onClick={() => removeFilter(f.axis)}
                                className="inline-flex items-center gap-1 rounded-full bg-violet-50 px-2.5 py-1 font-bold text-violet-800 ring-1 ring-violet-200 hover:bg-violet-100"
                                title="Remove filter"
                            >
                                {f.axis} · {f.values.join(",")} <span className="opacity-60">×</span>
                            </button>
                        ))}
                    </>
                ) : null}
            ...
            {filteredRows.length === 0 ? (
                ...
                <div className="grid grid-cols-1 gap-3 lg:grid-cols-[minmax(0,1fr)_380px]">
                    ...
                    <div>
                        <VariantLiveRail productMasterId={productMasterId} variant={selected} axes={axes} master={master} routeSteps={routeSteps} templateName={templateName} />
                    </div>
                </div>
            ) : mode === "cards" ? (
                renderCards ? <>{renderCards()}</> : <FallbackCards rows={filteredRows} axes={axes} onPick={setSelected} />
            ) : (
...
function ModeToggle({ mode, setMode, compareCount }: { mode: ViewMode; setMode: (m: ViewMode) => void; compareCount: number }) {
    return (
        <div className="inline-flex rounded-xl bg-slate-100 p-0.5 shadow-inner">
            {([
                { key: "matrix", label: "Matrix", icon: <LayoutGrid className="h-3.5 w-3.5" /> },
                { key: "cards", label: "Cards", icon: <Boxes className="h-3.5 w-3.5" /> },
                { key: "table", label: "Table", icon: <Rows3 className="h-3.5 w-3.5" /> },
            ] as { key: ViewMode; label: string; icon: React.ReactNode }[]).map((b) => (
                <button
                    key={b.key}
            ...
            {compareCount > 0 ? (
                <span className="ml-1 inline-flex h-8 items-center gap-1 rounded-lg bg-emerald-100 px-2.5 text-[11px] font-bold text-emerald-800 ring-1 ring-emerald-200">
...
function PivotMatrix({
    ...
    const weights = React.useMemo(() => rows.map(unitWeightOf).filter((x): x is number => x != null), [rows])
    const minW = weights.length ? Math.min(...weights) : 0
    const maxW = weights.length ? Math.max(...weights) : 0
    const avgUnit = (weight: number | null) => weight != null ? `${weight.toFixed(0)} g` : "—"
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <header className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        <h3 className="font-display text-sm font-bold text-slate-900">{labelOf(axes, yAxis)} × {labelOf(axes, xAxis)} pivot · {rows.length} variants</h3>
                        <div className="text-[10px] text-slate-500">Each cell = one variant · color = total unit weight · click for full BOM</div>
                    </div>
                </div>
                <div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-500">
                    <span>density</span>
                    <span className="rounded bg-violet-100 px-1.5 py-0.5 text-violet-700">light</span>
                    <span className="rounded bg-violet-400 px-1.5 py-0.5 text-white">▮</span>
                    <span className="rounded bg-violet-700 px-1.5 py-0.5 text-white">heavy</span>
                </div>
            </header>
            <div className="overflow-x-auto p-3">
                <table className="min-w-full text-[12px] border-separate border-spacing-y-0">
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {ys.map((y) => {
                            const rowVariants = rows.filter((r) => axisValueOf(r, yAxis) === y)
                            const yTotal = rowVariants.length
                            const yWeights = rowVariants.map(unitWeightOf).filter((x): x is number => x != null)
                                    ...
                                    {xs.map((x) => {
                                        ...
                                        if (!count) return (
                                            <td key={x} className="px-1 py-1 text-center">
                                                <span className="inline-flex h-12 w-16 items-center justify-center rounded-lg bg-slate-50 text-[10px] text-slate-300">—</span>
                                            </td>
                                        )
                                        const v0 = list[0]
                                        const w = unitWeightOf(v0)
                                        const intensity = w != null && maxW > minW ? (w - minW) / (maxW - minW) : 0.4
                                        ...
                                        return (
                                            <td key={x} className="px-1 py-1 text-center">
                                                ...
                                                >
                                                    <span className="text-[10px] font-bold leading-none">{v0.code}</span>
                                                    {w != null ? <span className="mt-0.5 text-[9px] opacity-80">{w.toFixed(0)} g</span> : null}
                                                    {count > 1 ? <span className="text-[8px] opacity-70">+{count - 1}</span> : null}
                                                </button>
                                            </td>
                                        )
                                    })}
...
function cellTone(intensity: number, selected: boolean, compared: boolean): string {
    let base: string
    if (intensity >= 0.66) base = "bg-violet-700 text-white ring-violet-800"
    else if (intensity >= 0.33) base = "bg-violet-400 text-white ring-violet-500"
    else base = "bg-violet-100 text-violet-900 ring-violet-200"
    if (selected) return cn(base, "ring-2 ring-offset-2 ring-violet-600")
    if (compared) return cn(base, "ring-2 ring-emerald-500")
    return base
}
...
function CompareBar({ ids, rows, onClear }: { ids: string[]; rows: ProductVariant[]; onClear: () => void }) {
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-2.5">
            ...
            <div className="flex items-center gap-2">
                <button
                    ...
                    className="inline-flex items-center gap-1 rounded-lg bg-emerald-600 px-3 py-1.5 text-[11px] font-bold text-white disabled:opacity-50 hover:bg-emerald-700"
                >
                    Compare {picked.length} → side-by-side
                </button>
                <button onClick={onClear} className="rounded-lg px-2.5 py-1.5 text-[11px] font-bold text-emerald-800 hover:bg-emerald-100">Clear</button>
            </div>
        </div>
    )
}
...
export function VariantLiveRail({
    ...
    const previewQuery = useQuery({
        queryKey: ["variant-preview-bom", productMasterId, variant?.id || "_none_", customerId || "_no_cust_"],
        queryFn: async () => {
    ...
    if (!variant) {
        return (
            <LiveBomRail
                title={scope === "variant" ? "Engineering BOM · variant scope" : "Live BOM preview"}
    ...
    return (
        <LiveBomRail
            ...
            preview={(previewQuery.data || synthesizePreviewFromVariant(variant, axes)) as PreviewBomResult}
...
function shortVariantTitle(variant: ProductVariant): string {
    ...
    else if (g.width_mm && g.height_mm) parts.push(`${g.width_mm}×${g.height_mm}`)
...
function variantSubtitle(variant: ProductVariant): string {
    ...
    if (axisCount) parts.push(`${axisCount} ${axisCount === 1 ? "axis" : "axes"}`)
    return parts.join(" · ")
}

function deriveMasterFlags(master: ProductMaster) {
    const fa: any = master.fixed_attributes || {}
    const addonsAxisDef = (master.variant_axes || []).find((a) => String(a.axis) === "addon" || String(a.axis) === "addons")
    const addonsAxis: "off" | "optional" | "required" = !addonsAxisDef ? "off" : addonsAxisDef.required ? "required" : "optional"
    return {
...
function FallbackCards({ rows, axes, onPick }: { rows: ProductVariant[]; axes: VariantAxisDef[]; onPick: (r: ProductVariant) => void }) {
    return (
                ...
                <button key={r.id || idx} onClick={() => onPick(r)} className="text-left rounded-2xl border border-slate-200 bg-white p-3 shadow-sm hover:border-violet-300">
                    ...
                    <div className="mt-1 flex flex-wrap gap-1">
                        {axes.slice(0, 4).map((a) => {
                            ...
                            if (!v) return null
                            return <span key={String(a.axis)} className="rounded bg-blue-50 px-1.5 py-0.5 text-[10px] font-mono font-bold text-blue-800 ring-1 ring-blue-200">{fmtAxisLabel(a)}: {v}</span>
                        })}
                    </div>
                </button>
            ))}
            ...
            <div className="overflow-x-auto">
                <table className="min-w-full text-[12px]">
                    <thead className="bg-slate-50/60 border-b border-slate-200 text-slate-500">
                        <tr>
                            <th className="px-3 py-2 text-left font-bold uppercase tracking-wider text-[9px]">Code</th>
                            {axes.slice(0, 6).map((a) => (
                                <th key={String(a.axis)} className="px-3 py-2 text-left font-bold uppercase tracking-wider text-[9px]">{fmtAxisLabel(a)}</th>
                            ))}
                            <th className="px-3 py-2 text-right font-bold uppercase tracking-wider text-[9px]">Unit g</th>
                            <th className="px-3 py-2 text-right font-bold uppercase tracking-wider text-[9px]">Status</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                        {rows.map((r, idx) => (
                            <tr key={r.id || idx} onClick={() => onPick(r)} className="cursor-pointer hover:bg-violet-50/30">
                                <td className="px-3 py-2 font-mono font-bold text-violet-700">{r.code || `V${idx + 1}`}</td>
                                {axes.slice(0, 6).map((a) => (
                                    <td key={String(a.axis)} className="px-3 py-2 font-mono text-[11px] text-slate-700">{axisValueOf(r, String(a.axis)) || "—"}</td>
                                ))}
