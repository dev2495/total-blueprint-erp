...
import { Copy, Layers, Loader2, Plus, Route, ShieldCheck, Trash2 } from "lucide-react"
...
function versionRoot(code?: string | null) {
    const root = String(code || "").trim().toUpperCase()
...
function cleanLayers(rows: LayerTemplateRow[]) {
    return rows
        .map((row, index) => ({
            ...
            film_variant_code: String(row.film_variant_code || "").trim().toUpperCase(),
            thickness_micron: Number(row.thickness_micron || 0),
            default_grade: String(row.default_grade || "").trim(),
        }))
        .filter((row) => row.role || row.film_variant_code || row.thickness_micron > 0)
}

export function ProductMasterCloneDialog({ open, onOpenChange, source }: Props) {
    ...
    const cloneMutation = useMutation({
        mutationFn: async () => {
            ...
            const payload: ProductMasterClonePayload = {
                ...
                fixed_attributes: { ...(source.fixed_attributes || {}), layer_count: nextLayers.length },
                description,
        ...
        onSuccess: async (created) => {
            ...
            await queryClient.invalidateQueries({ queryKey: ["product-master", source?.id] })
            await queryClient.invalidateQueries({ queryKey: ["product-master-sizes", created.id] })
            toast({
        ...
        onError: (err: any) => {
            toast({ title: "Clone failed", description: err?.message || "Try again", variant: "destructive" })
        },
    })
    ...
    const routeName = templates.find((template: any) => template.id === templateId)?.name || "No route selected"
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="flex max-h-[92vh] max-w-5xl flex-col overflow-hidden rounded-2xl p-0">
                <DialogHeader className="border-b border-slate-100 bg-gradient-to-r from-indigo-50 via-white to-violet-50 px-5 py-4">
                ...
                <div className="grid min-h-0 flex-1 gap-4 overflow-y-auto p-5 lg:grid-cols-[330px_minmax(0,1fr)]">
                    <aside className="space-y-4">
                        ...
                        <section className="space-y-3 rounded-2xl border border-slate-200 bg-surface-1 p-4 shadow-sm">
                            ...
                            <div className="grid gap-2">
                                <Label className="text-[10px] font-black uppercase tracking-wider text-slate-500">New name</Label>
                                <Input value={name} onChange={(event) => setName(event.target.value)} />
                            </div>
                            <div className="grid gap-2">
                                <Label className="text-[10px] font-black uppercase tracking-wider text-slate-500">Route at create</Label>
                                ...
                                <div className="inline-flex items-center gap-1.5 rounded-lg bg-blue-50 px-2 py-1 text-[10px] font-bold text-blue-800 ring-1 ring-blue-100">
                            ...
                            <label className="flex items-start gap-3 rounded-xl border border-warning-border bg-amber-50/70 p-3">
                                ...
                                <span>
                                    ...
                                    <span className="mt-0.5 block text-[11px] leading-snug text-amber-800">
                                        Disable the current master after creating this one. Existing orders keep the old disabled record.
                                    </span>
                                </span>
                            </label>
                        </section>
                    ...
                    <section className="min-w-0 rounded-2xl border border-slate-200 bg-surface-1 shadow-sm">
                        <header className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 px-4 py-3">
                            <div>
                                ...
                                <div className="mt-0.5 text-xs text-slate-500">Edit the copied stack now, or leave it unchanged and refine in the editor.</div>
                            </div>
                            <Button type="button" variant="outline" size="sm" onClick={addLayer} className="gap-1.5 rounded-xl">
                        ...
                        <div className="space-y-3 p-4">
                            {layers.length === 0 ? (
                                <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50 p-6 text-center text-sm text-slate-500">
                                    No layer rows. This is valid for packaging/POD setup that will be completed later.
                                </div>
                            ) : (
                                    ...
                                    <div key={index} className="rounded-xl border border-slate-200 bg-slate-50/60 p-3">
                                        ...
                                        <div className="grid gap-2 md:grid-cols-[1fr_1.5fr_120px_120px]">
                                            <div className="grid gap-1">
                                                <Label className="text-[9px] font-black uppercase tracking-wider text-slate-500">Role</Label>
                                                <Input value={layer.role || ""} onChange={(event) => patchLayer(index, { role: event.target.value })} />
                                            </div>
                                            <div className="grid gap-1">
                                                <Label className="text-[9px] font-black uppercase tracking-wider text-slate-500">Film variant</Label>
                                                <Select
                                                    value={layer.film_variant_code || "__none__"}
                                                    onValueChange={(value) => {
                                                        const picked = filmVariants.find((film) => film.code === value)
                                                        patchLayer(index, {
                                                            film_variant_code: value === "__none__" ? "" : value,
                                                ...
                                                >
                                                    ...
                                                    <SelectContent>
                                                        <SelectItem value="__none__">Select film</SelectItem>
                                            ...
                                            <div className="grid gap-1">
                                                ...
                                                <Input type="number" value={layer.thickness_micron || 0} onChange={(event) => patchLayer(index, { thickness_micron: Number(event.target.value || 0) })} />
                                            </div>
                                            <div className="grid gap-1">
                                                <Label className="text-[9px] font-black uppercase tracking-wider text-slate-500">Grade</Label>
                                                <Input value={layer.default_grade || ""} onChange={(event) => patchLayer(index, { default_grade: event.target.value.toUpperCase() })} placeholder="GP" />
                                            </div>
                                        </div>
                                    </div>
                                ))
                ...
                <DialogFooter className="border-t border-slate-100 bg-slate-50 px-5 py-3">
                    <div className="mr-auto hidden items-center gap-1.5 text-[11px] font-bold text-emerald-800 sm:flex">
                        <ShieldCheck className="h-3.5 w-3.5" />
                        New record keeps historical orders on the old master.
                    </div>
                    <Button type="button" variant="outline" className="rounded-xl" onClick={() => onOpenChange(false)} disabled={cloneMutation.isPending}>
                        Cancel
                    </Button>
                    <Button type="button" className="gap-1.5 rounded-xl" onClick={() => cloneMutation.mutate()} disabled={!isValid || cloneMutation.isPending}>
