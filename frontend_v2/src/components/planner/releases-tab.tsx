...
export function ReleasesTab({ context: _context }: ReleasesTabProps) {
    ...
    useEffect(() => {
        if (urlMode === "direct-fg" || urlMode === "match-wip" || urlMode === "fresh") {
            setMode(urlMode)
    ...
    if (mode === "match-wip") {
        return <MatchWipWizard seedOrderKey={seedOrderKey} seedSoItemId={seedSoItemId} onClose={handleClose} />
    }
    if (mode === "fresh") {
    ...
    return (
        <div className="flex flex-col gap-6 px-5 py-5">
            <header className="flex items-center justify-between">
                <div>
                    <h2 className="font-display text-base font-semibold text-slate-900">Release a job</h2>
                    <p className="text-[12px] text-slate-500">Three release modes plus internal launches. Pick a mode below.</p>
                </div>
                <Chip kind="info" size="sm">3 release modes · 2 launch kinds</Chip>
            </header>

            <section className="space-y-2">
                <h3 className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">For sales demand</h3>
            ...
            <section className="space-y-2">
                <h3 className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">In-house demand</h3>
            ...
            {!isWizardActive ? (
                <section className="rounded-lg border border-slate-200 bg-slate-50/60 p-4 text-[12px] text-slate-600">
                    ...
                    <ol className="mt-2 list-inside list-decimal space-y-1">
                        ...
                        <li>Each successful confirm invalidates the planner queries — Demand, Pool and Queue tabs update live.</li>
                    </ol>
                </section>
            ) : null}
        </div>
