...
import { BomPreviewPanel, type BomPreviewData, type BomStep, type BomLine } from "@/components/ds/bom-preview-panel"
...
function bucketIncludes(filter: DueBucket, row: DueBucket): boolean {
    ...
    if (filter === "7d") return row === "today" || row === "overdue" || row === "7d"
    if (filter === "14d") return row !== "30d" && row !== "all"
    if (filter === "30d") return row !== "all"
    return true
}

function statusKind(status: string): "info" | "warn" | "danger" | "success" | "neutral" {
...
function effectiveStatusBucket(order: PlannerControlOrder): "blocked" | "ready" | "review" | "other" {
    ...
    if (order.artwork_gate?.active || order.partial_replan_required) return "review"
    return "other"
}

function formatQty(value: number | undefined | null, uom?: string) {
    if (value === undefined || value === null || Number.isNaN(value)) return "—"
    const fixed = Math.abs(value) >= 100 ? Math.round(value).toLocaleString("en-IN") : value.toFixed(2)
...
function estimatedPackingQty(order: PlannerControlOrder, line: any) {
    ...
    const requiredPcs = explicitRequiredPcs > 0
        ? explicitRequiredPcs
        : requiredKg > 0 && unitWeightG > 0
            ? (requiredKg * 1000) / unitWeightG
            : 0
    if ((basis === "PCS_PER_PACK" || basis === "PRIMARY_INNER_PACK" || pcsPerPack > 0) && requiredPcs > 0 && pcsPerPack > 0) {
        return ceilPositive(requiredPcs / pcsPerPack)
    }
    if ((basis === "KG_PER_PACK" || kgPerPack > 0) && requiredKg > 0 && kgPerPack > 0) {
...
function buildBomData(order: PlannerControlOrder): BomPreviewData {
    ...
    for (const line of lines) {
        ...
        if (existing) {
            ...
            existing.theoretical_qty = (existing.theoretical_qty ?? 0) + (Number(line.theoretical_qty) || 0)
        } else {
            grouped.set(key, {
    ...
    const frozenPackagingLines = pkg && Array.isArray(pkg.packaging_lines)
        ? pkg.packaging_lines
    ...
    const pod_lines: BomLine[] = pod?.enabled
        ? [{
            ...
            qty: Number(pod.qty ?? podPlanLine?.theoretical_qty ?? podPlanLine?.planned_issue_qty ?? 0),
            uom: "KG",
    ...
    return {
        ...
        packaging_lines,
        pod_lines,
        variant_code: order.spec_signature ? order.spec_signature.slice(0, 8) : undefined,
...
function geometryTiles(order: PlannerControlOrder): React.ReactNode[] {
    ...
    if (order.fg_type) {
        const fgKind = String(order.fg_type).toUpperCase().includes("ROLL") ? "fg-roll" : "fg-pouch"
        tiles.push(
            <GeoTile
                key="fg-type"
                label="FG type"
...
export function DemandTab({ context }: DemandTabProps) {
    ...
    const salesOrders = useMemo(
        () => allOrders.filter((o) => String(o.order_kind || "").toLowerCase() === "sales"),
        [allOrders],
    )
    ...
    const filtered = useMemo(() => {
        ...
        return salesOrders.filter((order) => {
            ...
            if (statusFilter !== "all" && effectiveStatusBucket(order) !== statusFilter) return false
    ...
    const selected = useMemo(
        () => salesOrders.find((o) => `${o.order_kind}:${o.order_id}` === selectedId) ?? null,
    ...
    const goToReleases = (mode: "direct-fg" | "match-wip" | "fresh") => {
        ...
        if (selected.sales_order_item_id) params.set("so_item_id", selected.sales_order_item_id)
        router.push(`/dashboard/planner/control-tower/plan-queue?${params.toString()}`)
    }
    ...
    return (
        <div className="flex flex-col">
            <div className="flex flex-wrap items-center gap-2 border-b border-slate-100 px-5 py-3">
                ...
                <FilterChip
                    label="Status"
                    value={statusFilter !== "all" ? STATUS_OPTIONS.find((o) => o.value === statusFilter)?.label : undefined}
                    onClick={() => {
                ...
                <FilterChip
                    ...
                    onClear={customerFilter !== "all" ? () => setCustomerFilter("all") : undefined}
                    active={customerFilter !== "all"}
                />
                <FilterChip
                    label="Due"
                    value={dueFilter !== "all" ? DUE_OPTIONS.find((o) => o.value === dueFilter)?.label : undefined}
                    onClick={() => {
                                    ...
                                    >
                                        ...
                                        <td className="px-3 py-2.5 align-top">
                                            <ChipGroup spacing="tight">
                                                ...
                                                {blockers > 0 ? (
                                                    <Chip kind="danger" size="sm">{blockers} blockers</Chip>
                                                ) : null}
                                            </ChipGroup>
                                        </td>
                                        <td className="px-5 py-2.5 text-right align-top">
                                            <Button size="sm" variant="outline" onClick={(e) => { e.stopPropagation(); setSelectedId(id) }}>
                                                Open
                                            </Button>
                                        </td>
                                    </tr>
                        ...
                        <div className="flex flex-col gap-4">
                            <SheetHeader className="space-y-1 pb-2 text-left">
                                ...
                                <ChipGroup className="pt-1" spacing="tight">
                                    <Chip kind={statusKind(selected.status)}>{selected.status || "—"}</Chip>
                                    <Chip kind="neutral">{selected.fg_type || "—"}</Chip>
                                    {selected.delivery_date ? (
                                        <Chip kind="info">Due {formatDate(selected.delivery_date)}</Chip>
                                    ) : null}
                                </ChipGroup>
                            </SheetHeader>

                            <section className="space-y-2">
                                <h3 className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Spec</h3>
                            ...
                            <section className="space-y-2">
                                <h3 className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">BOM</h3>
                                <BomPreviewPanel
                            ...
                            <section className="space-y-2">
                                ...
                                <div className="grid gap-2">
                                    <Button
                                        variant="default"
                                        disabled={!selected.sales_order_item_id || !!selected.release_checklist?.blocked_count}
