...
function classifyRow(row: RollExplorerRow): PoolKind {
  ...
  const finished =
    row.stock_strategy === "FINAL_STOCK" || (row as any).is_fg === true;
    ...
    <div className="flex flex-col">
      <div className="flex flex-wrap gap-2 border-b border-line px-5 py-3">
        {KIND_TABS.map((tab) => (
          ...
          >
            <span>{tab.label}</span>
            <span className="font-mono-token text-[10px] text-content-3">
              {counts[tab.id]}
            </span>
          </button>
        ))}
        <div className="ml-2 flex flex-wrap gap-2">
          <FilterChip
            ...
            onClear={
              ageFilter !== "all" ? () => setAgeFilter("all") : undefined
            }
        ...
        <span className="ml-auto text-[11px] text-content-3">
          ...
          {data?.totals?.weight_kg ? formatKg(data.totals.weight_kg) : "—"}{" "}
          total
        </span>
      </div>
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5 align-top">
                      <div className="text-content-2">
                        {row.display_name || row.variant_display_name || "—"}
                      </div>
          ...
          {selectedRow && (
            <div className="flex flex-col gap-4">
              ...
              <section className="grid grid-cols-2 gap-2 text-[12px]">
                <PoolMeta
                  label="Weight"
                  value={formatKg((selectedRow as any).weight_kg)}
                />
                <PoolMeta
                  label="Width"
                  value={
                    (selectedRow as any).width_mm
                      ? `${Math.round((selectedRow as any).width_mm)} mm`
                      : "—"
                  }
                />
                <PoolMeta
                  label="Thickness"
                  value={
                    (selectedRow as any).thickness_micron
                      ? `${Math.round((selectedRow as any).thickness_micron)} μ`
                      : "—"
                  }
                />
                <PoolMeta
                  label="Length"
                  value={
                    (selectedRow as any).length_m
                      ? `${Math.round((selectedRow as any).length_m)} m`
                      : "—"
                  }
                />
                <PoolMeta
                  label="Location"
                  value={
                    (selectedRow as any).location_name ||
                    (selectedRow as any).location ||
                    "—"
                  }
                />
                <PoolMeta
                  label="Plant"
                  value={(selectedRow as any).plant_name || "—"}
                />
                <PoolMeta
                  label="Origin"
                  value={
                    selectedRow.origin_label || selectedRow.origin_type || "—"
                  }
                />
              ...
              {selectedRow.availability_label ? (
                <section className="rounded-lg border border-line bg-surface-2 px-3 py-2 text-[11px] text-content-2">
