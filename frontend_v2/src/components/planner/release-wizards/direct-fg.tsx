...
import { plannerService, type PlannerInventoryOption } from "@/services/planner"
...
export function DirectFgWizard({ seedOrderKey, onClose }: DirectFgWizardProps) {
    ...
    const [step, setStep] = useState<"pick-line" | "pick-stock" | "confirm">("pick-line")
    ...
    const claimMut = useMutation({
        ...
        onError: (e: any) => setError(e?.response?.data?.detail || e?.message || "Failed to claim"),
    })
    ...
    const togglePick = (option: PlannerInventoryOption) => {
        setPicks((prev) => {
            ...
            if (next[id]) {
                ...
                const cap = Number(option.allocatable_qty_kg || option.quantity_kg || 0)
    ...
    const updateQty = (id: string, value: number) => {
        setPicks((prev) => {
            ...
            const cap = Number(prev[id].option.allocatable_qty_kg || prev[id].option.quantity_kg || 0)
    ...
    return (
        <WizardShell
            ...
            footer={
                <>
                    {step !== "pick-line" ? (
                        <Button variant="ghost" onClick={() => setStep(step === "confirm" ? "pick-stock" : "pick-line")}>
                            Previous
                        </Button>
                    ) : null}
                    {step === "pick-line" ? (
                        <Button disabled={!selected || !selected.sales_order_item_id} onClick={() => setStep("pick-stock")}>
                            Next · pick FG
                        </Button>
                    ) : step === "pick-stock" ? (
            ...
            {step === "pick-line" ? (
                <section className="space-y-2">
                    ...
                    <DemandLineList
                        ...
                        filter={(o) => Boolean(o.sales_order_item_id) && Boolean(o.source_availability?.has_fg)}
                    ...
                    {finishedOptions.length === 0 ? (
                        ...
                        <div className="grid max-h-[320px] gap-2 overflow-y-auto pr-1">
                            {finishedOptions.map((opt) => {
                                ...
                                const pick = picks[id]
                                const cap = Number(opt.allocatable_qty_kg || opt.quantity_kg || 0)
                                return (
                                    <div
                                        key={id}
                                        className={cn(
                                        ...
                                        <div className="flex items-baseline justify-between gap-2">
                                            <div>
                                                ...
                                                <div className="text-[11px] text-slate-500">
                                                    {opt.family_display_name || opt.process_state_label || ""}
                                                </div>
                                            </div>
                                            <ChipGroup spacing="tight">
                                                <Chip kind="info" size="sm" mono>{cap.toFixed(1)} kg</Chip>
                                                {opt.size_line ? <Chip kind="neutral" size="sm">{opt.size_line}</Chip> : null}
                                            </ChipGroup>
                                        </div>
            ...
            {step === "confirm" && selected ? (
                <section className="space-y-3">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                        <SummaryStat label="Required" value={`${required.toFixed(1)} kg`} tone="info" />
                        <SummaryStat label="Allocated" value={`${allocatedTotal.toFixed(1)} kg`} tone={allocatedTotal >= required ? "success" : "warn"} />
                        <SummaryStat label="Picks" value={String(Object.keys(picks).length)} />
                    </div>
