...
export function MatchWipWizard({
  seedOrderKey,
  seedSoItemId,
  onClose,
}: MatchWipWizardProps) {
  ...
  const [step, setStep] = useState<
    "pick-line" | "load" | "pick-rolls" | "confirm"
  >("pick-line");
  ...
  const candidatesQuery = useQuery({
    ...
    enabled:
      !!soItemId &&
      (step === "load" || step === "pick-rolls" || step === "confirm"),
  });
  ...
  const required = Number(
    candidatesQuery.data?.remaining_qty_kg ?? selected?.required_qty_kg ?? 0,
  );
  ...
  const claimMut = useMutation({
    ...
    onError: (e: any) =>
      setError(
        e?.response?.data?.detail || e?.message || "Failed to claim WIP",
      ),
  });
  ...
  return (
        ...
        <>
          {step !== "pick-line" ? (
            <Button
              variant="ghost"
              onClick={() =>
                setStep(step === "confirm" ? "pick-rolls" : "pick-line")
              }
            >
              Previous
            </Button>
    ...
    >
        ...
        <section className="space-y-2">
          ...
          <DemandLineList
            ...
            filter={(o) =>
              ...
              Boolean(o.source_availability?.has_wip)
            }
      ...
      {step === "load" ? (
        <section className="rounded-lg border border-line bg-surface-2 p-4 text-center text-sm text-content-3">
                    ...
                    <div className="flex items-baseline justify-between gap-2">
                      <div>
                        ...
                        <div className="text-[11px] text-content-3">
                          {cand.source_stock_order_no
                            ? `From PSO ${cand.source_stock_order_no}`
                            : "Free WIP"}
                        </div>
                      </div>
