...
import { plannerService, type PlannerSkuVariantPreset } from "@/services/planner"
...
const META: Record<LaunchKind, { title: string; subtitle: string; tone: string }> = {
    PACKAGING_STOCK: {
...
export function InHouseWizard({ kind, onClose }: InHouseWizardProps) {
    ...
    const [step, setStep] = useState<"pick-variant" | "configure" | "confirm">("pick-variant")
    ...
    const variantsQuery = useQuery({
        ...
        queryFn: () => plannerService.getPlannerSkuVariants({ launch_kind: kind, active: true }),
    })
    ...
    const targetQty = Number(qty) || (selected ? Number(selected.default_qty || 0) : 0)
    ...
    const planMut = useMutation({
        ...
        onError: (e: any) => setError(e?.response?.data?.detail || e?.message || "Failed to launch"),
    })

    const stepDefs = [
        { id: "pick-variant", label: "Pick preset", hint: "Internal Product Master preset" },
    ...
    return (
        <WizardShell
            ...
            footer={
                <>
                    {step !== "pick-variant" ? (
                        <Button variant="ghost" onClick={() => setStep(step === "confirm" ? "configure" : "pick-variant")}>
                            Previous
                        </Button>
                    ) : null}
                    {step === "pick-variant" ? (
                        ...
                        <Button disabled={targetQty <= 0} onClick={() => setStep("confirm")}>
                            Next · review
                        </Button>
                    ) : (
                        <Button disabled={planMut.isPending} onClick={() => planMut.mutate()}>
                            ...
                            {variants.map((v) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="truncate text-[12px] text-slate-700">{v.name}</div>
                                        <ChipGroup spacing="tight">
                                            {v.template_name ? <Chip kind="neutral" size="sm">{v.template_name}</Chip> : null}
                                            {v.committed_customer_name ? <Chip kind="info" size="sm">{v.committed_customer_name}</Chip> : null}
                                            {v.committed_artwork_design_code ? <Chip kind="accent" size="sm">{v.committed_artwork_design_code}</Chip> : null}
                                        </ChipGroup>
                                    </button>
                                )
                            })}
