...
export function FreshWizard({ seedOrderKey, onClose }: FreshWizardProps) {
    ...
    const [step, setStep] = useState<"pick-line" | "configure" | "confirm">("pick-line")
    ...
    useEffect(() => {
        if (selected?.stock_strategy === "INTERMEDIATE_POOL") setStrategy("INTERMEDIATE_POOL")
        else if (selected?.stock_strategy === "FINAL_STOCK") setStrategy("FINAL_STOCK")
    }, [selected])

    const planMut = useMutation({
        mutationFn: async () => {
            if (!selected) throw new Error("No demand line selected")
            return plannerService.planOrder(selected.order_kind, String(selected.order_id), {
                option: "FRESH",
        ...
        onError: (e: any) => setError(e?.response?.data?.detail || e?.message || "Failed to create fresh PSO"),
    })
    ...
    return (
        <WizardShell
            ...
            footer={
                <>
                    {step !== "pick-line" ? (
                        <Button variant="ghost" onClick={() => setStep(step === "confirm" ? "configure" : "pick-line")}>
                            Previous
                        </Button>
                    ) : null}
                    {step === "pick-line" ? (
            ...
            {step === "configure" && selected ? (
                <section className="space-y-3">
                        ...
                        <ChipGroup className="mt-2" spacing="tight">
                            ...
                            {selected.effective_dims?.width_mm ? (
                                <Chip kind="info" size="sm" mono>
                                    ...
                                    {selected.effective_dims.height_mm ? `×${Math.round(selected.effective_dims.height_mm)}mm` : "mm"}
                                </Chip>
                            ) : null}
                            {selected.roll_invariants?.thickness_micron ? (
                                <Chip kind="thick" size="sm" mono>
                                    {Math.round(Number(selected.roll_invariants.thickness_micron))}μ
                                </Chip>
                            ) : null}
                        </ChipGroup>
                    </div>
                    ...
                    <div className="space-y-2">
                        <span className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Stock strategy</span>
                        <div className="grid grid-cols-2 gap-2">
                            <button
                                type="button"
                            ...
                            >
                                ...
                                <span className="text-[11px] text-slate-500">Run the full route to FG.</span>
                            </button>
                            <button
                                type="button"
                            ...
                            >
                                ...
                                <span className="text-[11px] text-slate-500">Stop at WIP for shared use.</span>
                            </button>
                        </div>
                    </div>
                </section>
