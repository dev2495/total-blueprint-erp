...
export function WizardShell({
  ...
  return (
    <div className="flex flex-col">
      ...
      {footer ? (
        <div className="sticky bottom-0 flex items-center justify-end gap-2 border-t border-line bg-surface-1/95 px-5 py-3 backdrop-blur">
          {footer}
        </div>
      ) : null}
    </div>
        ...
        return (
          <button
            ...
            className={cn(
              ...
              selected ? "border-info-border bg-info-bg" : "border-line",
            )}
          >
            ...
            <ChipGroup spacing="tight">
              {order.fg_type ? (
                <Chip
                  kind={
                    String(order.fg_type).toUpperCase().includes("ROLL")
                      ? "fg-roll"
                      : "fg-pouch"
                  }
                  size="sm"
...
export function FieldRow({
  ...
  return (
    <label className="flex flex-col gap-1.5">
      ...
      {children}
      {hint ? <span className="text-[11px] text-content-3">{hint}</span> : null}
    </label>
  );
}
