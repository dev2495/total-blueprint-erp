...
const BANDS: Array<{ id: Band; label: string; hint: string; tone: "info" | "process" | "warn" | "success" }> = [
    { id: "planned", label: "Planned", hint: "Awaiting release", tone: "info" },
    { id: "released", label: "Released", hint: "Cleared to run", tone: "process" },
    { id: "running", label: "Running", hint: "On the floor now", tone: "warn" },
    { id: "done", label: "Done · 24h", hint: "Recently completed", tone: "success" },
]
...
function progressPct(job: ProductionJob): number {
    ...
    const done = Number(job.step_produced_primary || (job as any).produced_qty || 0)
    if (!target) return 0
...
export function QueueTab({ context }: { context: TowerTabContext }) {
    ...
    const grouped = useMemo(() => {
        ...
        const buckets: Record<Band, ProductionJob[]> = { planned: [], released: [], running: [], done: [] }
    ...
    return (
        <div className="flex flex-col">
            <div className="flex flex-wrap gap-2 border-b border-slate-100 px-5 py-3 text-[11px] text-slate-500">
            ...
            <div className="grid gap-4 px-5 py-4 lg:grid-cols-2 xl:grid-cols-4">
                {BANDS.map((band) => {
                    ...
                    return (
                        <section
                            key={band.id}
                            className="flex min-h-[180px] flex-col gap-2 rounded-xl border border-slate-200 bg-slate-50/40 p-3"
                        >
                            <header className="flex items-center justify-between">
                                <div>
                                    <div className="font-display text-sm font-semibold text-slate-900">{band.label}</div>
                                    <div className="text-[10px] text-slate-500">{band.hint}</div>
                                </div>
                                <Chip kind={band.tone} size="sm">{list.length}</Chip>
                            </header>
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="flex items-baseline justify-between gap-2">
                                                <span className="font-mono-token text-[12px] font-semibold text-slate-900">{job.job_number}</span>
                                                <Chip kind={progressKind(pct)} size="sm" mono>{pct}%</Chip>
                                            </div>
                                            <div className="truncate text-[12px] text-slate-700">{job.product_name || job.template_name || "—"}</div>
                                            <div className="text-[10px] text-slate-500">
                                                {job.order_number || "—"} · {job.work_center_name || "WC pending"}
                                            </div>
                                            <ChipGroup spacing="tight">
                                                {job.process_code && (
                                                    <Chip kind="process" size="sm">{job.process_code}</Chip>
                                                )}
                                                {job.is_on_hold && (
                                                    <Chip kind="warn" size="sm">Hold</Chip>
                                                )}
                                            </ChipGroup>
                                        </button>
                                    )
                        ...
                        <div className="flex flex-col gap-4">
                            <SheetHeader className="space-y-1 pb-2 text-left">
                                ...
                                <ChipGroup className="pt-1" spacing="tight">
                                    <Chip kind={selectedJob.job_state === "EXECUTING" ? "warn" : "info"}>{selectedJob.job_state}</Chip>
                                    {selectedJob.is_on_hold && <Chip kind="danger">On hold</Chip>}
                                    {selectedJob.work_center_name && <Chip kind="neutral">{selectedJob.work_center_name}</Chip>}
                                </ChipGroup>
                            </SheetHeader>

                            <section className="grid grid-cols-2 gap-2 text-[12px]">
                                <Meta label="Sales order" value={selectedJob.order_number || "—"} />
                                <Meta label="Customer" value={selectedJob.customer_name || "—"} />
                                <Meta label="Quantity" value={formatQty(selectedJob.quantity, selectedJob.uom)} />
                                <Meta label="Remaining" value={formatQty(selectedJob.remaining_qty, selectedJob.uom)} />
                                <Meta label="Step target" value={formatQty(selectedJob.step_target_primary, selectedJob.primary_uom)} />
                                <Meta label="Produced" value={formatQty(selectedJob.step_produced_primary, selectedJob.primary_uom)} />
                                <Meta label="Planned date" value={formatDateTime(selectedJob.planned_date)} />
                                <Meta label="Priority" value={String(selectedJob.priority ?? "—")} />
                            </section>
