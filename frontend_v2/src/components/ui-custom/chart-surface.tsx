...
type ChartSurfaceRender = (dimensions: { width: number; height: number }) => React.ReactNode
...
export function ChartSurface({
  ...
  useEffect(() => {
    if (!mounted || !ref.current || typeof ResizeObserver === "undefined") return
  ...
  return (
    <div ref={ref} className={cn("h-full min-h-[280px] w-full", className)}>
      {typeof children === "function" ? (children as ChartSurfaceRender)(dimensions) : children}
    </div>
  )
}
