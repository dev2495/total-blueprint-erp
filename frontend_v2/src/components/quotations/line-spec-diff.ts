...
import type { QuoteLineSpec, BomLayer, BomAdhesive, BomInk, BomAddon } from "@/services/quotation"
...
export function diffSpecVsMaster(
    ...
    for (let i = 0; i < maxL; i += 1) {
        ...
        const pos = (b as BomLayer).position || (a as BomLayer).position || `L${i + 1}`
    ...
    const specFeatures = (spec as { features?: Record<string, boolean> }).features || {}
    const allKeys = new Set([...Object.keys(masterFeatures), ...Object.keys(specFeatures)])
...
export function bomStripText(spec: QuoteLineSpec & { features?: Record<string, boolean> }): string {
    const parts: string[] = []
    ...
    const feats = Object.entries((spec as { features?: Record<string, boolean> }).features || {})
        .filter(([, v]) => v)
