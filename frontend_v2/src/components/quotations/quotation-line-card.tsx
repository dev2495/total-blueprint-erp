...
const EMPTY_SPEC: LineSpecValue = {
    ...
    flap_mm: 0,
    layers: [
        { position: "L1", material_name: "PET 12", micron: 12, gsm: 14.4, rate_per_kg: 142 },
        { position: "L2", material_name: "MET-PE 25", micron: 25, gsm: 22.5, rate_per_kg: 198 },
        { position: "L3", material_name: "PE 65", micron: 65, gsm: 59.5, rate_per_kg: 128 },
    ],
...
function specSnapshotToLineSpec(item: DraftItem): LineSpecValue {
    const s = (item.spec_snapshot || {}) as Record<string, unknown> & QuoteLineSpec & {
        adhesive?: { material_id?: string; code?: string; name?: string; gsm?: number; rate_per_kg?: number }
        ink?: { material_id?: string; code?: string; name?: string; gsm?: number; rate_per_kg?: number; coverage?: string }
        addons?: Array<{ material_id?: string; code?: string; name: string; qty_per_pouch?: number; rate_per_kg?: number }>
    ...
    const adhesive = s.adhesive || {
        ...
        rate_per_kg: Number(s.adhesive_rate_per_kg ?? EMPTY_SPEC.adhesive.rate_per_kg),
    ...
    const layers = (s.layers as Array<Record<string, unknown>> | undefined)?.map((l, idx) => ({
...
function lineSpecToSpecSnapshot(value: LineSpecValue, masterSnapshot?: MasterSnapshot): Record<string, unknown> & QuoteLineSpec {
    return {
...
export default function QuotationLineCard({
    ...
    const masterSnapshot = (item.spec_snapshot as { master_snapshot?: MasterSnapshot } | undefined)?.master_snapshot
    ...
    const bomMut = useMutation({
        ...
        onSuccess: (data, pmId) => {
            ...
            const masterAlreadyAttached =
                (currentSpec.master_snapshot as { product_master_id?: string } | undefined)?.product_master_id ===
                pmId
    ...
    useEffect(() => {
        const pmId = (item.spec_snapshot as { product_master_id?: string } | undefined)?.product_master_id
        if (!pmId) {
            setBomData(null)
            return
        }
    ...
    }, [(item.spec_snapshot as { product_master_id?: string } | undefined)?.product_master_id])

    // ── Diff detection ─────────────────────────────────────────────────────
    const modifiedFields = useMemo(() => {
        if (item.line_kind !== "CATALOG" || !masterSnapshot) return []
        const spec = item.spec_snapshot as QuoteLineSpec & { features?: Record<string, boolean> }
    ...
    const previewMut = useMutation({
        ...
        onSuccess: (data) => {
            onChange({
                ...item,
                margin_pct: item.margin_lock ? item.margin_pct : Number(data.margin_pct || 0),
                rate:
    ...
    const handleCatalogPick = (sel: CatalogPickerSelection | null) => {
        ...
        onChange({
            ...
            spec_snapshot: {
                ...
                width_mm: sel.width_mm || (item.spec_snapshot as QuoteLineSpec).width_mm,
                height_mm: sel.height_mm || (item.spec_snapshot as QuoteLineSpec).height_mm,
                gusset_mm: sel.gusset_mm || (item.spec_snapshot as QuoteLineSpec).gusset_mm,
            },
        })
    }
    ...
    const kindBadge =
        item.line_kind === "AD_HOC" ? (
            <span className="inline-flex items-center gap-1 h-5 px-2 rounded-full text-[9px] font-extrabold uppercase tracking-widest bg-violet-100 text-violet-700 ring-1 ring-violet-200">
                <Wand2 className="h-3 w-3" strokeWidth={2.5} />
                Ad-hoc
            </span>
        ) : (
            <span className="inline-flex items-center gap-1 h-5 px-2 rounded-full text-[9px] font-extrabold uppercase tracking-widest bg-emerald-100 text-emerald-700 ring-1 ring-emerald-200">
    ...
    const stripText = bomStripText(lineSpec as unknown as QuoteLineSpec & { features?: Record<string, boolean> })
    ...
    return (
        <div className="rounded-2xl bg-white ring-1 ring-slate-200/60 shadow-[0_18px_42px_-34px_rgba(15,23,42,0.32)] overflow-hidden">
            {/* Compact row */}
            <div className="px-4 py-3 flex items-center gap-3">
                <button
                    onClick={onToggle}
                    className="h-7 w-7 inline-flex items-center justify-center rounded-lg text-slate-500 hover:bg-slate-100"
                ...
                <span className="font-mono text-[11px] font-extrabold text-slate-400 w-8 text-center">
                    #{item.line_no}
                </span>
                {kindBadge}
                <div className="flex-1 min-w-0">
                    <input
                        ...
                        className="w-full text-sm font-extrabold text-slate-900 bg-transparent outline-none truncate"
                        placeholder="Line name"
                    />
                    {stripText ? (
                        <div className="mt-0.5 text-[10px] font-bold font-mono text-slate-500 truncate">
                            {stripText}
                        </div>
                    ) : null}
                </div>
                {item.line_kind === "CATALOG" && modifiedFields.length > 0 ? (
                    <span className="inline-flex items-center h-5 px-2 rounded-full text-[10px] font-extrabold uppercase tracking-widest bg-amber-100 text-amber-700 ring-1 ring-amber-200">
                        Modified · {modifiedFields.length}
                    </span>
                ) : null}
                {item.line_kind === "CATALOG" && masterSnapshot && modifiedFields.length === 0 ? (
                    <span className="inline-flex items-center h-5 px-2 rounded-full text-[10px] font-extrabold uppercase tracking-widest bg-emerald-100 text-emerald-700 ring-1 ring-emerald-200">
                        Master
                    </span>
                ) : null}
                <div className="hidden md:flex items-center gap-2">
                    <div className="font-mono text-xs font-bold text-slate-500">
                        QTY <span className="text-slate-900">{inr(item.qty)}</span> {item.uom}
                    </div>
                    <div className="font-mono text-xs font-bold text-slate-500">
                        ₹ <span className="text-slate-900">{inr(item.rate)}</span>
                    </div>
                ...
                {item.margin_pct !== null && item.margin_pct !== undefined ? (
                    <span className="inline-flex items-center h-5 px-2 rounded-full text-[10px] font-extrabold uppercase tracking-widest bg-indigo-50 text-indigo-700 ring-1 ring-indigo-200 font-mono">
                        {inr(item.margin_pct)}%
                    </span>
                ) : null}
                <button
                    onClick={onDuplicate}
                    className="h-8 w-8 inline-flex items-center justify-center rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-100"
            ...
            {isOpen ? (
                <div className="border-t border-slate-100 p-4 bg-slate-50/40 space-y-4">
                    {/* Price/cost ribbon */}
                    <div className="rounded-xl bg-gradient-to-r from-indigo-600 via-violet-600 to-fuchsia-600 text-white p-3 flex flex-wrap items-center gap-3 shadow-[0_18px_42px_-30px_rgba(99,102,241,0.6)]">
                        ...
                        {costing?.margin_source ? (
                            <span className="ml-auto inline-flex items-center h-6 px-2.5 rounded-full text-[10px] font-extrabold uppercase tracking-widest bg-white/15 ring-1 ring-white/25">
                    ...
                    {item.line_kind === "CATALOG" ? (
                        <CatalogLinePicker
                            valuePmId={
                                (item.spec_snapshot as { product_master_id?: string }).product_master_id || null
                            }
                            valuePmCode={
                                (item.spec_snapshot as { product_master_code?: string }).product_master_code || null
                            }
                            valuePmName={item.line_name || null}
                            valueSizeId={(item.spec_snapshot as { size_id?: string }).size_id || null}
                    ...
                    {item.line_kind === "CATALOG" && masterSnapshot ? (
                        <VarianceRibbon
                            ...
                            masterCode={
                                (item.spec_snapshot as { product_master_code?: string }).product_master_code ||
                                undefined
                            }
                            onReset={handleResetToMaster}
                            onPromote={() => {
                                const pmId =
                                    (item.spec_snapshot as { product_master_id?: string }).product_master_id
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-4">
                        <div>
                            ...
                            {/* Qty + UOM */}
                            <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-3 rounded-xl border border-slate-200 bg-white p-3">
                                ...
                                <label className="block">
                                    ...
                                    <select
                                        value={item.uom}
                                        onChange={(e) =>
                                            onChange({ ...item, uom: e.target.value as DraftItem["uom"] })
                                        }
                                        className="mt-1 h-10 w-full rounded-lg border border-slate-200 px-3 text-sm font-bold outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-200"
                                    >
                                ...
                                <div>
                                    ...
                                    <div className="mt-1 h-10 inline-flex items-center font-mono font-extrabold text-slate-900">
                                        ₹ {inr(item.rate)}
                                    </div>
                                </div>
                                <div>
                        ...
                        <div className="space-y-3">
                                ...
                                result={
                                    costing
                                        ? ({
                                              ...
                                              conversion_cost_per_kg: costing.conversion_cost_per_kg || 0,
                            ...
                            {costing?.margin_source === "CUSTOMER" ? (
                                <div className="rounded-xl border border-emerald-200 bg-emerald-50/60 p-3 text-[11px] font-bold text-emerald-800">
                                    <div className="text-[10px] font-extrabold uppercase tracking-widest text-emerald-700 mb-1">
                                        Customer overlay applied
                                    </div>
                                    Margin floor of {inr(costing.margin_pct)}% comes from a customer-specific
                                    product overlay. Override only with approval.
                                </div>
                            ) : null}
                        </div>
                    </div>
                    ...
                    <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-100">
                        {canPersist && onSaveLine ? (
                            <button
                                onClick={onSaveLine}
                                className="h-9 px-3 inline-flex items-center gap-2 rounded-lg bg-indigo-600 text-white font-extrabold text-xs uppercase tracking-widest hover:bg-indigo-700"
                            >
                        ...
                        <button
                            onClick={onRemove}
                            className="h-9 px-3 inline-flex items-center gap-2 rounded-lg text-rose-600 hover:bg-rose-50 font-bold text-xs uppercase tracking-widest"
                        >
