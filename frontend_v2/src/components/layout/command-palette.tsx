...
function TypeIcon({ type }: { type: string }) {
  switch (type) {
    case "order":
      return <Package className="mr-2 h-4 w-4 text-primary" />;
    case "job":
      return <Factory className="mr-2 h-4 w-4 text-success-fg" />;
    case "customer":
      return <Users className="mr-2 h-4 w-4 text-warning-fg" />;
    case "machine":
      return <Cpu className="mr-2 h-4 w-4 text-primary" />;
    case "work_center":
      return <Building2 className="mr-2 h-4 w-4 text-info-fg" />;
    case "route":
      return <Compass className="mr-2 h-4 w-4 text-content-3" />;
    default:
...
function StatusBadge({ status }: { status?: string }) {
  ...
  if (value === "RUNNING" || value === "IN_PRODUCTION" || value === "EXECUTING")
    variant = "secondary";

  return (
    <Badge
      variant={variant}
      className="ml-auto h-4 px-1 text-[10px] font-bold uppercase tracking-wider"
    >
      {value}
    </Badge>
  );
...
export function CommandPalette({
    ...
    NAV_ITEMS.forEach((item) => {
      ...
      if (canAccessNavTarget(item, accessContext) && parentTarget) {
        entries.set(parentTarget, {
          ...
          label: item.title,
          subtitle:
            parentTarget === item.href
              ? item.href
              : `${item.href} -> ${parentTarget}`,
          href: parentTarget,
          status: "",
          score: 0,
        });
      ...
      (item.children || []).forEach((child) => {
        ...
        entries.set(childTarget, {
          ...
          label: child.title,
          subtitle:
            childTarget === child.href
              ? child.href
              : `${child.href} -> ${childTarget}`,
          href: childTarget,
          status: "",
          score: 0,
        });
  ...
  const localRouteMatches = React.useMemo<RouteIndexItem[]>(() => {
    ...
    return routeIndex
      .map((item) => {
        ...
        const score = item.label.toLowerCase().startsWith(q)
          ? 130
          : labelMatch
            ? 100
            : 80;
  ...
  React.useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      setLoading(true);
      try {
        const { data } = await api.get<SearchV2Response>(
          "/api/dashboard/search-v2",
          {
            params: { q, limit: 30 },
          },
        );
  ...
  const grouped = React.useMemo(() => {
    ...
    mergedResults.forEach((item) => {
      const key =
        item.group || (item.type === "route" ? "Navigate" : "Results");
    ...
    <>
      ...
      >
        ...
        <span className="truncate">
          {compact ? (
            "Search routes, orders, jobs..."
          ) : (
            <>
              <span className="hidden lg:inline-flex">
                Search commands, routes, records...
              </span>
      ...
      <CommandDialog open={open} onOpenChange={setOpen}>
        <div className="border-b px-3 pb-2">
          ...
          />
          {meta ? (
            <div className="flex flex-wrap items-center gap-2 px-2 pt-1 text-[10px] text-content-3">
              <Badge variant="outline">{mergedResults.length} results</Badge>
              <Badge variant="outline">{meta.took_ms} ms</Badge>
              {Object.entries(meta.counts_by_type || {}).map(
                ([type, count]) => (
        ...
        <CommandList className="max-h-[380px]">
          ...
          {!loading && query.trim().length < 2 ? (
            <div className="py-10 text-center text-content-4">
              ...
              <p className="mt-1 text-[10px]">
                Type at least 2 characters for full scoped search
              </p>
            </div>
          ) : null}
          ...
          {!loading && query.trim().length >= 2 && !hasResults ? (
            <CommandEmpty className="py-10 text-center">
              <AlertCircle className="mx-auto mb-2 h-8 w-8 text-content-4" />
                ...
                <CommandGroup key={groupName} heading={groupName}>
                  {items.map((item) => (
                    <CommandItem
                      ...
                      className="flex cursor-pointer items-center px-4 py-3 hover:bg-surface-2"
                    >
                      <TypeIcon type={item.type} />
                      <div className="flex min-w-0 flex-col">
                        <span className="truncate font-medium text-content-2">
                          {item.label}
                        </span>
                        <span className="truncate text-[10px] tracking-tight text-content-4">
