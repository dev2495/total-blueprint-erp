...
function locationLabel(location: Location) {
  return [location.plant_name, location.code, location.name]
    .filter(Boolean)
    .join(" · ");
}
...
function materialCategory(material: any) {
  return String(material?.category || "")
    .trim()
    .toUpperCase();
}
...
function defaultUomForReceiptClass(
  klass: ClassKind,
  material?: any,
): ReceiptUom {
  const category = materialCategory(material);
  if (category === "ADDON") {
    const addonUom = normalizeUom(
  ...
  if (klass === "PACKAGING" || klass === "TRADING" || category === "PACKAGING")
    return "PCS";
  return "KG";
}
...
function materialBaseUom(material: any, klass: ClassKind) {
  ...
  return isReceiptUom(normalized)
    ? normalized
...
function supportedUomsForMaterial(material: any, klass: ClassKind) {
  ...
  return [
    isReceiptUom(base) ? base : defaultUomForReceiptClass(klass, material),
  ];
}

function resolvedReceiptUom(
  ...
  return supported.includes(normalized as ReceiptUom)
    ? normalized
    : supported[0];
}
...
function computeNetFromGrossTare(
  gross?: string | number,
  tare?: string | number,
) {
...
function isExtrudableFilm(material: any) {
  return (
    materialCategory(material) === "FILM_VARIANT" &&
    (material?.is_extrudable === true ||
      String(material?.film_source || material?.source || "").toUpperCase() ===
        "EXTRUSION" ||
...
function isActiveMaterial(material: any) {
  return (
    ...
    (!material?.status || String(material.status).toUpperCase() === "ACTIVE")
  );
}
...
function isPurchasedAddon(material: any) {
  ...
  if (
    Object.prototype.hasOwnProperty.call(material || {}, "addon_is_purchased")
  ) {
    return material?.addon_is_purchased === true;
  }
  return ["KG", "PCS", "METER"].includes(
    String(
      material?.addon_purchase_uom || material?.base_uom || "",
    ).toUpperCase(),
  );
...
function materialMatchesReceiptClass(material: any, klass: ClassKind) {
  ...
  return (
    ["GRANULE", "INK", "ADHESIVE", "SOLVENT", "POD", "ADDON"].includes(
      category,
    ) && isPurchasedAddon(material)
  );
}
...
function poItemToDraft(item: POItem, locationId: string): ItemDraft {
  ...
  const qty =
    openQty > 0
      ...
      : Math.max(
          0,
          Number(item.qty_ordered || 0) - Number(item.qty_received || 0),
        );
  return {
    ...FRESH_ITEM(),
    ...
    location: locationId,
    width_mm:
      item.expected_width_mm != null ? String(item.expected_width_mm) : "",
    thickness_um:
      item.expected_thickness_micron != null
        ? String(item.expected_thickness_micron)
        : "",
    net_weight_kg:
      String(item.uom || "").toUpperCase() === "KG" && qty ? String(qty) : "",
    stock_form: ((item as any).stock_form as StockForm) || "OPEN_WEB",
    width_basis:
      ((item as any).width_basis as WidthBasis) ||
      widthBasisForStockForm((item as any).stock_form),
  };
}
...
function receiptTotalQty(receipt: any, fallback: number) {
  ...
  if (Array.isArray(receipt?.lines))
    return receipt.lines.reduce(
      (sum: number, line: any) => sum + (Number(line.qty_received) || 0),
      0,
    );
  return fallback;
}
...
export function GrnSmartV36() {
  ...
  const [bulkMaterialFilter, setBulkMaterialFilter] =
    React.useState<BulkMaterialFilter>("ALL");
  ...
  const [lastPosted, setLastPosted] = React.useState<PostedReceipt | null>(
    null,
  );
  const [rollUploadPreview, setRollUploadPreview] = React.useState<{
    rows: number;
    qty: number;
    value: number;
    vendorName?: string;
  ...
  const [tradingSummary, setTradingSummary] = React.useState({
    qty: 0,
    uom: "PCS",
    value: 0,
    valid: false,
  ...
  const granuleCodesQ = useQuery({
    queryKey: ["granule-codes", "active"],
  ...
  const lineQty = React.useCallback(
    (line: Pick<ItemDraft, "qty" | "net_weight_kg">) =>
      Number(klass === "ROLL" ? line.net_weight_kg || line.qty : line.qty) || 0,
    [klass],
  );
  const subtotal = items.reduce(
    (s, i) => s + lineQty(i) * (Number(i.unit_cost) || 0),
    0,
  );
  ...
  const queryError =
    ...
    gradesQ.error ||
    granuleCodesQ.error ||
    openPurchaseOrdersQ.error ||
    selectedPoQ.error;
  const lineComplete = React.useCallback(
    (line: ItemDraft) => {
      const material = (materials as any[]).find(
  ...
  const valid =
    !!warehouseId &&
    !!vendorId &&
    items.every(lineComplete) &&
    (sourceType !== "PO" || (!!poId && items.every((i) => i.po_item_id)));

  React.useEffect(() => {
    if (sourceType !== "PO" || !selectedPo) return;
    setVendorId(selectedPo.vendor || "");
    const samePlantLocation = locations.find(
      (loc: any) => String(loc.plant) === String(selectedPo.plant),
    );
    if (!warehouseId && samePlantLocation?.id)
      setWarehouseId(samePlantLocation.id);
    const lineLocation = warehouseId || samePlantLocation?.id || "";
    const poLines = (selectedPo.items || []).filter(
      (line) =>
        Number(line.qty_open || 0) > 0 &&
        poItemMatchesReceiptClass(line, klass),
    );
    setItems(
      poLines.length
        ? poLines.map((line) => poItemToDraft(line, lineLocation))
        : [FRESH_ITEM()],
    );
  ...
  const submitMutation = useMutation({
      ...
      if (sourceType === "PO") {
        ...
        const poLines = items
          ...
          .map((line) => ({
            ...
            thickness_micron: line.thickness_um
              ? Number(line.thickness_um)
              : undefined,
            stock_form: line.stock_form || "OPEN_WEB",
            width_basis:
              line.width_basis || widthBasisForStockForm(line.stock_form),
          }));
        if (!poLines.length)
          throw new Error(
      ...
      const basePayload: any = {
        ...
        lines: items.map(({ id, ...rest }) => {
          const material = materials.find(
            (entry) => String(entry.code) === String(rest.material_code),
          );
          return {
            material_code: rest.material_code,
            grade_id: klass === "ROLL" ? rest.grade || undefined : undefined,
            granule_code_id:
              klass === "BULK" ? rest.granule_code_id || undefined : undefined,
            ...
            gross_weight_kg: rest.gross_weight_kg
              ? Number(rest.gross_weight_kg)
              : undefined,
            tare_weight_kg: rest.tare_weight_kg
              ? Number(rest.tare_weight_kg)
              : undefined,
            length_m: rest.length_m ? Number(rest.length_m) : undefined,
            width_mm: rest.width_mm ? Number(rest.width_mm) : undefined,
            thickness_um: rest.thickness_um
              ? Number(rest.thickness_um)
              : undefined,
            core_size_inch: rest.core_size_inch
              ? Number(rest.core_size_inch)
              : undefined,
            stock_form:
              klass === "ROLL" ? rest.stock_form || "OPEN_WEB" : undefined,
            width_basis:
              klass === "ROLL"
                ? rest.width_basis || widthBasisForStockForm(rest.stock_form)
                : undefined,
          };
        }),
      };
    ...
    onSuccess: (receipt: any) => {
      ...
      toast({
        title: "GRN posted",
        description: `${posted.grn_no} · ${posted.total_qty} ${posted.uom} added to ledger`,
      });
    },
    onError: (err: any) =>
      toast({
        title: "Could not post GRN",
    ...
    onSuccess: (receipt: any) => {
      ...
      const posted: PostedReceipt = {
        ...
        movement_count:
          Number(
            receipt?.rows ||
              (Array.isArray(receipt?.stock_movements)
                ? receipt.stock_movements.length
                : 0),
          ) || 0,
      };
      ...
      setRollReviewRows([]);
      toast({
        title: "Roll GRN uploaded",
        description: `${posted.grn_no} · ${posted.movement_count} rolls · ${posted.total_qty.toLocaleString()} KG`,
      });
    },
    onError: (err: any) => {
      ...
      const first =
        Array.isArray(details) && details.length
          ? `Row ${details[0]?.row || "?"}: ${details[0]?.error || ""}`
          : "";
      toast({
        title: "Excel upload failed",
        description:
          first || describeApiError(err, "Fix the sheet and upload again"),
        variant: "destructive",
      });
    },
    onSettled: () => {
  ...
  const downloadRollTemplateMutation = useMutation({
    ...
    onSuccess: (blob) => {
      ...
      toast({
        ...
        description: "Dropdowns came from the current master database.",
      });
    },
    onError: (err: any) =>
      toast({
  ...
  const postRollReviewMutation = useMutation({
    mutationFn: () =>
      inventoryService.postRollGrnReview({
        ...
        warehouse_id: warehouseId || undefined,
        vendor_invoice_no:
          vendorInvoiceNo || rollUploadPreview?.invoiceNo || undefined,
        vendor_invoice_date:
    ...
    onError: (err: any) => {
      ...
      toast({
        ...
        description:
          first || describeApiError(err, "Fix the review rows and post again"),
        variant: "destructive",
      });
    },
  });

  const updateRollReviewRow = React.useCallback(
    (index: number, patch: Partial<RollReviewRow>) => {
      setRollReviewRows((rows) =>
  ...
  const validateRollUploadFile = React.useCallback(
    (file: File) => {
      setRollUploadPreview(null);
      setRollReviewRows([]);
  ...
  const footerUom =
    klass === "ROLL"
      ? "KG"
      : klass === "TRADING"
        ? tradingSummary.uom || "PCS"
        : items[0]?.uom || "units";
  const footerReady =
    klass === "TRADING"
      ? !!vendorId && !!warehouseId && tradingSummary.valid
      : valid;
  const footerLineCount = klass === "TRADING" ? 1 : items.length;
  const footerIncompleteCount =
    klass === "TRADING"
      ? tradingSummary.valid
        ? 0
        : 1
  ...
  const footerChecks = [
    ...
    {
      label: klass === "TRADING" ? "Receipt row" : "Materials",
      ok:
        klass === "TRADING"
          ? tradingSummary.valid
          : items.every((i) => i.material_code),
    },
    {
      label: "Qty",
      ok:
        klass === "TRADING"
          ? tradingSummary.qty > 0
          : items.every((i) => lineQty(i) > 0),
    },
  ...
  return (
    <div data-testid="smart-grn" className="w-full max-w-none space-y-4 pb-48">
      ...
      <div className="flex items-center justify-between">
        <Link
          href="/inventory"
          className="inline-flex items-center gap-1 text-xs font-bold text-content-3 hover:text-primary"
        >
          ← Stock workspace
        </Link>
      ...
      {queryError && (
        <div className="rounded-2xl border border-danger-border bg-danger-bg px-4 py-3 text-xs text-danger-fg">
          ...
          <div className="mt-0.5">
            {describeApiError(queryError, "Check backend and retry.")}
          </div>
        </div>
      )}
      {lastPosted && (
        <div
          ...
          className="rounded-2xl border border-success-border bg-success-bg px-4 py-3 text-sm text-success-fg shadow-sm ring-1 ring-success-border"
        >
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-start gap-2">
              <CheckCircle2 className="mt-0.5 h-4 w-4 flex-none text-success-fg" />
              <div>
                <div className="font-bold">Posted {lastPosted.grn_no}</div>
                <div className="mt-0.5 text-xs text-success-fg">
                  {lastPosted.klass} inward saved ·{" "}
                  {lastPosted.total_qty.toLocaleString()} {lastPosted.uom} ·{" "}
                  {lastPosted.movement_count} stock movement
                  {lastPosted.movement_count === 1 ? "" : "s"}. The form below
                  is reset for the next receipt.
                </div>
              </div>
            </div>
            <Button
              ...
              className="h-8 rounded-lg border-success-border bg-surface-1 text-xs font-bold text-success-fg hover:bg-success-bg"
            >
              Clear
            </Button>
          </div>
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <h2 className="font-display mt-1 text-2xl font-bold leading-tight">
              Receive goods at spreadsheet speed
            </h2>
            <p className="mt-1 max-w-5xl text-sm text-white/80">
              One invoice can carry many rolls or materials. Use the full-width
              grid below; readiness, totals, GST, freight, incomplete rows, and
              post action stay in the sticky bottom bar.
            </p>
          </div>
        </div>
      </section>
      ...
      <main className="space-y-5">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            {sourceType === "PO" && (
              <Field label="System purchase order" col2>
                ...
                >
                  <SelectTrigger
                    data-testid="smart-grn-system-po"
                    className="h-10 rounded-xl border-line font-mono shadow-sm"
                  >
                    <SelectValue
                      placeholder={
                        openPurchaseOrdersQ.isLoading
                          ? "Loading open purchase orders..."
                          : "Pick open PO waiting to receive"
                      }
                    />
                  </SelectTrigger>
                  <SelectContent>
                    {safeSelectRows(
                      openPurchaseOrders,
                      (po: PurchaseOrderListItem) => po.id,
                    ).map((po: PurchaseOrderListItem) => (
                      <SelectItem
                        key={safeSelectValue(po.id)}
                        value={safeSelectValue(po.id)}
                      >
                        {po.code} · {po.vendor_name || "Vendor"} · open{" "}
                        {(
                          Number(
                            po.open_qty_total ??
                              po.qty_ordered_total - po.qty_received_total,
                          ) || 0
                        ).toLocaleString()}{" "}
                        units
                      </SelectItem>
                    ...
                    {!openPurchaseOrdersQ.isLoading &&
                      openPurchaseOrders.length === 0 && (
                        <SelectItem value="__none__" disabled>
                          No sent/ack/partial PO has open quantity
                        </SelectItem>
                      )}
                  </SelectContent>
                </Select>
                {selectedPo && (
                  <div className="mt-2 rounded-xl border border-info-border bg-info-bg px-3 py-2 text-[11px] text-primary">
                    <div className="font-bold">
                      {selectedPo.code} · {selectedPo.vendor_name || "Vendor"} ·{" "}
                      {selectedPo.status}
                    </div>
                    <div className="mt-0.5">
                      {
                        (selectedPo.items || []).filter(
                          (line) => Number(line.qty_open || 0) > 0,
                        ).length
                      }{" "}
                      open lines ·{" "}
                      {Number(selectedPo.open_qty_total || 0).toLocaleString()}{" "}
                      total open qty
                    </div>
                    {(selectedPo.items || []).filter(
                      (line) =>
                      ...
                      <div className="mt-1 font-bold text-warning-fg">
                        No open {klass.toLowerCase()} lines on this PO. Pick
                        another stock class or PO.
                      </div>
                    )}
                  </div>
                )}
            ...
            <Field label="Vendor">
              <Select
                ...
                disabled={sourceType === "PO"}
              >
                <SelectTrigger
                  data-testid="smart-grn-vendor"
                  className="h-10 rounded-xl border-line shadow-sm"
                >
                  <SelectValue placeholder="Pick vendor" />
                </SelectTrigger>
                <SelectContent>
                  {safeSelectRows(vendors as Vendor[], (v) => v.id).map((v) => (
                    <SelectItem
                      key={safeSelectValue(v.id)}
                      value={safeSelectValue(v.id)}
                    >
                      {v.name}
                    </SelectItem>
                  ))}
            ...
            <Field label="LR / vehicle / driver">
              <Input
                value={lrVehicle}
                onChange={(e) => setLrVehicle(e.target.value)}
                placeholder="LR-MH04-AB-1234 · Suresh K."
                className="h-10 rounded-xl border-line font-mono shadow-sm"
              />
            </Field>
            <Field label="Receiving warehouse" required col2>
              <Select value={warehouseId} onValueChange={setWarehouseId}>
                <SelectTrigger
                  data-testid="smart-grn-warehouse"
                  className="h-10 rounded-xl border-line shadow-sm"
                >
                  <SelectValue placeholder="Pick warehouse" />
                </SelectTrigger>
                <SelectContent>
                  {safeSelectRows(locations as Location[], (l) => l.id).map(
                    (l) => (
                      <SelectItem
                        key={safeSelectValue(l.id)}
                        value={safeSelectValue(l.id)}
                      >
                        {locationLabel(l)}
                      </SelectItem>
                    ),
        ...
        {klass === "TRADING" && (
          <Section
            ...
            title="What did you actually receive?"
            tone="violet"
              ...
              <div className="flex flex-wrap justify-end gap-2">
                {klass === "ROLL" && (
                  <>
                    ...
                    >
                      {downloadRollTemplateMutation.isPending ? (
                ...
                {klass !== "ROLL" && (
                  <Button
                    ...
                    variant="outline"
                    disabled={sourceType === "PO"}
                    onClick={() => setItems([...items, FRESH_ITEM()])}
                    className="rounded-lg gap-1.5"
                  >
            ...
            <div className="space-y-3">
                  ...
                  <div className="flex flex-col gap-3 border-b border-success-border pb-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      ...
                      <div className="mt-1 font-bold">
                        {rollUploadPreview.rows.toLocaleString()} rolls ·{" "}
                        {rollUploadPreview.qty.toLocaleString()} KG
                        {rollUploadPreview.vendorName
                          ? ` · ${rollUploadPreview.vendorName}`
                          : ""}
                      </div>
                      <div className="mt-0.5 text-xs text-success-fg">
                        Edit any row below. Posting creates stock only from this
                        reviewed list.
                      </div>
                    </div>
                    <div className="flex gap-2">
                      <Button
                        ...
                        variant="outline"
                        size="sm"
                        className="rounded-lg border-success-border bg-surface-1 text-success-fg hover:bg-success-bg"
                        onClick={() => {
                          setRollUploadPreview(null);
                          setRollReviewRows([]);
                        }}
                    ...
                    <table className="min-w-[1500px] text-left text-[11px]">
                      ...
                      <tbody>
                        {rollReviewRows.map((row, index) => (
                          ...
                          >
                            <td className="px-2 py-2 font-mono text-content-3">
                            ...
                            <td className="px-2 py-2">
                              <Input
                                value={String(row.material_code || "")}
                                onChange={(e) =>
                                  updateRollReviewRow(index, {
                                    material_code: e.target.value,
                                  })
                                }
                                className="h-8 min-w-[180px] rounded-lg font-mono text-xs"
                              />
                            </td>
                            <td className="px-2 py-2">
                              <Input
                                ...
                                className="h-8 min-w-[260px] rounded-lg text-xs"
                              />
                            </td>
                            <td className="px-2 py-2">
                              <Input
                                value={String(row.batch_no || "")}
                                onChange={(e) =>
                                  updateRollReviewRow(index, {
                            ...
                            <td className="px-2 py-2">
                              <Input
                                value={String(row.gross_weight_kg ?? "")}
                                onChange={(e) =>
                                  updateRollReviewRow(index, {
                            ...
                            <td className="px-2 py-2">
                              <Input
                                value={String(row.tare_weight_kg ?? "")}
                                onChange={(e) =>
                                  updateRollReviewRow(index, {
                                    tare_weight_kg: e.target.value,
                                  })
                                }
                                className="h-8 w-24 rounded-lg font-mono text-xs"
                              />
                            ...
                            <td className="px-2 py-2">
                              <Input
                                value={String(row.net_weight_kg ?? "")}
                                onChange={(e) =>
                                  updateRollReviewRow(index, {
                                    net_weight_kg: e.target.value,
                                  })
                                }
                                className="h-8 w-24 rounded-lg font-mono text-xs"
                              />
                            ...
                            <td className="px-2 py-2">
                              <Input
                                value={String(row.width_mm ?? "")}
                                onChange={(e) =>
                                  updateRollReviewRow(index, {
                                    width_mm: e.target.value,
                                  })
                                }
                                className="h-8 w-24 rounded-lg font-mono text-xs"
                              />
                            ...
                            <td className="px-2 py-2">
                              <Input
                                value={String(row.thickness_micron ?? "")}
                                onChange={(e) =>
                                  updateRollReviewRow(index, {
                                    thickness_micron: e.target.value,
                                  })
                                }
                                className="h-8 w-24 rounded-lg font-mono text-xs"
                              />
                            ...
                            <td className="px-2 py-2">
                              <Input
                                value={String(row.location_code || "")}
                                onChange={(e) =>
                                  updateRollReviewRow(index, {
                            ...
                            <td className="px-2 py-2">
                              <Input
                                value={String(row.qc_status || "PENDING")}
                                onChange={(e) =>
                                  updateRollReviewRow(index, {
                            ...
                            <td className="px-2 py-2">
                              <Input
                                ...
                                className="h-8 min-w-[200px] rounded-lg text-xs"
                              />
                            </td>
                            <td className="px-2 py-2">
                              <Button
                                ...
                                variant="ghost"
                                size="sm"
                                className="h-8 w-8 rounded-lg p-0 text-danger-fg"
                                onClick={() => removeRollReviewRow(index)}
                              >
              ...
              {rollReviewRows.length === 0 && klass !== "ROLL" && (
                <ReceiptFastEntryGrid
                  ...
                  locations={locations as Location[]}
                  granuleCodes={
                    (granuleCodesQ.data || []) as GranuleQualityCode[]
                  }
          ...
          <Section
            ...
            actions={
              ...
              >
                ...
                <ChevronDown
                  className={cn(
                    "ml-1 h-3.5 w-3.5 transition",
                    showQualityDetails && "rotate-180",
                  )}
                />
              </Button>
            }
            ...
            {showQualityDetails ? (
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <Field label="Vendor COA">
                  <label className="flex h-10 items-center gap-2 rounded-xl border border-line bg-surface-1 px-3 text-sm shadow-sm cursor-pointer">
                    <input
                      type="checkbox"
                      checked={coaAttached}
                      onChange={(e) => setCoaAttached(e.target.checked)}
                      className="h-4 w-4 rounded text-success-fg"
                    />
                ...
                <Field label="In-house QC">
                  <label className="flex h-10 items-center gap-2 rounded-xl border border-line bg-surface-1 px-3 text-sm shadow-sm cursor-pointer">
                    <input
                      ...
                      className="h-4 w-4 rounded text-warning-fg"
                    />
              ...
              <div className="rounded-xl border border-warning-border bg-warning-bg px-3 py-2 text-xs text-warning-fg">
                QC and document details are optional. Open this only when COA,
                lab values, or inspector notes are needed.
              </div>
            )}
          </Section>
        )}
          ...
          >
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
              <Field label="Material subtotal">
                <div className="flex h-10 items-center justify-end rounded-xl border border-line bg-surface-2 px-3 text-sm font-mono shadow-sm">
              ...
              <Field label="Taxable base">
                <div className="flex h-10 items-center justify-end rounded-xl border border-info-border bg-info-bg px-3 text-sm font-mono font-bold text-primary shadow-sm">
                  ₹
                  {taxableBase.toLocaleString(undefined, {
                    maximumFractionDigits: 0,
                  })}
                </div>
              </Field>
              <Field label="Grand total">
                <div className="flex h-10 items-center justify-end rounded-xl border border-success-border bg-success-bg px-3 text-sm font-mono font-bold text-success-fg shadow-sm">
                  ₹
        ...
        {klass !== "TRADING" && (
          ...
          >
            <Textarea
              ...
              className="rounded-xl border-line shadow-sm"
            />
          </Section>
        )}
      </main>
        ...
        <div className="mx-auto grid max-w-none gap-2 px-3 py-2 sm:px-5">
          <div className="flex min-w-0 flex-wrap items-center gap-2 text-xs">
            ...
            {klass !== "TRADING" && (
              <span className="rounded-full bg-warning-bg px-2.5 py-0.5 font-bold text-warning-fg ring-1 ring-warning-border">
                Freight/other ₹
                {footerFreightCharges.toLocaleString(undefined, {
                  maximumFractionDigits: 0,
                })}
              </span>
            )}
            <span className="rounded-full bg-[#10233f] px-2.5 py-0.5 font-bold text-white ring-1 ring-primary">
              Total ₹
              {footerValue.toLocaleString(undefined, {
                maximumFractionDigits: 0,
              })}
            </span>
          </div>
          <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex items-center justify-end gap-2">
              ...
              <Button
                variant="outline"
                className="rounded-xl border-info-border bg-info-bg text-primary shadow-sm"
              >
                📂 Save draft
              </Button>
              {klass === "TRADING" ? (
                <span className="rounded-xl bg-order-bg px-3 py-2 text-xs font-bold text-order-fg ring-1 ring-order-border">
                  Post from trading row
                </span>
              ) : (
                <Button
                  data-testid="smart-grn-submit"
                  onClick={() => submitMutation.mutate()}
                  disabled={!valid || submitMutation.isPending}
                  className="gap-1.5 rounded-xl bg-gradient-to-r from-success-fg to-info-fg shadow-md"
                >
...
function Section({
  ...
  return (
    ...
    >
      <header
        className={cn(
          "flex items-start justify-between gap-3 border-b border-line bg-gradient-to-r via-white to-white px-5 py-3.5",
          TONE.bg,
        )}
      >
        <div className="flex items-start gap-3">
          <span
            className={cn(
              "flex h-7 w-7 flex-none items-center justify-center rounded-lg text-white text-xs font-bold shadow-sm ring-1",
              TONE.num,
            )}
          >
            {idx}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active
          ? "border-success-border bg-gradient-to-br from-success-bg to-white ring-2 ring-success-border"
    ...
    >
      <div className="flex items-center gap-2">
        <span
          className={cn(
            "flex h-9 w-9 items-center justify-center rounded-xl shadow-sm ring-1 ring-current/20",
            active ? "bg-success-fg text-white" : "bg-surface-2 text-content-3",
          )}
        >
          {icon}
        </span>
        <span className="text-sm font-bold text-content-1">{label}</span>
      </div>
      <div className="text-[11px] leading-snug text-content-3">{desc}</div>
    </button>
  );
}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active
          ? "bg-gradient-to-r from-success-fg to-info-fg text-white"
...
function BulkMaterialFilterChips({
  value,
  onChange,
  materials,
}: {
  value: BulkMaterialFilter;
  onChange: (value: BulkMaterialFilter) => void;
  materials: any[];
}) {
  const counts = React.useMemo(() => {
    const next: Record<BulkMaterialFilter, number> = {
      ALL: 0,
      GRANULE: 0,
      INK: 0,
      ADHESIVE: 0,
          ...
          >
            ...
            <span
              className={cn(
                ...
                value === filter.id ? "text-white/80" : "text-order-fg",
              )}
            >
              {counts[filter.id] || 0}
            </span>
...
function RollFastEntryGrid({
  ...
  const totalKg = items.reduce(
    (sum, item) => sum + (Number(item.net_weight_kg || item.qty) || 0),
    0,
  );
  const totalValue = items.reduce(
    (sum, item) =>
      sum +
      (Number(item.net_weight_kg || item.qty) || 0) *
        (Number(item.unit_cost) || 0),
    0,
  );
  const incomplete = items.filter(
    (item) => !isRollRowComplete(item, materials),
  ).length;

  const patch = React.useCallback(
    (index: number, rowPatch: Partial<ItemDraft>) => {
      onChange(
        items.map((item, rowIndex) =>
          rowIndex === index ? { ...item, ...rowPatch } : item,
        ),
      );
    },
    [items, onChange],
  ...
  const addRows = React.useCallback(
    (count: number) => {
      ...
      const clones = Array.from({ length: count }).map((_, cloneIndex) => ({
        ...
        width_basis:
          seed.width_basis || widthBasisForStockForm(seed.stock_form),
      ...
      window.setTimeout(() => {
        const next = document.querySelector<HTMLInputElement>(
          `[data-roll-gross-row="${items.length}"]`,
        );
        next?.focus();
        next?.select();
      }, 0);
  ...
  const handleMaterialChange = React.useCallback(
    (index: number, code: string) => {
      const selected = rollMaterials.find(
        (m) => String(m.code) === String(code),
      );
      patch(index, {
        material_code: code,
        grade: "",
  ...
  const handleGrossTareChange = React.useCallback(
    (
      index: number,
      key: "gross_weight_kg" | "tare_weight_kg",
      value: string,
    ) => {
  ...
  const handleStockFormChange = React.useCallback(
    (index: number, value: StockForm) => {
      patch(index, {
        stock_form: value,
        width_basis: widthBasisForStockForm(value),
      });
    },
    [patch],
  );

  const cloneAfter = React.useCallback(
    (index: number) => {
      const current = items[index];
      if (!current) return;
      const seed: Partial<ItemDraft> = {
        ...
        width_basis:
          current.width_basis || widthBasisForStockForm(current.stock_form),
      };
  ...
  const fillDown = React.useCallback(
    (index: number, key: keyof ItemDraft) => {
  ...
  const handlePaste = React.useCallback(
    (event: React.ClipboardEvent<HTMLDivElement>) => {
      ...
      const expanded = parsedRows.map((cols, index) => {
        const material = rollMaterials.find(
          (m) =>
        ...
        const rawStockForm = String(cols[1] || "")
          .trim()
          .toUpperCase()
          .replace(/[\s-]+/g, "_");
        const hasStockFormColumn = [
          "OPEN_WEB",
          "SHEET",
          "LAYFLAT_TUBE",
          "LAY_FLAT_TUBE",
          ...
          "FOLDED_WEB",
          "FOLDED",
        ].includes(rawStockForm);
        const stockForm = hasStockFormColumn
          ? ((rawStockForm === "SHEET"
              ? "OPEN_WEB"
              : rawStockForm === "TUBE" || rawStockForm === "LAY_FLAT_TUBE"
                ? "LAYFLAT_TUBE"
                : rawStockForm === "FOLDED"
                  ? "FOLDED_WEB"
                  : rawStockForm) as StockForm)
          : "OPEN_WEB";
        ...
        const newGrossTareFormat =
          cols.length >= (hasStockFormColumn ? 11 : 10);
        const grossValue = newGrossTareFormat
          ? String(cols[grossCol] || "").trim()
          : "";
        const tareValue = newGrossTareFormat
          ? String(cols[tareCol] || "").trim()
          : "";
        const computedNet =
          Number(grossValue) > 0 &&
          Number(tareValue) >= 0 &&
          Number(grossValue) >= Number(tareValue)
            ? String(
                Number((Number(grossValue) - Number(tareValue)).toFixed(3)),
              )
            : "";
        const net = newGrossTareFormat
          ? String(cols[netCol] || computedNet).trim()
          : String(cols[grossCol] || "").trim();
        const lengthValue = newGrossTareFormat
          ? String(cols[lengthCol] || "").trim()
          : String(cols[hasStockFormColumn ? 5 : 4] || "").trim();
        const gradeValue = newGrossTareFormat
          ? String(cols[gradeCol] || "").trim()
          : String(cols[hasStockFormColumn ? 6 : 5] || "").trim();
        const locationValue = newGrossTareFormat
          ? String(cols[locationCol] || "").trim()
          : String(cols[hasStockFormColumn ? 7 : 6] || "").trim();
        const rateValue = newGrossTareFormat
          ? String(cols[rateCol] || "").trim()
          : String(cols[hasStockFormColumn ? 8 : 7] || "").trim();
        const grade = grades.find(
          (g) =>
            String(g.name || "").toUpperCase() === gradeValue.toUpperCase() ||
            String((g as any).code || "").toUpperCase() ===
  ...
  return (
    <div
      className="overflow-hidden rounded-2xl border border-success-border bg-surface-1 shadow-sm"
      onPaste={handlePaste}
    >
      <div className="flex flex-col gap-3 border-b border-success-border bg-success-bg px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <div className="mt-1 text-sm font-semibold text-content-2">
            Fill row 1, then press Enter in net kg to clone the next roll. Gross
            minus tare auto-fills net. Paste from Excel: material, stock form,
            width, micron, gross, tare, net, length, grade, location, rate.
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <Button
            ...
            variant="outline"
            size="sm"
            className="h-8 rounded-lg bg-surface-1"
            onClick={() => addRow(items[items.length - 1])}
            disabled={lockedToPo}
          >
          ...
          <Button
            ...
            variant="outline"
            size="sm"
            className="h-8 rounded-lg bg-[#10233f] text-white hover:bg-[#18375f] hover:text-white"
            onClick={() => addRows(10)}
            disabled={lockedToPo}
          >
                ...
                >
                  ...
                  <td className="px-2 py-2">
                    <MaterialPicker
                      ...
                      onValueChange={(code) =>
                        handleMaterialChange(index, code)
                      }
                      disabled={lockedToPo}
                  ...
                  <td className="px-2 py-2">
                    <Select
                      value={item.stock_form || "OPEN_WEB"}
                      onValueChange={(value) =>
                        handleStockFormChange(index, value as StockForm)
                      }
                    >
                      <SelectTrigger className="h-8 rounded-lg border-line text-[11px]">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="OPEN_WEB">Open web</SelectItem>
                        <SelectItem value="LAYFLAT_TUBE">
                          Lay-flat tube
                        </SelectItem>
                  ...
                  <td className="px-2 py-2">
                    <Input
                      ...
                      value={item.width_mm || ""}
                      onChange={(e) =>
                        patch(index, { width_mm: e.target.value })
                      }
                      onKeyDown={(e) => handleCellKeyDown(e, index, "width_mm")}
                      placeholder={
                        item.stock_form === "LAYFLAT_TUBE"
                          ? "lay-flat"
                          : "open web"
                      }
                  ...
                  <td className="px-2 py-2">
                    <Input
                      ...
                      onChange={(e) =>
                        patch(index, { thickness_um: e.target.value })
                      }
                      onKeyDown={(e) =>
                        handleCellKeyDown(e, index, "thickness_um")
                      }
                      className="h-8 rounded-lg border-line font-mono text-[11px]"
                    />
                  </td>
                  <td className="px-2 py-2">
                    <Input
                      ...
                      onKeyDown={(e) =>
                        handleCellKeyDown(e, index, "gross_weight_kg")
                      }
                      className="h-8 rounded-lg border-line font-mono text-[11px]"
                    />
                  </td>
                  <td className="px-2 py-2">
                    <Input
                      ...
                      onKeyDown={(e) =>
                        handleCellKeyDown(e, index, "tare_weight_kg")
                      }
                      className="h-8 rounded-lg border-line font-mono text-[11px]"
                    />
                  </td>
                  <td className="px-2 py-2">
                    <Input
                      ...
                      value={item.net_weight_kg || item.qty || ""}
                      onChange={(e) =>
                  ...
                  <td className="px-2 py-2">
                    <Input
                      ...
                      onKeyDown={(e) => handleCellKeyDown(e, index, "length_m")}
                      className="h-8 rounded-lg border-line font-mono text-[11px]"
                    />
                  </td>
                  <td className="px-2 py-2">
                    <Select
                      value={item.grade || "__none__"}
                      onValueChange={(value) =>
                        patch(index, {
                    ...
                    >
                      <SelectTrigger
                        className={cn(
                          "h-8 rounded-lg border-line text-[11px]",
                          needsGrade &&
                            !item.grade &&
                      ...
                      >
                        <SelectValue
                          placeholder={needsGrade ? "Required" : "Optional"}
                        />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="__none__">
                          {needsGrade ? "Pick grade" : "No grade"}
                        </SelectItem>
                        {safeSelectRows(grades, (grade) => grade.id).map(
                          (grade) => (
                            <SelectItem
                              key={safeSelectValue(grade.id)}
                              value={safeSelectValue(grade.id)}
                            >
                              {grade.name}
                            </SelectItem>
                          ),
                    ...
                    >
                      <SelectTrigger
                        className={cn(
                          ...
                          item.location &&
                            defaultLocationId &&
                            item.location !== defaultLocationId &&
                            "border-warning-border bg-warning-bg",
                        )}
                      >
                      ...
                      <SelectContent>
                        ...
                        {safeSelectRows(
                          ...
                          <SelectItem
                            key={safeSelectValue(location.id)}
                            value={safeSelectValue(location.id)}
                          >
                  ...
                  <td className="px-2 py-2">
                    <Input
                      ...
                      onChange={(e) =>
                        patch(index, { unit_cost: e.target.value })
                      }
                      onKeyDown={(e) =>
                        handleCellKeyDown(e, index, "unit_cost")
                      }
                      className="h-8 rounded-lg border-line font-mono text-[11px]"
                    />
                  </td>
                  ...
                  <td className="px-2 py-2">
                    <Button
                      ...
                      disabled={items.length <= 1 || lockedToPo}
                    >
                      <X className="h-3.5 w-3.5" />
                    </Button>
                  </td>
      ...
      <div className="flex flex-col gap-2 border-t border-line bg-surface-1 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="text-[11px] font-semibold text-content-3">
          Width follows stock form: open web = full sheet width; tube = lay-flat
        ...
        <div className="flex flex-wrap items-center gap-2 text-[11px] font-black">
          ...
          <span className="rounded-lg border border-success-border bg-success-bg px-2.5 py-1 text-success-fg">
            {totalKg.toLocaleString(undefined, { maximumFractionDigits: 3 })}{" "}
            net kg
          </span>
          <span
            className={cn(
...
function isRollRowComplete(item: ItemDraft, materials: any[]) {
  ...
  if (!item.material_code || !(Number(item.net_weight_kg || item.qty) > 0))
    return false;
  if (!(Number(item.width_mm) > 0) || !(Number(item.thickness_um) > 0))
    return false;
...
function ReceiptFastEntryGrid({
  ...
  const filteredMaterials = React.useMemo(() => {
    return materials
      ...
      .filter(
        (material) =>
          klass !== "BULK" ||
          bulkMaterialFilter === "ALL" ||
          materialCategory(material) === bulkMaterialFilter,
      )
      .sort((a, b) => String(a.code || "").localeCompare(String(b.code || "")));
  ...
  const gridTitle =
    klass === "PACKAGING" ? "Fast packaging entry" : "Fast bulk entry";
  const pasteHint =
    klass === "PACKAGING"
  ...
  const addRows = React.useCallback(
    (count: number) => {
      ...
      window.setTimeout(() => {
        const next = document.querySelector<HTMLInputElement>(
          `[data-receipt-gross-row="${items.length}"], [data-receipt-qty-row="${items.length}"]`,
        );
        next?.focus();
        next?.select();
      }, 0);
  ...
  const handleMaterialChange = React.useCallback(
    (index: number, code: string) => {
      const selected = filteredMaterials.find(
        (m) => String(m.code) === String(code),
      );
      ...
      patch(index, {
        ...
        grade: "",
        uom: nextUom,
        gross_weight_kg: isWeightBasedUom(nextUom)
          ? items[index]?.gross_weight_kg || ""
          : "",
        tare_weight_kg: isWeightBasedUom(nextUom)
          ? items[index]?.tare_weight_kg || ""
          : "",
      });
    },
  ...
  const handleGrossTareChange = React.useCallback(
    (
      ...
      const selectedMaterial = materials.find(
        (material) => String(material.code) === String(current.material_code),
      );
      const uom = resolvedReceiptUom(current.uom, selectedMaterial, klass);
      const next = { ...current, [key]: value };
      const rowPatch: Partial<ItemDraft> = { [key]: value };
      if (isWeightBasedUom(uom)) {
        const net = computeNetFromGrossTare(
    ...
    (event: React.ClipboardEvent<HTMLDivElement>) => {
      ...
      const expanded = parsedRows.map((cols, index) => {
        ...
        const material = filteredMaterials.find(
          (m) =>
            String(m.code || "").toUpperCase() ===
              materialValue.toUpperCase() ||
            String(m.name || "").toUpperCase() === materialValue.toUpperCase(),
        );
        const newGrossTareFormat =
          klass === "BULK" ? cols.length >= 7 : cols.length >= 6;
        const grossValue = newGrossTareFormat
          ? String(cols[1] || "").trim()
          : "";
        const tareValue = newGrossTareFormat
        ...
        const qtyValue = newGrossTareFormat
          ? String(cols[3] || computedNet).trim()
          : String(cols[1] || "").trim();
        const codeValue =
          klass === "BULK"
            ? String(cols[newGrossTareFormat ? 4 : 2] || "").trim()
            : "";
        ...
        const granuleCode = granuleCodes.find(
          (code) =>
  ...
  return (
    <div
      className="overflow-hidden rounded-2xl border border-order-border bg-surface-1 shadow-sm"
      onPaste={handlePaste}
    >
      <div className="flex flex-col gap-3 border-b border-order-border bg-order-bg px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <div className="mt-1 text-sm font-semibold text-content-2">
            One row per received item. Weight-based rows auto-calculate net from
            gross minus tare; METER/PCS rows use direct received quantity.
          </div>
        ...
        <table className="min-w-[1520px] text-left text-[11px]">
          ...
          <tbody>
            {items.map((item, index) => {
              const selectedMaterial = materials.find(
                (material) =>
                  String(material.code) === String(item.material_code),
              );
              const selectedCategory = materialCategory(selectedMaterial);
              const showGranuleCode =
                klass === "BULK" && selectedCategory === "GRANULE";
              const selectedGranuleCodes = showGranuleCode
                ...
                : [];
              const supportedUoms = supportedUomsForMaterial(
                selectedMaterial,
                klass,
              );
              const rowUom = resolvedReceiptUom(
                item.uom,
                selectedMaterial,
                klass,
              );
              ...
              return (
                ...
                >
                  ...
                  <td className="px-2 py-2">
                    {showGranuleCode ? (
                      <Select
                        value={item.granule_code_id || "__none__"}
                        onValueChange={(value) =>
                          patch(index, {
                            granule_code_id: value === "__none__" ? "" : value,
                          })
                        }
                      >
                        <SelectTrigger
                          data-testid={`smart-grn-line-${index}-granule-code`}
                        ...
                        <SelectContent>
                          ...
                          {safeSelectRows(
                            ...
                            <SelectItem
                              key={safeSelectValue(code.id)}
                              value={safeSelectValue(code.id)}
                            >
                              {code.code}
                            </SelectItem>
                          ))}
                  ...
                  <td className="px-2 py-2">
                    <Input
                      ...
                      className={cn(
                        "h-8 rounded-lg border-line font-mono text-[11px]",
                        !usesGrossTare && "bg-surface-2 text-content-4",
                      )}
                    />
                  </td>
                  <td className="px-2 py-2">
                  ...
                  <td className="px-2 py-2">
                    <Select
                      ...
                      onValueChange={(value) =>
                        patch(index, {
                          uom: value,
                          gross_weight_kg: isWeightBasedUom(value)
                            ? item.gross_weight_kg || ""
                            : "",
                          tare_weight_kg: isWeightBasedUom(value)
                            ? item.tare_weight_kg || ""
                            : "",
                    ...
                    >
                      ...
                      <SelectContent>
                        {supportedUoms.map((uom) => (
                          <SelectItem
                            key={safeSelectValue(uom)}
                            value={safeSelectValue(uom)}
                          >
                            {uom}
                          </SelectItem>
                        ))}
      ...
      <div className="flex flex-col gap-2 border-t border-line bg-surface-1 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="text-[11px] font-semibold text-content-3">
          Header location is the default. The UOM is fixed from the selected
...
function ItemEditor({
  ...
  const selectedMaterial = React.useMemo(
    () =>
      materials.find(
        (material) => String(material.code) === String(item.material_code),
      ),
  ...
  return (
    <div className="rounded-xl border border-line bg-surface-1 p-4 shadow-sm">
      <div className="mb-3 flex items-center justify-between">
        ...
        <button
          onClick={onRemove}
          className="rounded p-1 text-danger-fg hover:bg-danger-bg"
        >
          <X className="h-3.5 w-3.5" />
        </button>
      </div>
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
        <Field label="Material" required>
          {filteredMaterials.length > 0 ? (
            <MaterialPicker
              ...
              placeholder={
                klass === "BULK" ? "Search code, name, type" : "Search material"
              }
              disabled={lockedToPo}
              testId={`smart-grn-line-${index}-material`}
        ...
        {showRollGrade && (
          <Field label="Film grade">
            <Select
              value={item.grade || "__none__"}
              onValueChange={(value) =>
                onChange({ grade: value === "__none__" ? "" : value })
              }
            >
              <SelectTrigger className="h-9 rounded-lg border-line text-xs shadow-sm">
                <SelectValue placeholder="Grade if required" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="__none__">No grade</SelectItem>
                {safeSelectRows(grades, (grade) => grade.id).map((grade) => (
                  <SelectItem
                    key={safeSelectValue(grade.id)}
                    value={safeSelectValue(grade.id)}
                  >
        ...
        {showGranuleCode && (
          <Field label="Granule code">
            <Select
              value={item.granule_code_id || "__none__"}
              onValueChange={(value) =>
                onChange({ granule_code_id: value === "__none__" ? "" : value })
              }
            >
              <SelectTrigger
                data-testid={`smart-grn-line-${index}-granule-code`}
                className="h-9 rounded-lg border-line text-xs shadow-sm"
              >
                <SelectValue placeholder="Pick code" />
              </SelectTrigger>
              <SelectContent>
        ...
        <Field label="UOM">
          <Select
            ...
            onValueChange={(v) =>
              onChange({
                uom: v,
                gross_weight_kg: isWeightBasedUom(v)
                  ? item.gross_weight_kg || ""
                  : "",
                tare_weight_kg: isWeightBasedUom(v)
                  ? item.tare_weight_kg || ""
                  : "",
              })
            }
          ...
          >
            <SelectTrigger
              data-testid={`smart-grn-line-${index}-uom`}
        ...
        <Field label="Vendor lot ref">
          <Input
            ...
            onChange={(e) => onChange({ vendor_lot_ref: e.target.value })}
            className="h-9 rounded-lg border-line font-mono text-xs shadow-sm"
          />
        </Field>
        ...
        <Field label="Storage location">
          <Select
            ...
            onValueChange={(v) => v !== "__none__" && onChange({ location: v })}
          >
            <SelectTrigger
              data-testid={`smart-grn-line-${index}-location`}
              className="h-9 rounded-lg border-line text-xs"
            >
            ...
            <SelectContent>
              ...
              {safeSelectRows(locations, (l) => l.id).map((l) => (
                <SelectItem
                  key={safeSelectValue(l.id)}
                  value={safeSelectValue(l.id)}
                >
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            <Field label="Gross (KG)">
              <Input
                ...
                value={item.gross_weight_kg || ""}
                onChange={(e) =>
                  patchWeight({ gross_weight_kg: e.target.value })
                }
                className="h-8 rounded-md border-line font-mono text-[11px]"
              />
            </Field>
            <Field label="Tare (KG)">
              <Input
                ...
                value={item.tare_weight_kg || ""}
                onChange={(e) =>
                  patchWeight({ tare_weight_kg: e.target.value })
                }
                className="h-8 rounded-md border-line font-mono text-[11px]"
              />
            </Field>
            <Field label="Net auto (KG)">
              <Input
                data-testid={`smart-grn-line-${index}-net`}
                value={item.net_weight_kg || ""}
                readOnly
                className="h-8 rounded-md border-line bg-surface-2 font-mono text-[11px]"
              />
            </Field>
            ...
            <Field label="Thickness (μ)">
              <Input
                data-testid={`smart-grn-line-${index}-thickness`}
                value={item.thickness_um || ""}
                onChange={(e) => onChange({ thickness_um: e.target.value })}
                className="h-8 rounded-md border-line font-mono text-[11px]"
              />
            </Field>
          </div>
  ...
  const post = useMutation({
    ...
    onSuccess: (receipt) => {
      toast({
        ...
        description: `${receipt.code} · ${receipt.qty_received} ${receipt.base_uom || ""}`,
      });
      onPosted(receipt);
      setTradingGoodId("");
      setGross("");
    ...
    <div className="overflow-hidden bg-surface-1 shadow-sm">
      <div className="flex flex-col gap-3 border-b border-order-border bg-order-bg px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <div className="mt-1 text-sm font-semibold text-content-2">
            One row per trading-good receipt. The grid language matches every
            GRN class; KG rows can use gross/tare/net, PCS and METER rows use
            direct quantity.
          </div>
          <div className="mt-1 text-[11px] font-semibold text-content-3">
            Trading goods post through the trading stock ledger, so stock value
        ...
        <table className="min-w-[1520px] text-left text-[11px]">
          ...
          <tbody>
            ...
            >
                ...
                <Select value={tradingGoodId} onValueChange={setTradingGoodId}>
                  ...
                  <SelectContent>
                    {safeSelectRows(tradingGoods, (t) => t.id).map((t) => (
                      <SelectItem
                        key={safeSelectValue(t.id)}
                        value={safeSelectValue(t.id)}
                      >
                        {t.code} · {t.name} ({t.base_uom})
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              <td className="px-2 py-2">
                <Input
                  value={warehouseLabel || "Pick receiving warehouse above"}
                  readOnly
                  className="h-8 rounded-lg border-line bg-surface-2 text-[11px] font-semibold text-content-3"
                />
              </td>
              <td className="px-2 py-2">
                <Input
              ...
              <td className="px-2 py-2">
                <Input
                  ...
                  className="h-8 min-w-[240px] rounded-lg border-line text-[11px]"
                />
              </td>
              <td className="px-2 py-2">
                <span
      ...
      <div className="flex flex-col gap-2 border-t border-line bg-surface-1 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            onClick={() => post.mutate()}
            className="h-8 rounded-lg bg-order-fg text-xs text-white hover:bg-order-fg"
          >
            {post.isPending ? (
