...
export function buildSalesAxisValues(
  ...
  for (const axis of master?.variant_axes || []) {
    ...
    if (canonical === "ARTWORK_MODE") {
      const mode = String(
        line.artwork_mode || axisValues.artwork_mode || "DEFER",
      ).trim();
      if (mode) {
        axisValues[key] = mode;
    ...
    if (canonical === "LAYER_THICKNESSES") {
      if (Object.keys(layerThicknesses).length)
        axisValues[key] = layerThicknesses;
      continue;
    }
  ...
  const missingLabels = missingRequired.map((axisKey) => {
    const axis = (master?.variant_axes || []).find(
      (item) => String(item.axis || "") === axisKey,
    );
...
function requiredAxisIssues(
  ...
  for (const axis of master?.variant_axes || []) {
    ...
    const canonical = normalizeCode(key);
    if (
      GENERATED_AXIS_KEYS.has(key) ||
      GENERATED_AXIS_KEYS.has(key.toLowerCase()) ||
      canonical === "LAYER_ROLL_WIDTHS"
    )
      continue;
    if (canonical === "ADDONS" || canonical === "ADDON") {
      if (
        !axisValuePresent(axisValues[key]) &&
        !axisValuePresent(axisValues.addons) &&
        !axisValuePresent(axisValues.addon)
      )
        missing.push(key);
      continue;
    }
...
export function buildPreviewBlocker(
  ...
  const layerSnapshot = (master.layer_template || [])
    ...
    .filter(
      (row) => row.film_variant_code || row.thickness_micron || row.grade,
    );
...
function filterAxisValuesForMaster(
  axisValues: Record<string, any>,
  master: ProductMaster | undefined,
) {
  if (!master) return;
  const declared = new Set(
    (master.variant_axes || [])
      .map((axis) => String(axis.axis || "").trim())
      .filter(Boolean),
  );
  ...
  Object.keys(axisValues).forEach((key) => {
    ...
    if (
      GENERATED_AXIS_KEYS.has(key) ||
      GENERATED_AXIS_KEYS.has(key.toLowerCase()) ||
      GENERATED_AXIS_KEYS.has(canonical)
    )
      return;
    if (
      canonical === "ADDONS" ||
      canonical === "ADDON" ||
      canonical === "ARTWORK_MODE"
    )
      return;
