...
function describeErrorDetail(detail: unknown): string {
  ...
  if (Array.isArray(detail))
    return detail
      .map((entry) => describeErrorDetail(entry))
      .filter(Boolean)
      .join(" ");
  ...
  return {
    ...
    items: (quotation.items || []).map((item) => ({
      ...
      geometry: {
        base: {
          width_mm: asNumber(
            item.geometry_snapshot?.base?.width_mm ||
              item.geometry_snapshot?.width_mm,
            0,
          ),
          height_mm: asNumber(
            item.geometry_snapshot?.base?.height_mm ||
              item.geometry_snapshot?.height_mm,
            0,
          ),
        },
        pouch_style: String(
          item.geometry_snapshot?.pouch_style || "",
        ).toUpperCase(),
        ...
        adjustments: Array.isArray(item.geometry_snapshot?.adjustments)
          ? item.geometry_snapshot.adjustments.map((adjustment: any) => ({
              ...
              impact: String(
                adjustment?.impact || "WIDTH",
              ).toUpperCase() as AdjustmentDraft["impact"],
            }))
          : [],
        multipliers: {},
      },
      ...
      printing: {
        ...
        type: String(
          item.printing_snapshot?.type || "FLEXO",
        ).toUpperCase() as LineDraft["printing"]["type"],
        substrate_mode: String(
          item.printing_snapshot?.substrate_mode || "SHEET",
        ).toUpperCase() as LineDraft["printing"]["substrate_mode"],
        front_colors_count: asNumber(
          item.printing_snapshot?.front_colors_count,
          0,
        ),
        back_colors_count:
          String(
            ...
            ? asNumber(item.printing_snapshot?.back_colors_count, 0)
            : 0,
      ...
      addons: Array.isArray(item.addons_snapshot)
        ? item.addons_snapshot.map((addon: any) => ({
            ...
            applies_to: String(
              addon?.applies_to || "PER_PIECE",
            ).toUpperCase() as AddonDraft["applies_to"],
          }))
        : [],
      packaging_snapshot: {
        pod: {
          ...
          pod_profile_id: String(
            item.packaging_snapshot?.pod?.pod_profile_id || "",
          ),
          pod_sku_variant_id: String(
            item.packaging_snapshot?.pod?.pod_sku_variant_id || "",
          ),
          pod_sku_code: String(
            item.packaging_snapshot?.pod?.pod_sku_code || "",
          ),
          pod_sku_name: String(
            item.packaging_snapshot?.pod?.pod_sku_name || "",
          ),
        },
        note: String(item.packaging_snapshot?.note || ""),
      },
      process_cost_rows: Array.isArray(item.process_cost_rows)
        ? item.process_cost_rows
        : [],
...
function hydrateLineFromSkuVariant(
  ...
  return {
    ...
    line_name:
      variant.name || sku?.default_line_name || sku?.name || "SKU quote line",
    ...
    film_layers:
      Array.isArray(variant.layer_snapshot) && variant.layer_snapshot.length > 0
    ...
    printing: {
      ...
      substrate_mode: String(
        printing.substrate_mode || "SHEET",
      ).toUpperCase() as LineDraft["printing"]["substrate_mode"],
      front_colors_count: asNumber(printing.front_colors_count, 0),
      back_colors_count:
        String(printing.substrate_mode || "SHEET").toUpperCase() === "TUBING"
          ? asNumber(printing.back_colors_count, 0)
          : 0,
...
export default function QuotationWorkspace() {
  ...
  const [activeLineId, setActiveLineId] = useState<string>(
    draft.items[0].localId,
  );
  const [activePreview, setActivePreview] = useState<QuotationPreview | null>(
    null,
  );
  ...
  const { data: skus = [] } = useQuery({
    ...
    queryFn: () =>
      salesService.getSalesSkus({
        customer_id: draft.customer || undefined,
        active: true,
      }),
  });
  ...
  const { data: families = [] } = useQuery({
    queryKey: ["quote-families"],
    queryFn: filmFamilyService.getAll,
  });
  const { data: variants = [] } = useQuery({
    queryKey: ["quote-variants"],
    queryFn: filmVariantService.getAll,
  });
  const { data: addons = [] } = useQuery({
    queryKey: ["quote-addons"],
    queryFn: masterDataService.getAddons,
  });
  const { data: podProfiles = [] } = useQuery({
    queryKey: ["pod-sku-variants", "quotation-workspace"],
    queryFn: () => masterDataService.getPodSkuVariants({ active: true }),
  });
  const { data: processes = [] } = useQuery({
    queryKey: ["quote-processes"],
    queryFn: factoryService.getProcesses,
  });
  const { data: processRates = [] } = useQuery({
    queryKey: ["quote-process-rates"],
    queryFn: costingService.getProcessRates,
  });

  const activeLineIndex = draft.items.findIndex(
    (item) => item.localId === activeLineId,
  );
  const activeLine = draft.items[activeLineIndex] || draft.items[0] || null;
  const activeSkuVariant = skuVariants.find(
    (variant) =>
      String(variant.id) === String(activeLine?.sku_variant_id || ""),
  );
  const activeSku = skus.find(
    (sku) => String(sku.id) === String(activeSkuVariant?.sku || ""),
  );
  const activeTemplate = templates.find(
    (template) => String(template.id) === String(activeLine?.template_id || ""),
  );
  const lockedPouchStyle = normalizePouchStyle(
    activeTemplate?.pouch_style || activeLine?.geometry.pouch_style || "",
  );
  const showGussetField =
    activeLine?.finished_good_type === "POUCH" &&
    isGussetStyle(lockedPouchStyle);
  const spoutStyle =
    activeLine?.finished_good_type === "POUCH" &&
    isSpoutStyle(lockedPouchStyle);
  const familyScopedVariants = useMemo(
    () =>
      variants.filter(
        (variant: any) =>
          !activeLine?.film_layers[0]?.family_id ||
          variant.parent_family === activeLine?.film_layers[0]?.family_id,
      ),
    [variants, activeLine],
  );
    ...
    () =>
      skuVariants
        ...
        .map((variant) => ({
          ...
          label:
            variant.name ||
            `${variant.sku_name || "SKU"} ${variant.code || ""}`.trim(),
  ...
  useEffect(() => {
    ...
    const currentVariantExists = skuVariantOptions.some(
      (variant) =>
        String(variant.id) === String(activeLine.sku_variant_id || ""),
    );
    ...
    activeLine?.localId,
    activeLine?.source_mode,
    activeLine?.sku_variant_id,
    draft.customer,
    skuVariantOptions,
  ]);
  ...
  useEffect(() => {
    ...
    const handle = window.setTimeout(async () => {
      try {
        setPreviewError("");
        const preview = await salesService.previewQuotationLine(
          buildLinePayload(activeLine, families, variants, addons, draft.plant),
        );
        setActivePreview(preview);
        if (
          (!activeLine.process_cost_rows ||
            activeLine.process_cost_rows.length === 0) &&
          preview.process_cost_rows?.length
        ) {
          setDraft((current) => ({
            ...current,
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const payload = {
        ...
        items: draft.items.map((item) =>
          buildLinePayload(item, families, variants, addons, draft.plant),
        ),
      };
      return draft.id
    ...
    onError: (error: any) => {
      toast({
        title: "Save failed",
        description:
          describeErrorDetail(error?.response?.data?.detail) ||
          error?.message ||
  ...
  const duplicateMutation = useMutation({
    ...
    onSuccess: (quotation) => {
      ...
      toast({
        ...
        description: `${quotation.quote_number} opened as a fresh draft.`,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const convertMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      window.open(
        `/sales/orders/${result.sales_order_id}`,
        "_blank",
        "noopener,noreferrer",
      );
    },
    onError: (error: any) => {
      ...
      toast({
        ...
        description:
          detail || error?.message || "Template mapping is incomplete.",
        variant: "destructive",
      });
    },
  });
  ...
  const metrics = {
    open: quotations.filter((quote) => ["DRAFT", "SENT"].includes(quote.status))
      .length,
    converted: quotations.filter((quote) => quote.status === "CONVERTED")
      .length,
    value: quotations.reduce(
      (sum, quote) => sum + Number(quote.totals_snapshot?.grand_total || 0),
      0,
    ),
    avgMargin: quotations.length
      ? quotations.reduce(
          (sum, quote) =>
            sum + Number(quote.totals_snapshot?.margin_percent || 0),
          0,
        ) / quotations.length
      : 0,
  };
  ...
  const updateLine = (
    lineId: string,
    updater: (line: LineDraft) => LineDraft,
  ) => {
    setDraft((current) => ({
      ...current,
      items: current.items.map((line) =>
        line.localId === lineId ? updater(line) : line,
      ),
    }));
  };
      ...
      items: current.items.map((line) => {
        ...
        if (mode === "SKU") {
          return {
            ...
            commercial_snapshot: {
              ...emptyLineDraft().commercial_snapshot,
      ...
      items: current.items.map((line) =>
        line.localId === lineId
          ? {
              ...
              localId: line.localId,
              qty_value:
                line.qty_value > 0 ? line.qty_value : hydrated.qty_value,
              commercial_snapshot: {
  ...
  const duplicateLine = (lineId: string) => {
    ...
    const clone: LineDraft = {
      ...
      geometry: {
        ...source.geometry,
        adjustments: source.geometry.adjustments.map((adjustment) => ({
          ...adjustment,
          localId: makeId(),
        })),
      },
      film_layers: source.film_layers.map((layer) => ({
        ...layer,
        localId: makeId(),
      })),
      addons: source.addons.map((addon) => ({ ...addon, localId: makeId() })),
      process_cost_rows: source.process_cost_rows.map((row, index) => ({
        ...row,
        sequence: index + 1,
      })),
    };
  ...
  const quotedUnitPrice =
    asNumber(activeLine?.commercial_snapshot?.manual_unit_price, 0) > 0
  ...
  return (
    <PremiumPageShell dataTestId="quotation-workspace" className="min-h-screen">
      <PremiumHero
        ...
        className="border-info-border bg-[linear-gradient(135deg,#2563eb_0%,#3b82f6_42%,#7dd3fc_100%)] shadow-[0_34px_80px_-46px_rgba(37,99,235,0.48)]"
        actions={
          <>
            <Button
              variant="outline"
              className="rounded-2xl border-surface-1/20 bg-surface-1/10 text-white hover:bg-surface-1/15"
              onClick={openNewQuote}
            >
              <Plus className="mr-2 h-4 w-4" /> New Quote
            </Button>
            <Button
              className="rounded-2xl bg-surface-1 text-content-1 hover:bg-surface-2"
        ...
        metrics={
          <PremiumMetricStrip className="xl:grid-cols-4">
            ...
            <PremiumMetricCard
              ...
              hint="Already turned into orders"
              tone="light"
            />
            <PremiumMetricCard
              label="Saved Value"
              value={formatMoney(metrics.value, draft.currency)}
              hint="From saved commercial totals"
              tone="light"
            />
            <PremiumMetricCard
              label="Avg Margin"
              value={`${metrics.avgMargin.toFixed(1)}%`}
              hint="Estimated quote margin"
              tone="light"
            />
          </PremiumMetricStrip>
        }
      ...
      <div className="grid gap-6 xl:grid-cols-[290px_minmax(0,1fr)]">
          ...
          <div className="overflow-hidden rounded-[2rem] border border-line bg-[linear-gradient(180deg,#f7fbff_0%,#eef6ff_30%,#f8f5ed_62%,#fffdf7_100%)] shadow-[0_28px_70px_-46px_rgba(15,23,42,0.22)]">
            <div className="px-5 py-5 text-content-1">
              ...
              <div className="mt-2 text-sm leading-6 text-content-3">
                Saved quotes, active lines, and the next line action stay
                together so sales never has to hunt around the page.
              </div>
            </div>
            ...
            <div className="space-y-5 px-4 pb-4 pt-1">
              <div className="rounded-[1.6rem] border border-info-border bg-surface-1/88 px-4 py-4 text-content-1 shadow-sm">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <Button
                    variant="outline"
                    className="rounded-2xl border-line bg-surface-1 text-content-2 hover:bg-surface-2"
                    onClick={openNewQuote}
                  >
              ...
              <div className="rounded-[1.8rem] border border-line bg-surface-1 p-4 shadow-[0_20px_40px_-36px_rgba(15,23,42,0.24)]">
                ...
                <ScrollArea className="mt-4 h-[250px] pr-2">
                  <div className="space-y-3">
                    {quotationsLoading ? (
                      <div className="rounded-[1.2rem] border border-line bg-surface-2 px-4 py-6 text-sm text-content-3">
                    ...
                    {quotations.map((quotation) => (
                      <button
                        ...
                        className={`w-full rounded-[1.35rem] border px-4 py-4 text-left transition ${draft.id === quotation.id ? "border-info-border bg-info-bg text-content-1 shadow-[0_18px_42px_-34px_rgba(59,130,246,0.22)]" : "border-line bg-surface-2 hover:border-line-strong hover:bg-surface-1"}`}
                      >
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <Badge
                            className={`border ${draft.id === quotation.id ? "border-info-border bg-surface-1 text-info-fg" : statusTone(quotation.status)}`}
                          >
                            {quotation.status}
                          </Badge>
                        </div>
              ...
              <div className="rounded-[1.8rem] border border-line bg-surface-1 p-4 shadow-[0_20px_40px_-36px_rgba(15,23,42,0.24)]">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <Button
                    ...
                    data-testid="quotation-add-line"
                    onClick={addLine}
                  >
                ...
                <div className="mt-4 space-y-3">
                  {draft.items.map((line, index) => (
                    <button
                      ...
                      className={`w-full rounded-[1.35rem] border px-4 py-4 text-left transition ${line.localId === activeLineId ? "border-info-border bg-info-bg text-content-1 shadow-[0_18px_42px_-34px_rgba(79,70,229,0.22)]" : "border-line bg-surface-2 hover:border-line-strong hover:bg-surface-1"}`}
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <div className="text-[10px] font-black uppercase tracking-[0.22em] opacity-70">
                          ...
                          <div className="mt-1 text-xs opacity-70">
                            {line.qty_value} {line.qty_uom} • {line.source_mode}
                          </div>
                        </div>
        ...
        <div className="space-y-5">
          <section className="overflow-hidden rounded-[2rem] border border-line bg-[linear-gradient(180deg,#fffdf8_0%,#ffffff_58%,#f7f9fc_100%)] shadow-[0_24px_60px_-42px_rgba(15,23,42,0.22)]">
            <div className="grid gap-5 border-b border-line px-6 py-6 xl:grid-cols-[minmax(0,1.2fr)_300px]">
              <div className="space-y-4">
                <div>
                  ...
                  <div className="mt-2 max-w-3xl text-sm leading-6 text-content-3">
                    Customer, plant, validity, and quote status stay fixed here.
                    The active line below should feel like one working plane,
                    not a document made of small cards.
                  </div>
                </div>
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                  <FieldSelect
                    label="Customer"
                    testId="quotation-customer"
                    value={draft.customer || "NONE"}
                    onChange={(value) =>
                      onCustomerChange(value === "NONE" ? "" : value)
                    }
                  >
                  ...
                  <FieldSelect
                    label="Plant"
                    testId="quotation-plant"
                    value={draft.plant || "NONE"}
                    onChange={(value) =>
                      updateDraft({ plant: value === "NONE" ? "" : value })
                    }
                  >
                  ...
                  <FieldShell label="Valid Until">
                    <Input
                      type="date"
                      className="h-12 rounded-2xl border-line bg-surface-1"
                      value={draft.valid_until}
                      onChange={(event) =>
                        updateDraft({ valid_until: event.target.value })
                      }
                    />
                  </FieldShell>
                  <FieldShell label="Currency">
                    <Input
                      className="h-12 rounded-2xl border-line bg-surface-1 uppercase"
              ...
              <div className="rounded-[1.7rem] border border-line-strong bg-surface-3 px-5 py-5 text-white shadow-[0_22px_56px_-42px_rgba(15,23,42,0.8)]">
                ...
                <div className="mt-2 text-sm text-content-4">
                  {draft.customer_name ||
                    "Customer will mirror the selected account unless sales overrides the commercial name below."}
                </div>
                <div className="mt-4 flex flex-wrap gap-2">
                  <Badge
                    className={`rounded-full border ${statusTone(draft.status)}`}
                  >
                    {draft.status.replaceAll("_", " ")}
                  </Badge>
                  <Badge className="rounded-full border border-surface-1/10 bg-surface-1/10 text-white">
                    SKU-first by default
                  </Badge>
                  <Badge className="rounded-full border border-surface-1/10 bg-surface-1/10 text-white">
                    Cost mode: ESTIMATED
                  </Badge>
                ...
                <FieldShell label="Customer Name Override">
                  <Input
                    className="mt-4 h-12 rounded-2xl border-surface-1/10 bg-surface-1/10 text-white placeholder:text-content-4"
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 px-6 py-4">
              <div className="text-sm text-content-3">
                Launch deck below: use SKU Quote for speed, switch to Custom
                Quote only when a sales line genuinely needs expert technical
                shaping.
              </div>
              {draft.id ? (
                <div className="flex flex-wrap gap-2">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      window.open(
                        salesService.getQuotationPdfUrl(draft.id!),
                        "_blank",
                        "noopener,noreferrer",
                      )
                    }
          ...
          <QuoteWorkbench
            ...
            onToggleAdvanced={() =>
              setShowAdvancedLineEditor((current) => !current)
            }
...
function QuoteWorkbench({
  ...
  skuVariantOptions: Array<
    SalesSkuVariant & { label: string; detailLabel: string }
  >;
  templates: any[];
  ...
  if (!activeLine) {
    return (
      <PremiumSection
        title="Sales Quote Studio"
        description="Pick a saved quote or add a line to begin."
      >
        <div className="rounded-[1.5rem] border border-dashed border-line bg-surface-2 px-6 py-10 text-sm text-content-3">
  ...
  return (
    <div className="overflow-hidden rounded-[2.2rem] border border-line bg-[linear-gradient(180deg,#f6f3eb_0%,#ffffff_55%,#f8fafc_100%)] shadow-[0_38px_90px_-62px_rgba(15,23,42,0.45)]">
      <div className="border-b border-line-strong bg-[linear-gradient(135deg,#0f172a_0%,#172554_62%,#1d4ed8_100%)] px-6 py-6 text-white">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
          <div className="max-w-3xl">
            ...
            <div className="mt-3 max-w-2xl text-sm leading-6 text-content-4">
              The default path is SKU-first: choose the saleable variant, keep
              only light size edits visible, enter the quoted price, and move.
              Custom Quote stays available, but it no longer contaminates the
              fast lane.
            </div>
          </div>
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
          <button
            ...
            className={`rounded-[1.8rem] border px-5 py-5 text-left transition ${isSkuMode ? "border-info-border bg-surface-1/14 shadow-[inset_0_1px_0_rgba(255,255,255,0.16)]" : "border-surface-1/10 bg-surface-1/5 hover:bg-surface-1/10"}`}
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <div className="mt-2 text-xs leading-5 text-content-4">
                  Fast lane for normal sales work. Geometry, layers, print, and
                  packaging stay inherited from the SKU so the user only touches
                  commercial fields.
                </div>
              </div>
              <CheckCircle2
                className={`mt-0.5 h-5 w-5 ${isSkuMode ? "text-info-border" : "text-white/30"}`}
              />
            </div>
          </button>
          <button
            ...
            className={`rounded-[1.8rem] border px-5 py-5 text-left transition ${!isSkuMode ? "border-warning-border bg-warning-bg shadow-[inset_0_1px_0_rgba(255,255,255,0.16)]" : "border-surface-1/10 bg-surface-1/5 hover:bg-surface-1/10"}`}
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <div className="mt-2 text-xs leading-5 text-content-4">
                  Secondary expert path for pre-catalogue work, unusual
                  structures, or deliberate template-led technical editing.
                </div>
              </div>
              <Layers3
                className={`mt-0.5 h-5 w-5 ${!isSkuMode ? "text-warning-border" : "text-white/30"}`}
              />
            </div>
          </button>
        </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-line px-6 py-4">
        <div className="text-sm text-content-3">
          Use the left rail to switch saved quotes or lines. This surface only
          shows the active commercial path.
        </div>
        <div className="flex flex-wrap gap-2">
          <Button
            variant="outline"
            className="rounded-2xl"
            onClick={() => onCloneLine(activeLine.localId)}
          >
          ...
          <Button
            ...
            onClick={() => onRemoveLine(activeLine.localId)}
        ...
        <div className="space-y-4 2xl:sticky 2xl:top-6">
          <div className="rounded-[1.7rem] border border-line bg-surface-3 px-5 py-5 text-white shadow-[0_26px_60px_-48px_rgba(15,23,42,0.78)]">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <div className="mt-1 text-sm text-content-4">
                  {activeLine.qty_value} {activeLine.qty_uom} •{" "}
                  {activeLine.finished_good_type}
                </div>
              </div>
              <div className="rounded-2xl bg-surface-1/10 p-3 text-white">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
              ...
              <StatPill
                label="Preview"
                value={`${activeLine.qty_value} ${activeLine.qty_uom}`}
                subtle
              />
            </div>
            <div className="mt-5 space-y-2">
              <InspectorRow
                ...
                value={formatMoney(
                  activePreview?.costing?.landed_cost,
                  draft.currency,
                )}
              />
              <InspectorRow
                ...
                value={formatMoney(
                  activePreview?.costing?.net_total,
                  draft.currency,
                )}
              />
              <InspectorRow
                inverted
                label="Sales Quoted Price"
                value={formatMoney(quotedUnitPrice, draft.currency)}
                highlight
              />
              <InspectorRow
                inverted
                label="Estimated Margin"
                value={`${asNumber(activePreview?.costing?.margin_percent, 0).toFixed(1)}%`}
              />
            </div>
          </div>
          ...
          <div className="rounded-[1.6rem] border border-line bg-surface-2 px-4 py-4">
            ...
            {previewError ? (
              <div className="mt-4 rounded-[1.2rem] border border-danger-border bg-danger-bg px-4 py-4 text-sm text-danger-fg">
                {previewError}
              </div>
            ) : activePreview ? (
              <div className="mt-4 grid gap-3 sm:grid-cols-2 2xl:grid-cols-1">
                <StatPill
                  label="Unit Weight"
                  value={`${asNumber(activePreview.unit_weight_g, 0).toFixed(3)} g`}
                />
                <StatPill
                  label="Total Weight"
                  value={`${asNumber(activePreview.total_weight_kg, 0).toFixed(3)} kg`}
                />
                <StatPill
                  label="Effective Width"
                  value={`${asNumber(activePreview.physics?.geometry_snapshot?.effective_width_mm, 0).toFixed(1)} mm`}
                />
                <StatPill
                  label="Area"
                  value={`${asNumber(activePreview.physics?.geometry_snapshot?.area_m2, 0).toFixed(4)} m²`}
                />
                {activePreview.roll_preview ? (
                  <StatPill
                    label="Derived Length"
                    value={`${asNumber(activePreview.roll_preview.derived_length_m, 0).toFixed(2)} m`}
                  />
                ) : null}
                {activePreview.roll_preview ? (
                  <StatPill
                    label="Roll Thickness"
                    value={`${asNumber(activePreview.roll_preview.thickness_micron, 0).toFixed(2)} μ`}
                  />
                ) : null}
              </div>
            ) : (
              <div className="mt-4 rounded-[1.2rem] border border-dashed border-line bg-surface-1 px-4 py-5 text-sm text-content-3">
                Choose a SKU or template and the inspector will render geometry,
                weight, and commercial guidance here.
              </div>
            )}
          </div>
          ...
          <div className="rounded-[1.7rem] border border-line bg-surface-1 px-5 py-5 shadow-[0_22px_48px_-44px_rgba(15,23,42,0.35)]">
            ...
            <div className="mt-4 space-y-2">
              <InspectorRow
                label="Subtotal"
                value={formatMoney(quoteSummary.subtotal, draft.currency)}
              />
              <InspectorRow
                label="Tax Total"
                value={formatMoney(quoteSummary.tax_total, draft.currency)}
              />
              <InspectorRow
                label="Grand Total"
                value={formatMoney(quoteSummary.grand_total, draft.currency)}
                highlight
              />
              <InspectorRow
                label="Margin"
                value={`${asNumber(quoteSummary.margin_percent, 0).toFixed(1)}%`}
              />
            </div>
...
function SkuQuoteLane({
  ...
  const specChips = [
    ...
    isRoll
      ? activeLine.roll_form || "ROLL"
      : `${activeLine.geometry.base.width_mm} × ${activeLine.geometry.base.height_mm} mm`,
  ].filter(Boolean);

  return (
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_292px]">
      <div className="overflow-hidden rounded-[1.95rem] border border-line bg-surface-1 shadow-[0_26px_60px_-48px_rgba(15,23,42,0.35)]">
        <div className="border-b border-line bg-[linear-gradient(135deg,#f9f4ea_0%,#ffffff_40%,#f3f6fb_100%)] px-6 py-6">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
            <div className="max-w-3xl">
              ...
              <div className="mt-2 text-sm leading-6 text-content-3">
                The normal path is now simple: live SKU first, light size edits,
                quantity, salesperson price, then save or PDF. Deep technical
                structure stays inherited from the SKU.
              </div>
            </div>
            <div className="flex flex-wrap gap-2">
              {specChips.map((chip) => (
                <Badge
                  key={chip}
                  className="rounded-full border border-line bg-surface-1 px-3 py-1 text-content-2 shadow-sm"
                >
                  {chip}
                </Badge>
              ))}
              <Badge className="rounded-full border border-line bg-surface-2 px-3 py-1 text-content-2 shadow-sm">
        ...
        <div className="grid gap-0 xl:grid-cols-[minmax(0,1.15fr)_320px]">
          <div className="relative z-10 min-w-0">
            <div className="border-b border-line px-6 py-6">
              <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_240px]">
                ...
                <FieldSelect
                  ...
                  onChange={(value) => {
                    ...
                    const selectedTemplate = templates.find(
                      (template: any) => String(template.id) === String(value),
                    );
                    onRevealCustomEditor();
                    onUpdateLine(activeLine.localId, (current) => ({
                      ...
                      geometry: {
                        ...current.geometry,
                        pouch_style:
                          current.finished_good_type === "POUCH"
                            ? normalizePouchStyle(
                                selectedTemplate?.pouch_style ||
                                  current.geometry.pouch_style ||
                                  "",
                              )
                            : "",
                      },
                ...
                >
                  <SelectItem value="NONE">
                    Switch to template-led custom line
                  </SelectItem>
                  {templates
                    .filter(
                      (template: any) =>
                        template.fg_type === activeLine.finished_good_type,
                    )
                    .map((template: any) => (
              ...
              <div className="relative z-20 mt-4 flex flex-wrap gap-2">
                ...
                <Button
                  type="button"
                  variant={
                    compatibilityTab === "materials" ? "default" : "outline"
                  }
                  className="rounded-2xl"
                ...
                <Button
                  type="button"
                  variant={
                    compatibilityTab === "pricing" ? "default" : "outline"
                  }
                  className="rounded-2xl"
              ...
              {featuredVariants.length ? (
                <div className="mt-5 grid gap-3 md:grid-cols-3">
                  {featuredVariants.map((variant, index) => {
                    const selected =
                      String(variant.id) ===
                      String(activeLine.sku_variant_id || "");
                    return (
                      <button
                        key={variant.id}
                        type="button"
                        onClick={() =>
                          onApplySkuVariant(activeLine.localId, variant.id)
                        }
                        className={`rounded-[1.45rem] border px-4 py-4 text-left transition ${
                          selected
                            ? "border-line-strong bg-surface-3 text-white shadow-[0_18px_40px_-34px_rgba(15,23,42,0.75)]"
            ...
            <div className="grid gap-5 px-6 py-6">
                  ...
                  <div className="mt-3 grid gap-4 md:grid-cols-2">
                    <FieldSelect
                      ...
                      onChange={(value) =>
                        onUpdateLine(activeLine.localId, (current) => ({
                          ...current,
                          film_layers: current.film_layers.map((row, idx) =>
                            idx === 0
                              ? {
                                  ...row,
                                  family_id: value === "NONE" ? "" : value,
                                  variant_id: "",
                                }
                              : row,
                          ),
              ...
              <div>
                ...
                <div className="mt-2 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                  <FieldNumber
                    ...
                    onChange={(value) =>
                      onUpdateLine(activeLine.localId, (current) => ({
                        ...
                        geometry: {
                          ...current.geometry,
                          base: { ...current.geometry.base, width_mm: value },
                        },
                      }))
                    }
                  />
                  <FieldNumber
                    ...
                    onChange={(value) =>
                      onUpdateLine(activeLine.localId, (current) => ({
                        ...
                        geometry: {
                          ...current.geometry,
                          base: { ...current.geometry.base, height_mm: value },
                        },
                      }))
                    }
                  />
                  ...
                  <FieldNumber
                    ...
                    value={asNumber(
                      activeLine.commercial_snapshot.manual_unit_price,
                      0,
                    )}
                    onChange={(value) =>
                      onUpdateLine(activeLine.localId, (current) => ({
                        ...current,
                        commercial_snapshot: {
                          ...current.commercial_snapshot,
                          manual_unit_price: value,
                        },
                      }))
                    }
                  />
              ...
              <div className="rounded-[1.6rem] border border-line bg-surface-2 px-5 py-5">
              ...
              {previewError ? (
                <div className="rounded-[1.4rem] border border-danger-border bg-danger-bg px-4 py-4 text-sm text-danger-fg">
                  {previewError}
                </div>
              ) : null}
            </div>
          ...
          <aside className="relative z-0 border-t border-line bg-surface-3 px-5 py-5 text-white xl:border-l xl:border-t-0 xl:border-line-strong">
            ...
            <div className="mt-2 text-sm leading-6 text-content-4">
              {activeSku
                ? `${activeSku.code} • ${activeSku.name}`
                : "Choose a live SKU variant and the line will inherit geometry, layers, print, and packaging."}
            </div>
            <div className="mt-5 grid gap-3">
              <StatPill
                label="Template"
                value={
                  activeSkuVariant?.template_name || "Will inherit on selection"
                }
                subtle
              />
              <StatPill
              ...
              <StatPill
                ...
                value={formatMoney(quotedUnitPrice, "INR")}
                subtle
              />
              <StatPill
                label="Preview Basis"
...
function CustomQuoteLane({
  ...
  const spoutStyle =
    activeLine.finished_good_type === "POUCH" && isSpoutStyle(lockedPouchStyle);
  return (
    <div className="space-y-5">
        ...
        <div className="rounded-[1.6rem] border border-line bg-surface-2 px-5 py-5">
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
              ...
              onChange={(value) => {
                ...
                onUpdateLine(activeLine.localId, (current) => ({
                  ...
                  line_name:
                    value === "NONE"
                      ? current.line_name
                      : selectedTemplate?.name || current.line_name,
                  geometry: {
                    ...current.geometry,
                    pouch_style:
                      current.finished_good_type === "POUCH"
            ...
            <FieldNumber
              label="Height (mm)"
              value={activeLine.geometry.base.height_mm}
              disabled={activeLine.finished_good_type === "ROLL"}
              onChange={(value) =>
                onUpdateLine(activeLine.localId, (current) => ({
                  ...current,
                  geometry: {
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <Badge className="border border-line bg-surface-1 text-content-2">
              {activeTemplate?.name || "Template-led custom line"}
            </Badge>
            {lockedPouchStyle ? (
              <Badge className="border border-info-border bg-info-bg text-primary">
                {lockedPouchStyle.replaceAll("_", " ")}
              </Badge>
            ) : null}
      ...
      <div className="rounded-[1.5rem] border border-line bg-surface-3 px-5 py-5 text-white">
        ...
        <>
            ...
            <TabsContent value="spec" className="space-y-5">
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                      ...
                      <button
                        ...
                        onClick={() =>
                          onUpdateLine(activeLine.localId, (current) => ({
                            ...
                            roll_form:
                              type === "ROLL"
                                ? current.roll_form || "FLAT"
                                : "",
                            qty_uom: type === "ROLL" ? "KG" : current.qty_uom,
                            price_basis:
                              type === "ROLL" ? "KG" : current.price_basis,
                            geometry: {
                              ...current.geometry,
                              base: {
                                ...
                                height_mm:
                                  type === "ROLL"
                                    ? 0
                                    : current.geometry.base.height_mm,
                              },
                            },
                          }))
                        }
                ...
                <FieldSelect
                  ...
                  value={
                    activeLine.finished_good_type === "ROLL"
                      ? activeLine.roll_form || "FLAT"
                      : "NONE"
                  }
                  onChange={(value) =>
                    onUpdateLine(activeLine.localId, (current) => ({
                      ...current,
                      roll_form: value as LineDraft["roll_form"],
                    }))
                  }
                >
                  {activeLine.finished_good_type !== "ROLL" ? (
                ...
                {showGussetField ? (
                  <FieldNumber
                    label="Gusset (mm)"
                    value={asNumber(activeLine.geometry.gusset_mm, 0)}
                    onChange={(value) =>
                      onUpdateLine(activeLine.localId, (current) => ({
                        ...current,
                        geometry: { ...current.geometry, gusset_mm: value },
                      }))
                    }
                  />
                ) : null}
              ...
              <div className="rounded-[1.4rem] border border-line bg-surface-2 px-4 py-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-sm text-content-3">
                      Use only when the line truly needs technical deviation
                      from the template.
                    </div>
                  </div>
                  <Button
                ...
                <div className="mt-4 space-y-3">
                  {activeLine.film_layers.map((layer, layerIndex) => (
                    <div
                      key={layer.localId}
                      className="grid gap-3 rounded-2xl border border-line bg-surface-1 p-3 xl:grid-cols-[1.2fr_1.2fr_140px_auto]"
                    >
                      <FieldSelect
                        label={`Layer ${layerIndex + 1} Family`}
                        testId={`quotation-line-${activeLineIndex}-layer-${layerIndex}-family`}
                        value={layer.family_id || "NONE"}
                        onChange={(value) =>
                          onUpdateLine(activeLine.localId, (current) => ({
                            ...current,
                            film_layers: current.film_layers.map((row, idx) =>
                      ...
                      <FieldSelect
                        label={`Layer ${layerIndex + 1} Variant`}
                        testId={`quotation-line-${activeLineIndex}-layer-${layerIndex}-variant`}
                        value={layer.variant_id || "NONE"}
                        onChange={(value) =>
                          onUpdateLine(activeLine.localId, (current) => ({
                            ...current,
                            film_layers: current.film_layers.map((row, idx) =>
                      ...
                      <Button
                        variant="outline"
                        className="rounded-2xl text-danger-fg self-end"
                        onClick={() =>
                          onUpdateLine(activeLine.localId, (current) => ({
                            ...current,
                            film_layers: current.film_layers.filter(
                              (_, idx) => idx !== layerIndex,
                            ),
                          }))
                        }
                        disabled={activeLine.film_layers.length === 1}
                      >
              ...
              <div className="grid gap-4 lg:grid-cols-2">
                <div className="rounded-[1.4rem] border border-line bg-surface-2 px-4 py-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2 text-sm text-content-3">
                      <Switch
                        checked={activeLine.printing.enabled}
                  ...
                  <div className="mt-4 grid gap-3 md:grid-cols-2">
                    <FieldSelect
                      label="Print Type"
                      value={activeLine.printing.type}
                      onChange={(value) =>
                        onUpdateLine(activeLine.localId, (current) => ({
                          ...current,
                          printing: {
                            ...current.printing,
                            type: value as LineDraft["printing"]["type"],
                          },
                        }))
                      }
                    >
                    ...
                    <FieldSelect
                      label="Film Type"
                      value={activeLine.printing.substrate_mode}
                      onChange={(value) =>
                        onUpdateLine(activeLine.localId, (current) => ({
                          ...current,
                          printing: {
                            ...current.printing,
                            substrate_mode:
                              value as LineDraft["printing"]["substrate_mode"],
                            back_colors_count:
                              value === "TUBING"
                    ...
                    <FieldNumber
                      label="Front Colors"
                      value={activeLine.printing.front_colors_count}
                      onChange={(value) =>
                        onUpdateLine(activeLine.localId, (current) => ({
                          ...current,
                          printing: {
                            ...current.printing,
                            front_colors_count: value,
                          },
                        }))
                      }
                    />
                    {activeLine.printing.substrate_mode === "TUBING" ? (
                      <FieldNumber
                        label="Back Colors"
                        value={activeLine.printing.back_colors_count}
                        onChange={(value) =>
                          onUpdateLine(activeLine.localId, (current) => ({
                            ...current,
                            printing: {
                              ...current.printing,
                              back_colors_count: value,
                            },
                          }))
                        }
                      />
                    ...
                    <FieldNumber
                      label="Ink GSM"
                      value={activeLine.printing.ink_gsm_total}
                      onChange={(value) =>
                        onUpdateLine(activeLine.localId, (current) => ({
                          ...current,
                          printing: {
                ...
                <div className="rounded-[1.4rem] border border-line bg-surface-2 px-4 py-4">
                  ...
                  <div className="mt-4 grid gap-3 md:grid-cols-2">
                    <FieldNumber
                      label="Adhesive GSM"
                      value={activeLine.chemicals.adhesive_gsm}
                      onChange={(value) =>
                        onUpdateLine(activeLine.localId, (current) => ({
                          ...current,
                          chemicals: {
                    ...
                    <FieldNumber
                      label="Solvent GSM"
                      value={activeLine.chemicals.solvent_gsm}
                      onChange={(value) =>
                        onUpdateLine(activeLine.localId, (current) => ({
                          ...current,
                          chemicals: {
                  ...
                  <div className="mt-4">
                    <FieldSelect
                      label="POD Profile"
                      value={
                        activeLine.packaging_snapshot.pod.pod_profile_id ||
                        "NONE"
                      }
                      onChange={(value) => {
                        const selected = podProfiles.find(
                          (entry: any) => String(entry.id) === String(value),
                        );
                        onUpdateLine(activeLine.localId, (current) => ({
                          ...current,
                          packaging_snapshot: {
              ...
              <div className="rounded-[1.4rem] border border-line bg-surface-2 px-4 py-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-sm text-content-3">
                      {spoutStyle
                        ? "Spout-compatible add-ons are ranked first."
                        : "Optional add-ons stay out of the common flow until explicitly opened."}
                    </div>
                  </div>
                  <Button
                    variant="outline"
                ...
                <div className="mt-4 space-y-3">
                  {activeLine.addons.length === 0 ? (
                    <div className="rounded-2xl border border-dashed border-line bg-surface-1 px-4 py-4 text-sm text-content-3">
                      No add-ons configured.
                    </div>
                  ) : null}
                  {activeLine.addons.map((addon, addonIndex) => {
                    const addonMeta = addons.find(
                      (entry: any) =>
                        String(entry.id) === String(addon.addon_id),
                    );
                    const mode = String(
                    ...
                    return (
                      <div
                        key={addon.localId}
                        className="grid gap-3 rounded-2xl border border-line bg-surface-1 p-3 xl:grid-cols-[1.2fr_120px_180px_auto]"
                      >
                        <FieldSelect
                          label={`Add-on ${addonIndex + 1}`}
                          value={addon.addon_id || "NONE"}
                          onChange={(value) =>
                            onUpdateLine(activeLine.localId, (current) => ({
                              ...current,
                              addons: current.addons.map((row, idx) =>
                                idx === addonIndex
                                  ? {
                                      ...row,
                                      addon_id: value === "NONE" ? "" : value,
                                    }
                                  : row,
                              ),
                            }))
                        ...
                        <div>
                          ...
                          />
                          <div className="mt-1 text-[10px] font-semibold leading-snug text-content-3">
                            {qtyHint}
                          </div>
                        </div>
                        {mode === "PER_MM" ? (
                                ...
                                addons: current.addons.map((row, idx) =>
                                  idx === addonIndex
                                    ? {
                                        ...row,
                                        applies_to:
                                          value as AddonDraft["applies_to"],
                                      }
                                    : row,
                                ),
                              }))
                          ...
                          <div className="self-end rounded-2xl border border-line bg-surface-2 px-3 py-3 text-xs text-content-3">
                            {mode === "FIXED"
                              ? "Fixed weight add-on"
                              : "Per piece add-on"}
                          </div>
              ...
              <div className="rounded-[1.4rem] border border-line bg-surface-2 px-4 py-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-sm text-content-3">
                      Template-seeded when available. Sales can refine
                      commercial route assumptions without touching the
                      manufacturing route master.
                    </div>
                  </div>
                  <Button
                    variant="outline"
                    ...
                    onClick={() =>
                      onUpdateLine(activeLine.localId, (current) => ({
                        ...current,
                        process_cost_rows: [
                          ...current.process_cost_rows,
                          {
                            ...makeProcessRow(),
                            sequence: current.process_cost_rows.length + 1,
                          },
                        ],
                      }))
                    }
                ...
                <div className="mt-4 space-y-3">
                  {activeLine.process_cost_rows.length === 0 ? (
                    <div className="rounded-2xl border border-dashed border-line bg-surface-1 px-4 py-4 text-sm text-content-3">
                      No process rows yet. Add them only if this custom quote
                      needs route-specific commercial logic.
                    </div>
                  ) : null}
                  {activeLine.process_cost_rows.map((row, rowIndex) => (
                    <div
                      key={`${activeLine.localId}-${rowIndex}`}
                      className="grid gap-3 rounded-2xl border border-line bg-surface-1 p-3 xl:grid-cols-[1.2fr_120px_120px_120px_auto]"
                    >
                      <FieldSelect
                        label={`Process ${rowIndex + 1}`}
                        value={row.process_id || "NONE"}
                        onChange={(value) =>
                          onProcessSelect(
                            activeLine.localId,
                            rowIndex,
                      ...
                      <FieldShell label="Rate / Hr">
                        <Input
                          ...
                          value={
                            row.hourly_rate ??
                            processRates.find(
                              (entry: any) => entry.process === row.process_id,
                            )?.cost_per_hour ??
                            0
                          }
                          onChange={(event) =>
                            onUpdateLine(activeLine.localId, (current) => ({
                              ...current,
                              process_cost_rows: current.process_cost_rows.map(
                                (entry, idx) =>
                                  idx === rowIndex
                                    ? {
                                        ...entry,
                                        hourly_rate: asNumber(
                                          event.target.value,
                                          0,
                                        ),
                                      }
                      ...
                      <Button
                        ...
                        onClick={() =>
                          onUpdateLine(activeLine.localId, (current) => ({
                            ...current,
                            process_cost_rows: current.process_cost_rows
              ...
              <div className="rounded-[1.4rem] border border-line bg-surface-2 px-4 py-4">
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                  <FieldNumber
                    label="Margin %"
                    value={asNumber(
                      activeLine.commercial_snapshot.margin_target_percent,
                      15,
                    )}
                    onChange={(value) =>
                      onUpdateLine(activeLine.localId, (current) => ({
                  ...
                  <FieldNumber
                    label="Tax %"
                    value={asNumber(
                      activeLine.commercial_snapshot.tax_percent,
                      18,
                    )}
                    onChange={(value) =>
                      onUpdateLine(activeLine.localId, (current) => ({
                  ...
                  <FieldNumber
                    label="Wastage %"
                    value={asNumber(
                      activeLine.commercial_snapshot.wastage_percent,
                      0,
                    )}
                    onChange={(value) =>
                      onUpdateLine(activeLine.localId, (current) => ({
                  ...
                  <FieldNumber
                    label="Packing Value"
                    value={asNumber(
                      activeLine.commercial_snapshot.packing_value,
                      0,
                    )}
                    onChange={(value) =>
                      onUpdateLine(activeLine.localId, (current) => ({
                  ...
                  <FieldNumber
                    label="Freight Value"
                    value={asNumber(
                      activeLine.commercial_snapshot.freight_value,
                      0,
                    )}
                    onChange={(value) =>
                      onUpdateLine(activeLine.localId, (current) => ({
                  ...
                  <FieldNumber
                    label="Misc Value"
                    value={asNumber(
                      activeLine.commercial_snapshot.misc_value,
                      0,
                    )}
                    onChange={(value) =>
                      onUpdateLine(activeLine.localId, (current) => ({
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <FieldArea
              ...
              value={draft.terms}
              onChange={(value) => onUpdateDraft({ terms: value })}
            />
            <FieldArea
              label="Notes"
              value={draft.notes}
...
function buildLinePayload(
  ...
  const filmLayers = line.film_layers.map((layer) => {
    ...
    const family = families.find(
      (entry) => String(entry.id) === String(layer.family_id),
    );
    ...
    return {
      ...
      roll_width_mm: asNumber(
        layer.roll_width_mm || line.geometry.base.width_mm,
        0,
      ),
  ...
  return {
    ...
    geometry: {
      base: {
        ...
        height_mm:
          line.finished_good_type === "ROLL"
            ? 0
            : asNumber(line.geometry.base.height_mm, 0),
      },
      pouch_style:
        line.finished_good_type === "POUCH"
          ? String(line.geometry.pouch_style || "").toUpperCase()
          : "",
      gusset_mm:
        line.finished_good_type === "POUCH"
          ? asNumber(line.geometry.gusset_mm, 0)
          : 0,
      trim_loss_mm:
        line.finished_good_type === "POUCH"
          ? asNumber(line.geometry.trim_loss_mm, 0)
          : 0,
      flap_tape_mm:
        line.finished_good_type === "POUCH"
          ? asNumber(line.geometry.flap_tape_mm, 0)
          : 0,
    ...
    printing: {
      ...
      back_colors_count:
        line.printing.substrate_mode === "TUBING"
          ? asNumber(line.printing.back_colors_count, 0)
          : 0,
    ...
    addons: line.addons.map((addon) => {
      const master = addons.find(
        (entry: any) => String(entry.id) === String(addon.addon_id),
      );
      const masterMode = String(
        master?.weight_mode || "PER_PIECE",
      ).toUpperCase();
      const finalWeightMode =
        masterMode === "PER_MM"
          ? "PER_MM"
          : masterMode === "FIXED"
            ? "FIXED"
      ...
      const finalAppliesTo =
        finalWeightMode === "PER_MM"
          ? addon.applies_to === "HEIGHT"
            ? "HEIGHT"
            : addon.applies_to === "BOTH"
              ? "BOTH"
              : "WIDTH"
          : "NONE";
      return {
    ...
    packaging_snapshot: {
      ...
      pod: {
        ...
        pod_sku_variant_id:
          line.packaging_snapshot.pod.pod_sku_variant_id || null,
    ...
    commercial_snapshot: {
      ...
      margin_target_percent: asNumber(
        line.commercial_snapshot.margin_target_percent,
        15,
      ),
      manual_unit_price: asNumber(
        line.commercial_snapshot.manual_unit_price,
        0,
      ),
      manual_line_total: asNumber(
        line.commercial_snapshot.manual_line_total,
        0,
      ),
    },
  };
...
function FieldArea({
  ...
  return (
    <FieldShell label={label}>
      <Textarea
        className="min-h-[128px] rounded-[1.4rem] border-line bg-surface-1"
        value={value}
...
function InspectorRow({
  ...
  return (
    <div
      className={`flex items-center justify-between rounded-2xl px-3 py-2 ${highlight ? (inverted ? "bg-surface-1/10" : "bg-surface-1") : "bg-transparent"}`}
    >
      <span className={inverted ? "text-content-4" : "text-content-3"}>
        {label}
      </span>
      <span
