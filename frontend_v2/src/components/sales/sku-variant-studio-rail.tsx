...
import { asNumber, type OrderItemDraft } from "@/components/sales/shared/order-draft"
...
export function SkuVariantStudioRail({
    ...
    const previewGeometry = preview?.physics?.geometry_snapshot || preview?.geometry_snapshot || {}
    const addOnKg = (preview?.bom?.addons || []).reduce((sum, row) => sum + asNumber(row?.weight_kg, 0), 0)
    const podKg = (preview?.bom?.pod || []).reduce((sum, row) => sum + asNumber(row?.weight_kg, 0), 0)
    const bomRows = preview?.bom_preview?.components || []

    return (
        <aside className="space-y-4 xl:sticky xl:top-6">
            <section className="overflow-hidden rounded-[1.9rem] border border-slate-200/80 bg-[linear-gradient(180deg,rgba(15,23,42,0.97),rgba(30,41,59,0.96))] text-white shadow-[0_32px_90px_-48px_rgba(15,23,42,0.6)]">
                <div className="border-b border-white/10 px-5 py-5">
                    <div className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1 text-[10px] font-black uppercase tracking-[0.24em] text-cyan-100">
                        <Sparkles className="h-3.5 w-3.5" />
                        Preview Studio
                    </div>
                    <div className="mt-3 space-y-2">
                        <h3 className="text-xl font-black tracking-tight text-white">Preview quantity and snapshot</h3>
                        <p className="max-w-md text-sm leading-6 text-slate-300">
                            The preview stays tied to the current draft, so BOM, weight, and geometry always reflect the quantity you are validating.
                        </p>
                    </div>
                </div>
                ...
                <div className="space-y-4 px-5 py-5">
                    <div className="grid gap-3 sm:grid-cols-[minmax(0,1.3fr)_110px]">
                        <div className="space-y-2">
                            <Label className="text-[11px] font-black uppercase tracking-[0.22em] text-slate-300">Preview quantity</Label>
                            <Input
                        ...
                        <div className="space-y-2">
                            <Label className="text-[11px] font-black uppercase tracking-[0.22em] text-slate-300">UOM</Label>
                            {isRoll ? (
                                <div className="flex h-12 items-center rounded-2xl border border-white/10 bg-white/10 px-4 text-sm font-semibold text-white">
                                    KG
                                </div>
                            ) : (
                                <Select value={resolvedPreviewUom} onValueChange={(value) => onPreviewQuantityUomChange(value as "PCS" | "KG")}>
                    ...
                    <div className="rounded-[1.5rem] border border-white/10 bg-white/10 px-4 py-4">
                        <div className="text-[10px] font-black uppercase tracking-[0.24em] text-cyan-100/80">Previewing for</div>
                        <div className="mt-2 text-lg font-black text-white">{previewContext}</div>
                        <p className="mt-1 text-sm leading-6 text-slate-300">
                            This is the exact snapshot the builder will calculate against before you save the variant.
                        </p>
                    </div>
            ...
            <section className="space-y-3 rounded-[1.8rem] border border-slate-200/80 bg-white/92 p-4 shadow-[0_24px_55px_-46px_rgba(15,23,42,0.45)]">
                ...
                {previewLoading ? (
                    <div className="flex items-center gap-2 rounded-[1.4rem] border border-slate-200 bg-slate-50 px-4 py-4 text-sm text-slate-500">
                    ...
                    <>
                            ...
                            <PreviewMetric
                                ...
                                value={
                                    <span className="text-[1.2rem]">
                                        {formatNumber(asNumber(previewGeometry?.effective_width_mm, 0), 1)} W × {formatNumber(asNumber(previewGeometry?.effective_height_mm, 0), 1)} H
                                    </span>
                                }
                                hint={
                        ...
                        <div className="space-y-3">
                            <div className="text-sm font-black uppercase tracking-[0.22em] text-slate-700">BOM snapshot</div>
                    ...
                    <div className="rounded-[1.4rem] border border-dashed border-slate-200 bg-slate-50 px-4 py-8 text-sm text-slate-500">
                        Preview will appear once the draft has a LIVE template, resolved layer stack, and valid geometry.
                    </div>
                )}
            </section>
        </aside>
