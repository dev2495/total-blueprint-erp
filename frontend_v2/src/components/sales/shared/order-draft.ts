...
export function isGussetStyle(style: unknown) {
  return [
    ...
    "FLAT_BOTTOM",
    "SPOUT",
  ].includes(normalizePouchStyle(style));
}
...
export function classifyAddonMaster(addon: any) {
  ...
  const isSpoutCompatible = ["SPOUT", "FITMENT", "VALVE"].some((token) =>
    code.includes(token),
  );
  return {
...
export function getOrderItemContractIssues(
  ...
  const lockedPouchStyle = normalizePouchStyle(
    templatePouchStyle || item.geometry.pouch_style,
  );
  ...
  if (fgType === "POUCH") {
    ...
    if (
      ...
      asNumber(item.geometry.gusset_mm, 0) <= 0
    ) {
      issues.push(`${lockedPouchStyle.replaceAll("_", " ")} requires gusset.`);
    }
    if (
      item.addons.some((addon) => addon.addon_id && asNumber(addon.qty, 0) <= 0)
    ) {
  ...
  const requiresExplicitLayerContract =
    item.sourceType === "CUSTOM" || item.advancedUnlocked;
  if (requiresExplicitLayerContract) {
    const incompleteLayerIndex = item.film_layers.findIndex(
      (layer) => !layer.family_id || !layer.variant_id,
    );
    if (incompleteLayerIndex >= 0) {
      issues.push(
        `Layer ${incompleteLayerIndex + 1} must include both film family and film variant.`,
      );
    }
    if (
      item.film_layers.some((layer) => asNumber(layer.thickness_micron, 0) <= 0)
    ) {
      issues.push("Each film layer must have thickness greater than zero.");
    }
    if (
      item.film_layers.some((layer) => asNumber(layer.roll_width_mm, 0) <= 0)
    ) {
  ...
  if (item.printing.enabled) {
    if (
      item.printing.substrate_mode === "SHEET" &&
      asNumber(item.printing.back_colors_count, 0) > 0
    ) {
...
export function normalizePackagingSnapshot(
  snapshot: any,
): OrderItemDraft["packaging_snapshot"] {
...
export function createEmptyOrderItemDraft(
  sourceType: OrderDraftSource = "CUSTOM",
): OrderItemDraft {
  return {
    localId: makeId(),
    sourceType,
...
export function cloneOrderItemDraft(item: OrderItemDraft): OrderItemDraft {
  return {
    ...
    geometry: {
      ...item.geometry,
      adjustments: item.geometry.adjustments.map((row) => ({
        ...row,
        localId: makeId(),
      })),
    },
    ...
    packaging_snapshot: {
      ...
      roll_dispatch_pack: {
        ...
        lines: item.packaging_snapshot.roll_dispatch_pack.lines.map((row) => ({
          ...row,
        })),
      },
    },
...
export function orderItemFromVariant(
  sku: SalesSku,
  ...
  const finishedGoodType = (variant.finished_good_type || "POUCH") as
    | "POUCH"
    | "ROLL";
  return {
    localId: makeId(),
    ...
    geometry: hydrateGeometry(
      geometry,
      resolvePouchStyleFallback(finishedGoodType, geometry?.pouch_style),
    ),
    film_layers:
      Array.isArray(variant.layer_snapshot) && variant.layer_snapshot.length
    ...
    printing: {
      ...
      type: String(
        printing?.type || "FLEXO",
      ).toUpperCase() as OrderItemDraft["printing"]["type"],
      substrate_mode: String(
        printing?.substrate_mode || "SHEET",
      ).toUpperCase() as OrderItemDraft["printing"]["substrate_mode"],
    ...
    addons: Array.isArray(variant.addons_snapshot)
      ? variant.addons_snapshot.map((row: any) => ({
          ...
          applies_to: String(
            row?.applies_to || row?.weight_mode || "PER_PIECE",
...
export function orderItemFromRepeat(
  candidate: RepeatLineCandidate,
): OrderItemDraft {
  ...
  const finishedGoodType = (candidate.summary?.finished_good_type ||
    "POUCH") as "POUCH" | "ROLL";
  return {
    localId: makeId(),
    sourceType: "REPEAT",
    ...
    finished_good_type: finishedGoodType,
    roll_form: (candidate.summary?.roll_form ||
      "") as OrderItemDraft["roll_form"],
    ...
    film_layers:
      Array.isArray(candidate.layer_snapshot) && candidate.layer_snapshot.length
    ...
    addons: Array.isArray(candidate.addons_snapshot)
      ...
      : [],
    packaging_snapshot: normalizePackagingSnapshot(
      candidate.packaging_snapshot,
    ),
    savedPreview: null,
  };
}
...
export function buildOrderItemPayload(
  ...
  const fgType = String(item.finished_good_type || "POUCH").toUpperCase() as
    | "POUCH"
    | "ROLL";
  ...
  const filmLayers = item.film_layers.map((layer) => {
    const family = families.find(
      (row: any) => String(row.id) === String(layer.family_id),
    );
    const variant = variants.find(
      (row: any) => String(row.id) === String(layer.variant_id),
    );
    ...
    ? {
        ...
        back_colors_count:
          substrateMode === "TUBING"
            ? asNumber(item.printing.back_colors_count, 0)
            : 0,
        ink_gsm_total: asNumber(item.printing.ink_gsm_total, 0),
        artwork_id: item.printing.defer_artwork_to_planner
          ? null
          : item.printing.artwork_id || null,
        defer_artwork_to_planner: Boolean(
          item.printing.defer_artwork_to_planner,
        ),
        chemicals,
      }
    : { enabled: false };
  ...
  const addons = item.addons
    ...
    .map((addon) => {
      const master = addonsMaster.find(
        (row: any) => String(row.id) === String(addon.addon_id),
      );
  ...
  return {
    ...
    geometry: {
      base: {
        width_mm:
          fgType === "ROLL"
            ? primaryRollWidth
            : asNumber(item.geometry.base.width_mm, 0),
        height_mm: fgType === "POUCH" ? pouchHeight : 0,
      },
      pouch_style:
        fgType === "POUCH"
          ? normalizePouchStyle(item.geometry.pouch_style)
          : "",
      gusset_mm: fgType === "POUCH" ? asNumber(item.geometry.gusset_mm, 0) : 0,
      trim_loss_mm:
        fgType === "POUCH" ? asNumber(item.geometry.trim_loss_mm, 0) : 0,
      flap_tape_mm:
        fgType === "POUCH" ? asNumber(item.geometry.flap_tape_mm, 0) : 0,
    ...
    packaging_snapshot: {
      primary_inner_pack: {
        enabled:
          fgType === "POUCH"
            ? Boolean(item.packaging_snapshot.primary_inner_pack.enabled)
            : false,
        material_id:
          fgType === "POUCH" &&
          item.packaging_snapshot.primary_inner_pack.enabled
            ? item.packaging_snapshot.primary_inner_pack.material_id || null
            : null,
        pcs_per_pack:
          fgType === "POUCH"
            ? asNumber(
      ...
      pod: {
        enabled:
          fgType === "POUCH"
            ? Boolean(item.packaging_snapshot.pod.enabled)
            : false,
        pod_profile_id:
          fgType === "POUCH" && item.packaging_snapshot.pod.enabled
            ? item.packaging_snapshot.pod.pod_profile_id || null
            : null,
        pod_sku_variant_id:
          fgType === "POUCH" && item.packaging_snapshot.pod.enabled
            ? item.packaging_snapshot.pod.pod_sku_variant_id || null
            : null,
        pod_sku_code:
          fgType === "POUCH" && item.packaging_snapshot.pod.enabled
            ? item.packaging_snapshot.pod.pod_sku_code || null
            : null,
        pod_sku_name:
          fgType === "POUCH" && item.packaging_snapshot.pod.enabled
            ? item.packaging_snapshot.pod.pod_sku_name || null
            : null,
      },
      roll_dispatch_pack: {
        enabled:
          fgType === "ROLL"
            ? Boolean(item.packaging_snapshot.roll_dispatch_pack.enabled)
            : false,
        lines:
          fgType === "ROLL"
