...
export type SalesSavedViewScope = "orders" | "sku_catalog" | "order_create" | "planner_queue" | "planner_sku"
export type SalesSavedViewFilters = Record<string, string | number | boolean | null | undefined>
...
export const SALES_MATERIAL_PRIORITY = ["PET", "PE", "LDPE", "LLDPE", "HDPE", "POLYESTER", "CPP", "BOPP", "PP", "METALLOCENE", "NYLON"]
export const SALES_GRADE_PRIORITY = ["GP", "SP", "MET10", "METALLOCENE", "SLIP", "NON-SLIP", "SURFACE", "REVERSE"]

export function salesMaterialFilterLabel(value: unknown) {
  const raw = String(value || "").trim()
  if (!raw) return ""
  const upper = raw.toUpperCase()
  const priorityHit = SALES_MATERIAL_PRIORITY.find((candidate) => new RegExp(`(^|[^A-Z0-9])${candidate}([^A-Z0-9]|$)`).test(upper))
...
export function salesSortChipOptions(options: SalesOverflowChipOption[], priority: string[] = []) {
  ...
  options.forEach((option) => {
    ...
    if (!current || Number(option.count || 0) > Number(current.count || 0)) byValue.set(key, option)
  })
...
function safeLoadSavedViews(scope: SalesSavedViewScope): SalesSavedView[] {
  ...
  try {
    ...
    return parsed
      .filter((view): view is SalesSavedView => Boolean(view?.id && view?.name && view?.filters))
      .slice(0, 40)
  } catch {
    return []
  }
...
function safeSaveSavedViews(scope: SalesSavedViewScope, views: SalesSavedView[]) {
  if (typeof window === "undefined") return
  try {
    window.localStorage.setItem(salesSavedViewsKey(scope), JSON.stringify(views.slice(0, 40)))
  } catch {
...
function defaultStarterViews(scope: SalesSavedViewScope): StarterView[] {
  if (scope === "orders") {
    return [
      { name: "My active queue", filters: { tab: "queue", statusFilter: "ALL" } },
      { name: "Awaiting planning", filters: { tab: "queue", statusFilter: "PLANNING_REQUIRED" } },
      { name: "This week's deliveries", filters: { tab: "queue", deliveryWindow: "THIS_WEEK" } },
      { name: "Overdue", filters: { tab: "queue", deliveryWindow: "OVERDUE" } },
      { name: "LDPE rolls", filters: { fgTypeFilter: "ROLL", materialFilter: "LDPE" } },
      { name: "Pouches this week", filters: { fgTypeFilter: "POUCH", deliveryWindow: "THIS_WEEK" } },
    ]
  }
  if (scope === "sku_catalog") {
    return [
      { name: "Active rolls", filters: { activeFilter: "active", fgTypeFilter: "ROLL" } },
      { name: "Active pouches", filters: { activeFilter: "active", fgTypeFilter: "POUCH" } },
      { name: "Customer-proven", filters: { usageOnly: true } },
      { name: "All live templates", filters: { activeFilter: "active", templateFilter: "all" } },
    ]
  }
  if (scope === "planner_queue") {
    return [
      { name: "My planning queue", filters: { activeTab: "planning", queueFilter: "ALL" } },
      { name: "Ready to release", filters: { activeTab: "planning", queueFilter: "READY" } },
      { name: "Artwork blocked", filters: { activeTab: "planning", queueFilter: "ARTWORK_GATE" } },
      { name: "FG direct", filters: { activeTab: "planning", sourcePathFilter: "FG" } },
      { name: "WIP convertible", filters: { activeTab: "planning", sourcePathFilter: "WIP" } },
    ]
  }
  if (scope === "planner_sku") {
    return [
      ...
      { name: "Invariant rolls", filters: { launchFilter: "SHARED_INVARIANT_ROLL" } },
...
function filterSignature(filters: SalesSavedViewFilters) {
  return JSON.stringify(
    Object.entries(filters)
      .filter(([, value]) => value !== "" && value !== "all" && value !== "ALL" && value !== false && value !== null && value !== undefined)
...
export function salesVariantSpecSource(sku: Partial<SalesSku> | null | undefined, variant: SalesSkuVariant) {
  return {
    product_name: [sku?.code, sku?.name].filter(Boolean).join(" · ") || variant.sku_name || variant.name,
...
export function specMatchesFilters(
  ...
  if (filters.fgTypeFilter && filters.fgTypeFilter !== "all" && filters.fgTypeFilter !== "ALL" && fgType !== filters.fgTypeFilter.toUpperCase()) {
    return false
  }
  if (filters.sizeFilter?.trim()) {
    const q = filters.sizeFilter.trim().toLowerCase()
    const numericMatch = numberMatchesRange(spec.size.widthMm, q) || numberMatchesRange(spec.size.heightMm, q)
    if (!numericMatch && !spec.size.label.toLowerCase().includes(q)) return false
  }
  if (filters.heightFilter?.trim() && !numberMatchesRange(spec.size.heightMm, filters.heightFilter)) return false
  if (filters.variantFilter?.trim()) {
    ...
    const haystack = [
      ...
      spec.variantName,
      spec.templateName,
      ...spec.layers.flatMap((layer) => [layer.variantCode, layer.variantName, layer.label]),
    ].join(" ").toLowerCase()
  ...
  if (filters.thicknessFilter?.trim()) {
    ...
    if (hasNumericRange && spec.layers.some((layer) => numberMatchesRange(layer.thicknessMicron, q))) return true
    const haystack = spec.layers.map((layer) => `${compactNumber(layer.thicknessMicron)} ${layer.label}`).join(" ").toLowerCase()
    if (!haystack.includes(q)) return false
  }
  return true
}
...
export function SalesSpecChip({
  ...
  return (
    <span className={cn("inline-flex min-h-7 items-center rounded-full border px-3 py-1 text-[11px] font-black leading-none", CHIP_CLASS[tone], className)}>
      {children}
    </span>
  )
}
...
export function SalesOverflowChipGroup({
  ...
  const selectedOverflow = cleanOptions.find((option) => option.value === value && !cleanOptions.slice(0, maxInline).some((inline) => inline.value === option.value))
  ...
  const hiddenOptions = cleanOptions.filter((option) => !inlineOptions.some((inline) => inline.value === option.value))
  ...
  const hasActiveOption = value !== allValue && cleanOptions.some((option) => option.value === value)
  ...
  function renderButton(option: SalesOverflowChipOption | null) {
    ...
    const active = option ? value === option.value : !hasActiveOption && value === allValue
    ...
    return (
      <button
        ...
        className={cn(
          "inline-flex h-9 shrink-0 items-center gap-2 rounded-full border px-3 text-xs font-black leading-none transition hover:-translate-y-0.5 hover:border-blue-300 hover:bg-blue-50 disabled:pointer-events-none disabled:opacity-50",
          active
  ...
  return (
    <div className="flex min-w-0 flex-wrap items-center gap-2">
      <span className="inline-flex h-9 shrink-0 items-center text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{label}</span>
      {renderButton(null)}
      ...
      {hiddenOptions.length ? (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="start" className="max-h-[22rem] w-[min(22rem,calc(100vw-2rem))] overflow-y-auto rounded-2xl border-slate-200 p-2">
            <DropdownMenuLabel className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-500">{label} options</DropdownMenuLabel>
            {hiddenOptions.length > 8 ? (
              <div className="px-1 pb-2">
                <Input
                  ...
                  className="h-9 rounded-xl border-slate-200 bg-slate-50 text-xs font-semibold"
                />
              </div>
            ) : null}
            {searchedHiddenOptions.map((option) => (
              <DropdownMenuItem
                ...
                onClick={() => onChange(value === option.value ? allValue : option.value)}
              >
            ...
            {!searchedHiddenOptions.length ? (
              <div className="px-3 py-5 text-center text-xs font-semibold text-slate-500">No {label.toLowerCase()} options match.</div>
            ) : null}
          </DropdownMenuContent>
        </DropdownMenu>
      ) : null}
...
export function SalesSmartRangeFilter({
  ...
  const cleanPresets = presets.filter((preset) => preset.value && preset.label).slice(0, 18)
    ...
    >
      ...
      {cleanPresets.length ? (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="start" className="max-h-[20rem] w-[min(18rem,calc(100vw-2rem))] overflow-y-auto rounded-2xl border-slate-200 p-2">
            <DropdownMenuLabel className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-500">{label} presets</DropdownMenuLabel>
            <DropdownMenuItem className="rounded-xl p-3" onClick={() => onChange("")}>
              <div className="font-black text-slate-900">Any {label.toLowerCase()}</div>
            </DropdownMenuItem>
...
export function SalesSpecChips({
  ...
  const resolvedFgType = String(spec.size.finishedGoodType || fgType || "").toUpperCase()
  const thicknesses = uniq(spec.layers.map((layer) => layer.thicknessMicron).filter((value) => value !== null).map((value) => `${compactNumber(value)}μ`))
  const materials = uniq(spec.layers.map((layer) => layer.variantName || layer.variantCode))
  const grades = uniq(spec.layers.map((layer) => layer.grade))
  const print = printingLabel || (spec.podLabels.find((label) => /print|flexo|roto|color/i.test(label)) || "")

  return (
    <div className={cn("flex flex-wrap gap-2", compact && "gap-1.5")}>
      {resolvedFgType ? <span className={cn("inline-flex min-h-7 items-center rounded-full border px-3 py-1 text-[11px] font-black leading-none", fgChipClass(resolvedFgType))}>{resolvedFgType}</span> : null}
      <SalesSpecChip tone="size"><Ruler className="mr-1.5 h-3.5 w-3.5" />{spec.size.label}</SalesSpecChip>
      {thicknesses.length ? <SalesSpecChip tone="thickness">{thicknesses.join(" + ")}</SalesSpecChip> : <SalesSpecChip tone="muted">Thickness —</SalesSpecChip>}
      {materials.slice(0, compact ? 1 : 3).map((label) => <SalesSpecChip key={`mat-${label}`} tone="material">{label}</SalesSpecChip>)}
      {materials.length > (compact ? 1 : 3) ? <SalesSpecChip tone="material">+{materials.length - (compact ? 1 : 3)} materials</SalesSpecChip> : null}
      {grades.length ? grades.slice(0, compact ? 1 : 3).map((label) => <SalesSpecChip key={`grade-${label}`} tone="grade">Grade {label}</SalesSpecChip>) : <SalesSpecChip tone="muted">Grade —</SalesSpecChip>}
      {print ? <SalesSpecChip tone="print">{print}</SalesSpecChip> : <SalesSpecChip tone="print">No print</SalesSpecChip>}
      {spec.templateName ? <SalesSpecChip tone="template">{spec.templateName}</SalesSpecChip> : null}
      {spec.hasPod ? spec.podLabels.filter((label) => !/print|flexo|roto|color/i.test(label)).slice(0, 2).map((label) => <SalesSpecChip key={`pod-${label}`} tone="pack">POD {label}</SalesSpecChip>) : <SalesSpecChip tone="muted">No POD</SalesSpecChip>}
      {spec.hasAddons ? spec.addonLabels.slice(0, maxAddonLabels).map((label) => <SalesSpecChip key={`addon-${label}`} tone="pack">{label}</SalesSpecChip>) : <SalesSpecChip tone="muted">No add-ons</SalesSpecChip>}
    </div>
  )
}
...
export function SalesLayerTable({
  ...
  return (
    <div className={cn("overflow-hidden rounded-2xl border border-slate-200 bg-white", className)}>
      ...
      {spec.layers.length ? spec.layers.map((layer) => (
        <div
          key={`${layer.index}-${layer.label}`}
          className={cn(
            "grid grid-cols-[3rem_minmax(0,1.35fr)_0.7fr_0.85fr_0.7fr] items-center gap-0 border-b border-slate-100 px-3 last:border-b-0",
    ...
    <section className={cn("rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-[0_18px_50px_-40px_rgba(15,23,42,0.35)]", className)}>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          ...
          <h2 className="mt-1 break-words text-2xl font-black tracking-tight text-slate-950">{spec.productName}</h2>
          <div className="mt-1 text-sm font-bold text-slate-500">
            {[spec.customerName, spec.orderNumber, spec.variantCode || spec.variantName].filter(Boolean).join(" · ")}
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          {status ? <SalesSpecChip tone="muted">{status}</SalesSpecChip> : null}
          {blockerCount > 0 ? <SalesSpecChip tone="fgRoll">{blockerCount} blocker{blockerCount === 1 ? "" : "s"}</SalesSpecChip> : <SalesSpecChip tone="grade">Ready</SalesSpecChip>}
        </div>
      </div>
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        ...
        <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
          <div className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-500">Order context</div>
          <div className="mt-2 text-sm font-black text-slate-900">{spec.templateName || spec.variantName || "Template pending"}</div>
        </div>
      </div>
      <div className="mt-4">
  ...
  return (
    <section className="rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-[0_18px_50px_-40px_rgba(15,23,42,0.35)]">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="text-[11px] font-black uppercase tracking-[0.18em] text-slate-500">{title}</div>
          <h3 className="mt-1 text-xl font-black tracking-tight text-slate-950">{outputLabel}</h3>
        </div>
        {status ? <SalesSpecChip tone="muted">{status}</SalesSpecChip> : null}
      </div>
      ...
      <div className="mt-4 h-2 rounded-full bg-slate-100">
        <div className="h-full rounded-full bg-slate-950" style={{ width: "28%" }} />
      </div>
    </section>
  )
}
...
export function SalesSavedViewsBar({
  ...
  const activeSavedId = savedViews.find((view) => filterSignature(view.filters) === activeSignature)?.id || ""
  const updateTarget = savedViews.find((view) => view.id === (activeSavedId || lastAppliedSavedId)) || null
  const activeStarterName = starters.find((view) => filterSignature(view.filters) === activeSignature)?.name || ""
  ...
  function openSaveDialog() {
    const now = new Date()
    const viewPrefix = scope.startsWith("planner") ? "Planner view" : "Sales view"
    setSaveName(`${viewPrefix} · ${now.toLocaleDateString("en-IN", { day: "2-digit", month: "short" })}`)
    setMenuOpen(false)
    setSaveOpen(true)
  }
  ...
  function updateSavedView(id: string) {
    ...
    if (!target) return
    const now = new Date().toISOString()
    const next = savedViews.map((view) => view.id === id ? { ...view, filters: currentFilters, updatedAt: now } : view)
    persist(next)
  ...
  return (
    <div className={cn("rounded-[1.35rem] border border-slate-200 bg-white p-4 shadow-[0_16px_38px_-34px_rgba(15,23,42,0.35)]", className)} data-testid={`sales-saved-views-${scope}`}>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {updateTarget ? (
          <Button type="button" variant="outline" className="h-9 rounded-full border-blue-200 bg-blue-50 px-4 text-xs font-black text-blue-700 hover:bg-blue-100" onClick={() => updateSavedView(updateTarget.id)}>
        ...
        <DropdownMenu open={menuOpen} onOpenChange={setMenuOpen}>
          <DropdownMenuTrigger asChild>
            ...
            <DropdownMenuContent align="end" className="w-[min(28rem,calc(100vw-2rem))] rounded-2xl border-slate-200 p-2 shadow-[0_24px_58px_-28px_rgba(15,23,42,0.38)]">
              <DropdownMenuLabel className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-500">Starter views</DropdownMenuLabel>
              {starters.map((view) => (
                <DropdownMenuItem key={`menu-starter-${view.name}`} className="rounded-xl p-3" onClick={() => {
                  setLastAppliedSavedId("")
                  onApply(view.filters)
                }}>
                  <div className="min-w-0">
                    <div className="font-black text-slate-900">{view.name}</div>
                    <div className="mt-0.5 truncate text-xs text-slate-500">{summarizeFilters(view.filters)}</div>
                  </div>
                </DropdownMenuItem>
              ))}
              <DropdownMenuSeparator />
              <DropdownMenuLabel className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-500">My saved views</DropdownMenuLabel>
              {!savedViews.length ? (
                <div className="px-3 py-5 text-center text-sm font-semibold text-slate-500">No saved views yet.</div>
              ) : savedViews.map((view) => (
                <div key={`menu-${view.id}`} className="flex items-center gap-2 rounded-xl px-1 py-1 hover:bg-slate-50">
                  <button
                    type="button"
                  ...
                  >
                    <div className="truncate text-sm font-black text-slate-900">{view.name}</div>
                    <div className="mt-0.5 truncate text-xs text-slate-500">{summarizeFilters(view.filters)}</div>
                  </button>
                  <button
                    type="button"
      ...
      <Dialog open={saveOpen} onOpenChange={setSaveOpen}>
        <DialogContent className="sm:max-w-[28rem]">
          <DialogHeader>
            ...
            <DialogDescription>Name this filter setup so it can be reused on this browser.</DialogDescription>
          </DialogHeader>
          <div className="space-y-2">
            <Label>View name</Label>
            <Input value={saveName} onChange={(event) => setSaveName(event.target.value)} autoFocus />
            <div className="rounded-2xl bg-slate-50 px-3 py-2 text-xs font-semibold text-slate-500">{summarizeFilters(currentFilters)}</div>
          </div>
          <DialogFooter>
  ...
  return (
    ...
    >
      <button type="button" className="w-full text-left" onClick={onSelect}>
        ...
        <div className="mt-3">
          <SalesSpecChips spec={spec} fgType={variant.finished_good_type} printingLabel={printingLabel} compact maxAddonLabels={1} />
        </div>
        <div className="mt-3 rounded-2xl border border-slate-200 bg-white/88 p-2">
          <SalesLayerTable spec={spec} dense />
        </div>
      </button>
      {onAdd ? (
        <Button type="button" className="mt-3 h-10 w-full rounded-full bg-slate-950 text-xs font-black uppercase tracking-[0.14em] hover:bg-slate-800" onClick={onAdd}>
