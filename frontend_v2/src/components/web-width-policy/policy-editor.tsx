...
import {
  factoryService,
  type Machine as FactoryMachine,
  type Process as FactoryProcess,
...
export function PolicyEditor({ id, initialMode }: Props) {
  ...
  const set = <K extends keyof WebWidthPolicy>(k: K, v: WebWidthPolicy[K]) =>
    setDraft((d) => ({ ...d, [k]: v }));
  const effectiveScopeType = draft.scope_type || "GLOBAL";

  const { data: productMasters = [], isLoading: productMastersLoading } =
    useQuery({
  ...
  const { data: pouchStyles = [], isLoading: pouchStylesLoading } = useQuery({
    ...
    queryFn: () =>
      pouchStyleService.list({ deprecated: false, page_size: 500 }),
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: (saved) => {
      toast({
        title: isNew ? "Policy created" : "Saved",
  ...
  const removeMutation = useMutation({
    ...
    onSuccess: () => {
      toast({ title: "Deleted" });
      router.push("/master/web-width-policies");
    },
    onError: (e: any) =>
      toast({
        title: "Delete failed",
  ...
  const setRule = (k: string, v: any) =>
    set("slitting_waste_rule", { ...rule, [k]: v } as any);
  const [previewChild, setPreviewChild] = React.useState(440);
  const [previewLane, setPreviewLane] = React.useState(2);
  const previewPlan = computeWebWidthPlan(
    previewChild,
    previewLane,
    draft as WebWidthPolicy,
  );
  const scopeNeedsRef = effectiveScopeType !== "GLOBAL";
  const scopeOptions = React.useMemo(
    () =>
      buildScopeRefOptions(effectiveScopeType, {
        productMasters,
        pouchStyles,
        processes,
        machines,
  ...
  const visibleScopeOptions = React.useMemo(() => {
    if (
      !draft.scope_ref ||
      scopeOptions.some((o) => o.value === draft.scope_ref)
    )
      return scopeOptions;
    return [
      {
        ...
        detail:
          "This saved ref is not in the current active master list. Pick a listed master before changing this policy.",
      },
      ...scopeOptions,
    ];
  ...
  const selectedScopeOption = visibleScopeOptions.find(
    (o) => o.value === draft.scope_ref,
  );
  const scopeRefMissing = scopeNeedsRef && !draft.scope_ref;

  const exampleParent = React.useMemo(() => {
    return computeWebWidthPlan(440, 3, draft as WebWidthPolicy)
        ...
        <div className="space-y-5">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <Field label="Name">
                <Input
                  ...
                  placeholder="Standard slit · 1-up to 3-up"
                />
              </Field>
              ...
              <label className="flex items-center gap-2 text-[12px]">
                <input
                  ...
                  onChange={(e) => set("is_default", e.target.checked as any)}
                />
                <span className="font-bold">Default policy</span>
                <span className="text-[10px] text-content-3">
                  (fallback when no scoped policy matches)
                </span>
              </label>
              <Field label="Scope">
                <select
                  ...
                  onChange={(e) =>
                    setDraft((d) => ({
                      ...d,
                      scope_type: e.target
                        .value as WebWidthPolicy["scope_type"],
                      scope_ref: "",
                    }))
                  }
                  className="h-9 w-full rounded-xl border border-line bg-surface-1 px-3 text-[12px]"
                >
              ...
              {scopeNeedsRef ? (
                <Field label="Scope ref">
                  <select
                    ...
                    className="h-9 w-full rounded-xl border border-line bg-surface-1 px-3 font-mono text-[12px] disabled:bg-surface-2 disabled:text-content-4"
                  >
                    <option value="">
                      {scopeLoading
                        ? "Loading options..."
                  ...
                  <div className="mt-1 min-h-4 text-[10px] text-content-3">
                    {selectedScopeOption?.detail ||
                      "Only listed master records can be selected for this scope."}
                  </div>
                </Field>
              ) : null}
            </div>
          ...
          <Card title="Allowed lane counts">
            <p className="-mt-1 mb-3 text-[11px] text-content-3">
              Which N-up runs production can do for this policy scope.
              Sales-order lane picker shows only these.
            </p>
              ...
              {[1, 2, 3, 4, 5, 6].map((n) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      const next = on
                        ? allowedLanes.filter((v) => v !== n)
                        : [...allowedLanes, n].sort((a, b) => a - b);
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              <Field label="Strategy">
                <select
                  ...
                  onChange={(e) =>
                    set("parent_width_strategy", e.target.value as any)
                  }
                  className="h-9 w-full rounded-xl border border-line bg-surface-1 px-3 text-[12px]"
                >
                  <option value="CALCULATED">Calculated width</option>
                  <option value="NEAREST_STANDARD">
                    Nearest standard parent
                  </option>
                  <option value="STRICT_STANDARD">
                    Require standard parent
                  </option>
                </select>
              </Field>
              <Field label="Min parent width (mm)">
                <Input
                  type="number"
                  value={String(draft.min_parent_width_mm ?? "")}
                  onChange={(e) =>
                    set("min_parent_width_mm", e.target.value as any)
                  }
              ...
              <Field label="Max parent width (mm)">
                <Input
                  type="number"
                  value={String(draft.max_parent_width_mm ?? "")}
                  onChange={(e) =>
                    set("max_parent_width_mm", e.target.value as any)
                  }
                  placeholder="optional"
                />
              </Field>
            </div>
            <p className="mt-3 text-[11px] text-content-3">
              Calculated mode keeps exact math. Nearest standard snaps to the
              next configured parent width. Strict standard blocks the order if
              no configured width fits.
            </p>
          </Card>

          <Card title="Standard parent widths">
            <p className="-mt-1 mb-3 text-[11px] text-content-3">
              Use for standard jumbo/slit widths such as 880, 1320, 1500. Leave
              empty if this policy should only use calculated widths.
            </p>
            <div className="flex flex-wrap items-center gap-2">
              {allowedParents.map((w, i) => (
                <span
                  key={i}
                  className="flex items-center gap-1 rounded-full bg-order-bg px-2 py-1 font-mono text-[11px] font-bold text-order-fg"
                >
                  {w} mm
                  <button
                    onClick={() =>
                      set(
                        "allowed_parent_widths",
                        allowedParents.filter((_, j) => j !== i) as any,
                      )
                    }
              ...
              <ParentWidthAdder
                onAdd={(w) =>
                  set(
                    "allowed_parent_widths",
                    [...allowedParents, w].sort((a, b) => a - b) as any,
                  )
                }
              />
            </div>
          ...
          <Card title="Slitting trim rule">
            <div className="grid gap-3 sm:grid-cols-3">
              <Field label="Inter-cut trim (mm)">
                <Input
                  type="number"
                  value={Number(rule.inter_cut_mm ?? 5)}
                  onChange={(e) =>
                    setRule("inter_cut_mm", Number(e.target.value || 0))
                  }
                />
              </Field>
              <Field label="Edge trim (mm)">
                <Input
                  type="number"
                  value={Number(rule.edge_trim_mm ?? 2)}
                  onChange={(e) =>
                    setRule("edge_trim_mm", Number(e.target.value || 0))
                  }
                />
              </Field>
              <Field label="Formula">
            ...
            <div className="mt-3 rounded-xl bg-surface-3 px-3 py-2 font-mono text-[12px] text-success-border">
              For 440 mm child × 3 lanes: parent ={" "}
              <b className="text-white">{exampleParent} mm</b> using this
              policy.
            </div>
          </Card>
          ...
          <Card title="Remainder handling">
            <div className="grid gap-3 sm:grid-cols-2">
              <Field label="Min remainder to keep (mm)">
                <Input
                  type="number"
                  value={Number(draft.min_remainder_mm || 50)}
                  onChange={(e) =>
                    set("min_remainder_mm", Number(e.target.value || 0) as any)
                  }
                />
                <div className="mt-1 text-[10px] text-content-3">
                  Remainders narrower than this go to scrap.
                </div>
              </Field>
              <label className="flex flex-col gap-1 self-end">
                ...
                <label className="mt-1 flex items-center gap-2 text-[12px]">
                  <input
                    type="checkbox"
                    checked={!!draft.prefer_remainder_first}
                    onChange={(e) =>
                      set("prefer_remainder_first", e.target.checked as any)
                    }
                  />
                  <span>
                    <b>Prefer remainders first</b> when allocating
                  </span>
                </label>
              </label>
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              {!isNew ? (
                <Button
                  variant="outline"
                  onClick={() => {
                    if (confirm("Delete this policy?")) removeMutation.mutate();
                  }}
            ...
            <div className="grid gap-2">
              <div className="grid grid-cols-2 gap-2">
                <Field label="Child width">
                  <Input
                    ...
                    value={previewChild}
                    onChange={(e) =>
                      setPreviewChild(Number(e.target.value || 0))
                    }
                  />
                </Field>
                <Field label="Lane">
                  <Input
                    ...
                    value={previewLane}
                    onChange={(e) =>
                      setPreviewLane(Number(e.target.value || 1))
                    }
                  />
                </Field>
              </div>
              <div className="rounded-xl bg-order-fg p-3 text-white">
                ...
                <div className="mt-1 text-[10px] text-order-border">
                  run {Math.round(previewPlan.computed_run_width_mm)} · trim{" "}
                  {Math.round(previewPlan.trim_mm)} · rem{" "}
                  {Math.round(previewPlan.remainder_mm)}{" "}
                  {previewPlan.remainder_disposition.toLowerCase()}
                </div>
              </div>
              {previewPlan.warnings.length ? (
                <div className="rounded-lg bg-warning-bg px-2 py-1 text-[11px] font-bold text-warning-fg">
                  {previewPlan.warnings.join(" ")}
                </div>
              ) : null}
            </div>
          </Card>
...
const PRODUCT_KIND_SCOPE_OPTIONS: ScopeRefOption[] = [
  {
    value: "POUCH",
    label: "POUCH · finished pouch products",
    detail:
      "Applies to all pouch product masters unless a more specific policy exists.",
  },
  {
    value: "ROLL",
    label: "ROLL · film roll products",
    detail:
      "Applies to roll-form product masters unless a more specific policy exists.",
  },
  {
    value: "PACKAGING",
    label: "PACKAGING · in-house packing products",
    detail: "Applies to packaging product masters and packing stock launchers.",
  },
  {
    value: "POD",
    label: "POD · POD roll products",
    detail: "Applies to POD product masters and POD stock launchers.",
  },
  {
    value: "OTHER",
    label: "OTHER · fallback product kind",
    detail: "Use only when a master is intentionally classified as OTHER.",
  },
];
...
function Card({
  ...
  return (
    <section className="rounded-2xl border border-line bg-surface-1 p-4 shadow-sm">
      <h2 className="mb-3 font-display text-sm font-bold text-content-1">
        {title}
      </h2>
      {children}
    </section>
