...
export function LiveBomRail({
  ...
  const effectiveTitle =
    title ??
    (isVariantScope ? "Engineering BOM · variant scope" : "Live BOM preview");
  return (
    <aside
      className={cn(
        "flex flex-col rounded-2xl border border-order-border bg-gradient-to-b from-order-bg via-white to-white shadow-md overflow-hidden",
        sticky ? "max-h-[calc(100vh-6rem)] lg:sticky lg:top-4" : "",
        className,
      )}
    >
      {/* ─── Header pill row ─── */}
      <header className="flex items-center justify-between gap-2 border-b border-order-border bg-gradient-to-r from-order-bg via-white to-success-bg px-5 py-3">
        <div className="flex items-center gap-2">
          <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-gradient-to-br from-order-fg to-success-fg text-white shadow-sm">
            ⚡
          </span>
          <div>
        ...
        {badge ??
          (preview ? (
            blockers === 0 ? (
              <span className="inline-flex items-center gap-1 rounded-full bg-success-bg px-2 py-0.5 text-[10px] font-bold text-success-fg ring-1 ring-success-border">
      ...
      {isVariantScope && !empty ? (
        <ScopeChipRow masterFlags={masterFlags} />
      ) : null}

      {/* ─── Body ─── */}
      <div
        className={cn(
          "flex-1 px-5 py-3 space-y-3",
          sticky && "overflow-y-auto",
        )}
      >
        {routeSteps && routeSteps.length > 0 ? (
          <RouteRibbon steps={routeSteps} templateName={routeTemplateName} />
        ) : null}
        {empty ? (
...
function RouteRibbon({
  ...
  steps: NonNullable<LiveBomRailProps["routeSteps"]>;
  templateName?: string;
}) {
  const TONE: Record<string, string> = {
    EXTRUSION: "bg-info-bg text-primary ring-info-border",
    PRINTING: "bg-danger-bg text-danger-fg ring-danger-border",
    ROTO: "bg-danger-bg text-danger-fg ring-danger-border",
    FLEXO: "bg-danger-bg text-danger-fg ring-danger-border",
    LAMINATION: "bg-warning-bg text-warning-fg ring-warning-border",
    SLITTING: "bg-info-bg text-info-fg ring-info-border",
    POUCHING: "bg-success-bg text-success-fg ring-success-border",
    PACKING: "bg-order-bg text-order-fg ring-order-border",
    DEFAULT: "bg-surface-2 text-content-2 ring-line-strong",
  };
  const toneFor = (name?: string, code?: string) => {
    const key = (code || name || "").toUpperCase();
    return (
      TONE[key] ||
      Object.entries(TONE).find(([k]) => key.includes(k))?.[1] ||
      TONE.DEFAULT
    );
  };
  return (
    <section className="rounded-2xl border border-order-border bg-gradient-to-br from-order-bg via-white to-order-bg p-3 shadow-sm">
      <div className="flex items-center justify-between mb-2">
        <div className="text-[10px] font-bold uppercase tracking-wider text-order-fg">
          Live route{templateName ? ` · ${templateName}` : ""}
        </div>
        ...
        {steps.map((s, i) => {
          ...
          return (
            <React.Fragment key={`r-${s.index ?? i}`}>
              <span
                className={cn(
                  "inline-flex items-center gap-1 rounded-lg px-2 py-1 text-[10px] font-bold ring-1 capitalize",
                  tone,
                )}
              >
              ...
              {i < steps.length - 1 ? (
                <span className="text-content-4 text-[12px] font-bold">›</span>
              ) : null}
            </React.Fragment>
          );
        })}
...
function ScopeChipRow({
  ...
  masterFlags?: LiveBomRailProps["masterFlags"];
}) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5 border-b border-order-border bg-order-bg px-5 py-2 text-[10px]">
      ...
      {deferred.map((d) => (
        <span
          key={d}
          className="inline-flex items-center rounded-full bg-surface-1 px-2 py-0.5 font-bold text-order-fg ring-1 ring-order-border"
        >
          + {d}
        </span>
      ))}
      {podLocked ? (
        <span className="inline-flex items-center rounded-full bg-success-bg px-2 py-0.5 font-bold text-success-fg ring-1 ring-success-border">
          POD locked at master
        </span>
      ) : null}
    </div>
  );
...
function EmptyState({ loading }: { loading?: boolean }) {
  return (
    <div className="flex h-full min-h-[260px] flex-col items-center justify-center rounded-2xl border border-dashed border-line bg-surface-1/70 p-6 text-center">
      <Sparkles className="h-6 w-6 text-content-4" />
      <div className="mt-2 text-sm font-semibold text-content-2">
        {loading ? "Computing live preview…" : "Pick axes to preview"}
      </div>
      <p className="mt-1 max-w-xs text-xs text-content-3">
        Pick a customer, product master, size and per-layer axes to compute
        geometry, layers, packaging and BOM in real time.
      </p>
    </div>
  );
}
...
function QuantityConversion({ preview }: { preview: PreviewBomResult }) {
  ...
  const orderQty = Number(
    previewPayload.order_qty ?? previewPayload.quantity ?? 0,
  );
  const orderUom = String(
    previewPayload.uom || previewPayload.quantity_uom || "",
  ).toUpperCase();
  const unitWeightG = Number(preview.unit_weight_g || 0);
  const totalKg = Number(
    preview.total_weight_kg || (fgType === "ROLL" ? orderQty : 0),
  );
  if (fgType === "ROLL") {
    return (
      <section className="rounded-2xl border border-line bg-surface-1 p-3">
        <div className="text-[10px] font-bold uppercase tracking-wider text-content-3 mb-2">
          Quantity conversion
        </div>
        <div className="rounded-xl bg-surface-2 px-3 py-2 ring-1 ring-line">
          ...
          <div className="mt-1 text-[10px] font-semibold text-content-3">
            Roll masters are planned and issued in KG only.
          </div>
        </div>
      </section>
    );
  ...
  const innerPacks =
    pieces > 0 && pcsPerInner > 0 ? Math.ceil(pieces / pcsPerInner) : 0;
  const cells = [
    {
      label: "Order",
      value: orderQty > 0 ? `${fmtQty(orderQty)} ${orderUom || "KG"}` : "—",
    },
    { label: "Order kg", value: totalKg > 0 ? `${fmtQty(totalKg)} kg` : "—" },
    { label: "Pieces", value: pieces > 0 ? fmtNum(Math.round(pieces)) : "—" },
    {
      label: "Pcs / inner",
      value: pcsPerInner > 0 ? fmtNum(pcsPerInner) : "not set",
    },
    {
      label: "Inner pouches",
      value: innerPacks > 0 ? `${fmtNum(innerPacks)} pcs` : "—",
    },
  ];
  return (
    <section className="rounded-2xl border border-line bg-surface-1 p-3">
      ...
      <div className="mt-2 text-[10px] font-semibold text-content-3">
        Pouches use unit weight for KG ↔ pieces conversion. Inner-pouch demand
        is ceil(pieces / pcs per inner).
      </div>
    </section>
  );
}
  ...
  return (
    <section className="rounded-2xl bg-gradient-to-br from-order-bg via-order-bg to-white ring-1 ring-order-border px-3 py-2.5 shadow-sm">
      ...
      <div className="mt-2 flex flex-wrap items-center gap-1.5 text-[10px]">
        ...
        >
          {preview.variant_status === "EXISTS" ? "EXISTS" : "NEW"}
          {preview.variant_code ? ` · ${preview.variant_code}` : ""}
        </span>
        {preview.overlay_match?.source &&
        preview.overlay_match.source !== "NONE" ? (
          <span className="rounded bg-order-bg px-1.5 py-0.5 font-bold text-order-fg ring-1 ring-order-border">
            overlay ·{" "}
            {preview.overlay_match.source.toLowerCase().replace("_", " ")}
          </span>
        ) : null}
      </div>
    </section>
...
function PouchRender({ preview }: { preview: PreviewBomResult }) {
  ...
  const height = Number(
    g.height_mm ??
      ...
      g.size_height_mm ??
      g.height ??
      g.effective_height_mm ??
      0,
  );
  const rollWidth = Number(g.roll_width_mm ?? g.roll_width ?? 0);
  const styleCode = String(
    g.pouch_style_master_code || g.pouch_style_master || g.pouch_style || "",
  ).toUpperCase();
  const featureHaystack =
    `${styleCode} ${JSON.stringify(preview.addons_snapshot || [])} ${JSON.stringify(preview.bom?.planning_lines || preview.bom_snapshot?.planning_lines || [])}`.toUpperCase();
  ...
  const hasDcut = /D[\s_-]?CUT|DIE[\s_-]?CUT|ROUND[\s_-]?CUT|HANDLE/.test(
    featureHaystack,
  );
  ...
  const featureTags = [
    ...
    .slice(0, 4);
  return (
    <section className="rounded-2xl border border-line bg-surface-1 px-3 py-3">
      <div className="flex items-center justify-between text-[10px] font-bold uppercase tracking-wider text-content-3 mb-2">
        <span>Visual</span>
        <span className="font-mono">{kind}</span>
      </div>
      <div className="flex items-center justify-center bg-surface-2 rounded-xl py-3">
        {kind === "POUCH" ? (
          <svg
            viewBox="0 0 180 190"
            width="150"
            ...
            role="img"
            aria-label="Pouch visual preview"
          >
            <defs>
              <linearGradient
                id={`lbr-pouch-${gradientId}`}
                x1="0"
                x2="1"
                y1="0"
                y2="1"
            ...
            <path
              d="M 46 18 L 134 18 L 143 28 L 143 165 L 37 165 L 37 28 Z"
              fill={`url(#lbr-pouch-${gradientId})`}
              stroke="#047857"
              strokeWidth="1.4"
            />
            <rect
            ...
            <rect
              ...
              rx="3"
              fill="#065f46"
              opacity={isStandup ? 0.26 : 0.12}
            />
            <path
              d="M 47 18 L 37 28 L 37 165"
              fill="none"
              stroke="#047857"
              strokeWidth="0.8"
              strokeDasharray="3 3"
            ...
            {isSideGusset ? (
              <>
                <path
                  d="M 51 35 C 61 62 61 123 51 150"
                  fill="none"
                  stroke="#065f46"
                  strokeWidth="1"
                  opacity="0.45"
                />
                <path
                  d="M 129 35 C 119 62 119 123 129 150"
                  fill="none"
                  stroke="#065f46"
                  strokeWidth="1"
                  opacity="0.45"
            ...
            {hasDcut ? (
              <path
                d="M 72 37 C 75 52 105 52 108 37"
                fill="none"
                stroke="#064e3b"
                strokeWidth="2.2"
                strokeLinecap="round"
            ...
            {isSpout ? (
              <>
                <path
                  d="M 123 16 L 148 7 L 154 21 L 134 31 Z"
                  fill="#99f6e4"
                  stroke="#047857"
                  strokeWidth="1"
                />
            ...
            {isStandup ? (
              <path
                d="M 48 159 C 71 171 109 171 132 159"
                fill="none"
                stroke="#064e3b"
                strokeWidth="1.3"
                opacity="0.5"
...
function GeometryStrip({ preview }: { preview: PreviewBomResult }) {
  ...
  const totalPouches =
    unitG > 0 && totalKg > 0 ? Math.round((totalKg * 1000) / unitG) : 0;
  ...
  const cells: Array<{
    ...
    {
      label: "W × H",
      value: widthMm > 0 ? `${widthMm} × ${heightMm || 0} mm` : "—",
    },
    ...(webWidthMm > 0 && pitchMm > 0
      ? [
          {
            label:
              areaBasis === "LEGACY_WEB_BASIS"
                ? "Film basis (legacy)"
                : "Film basis",
            value: `${webWidthMm} × ${pitchMm} mm`,
    ...
    {
      label: "Unit wt",
      value: unitG > 0 ? `${unitG} g` : "—",
      tone: unitG > 0 ? "emerald" : "default",
    },
    {
      label: "Order wt",
      value: totalKg > 0 ? `${fmtQty(totalKg)} kg` : "—",
      tone: totalKg > 0 ? "emerald" : "default",
    },
  ];
  if (totalPouches > 0)
    cells.push({
      label: "Pouches",
      value: fmtNum(totalPouches),
      tone: "emerald",
    });
  return (
    <section className="rounded-2xl border border-line bg-surface-1 p-3">
          ...
          >
            ...
            <div
              className={cn(
                ...
                c.tone === "emerald" ? "text-success-fg" : "text-content-1",
              )}
            >
              {c.value}
            </div>
...
function LayerStack({ preview }: { preview: PreviewBomResult }) {
  ...
  const total = preview.layer_snapshot.reduce(
    (s: number, l: any) => s + Math.max(Number(l?.thickness_micron || 0), 0),
    0,
  );
  ...
  const FILM_COLOR_ORDER: Array<[string, string]> = [
    ["MET", "from-surface-2 to-surface-2"],
    ["METALLIZED", "from-surface-2 to-surface-2"],
    ["ALU", "from-surface-2 to-surface-2"],
    ["LDPE", "from-success-fg to-success-fg"],
    ["LD-NAT", "from-success-fg to-success-fg"],
    ["LD", "from-success-fg to-success-fg"],
    ["HDPE", "from-success-fg to-success-fg"],
    ["BOPP", "from-info-fg to-info-fg"],
    ["BOPA", "from-order-fg to-order-fg"],
    ["BOPE", "from-order-fg to-order-fg"],
    ["PET", "from-info-fg to-primary"],
    ["NYL", "from-order-fg to-order-fg"],
    ["NYLON", "from-order-fg to-order-fg"],
    ["PP", "from-info-fg to-info-fg"],
    ["PE", "from-success-fg to-success-fg"],
    ["PAPER", "from-warning-fg to-warning-fg"],
  ];
  const colorOf = (code: string): string => {
    const upper = String(code || "").toUpperCase();
    const found = FILM_COLOR_ORDER.find(
      ([k]) =>
        upper.startsWith(k) ||
        upper.includes(`-${k}-`) ||
        upper.includes(`-${k}`),
    ...
    return found
      ? `bg-gradient-to-r ${found[1]}`
      : "bg-gradient-to-r from-surface-2 to-surface-2";
  };
  return (
        ...
        {preview.layer_snapshot.map((l: any, i: number) => {
          ...
          return (
            <div key={i} className="flex items-center gap-2">
              <span className="w-14 font-bold text-content-3 text-[10px] uppercase tracking-wider truncate">
                {l.role || `L${i + 1}`}
              </span>
              <div className="flex-1 h-6 rounded-md bg-surface-2 ring-1 ring-line overflow-hidden flex items-center">
                <span
                  className={cn(
                    "h-full shadow-inner",
...
function matchCategory(cat: string, code = "", name = ""): string {
  ...
  if (/INNER[_-]?POUCH|INNER\s+POUCH|PRIMARY\s+INNER|^IP[_-]/.test(hay))
    return "INNER_POUCH";
  if (/GUNNY|GONNY|SHEET|CARTON|BOX|TAPE|LABEL|TAG|PACKAGING/.test(hay))
    return "PACKAGING";
  if (/FILM|RESIN|PE\b|PET\b|HDPE|LDPE|PP\b|BOPP|LAMINATE/.test(hay))
    return "FILM";
  return "OTHER";
}
...
function MaterialBreakdown({
  ...
  const planningLines: any[] = (preview.bom?.planning_lines ||
    preview.bom_snapshot?.planning_lines ||
    []) as any[];
  const rows: Array<{
    category: string;
    code: string;
  ...
  planningLines.forEach((row) => {
    const category = String(
      row.category_code || row.category || "OTHER",
    ).toUpperCase();
    const code = String(row.material_code || row.code || "—");
    const name = String(row.material_name || row.name || "");
    if (artworkDeferred && matchCategory(category, code, name) === "INK")
      return;
    rows.push({
      category,
      code,
  ...
  if (rows.length === 0) {
    (preview.addons_snapshot || []).forEach((r: any) =>
      rows.push({
        category: "ADDON",
    ...
    (preview.packaging_lines || []).forEach((r: any) =>
      rows.push({
        category: "PACKAGING",
    ...
    (preview.pod_lines || []).forEach((r: any) =>
      rows.push({
        category: "POD",
  ...
  const hasCat = (cat: string) =>
    rows.some(
      (r) =>
        r.category === cat || matchCategory(r.category, r.code, r.name) === cat,
    );
  const printCapable = !!masterFlags?.print_capable;
  if (scope === "variant") {
    const podLocked = !!masterFlags?.pod_locked;
    ...
    if (addonsAxis !== "off" && !hasCat("ADDON")) {
      rows.push({
        ...
        name:
          addonsAxis === "required"
            ? "required — picked on order"
            : "optional — picked on order",
        qty: 0,
        uom: "KG",
        placeholder: true,
      });
    ...
    if (!podLocked && !hasCat("POD")) {
      rows.push({
        category: "POD",
        code: "POD-?",
        name: "POD variant picked on order line",
        qty: 0,
        uom: "KG",
        placeholder: true,
      });
  ...
  const groups: Array<{
    ...
    {
      key: "CHEMICAL",
      label: "Adhesive / solvent",
      eyebrow: "text-info-fg",
      tile: "bg-info-bg",
    ...
    },
    {
      ...
      tile: "bg-order-bg",
      chip: "bg-order-bg text-order-fg ring-order-border",
      note: "doesn't change per-pouch weight",
    },
    {
      key: "INNER_POUCH",
      label: "Inner pouch",
      ...
      note: "carrier · doesn't change per-pouch weight",
    },
  ];
            ...
            <section key={g.key} className={cn("px-3 py-2", g.tile)}>
              <div className="flex items-center justify-between text-[9px] font-black uppercase tracking-wider mb-1">
                <span className={cn("flex items-center gap-1.5", g.eyebrow)}>
                  ...
                  {g.note ? (
                    <span className="rounded-full bg-surface-1/70 px-1.5 py-0.5 text-[8px] font-bold normal-case tracking-normal opacity-80 ring-1 ring-surface-1/40">
                      {g.note}
                    </span>
                  ) : null}
                </span>
                ...
                <tbody className="divide-y divide-line">
                  {list.slice(0, 8).map((r, i) => (
                    <tr key={i} className={cn(r.placeholder && "opacity-70")}>
                      <td className="py-1 pr-2 align-top">
                        {r.placeholder ? (
                          <span
                            className="inline-block rounded px-1.5 py-0.5 text-[10px] font-mono font-bold text-content-3 bg-surface-1/60"
                            style={{
                              borderStyle: "dashed",
                              borderWidth: 1,
                          ...
                          <span className="font-mono text-[11px] font-bold text-content-1">
                            {r.code}
                          </span>
                        )}
                        {r.name ? (
                          <div
                            className={cn(
                              "mt-0.5 text-[10px] truncate max-w-[180px]",
                              r.placeholder
                                ? "text-content-4 italic"
                                : "text-content-3",
                            )}
                          >
                            {r.name}
                      ...
                      <td
                        className={cn(
                          "py-1 text-right font-mono font-bold tabular-nums",
                          r.placeholder ? "text-content-4" : "text-content-2",
                        )}
                        colSpan={2}
                      >
                  ...
                  {list.length > 8 ? (
                    <tr>
                      <td
                        colSpan={3}
                        className="py-1 text-center text-[10px] text-content-4"
                      >
                        + {list.length - 8} more in {g.label.toLowerCase()}
                      </td>
                    </tr>
                  ) : null}
                </tbody>
  ...
  const tiles = [
    {
      ...
      value: fg,
      pct: pct(fg),
      tile: "bg-success-bg text-success-fg ring-success-border",
      bar: "bg-success-fg",
    },
    {
      label: "Shared WIP",
      value: wip,
      pct: pct(wip),
      tile: "bg-info-bg text-primary ring-info-border",
      bar: "bg-primary",
    },
    {
      label: "Fresh route",
      value: fresh,
      pct: pct(fresh),
      tile: "bg-warning-bg text-warning-fg ring-warning-border",
      bar: "bg-warning-fg",
    },
  ];
  return (
    <section className="rounded-2xl border border-line bg-surface-1 p-3">
          ...
          ? tiles.map((t) =>
              t.value > 0 ? (
                <div
                  ...
                  className={t.bar}
                  style={{ width: `${t.pct}%` }}
                  title={`${t.label} ${t.value} · ${t.pct.toFixed(0)}%`}
                />
              ) : null,
            )
          : null}
      ...
      <div className="mt-2 text-[10px] text-content-3">
        FG ships first · WIP next · Fresh route only when no stock found.
      </div>
    </section>
  );
}
...
function ChecksList({ preview }: { preview: PreviewBomResult }) {
  ...
  return (
    <section className="rounded-2xl border border-line bg-surface-1 p-3 space-y-1.5">
      <div className="text-[10px] font-bold uppercase tracking-wider text-content-3">
        Checks
      </div>
      {checks.map((c, i) => (
        <div key={`c-${i}`} className="flex items-start gap-2 text-[11px]">
          ...
          <span className="text-content-2">{c.label}</span>
        </div>
      ))}
      {warnings.map((w, i) => (
        <div key={`w-${i}`} className="flex items-start gap-2 text-[11px]">
          <AlertTriangle className="mt-0.5 h-3.5 w-3.5 flex-none text-warning-fg" />
          <span className="text-warning-fg">{w}</span>
        </div>
      ))}
      {blockers.map((b, i) => (
        <div key={`b-${i}`} className="flex items-start gap-2 text-[11px]">
          <AlertTriangle className="mt-0.5 h-3.5 w-3.5 flex-none text-danger-fg" />
          <span className="text-danger-fg font-bold">{b}</span>
        </div>
      ))}
    </section>
  );
...
function LineFooter({
  ...
  line: NonNullable<LiveBomRailProps["line"]>;
  preview: PreviewBomResult;
}) {
    ...
    <footer className="bg-success-fg px-5 py-3 text-white sticky bottom-0">
      <div className="flex items-center justify-between gap-2">
        <div className="min-w-0">
          ...
          <div className="text-[10px] font-bold text-success-border">
            {pouches > 0
              ? `${fmtNum(pouches)} pouches · ${fmtQty(totalKg)} kg total`
              : "—"}
          </div>
        </div>
        {line.onAdd ? (
          ...
          <span className="inline-flex items-center gap-1 rounded-xl bg-surface-1/15 px-3 py-1.5 text-[11px] font-bold text-white ring-1 ring-surface-1/30">
