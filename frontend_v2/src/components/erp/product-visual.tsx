...
export function ProductVisual({
  ...
  const totalThickness =
    layers.reduce((sum, l) => sum + (l.thickness_micron || 0), 0) || 1;
  return (
    <div
      className={cn(
        "relative overflow-hidden rounded-2xl border border-line bg-gradient-to-br from-surface-2 via-white to-info-bg p-4 ring-1 ring-line",
        className,
      )}
    >
      <div
      ...
      >
        <div
          className={cn(
            "relative flex flex-1 items-end justify-center rounded-xl bg-surface-1/70 ring-1 ring-line",
            compact ? "min-h-[120px]" : "min-h-[180px]",
          )}
        >
          {kind === "POUCH" ? (
            ...
            <Roll
              width_mm={roll_width_mm || width_mm}
              accent="violet"
              compact={compact}
            />
          ) : (
          ...
          {(title || subtitle) && (
            <div className="absolute left-3 top-3 right-3 flex items-start justify-between">
              ...
              <span className="rounded-full bg-surface-1/80 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-content-3 ring-1 ring-line">
                {kind}
              </span>
            </div>
          )}
      ...
      {addons.length > 0 ? (
        <div className="mt-3 flex flex-wrap gap-1.5">
          {addons.map((a) => (
            <span
              key={a}
              className="rounded-full bg-order-bg px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-order-fg ring-1 ring-order-border"
            >
              {a}
            </span>
          ))}
...
function Roll({
  ...
  return (
    <svg
      viewBox={`-15 -15 ${drawW + 30} ${drawH + 40}`}
      width="100%"
      height="100%"
      preserveAspectRatio="xMidYMax meet"
    >
...
function Generic({ compact }: { compact?: boolean }) {
  return (
    <div
      className={cn(
        "flex flex-col items-center gap-1 text-content-3",
        compact ? "py-3" : "py-6",
      )}
    >
      <div className="h-12 w-12 rounded-2xl bg-surface-2 ring-1 ring-line" />
