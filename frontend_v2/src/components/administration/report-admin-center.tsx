...
export function ReportAdminCenter() {
  ...
  const [draftProfiles, setDraftProfiles] = useState<
    Record<string, ReportDistributionProfile>
  >({});
  ...
  const saveMutation = useMutation({
    mutationFn: async () =>
      analyticsApi.updateReportDistributions(Object.values(draftProfiles)),
    onSuccess: () => {
      toast({
        title: "Report center saved",
        description: "Daily archive generation states were updated.",
      });
  ...
  const totals = useMemo(() => {
    const activeProfiles = Object.values(draftProfiles).filter(
      (profile) => profile.active,
    );
    const successfulRuns = runs.filter((run) => run.status === "SUCCEEDED");
    const pdfBytes = successfulRuns.reduce(
      (sum, run) => sum + Number(run.pdf_size_bytes || 0),
      0,
    );
    return {
  ...
  const updateProfile = (
    reportCode: string,
    patch: Partial<ReportDistributionProfile>,
  ) => {
      ...
      <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(96,165,250,0.16),_transparent_24%),linear-gradient(180deg,#f8fbff_0%,#f7f5ef_100%)] p-6">
        <Card className="mx-auto max-w-3xl rounded-[2rem] border-surface-1/10 bg-surface-1/95 shadow-[0_30px_80px_-40px_rgba(15,23,42,0.6)]">
          <CardHeader>
            ...
            <CardDescription>
              Daily archives and generation logs are available only to report
              admins.
            </CardDescription>
          </CardHeader>
        </Card>
      ...
      <div className="mx-auto max-w-[1500px] space-y-6">
        <section className="rounded-[2.2rem] border border-line bg-[linear-gradient(135deg,#ffffff_0%,#eef5ff_48%,#ecfeff_100%)] p-6 shadow-[0_35px_90px_-52px_rgba(15,23,42,0.22)]">
          <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
            <div className="max-w-3xl">
              ...
              <p className="mt-3 max-w-2xl text-sm leading-6 text-content-3">
                Daily report packs are generated here, archived for audit, and
                announced to owner/admin in the in-app notification tray. Email
                delivery and per-pack scheduling are intentionally removed from
                the workflow.
              </p>
            </div>
            <div className="flex flex-wrap gap-3">
              <Button
                variant="outline"
                className="rounded-full border-line bg-surface-1/90 text-content-2 hover:bg-surface-2"
                onClick={() => {
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <MetricTile
              label="Active daily packs"
              value={String(totals.activePacks)}
              sublabel="Included in the next daily archive cycle"
            />
            <MetricTile
              label="Successful archives"
              value={String(totals.successfulRuns)}
              sublabel="Runs stored and ready for review"
            />
            <MetricTile
              label="Archive shelf"
              value={String(totals.archivedRuns)}
              sublabel="Recent generated runs retained for audit"
            />
            <MetricTile
              label="Stored PDF size"
              value={`${totals.archivedSizeMb.toFixed(1)} MB`}
              sublabel="Archive footprint across the recent window"
            />
          </div>
        </section>
        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1.05fr)_430px]">
          <section className="rounded-[2rem] border border-line bg-surface-1/95 p-5 shadow-[0_24px_70px_-42px_rgba(15,23,42,0.45)]">
            ...
            <div className="mt-5 grid max-h-[calc(100vh-20rem)] gap-4 overflow-y-auto pr-2">
              {(profiles.length ? profiles : Object.values(draftProfiles)).map(
                (profile) => {
                  ...
                  return (
                    <div
                      ...
                      className="rounded-[1.7rem] border border-line bg-surface-2 p-4"
                    >
                      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <div className="text-sm text-content-3">
                                {profile.report_code.replaceAll("_", " ")}
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="mt-4 grid gap-3 md:grid-cols-3">
                            <InfoPill
                              ...
                              value={(
                                draft.target_roles || ["OWNER", "ADMIN"]
                              ).join(" · ")}
                            />
                            <InfoPill
                              ...
                              value={
                                latestRun
                                  ? formatTimestamp(
                                      latestRun.sent_at || latestRun.created_at,
                                    )
                                  : "—"
                              }
                            />
                            <InfoPill
                              title="Latest status"
                              value={latestRun?.status || "No run yet"}
                            />
                          </div>
                        </div>
                        ...
                        <div className="w-full shrink-0 rounded-[1.5rem] border border-line bg-surface-1 p-4 xl:w-[280px]">
                          ...
                          <div className="mt-4 rounded-[1rem] border border-line bg-surface-2 p-3 text-sm text-content-3">
                            Generating a pack writes the PDF/detail files to
                            archive and pushes a live inbox notification instead
                            of sending email.
                          </div>
                          <Button
          ...
          <div className="space-y-6">
            <section className="rounded-[2rem] border border-line bg-surface-1/95 p-5 shadow-[0_24px_70px_-42px_rgba(15,23,42,0.45)]">
              ...
              <div className="mt-5 h-[calc(100vh-20rem)] min-h-[520px] space-y-3 overflow-y-auto pr-4">
                {runs.map((run) => (
                  <div
                    key={run.id}
                    className="rounded-[1.4rem] border border-line bg-surface-2 p-4"
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <Badge
                        className={`rounded-full border ${statusTone(run.status)}`}
                      >
                        {run.status}
                      </Badge>
                    </div>
                    <div className="mt-3 grid gap-2 text-xs text-content-3">
                      <div className="flex items-center gap-2">
                        ...
                        Audience:{" "}
                        {(run.recipients || []).join(" · ") || "OWNER · ADMIN"}
                      </div>
                      <div className="flex items-center gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {run.pdf_file_name || "PDF ready"}
                        {run.pdf_size_bytes
                          ? ` • ${(Number(run.pdf_size_bytes) / 1024).toFixed(0)} KB`
                          : ""}
                      </div>
                      {run.warning_text ? (
                        <div className="text-warning-fg">
                          Warning: {run.warning_text}
                        </div>
                      ) : null}
                      {run.error_text ? (
                ...
                {!runs.length ? (
                  <div className="rounded-[1.6rem] border border-dashed border-line bg-surface-2 px-4 py-12 text-center text-sm text-content-3">
            ...
            <Card className="rounded-[2rem] border-line bg-surface-1/95 shadow-[0_24px_70px_-42px_rgba(15,23,42,0.45)]">
              <CardHeader>
                ...
                <CardDescription>
                  Analytics stays in the workspace; this center handles archive
                  generation and audit retrieval.
                </CardDescription>
              </CardHeader>
