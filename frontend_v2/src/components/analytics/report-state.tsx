    ...
    <Card className={`rounded-[1.35rem] ${toneClasses}`}>
      <CardContent className="flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-start gap-3">
          <div className="rounded-full border border-current/15 bg-surface-1/70 p-2">
        ...
        {onAction && actionLabel ? (
          <Button
            variant="outline"
            onClick={onAction}
            className="rounded-full border-current/20 bg-surface-1/80"
          >
...
export function ReportEmptyCard({
  title,
  message,
}: {
  title: string;
