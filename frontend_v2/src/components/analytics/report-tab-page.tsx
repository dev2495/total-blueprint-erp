...
const ACCENT_STYLES: Record<
  NonNullable<ReportTabPageProps["accent"]>,
  { panel: string; chip: string; badge: string }
> = {
  indigo: {
    panel: "from-surface-3 via-primary to-primary",
    chip: "border-info-border bg-info-bg text-primary",
    badge: "bg-primary",
  },
  emerald: {
    panel: "from-surface-3 via-success-fg to-success-fg",
    chip: "border-success-border bg-success-bg text-success-fg",
    badge: "bg-success-fg",
  },
  amber: {
    panel: "from-surface-3 via-warning-fg to-warm",
    chip: "border-warning-border bg-warning-bg text-warning-fg",
    badge: "bg-warning-fg",
  },
  cyan: {
    panel: "from-surface-3 via-info-fg to-info-fg",
    chip: "border-info-border bg-info-bg text-info-fg",
    badge: "bg-info-fg",
  },
  rose: {
    panel: "from-surface-3 via-danger-solid to-info-fg",
...
function normalizeChartRows(rows: unknown) {
  if (!Array.isArray(rows))
    return { rows: [], primaryKey: "", secondaryKey: "" };
...
function normalizeBreakdownGroups(payload: ReportTabResponse) {
  ...
  const fallbackBreakdowns =
    !payload.breakdowns &&
    Array.isArray(payload.charts?.distribution) &&
    payload.charts.distribution.length
...
function formatMetricValue(key: string, value: unknown) {
  ...
  if (normalized.includes("kg")) {
    ...
    return Number.isFinite(numeric)
      ? `${numeric.toLocaleString("en-IN", { maximumFractionDigits: 2 })} kg`
      : String(value);
  }
...
function sortRowsByKey(rows: Array<Record<string, any>>, key: string) {
  return [...rows].sort(
    (left, right) => toNumber(right[key]) - toNumber(left[key]),
  );
}
...
export function ReportTabPage({
  ...
  const prefersWideDefaultWindow = [
    "scrap",
    "mrp",
    "material-variance",
    "ink-intelligence",
  ...
  const payload =
    ...
    ({
      ...
      series: [],
      warnings: [],
    } satisfies Partial<ReportTabResponse>);
  const rawPayload = payload as ReportTabResponse & Record<string, any>;
  const reportPending =
    !reportQuery.data && (reportQuery.isPending || reportQuery.isFetching);
  const normalizedSummary = (payload.summary || payload.kpis || {}) as Record<
    string,
    any
  >;
  const summaryEntries = Object.entries(normalizedSummary)
    .filter(
  ...
  const visibleWarnings = useMemo(() => {
    ...
    return reportHasVisibleEvidence
      ? warnings.filter((warning) => !isGenericCoverageWarning(warning))
      : warnings;
    ...
    return [
      ...
      },
      {
        ...
        value: formatMetricValue(
          "global_availability",
          normalizedSummary.global_availability,
        ),
        hint: "Machine ready-time ratio",
      },
      {
        label: "Performance",
        value: formatMetricValue(
          "global_performance",
          normalizedSummary.global_performance,
        ),
        hint: "Actual versus rated throughput",
      },
      {
        label: "Quality",
        value: formatMetricValue(
          "global_quality",
          normalizedSummary.global_quality,
        ),
        hint: "Good output as a share of processed mass",
      },
      {
        label: "Top machine",
        value: formatMetricValue("top_machine", normalizedSummary.top_machine),
        hint: "Highest visible OEE in current window",
      },
      {
        label: "Output",
        value: formatMetricValue("output_kg", normalizedSummary.output_kg),
        hint: "Produced quantity in current lens",
      },
    ];
  ...
  const mrpFlowRows = useMemo(() => {
    ...
    return [
      {
        label: "Theoretical",
        value: toNumber(normalizedSummary.theoretical_kg),
        color: "#334155",
      },
      {
        label: "Required",
        value: toNumber(normalizedSummary.required_kg),
        color: "#f59e0b",
      },
      {
        label: "Planned issue",
        value: toNumber(normalizedSummary.planned_issue_kg),
        color: "#2563eb",
      },
      {
        label: "Actual issued",
        value: toNumber(normalizedSummary.actual_issued_kg),
        color: "#0ea5e9",
      },
      {
        label: "Consumed",
        value: toNumber(normalizedSummary.consumed_kg),
        color: "#10b981",
      },
      {
        label: "Returned",
        value: toNumber(normalizedSummary.returned_kg),
        color: "#94a3b8",
      },
      {
        label: "Scrap",
        value: toNumber(normalizedSummary.scrap_kg),
        color: "#ef4444",
      },
    ];
  ...
  const interplantRows = useMemo(
    () =>
      isInterplantTab
        ? tableRows.filter((row) => row.dc_no || row.from_plant || row.to_plant)
        : [],
  ...
  const interplantStatusRows = useMemo(() => {
    ...
    return [
      {
        label: "Draft",
        value: toNumber(normalizedSummary.draft),
        color: "#64748b",
      },
      {
        label: "In transit",
        value: toNumber(normalizedSummary.in_transit),
        color: "#f59e0b",
      },
      {
        label: "Received",
        value: toNumber(normalizedSummary.received),
        color: "#10b981",
      },
    ];
  ...
  const interplantRouteRows = useMemo(() => {
    ...
    const grouped = new Map<
      string,
      { route: string; challans: number; weight_kg: number }
    >();
  ...
  const interplantKpis = useMemo(() => {
    ...
    return [
      {
        label: "Challans",
        value: formatMetricValue("count", normalizedSummary.total_challans),
        hint: "Visible transfer documents",
        tone: "border-line bg-surface-2 text-content-2",
      },
      {
        label: "Dispatched",
        value: formatMetricValue(
          "dispatched_total_kg",
          normalizedSummary.dispatched_total_kg,
        ),
        hint: "Total kg sent between plants",
        tone: "border-info-border bg-info-bg text-primary",
      },
      {
        label: "Received",
        value: formatMetricValue(
          "received_total_kg",
          normalizedSummary.received_total_kg,
        ),
        hint: "Total kg booked in at destination",
        tone: "border-success-border bg-success-bg text-success-fg",
      },
      {
        label: "Output in transit",
        value: formatMetricValue(
          "output_in_transit_kg",
          normalizedSummary.output_in_transit_kg,
        ),
        hint: "Saleable mass still moving",
        tone: "border-warning-border bg-warning-bg text-warning-fg",
      },
      {
        label: "Remainder in transit",
        value: formatMetricValue(
          "remainder_in_transit_kg",
          normalizedSummary.remainder_in_transit_kg,
        ),
        hint: "Remainder rolls still moving",
        tone: "border-info-border bg-info-bg text-info-fg",
      },
      {
        label: "Execution coverage",
        value: `${formatMaybeNumber(payload.coverage?.execution_log_coverage, 0)}%`,
        hint: "Transfer telemetry linked to execution",
        tone: "border-line bg-surface-2 text-content-2",
      },
    ];
  ...
  const inventoryKpis = useMemo(() => {
    ...
    return [
      {
        ...
        value: formatMetricValue(
          "total_weight_kg",
          normalizedSummary.total_weight_kg,
        ),
        hint: `${formatMetricValue("total_items", normalizedSummary.total_items)} physical rolls`,
        tone: "border-info-border bg-info-bg text-info-fg",
      },
      {
        label: "Families",
        value: formatMetricValue("count", inventoryBreakdowns.byFamily.length),
        hint: "Business-facing stock groups",
        tone: "border-info-border bg-info-bg text-primary",
      },
      {
        label: "Variants",
        value: formatMetricValue("count", inventoryBreakdowns.byVariant.length),
        hint: "Material variants currently visible",
        tone: "border-line bg-surface-2 text-content-2",
      },
      {
        label: "Aged 90d+",
        value: formatMetricValue(
          "aged_stock_weight_kg",
          normalizedSummary.aged_stock_weight_kg,
        ),
        hint: `${formatMetricValue("count", normalizedSummary.aged_stock_items)} aged rolls`,
        tone: "border-warning-border bg-warning-bg text-warning-fg",
      },
      {
        label: "Bulk stock",
        value: formatMetricValue(
          "bulk_stock_kg",
          normalizedSummary.bulk_stock_kg,
        ),
        hint: `${formatMetricValue("count", normalizedSummary.bulk_items)} bulk rows`,
        tone: "border-success-border bg-success-bg text-success-fg",
      },
      {
        label: "Stock value",
    ...
    inventoryBreakdowns.byVariant.length,
    normalizedSummary,
  ]);
  ...
  const productionBreakdowns = useMemo(() => {
    ...
    return {
      ...
      byWorkCenter: sortRowsByKey(
        asRecordRows(raw.by_work_center),
        "output_kg",
      ),
    ...
    return [
      {
        ...
        value: formatMetricValue(
          "total_output_kg",
          normalizedSummary.total_output_kg,
        ),
        hint: "Logged good output in the active window",
        tone: "border-info-border bg-info-bg text-info-fg",
      },
      {
        label: "Yield",
      ...
      },
      {
        ...
        value: formatMetricValue(
          "completion_rate",
          normalizedSummary.completion_rate,
        ),
        hint: `${formatMetricValue("completed_jobs", normalizedSummary.completed_jobs)} finished jobs`,
        tone: "border-info-border bg-info-bg text-primary",
      },
      {
        ...
        value: formatMetricValue(
          "active_machines",
          normalizedSummary.active_machines,
        ),
        hint: `${formatMetricValue("active_work_centers", normalizedSummary.active_work_centers)} work centers engaged`,
        tone: "border-line bg-surface-2 text-content-2",
      },
      {
        label: "Jobs with scrap",
        value: formatMetricValue(
          "jobs_with_scrap",
          normalizedSummary.jobs_with_scrap,
        ),
        hint: `${formatMetricValue("total_jobs", normalizedSummary.total_jobs)} total jobs in the lens`,
        tone: "border-warning-border bg-warning-bg text-warning-fg",
      },
      {
        label: "Output trend",
        value: formatMetricValue(
          "output_trend_pct",
          normalizedSummary.output_trend_pct,
        ),
        hint: `${formatMetricValue("target_daily_output", normalizedSummary.target_daily_output)} daily target`,
        tone: "border-info-border bg-info-bg text-primary",
      },
    ];
  ...
  const salesAnalytics = useMemo(() => {
    ...
    return {
      pipeline: sortRowsByKey(
        asRecordRows(payload.breakdowns?.pipeline || rawPayload.pipeline),
        "count",
      ),
      topCustomers: sortRowsByKey(
        asRecordRows(
          payload.breakdowns?.top_customers || rawPayload.top_customers,
        ),
        "weight_kg",
      ),
      skuBreakdown: sortRowsByKey(
        asRecordRows(rawPayload.sku_breakdown),
        "weight_kg",
      ),
      ...
      quoteConversion: rawPayload.quote_conversion || {
        total_quotes: 0,
        converted_quotes: 0,
        conversion_pct: 0,
      },
    ...
    return [
      {
        ...
        value: formatMetricValue(
          "backlog_count",
          normalizedSummary.backlog_count,
        ),
        hint: "Open orders still in the system",
        tone: "border-line bg-surface-2 text-content-2",
      },
      {
        label: "Overdue",
        value: formatMetricValue(
          "overdue_count",
          normalizedSummary.overdue_count,
        ),
        hint: "Delivery dates already missed",
        tone: "border-danger-border bg-danger-bg text-danger-fg",
      },
      {
        label: "OTIF",
        value: formatMetricValue("otif_rate", normalizedSummary.otif_rate),
        hint: "Completed orders on time and in full",
        tone: "border-success-border bg-success-bg text-success-fg",
      },
      {
        label: "Revenue",
        value: formatMetricValue(
          "total_revenue",
          normalizedSummary.total_revenue,
        ),
        hint: `${formatMetricValue("total_weight_ordered_kg", normalizedSummary.total_weight_ordered_kg)} ordered`,
        tone: "border-warning-border bg-warning-bg text-warning-fg",
      },
      {
        label: "Repeat share",
        value: formatMetricValue(
          "repeat_share_pct",
          normalizedSummary.repeat_share_pct,
        ),
        hint: "Repeat demand across order items",
        tone: "border-info-border bg-info-bg text-primary",
      },
      {
        ...
        value: formatMetricValue(
          ...
          normalizedSummary.quote_conversion_pct,
        ),
        hint: `${formatMetricValue("count", salesAnalytics.quoteConversion.converted_quotes)} converted quotes`,
        tone: "border-info-border bg-info-bg text-info-fg",
      },
    ];
  ...
  const dispatchAnalytics = useMemo(() => {
    ...
    return {
      ...
      byCustomer: sortRowsByKey(
        asRecordRows(payload.breakdowns?.by_customer || rawPayload.by_customer),
        "weight_kg",
      ),
    ...
    return [
      {
        ...
        value: formatMetricValue(
          "total_challans",
          normalizedSummary.total_challans,
        ),
        hint: "Visible dispatch documents",
        tone: "border-line bg-surface-2 text-content-2",
      },
      {
        label: "Dispatched",
        value: formatMetricValue("dispatched", normalizedSummary.dispatched),
        hint: "In transit, delivered, or dispatched",
        tone: "border-success-border bg-success-bg text-success-fg",
      },
      {
        label: "Pending",
        value: formatMetricValue("pending", normalizedSummary.pending),
        hint: "Draft challans not yet sent",
        tone: "border-warning-border bg-warning-bg text-warning-fg",
      },
      {
        label: "Gross load",
        ...
        hint: `${formatMetricValue("count", normalizedSummary.total_pcs)} pieces on challans`,
        tone: "border-info-border bg-info-bg text-info-fg",
      },
      {
        label: "Customers",
        value: formatMetricValue("count", dispatchAnalytics.byCustomer.length),
        hint: "Accounts visible in this dispatch window",
        tone: "border-info-border bg-info-bg text-primary",
      },
      {
        label: "Execution coverage",
        value: `${formatMaybeNumber(payload.coverage?.execution_log_coverage, 0)}%`,
        hint: "Dispatch window tied to execution telemetry",
        tone: "border-info-border bg-info-bg text-primary",
      },
    ];
  }, [
  ...
  const downtimeAnalytics = useMemo(() => {
    ...
    return {
      ...
      topMachines: sortRowsByKey(
        asRecordRows(rawPayload.top_machines),
        "minutes",
      ),
      dailyTrend: asRecordRows(rawPayload.daily_trend || payload.series),
    };
    ...
    return [
      {
        ...
        value: formatMetricValue(
          ...
          normalizedSummary.total_downtime_hours,
        ),
        hint: `${formatMetricValue("total_downtime_minutes", normalizedSummary.total_downtime_minutes)} total minutes`,
        tone: "border-danger-border bg-danger-bg text-danger-fg",
      },
      {
        label: "Events",
        value: formatMetricValue(
          "total_events",
          normalizedSummary.total_events,
        ),
        hint: "Recorded machine stoppages",
        tone: "border-line bg-surface-2 text-content-2",
      },
      {
        label: "MTTR",
        value: `${formatMaybeNumber(normalizedSummary.mttr_minutes, 1)} min`,
      ...
      },
      {
        ...
        value: formatMetricValue(
          "avg_events_per_day",
          normalizedSummary.avg_events_per_day,
        ),
        hint: "Average stop count per active day",
        tone: "border-info-border bg-info-bg text-primary",
      },
    ];
  ...
  const operatorAnalytics = useMemo(() => {
    ...
    return {
      leaderboard: sortRowsByKey(
        asRecordRows(rawPayload.leaderboard || payload.rows),
        "produced_kg",
      ),
    ...
    return [
      {
        ...
        value: formatMetricValue(
          "active_operators",
          normalizedSummary.active_operators,
        ),
        hint: "Users with work in the lens",
        tone: "border-line bg-surface-2 text-content-2",
      },
      {
        label: "Output",
        ...
        hint: "Production attributed to operators",
        tone: "border-info-border bg-info-bg text-info-fg",
      },
      {
        ...
        value: formatMetricValue(
          "avg_efficiency",
          normalizedSummary.avg_efficiency,
        ),
        hint: "Good output share of processed mass",
        tone: "border-success-border bg-success-bg text-success-fg",
      },
      {
        label: "Best",
        value: formatMetricValue(
          "best_operator",
          normalizedSummary.best_operator,
        ),
        hint: "Top operator in the visible leaderboard",
        tone: "border-info-border bg-info-bg text-primary",
      },
      {
        label: "Worst",
        value: formatMetricValue(
          "worst_operator",
          normalizedSummary.worst_operator,
        ),
        hint: "Lowest efficiency in the same lens",
        tone: "border-warning-border bg-warning-bg text-warning-fg",
      },
    ];
  ...
  const costingAnalytics = useMemo(() => {
    ...
    return {
      ...
      customerMargin: sortRowsByKey(
        asRecordRows(rawPayload.customer_margin),
        "revenue",
      ),
    ...
    return [
      ...
      },
      {
        ...
        value: formatMetricValue(
          ...
          normalizedSummary.total_production_cost,
        ),
        hint: "Material + conversion + absorbed overhead",
        tone: "border-danger-border bg-danger-bg text-danger-fg",
      },
      {
        label: "Contribution",
        value: formatMetricValue(
          "total_contribution",
          normalizedSummary.total_contribution,
        ),
        hint: "Contribution margin across costed orders",
        tone: "border-info-border bg-info-bg text-info-fg",
      },
      {
        label: "Margin",
        value: formatMetricValue(
          "total_margin",
          normalizedSummary.total_margin,
        ),
        hint: `${formatMetricValue("avg_margin_pct", normalizedSummary.avg_margin_pct)} average margin`,
        tone: "border-info-border bg-info-bg text-primary",
      },
      {
        label: "Cost / kg",
        value: formatMetricValue(
          "avg_cost_per_kg",
          normalizedSummary.avg_cost_per_kg,
        ),
        hint: `${formatMetricValue("total_jobs_costed", normalizedSummary.total_jobs_costed)} jobs costed`,
        tone: "border-warning-border bg-warning-bg text-warning-fg",
      },
      {
        label: "Coverage",
        value: formatMetricValue(
          "avg_actual_cost_coverage_pct",
          normalizedSummary.avg_actual_cost_coverage_pct,
        ),
        hint: `${formatMetricValue("avg_order_coverage_pct", normalizedSummary.avg_order_coverage_pct)} order coverage`,
        tone: "border-line bg-surface-2 text-content-2",
      },
    ];
  ...
  const shiftAnalytics = useMemo(() => {
    ...
    return {
      rows: sortRowsByKey(
        asRecordRows(payload.breakdowns?.shift_oee_like || payload.rows),
        "output_kg",
      ),
    ...
    return [
      {
        ...
        hint: "Shift-tagged output in the window",
        tone: "border-info-border bg-info-bg text-info-fg",
      },
      {
        label: "Scrap",
        value: formatMetricValue(
          "total_scrap_kg",
          normalizedSummary.total_scrap_kg,
        ),
        hint: "Scrap attributed to shifts",
        tone: "border-danger-border bg-danger-bg text-danger-fg",
      },
      {
        label: "Downtime",
        value: `${formatMaybeNumber(normalizedSummary.total_downtime_minutes, 0)} min`,
        hint: "Downtime linked to shift tags",
        tone: "border-warning-border bg-warning-bg text-warning-fg",
      },
      {
        label: "Shift count",
        value: formatMetricValue("shift_count", normalizedSummary.shift_count),
        hint: "Visible shift buckets",
        tone: "border-info-border bg-info-bg text-primary",
      },
    ];
  ...
  const inkAnalytics = useMemo(() => {
    ...
    return {
      topVariance: sortRowsByKey(
        asRecordRows(payload.breakdowns?.top_variance || payload.rows),
        "variance_qty",
      ),
      flow: [
        {
          label: "Theoretical",
          value: toNumber(normalizedSummary.ink_theoretical_kg),
        },
        {
          label: "Planned issue",
          value: toNumber(normalizedSummary.ink_planned_issue_kg),
        },
        {
          label: "Actual issued",
          value: toNumber(normalizedSummary.ink_actual_issued_kg),
        },
        {
          label: "Returned",
          value: toNumber(normalizedSummary.ink_returned_kg),
        },
        {
          label: "Consumed",
          value: toNumber(normalizedSummary.ink_consumed_kg),
        },
        {
          label: "Variance",
          value: Math.abs(toNumber(normalizedSummary.ink_variance_kg)),
        },
      ],
    };
    ...
    return [
      {
        ...
        value: formatMetricValue("count", normalizedSummary.color_families),
        hint: "Resolved CMYK and custom ink groups in the window",
        tone: "border-line bg-surface-2 text-content-2",
      },
      {
        label: "Theoretical",
  ...
  const scrapBreakdowns = useMemo(() => {
    const raw = payload.breakdowns || {};
    return {
      byReason: sortRowsByKey(
        Array.isArray(raw.by_reason) ? raw.by_reason : [],
        "scrap_kg",
      ),
      byProcess: sortRowsByKey(
        Array.isArray(raw.by_process) ? raw.by_process : [],
        "scrap_kg",
      ),
      byOperator: sortRowsByKey(
        Array.isArray(raw.by_operator) ? raw.by_operator : [],
        "scrap_kg",
      ),
      byMachine: sortRowsByKey(
        Array.isArray(raw.by_machine) ? raw.by_machine : [],
        "scrap_kg",
      ),
      topJobs: sortRowsByKey(
        Array.isArray(raw.top_jobs) ? raw.top_jobs : [],
        "scrap_kg",
      ),
    };
  }, [payload.breakdowns]);
    ...
    return [
      {
        key: "total_scrap_kg",
        label: "Total scrap",
        value: formatMetricValue("total_scrap_kg", summary.total_scrap_kg),
        tone: "border-danger-border bg-danger-bg text-danger-fg",
        icon: Scissors,
      },
      {
        key: "total_processed_kg",
        label: "Processed",
        value: formatMetricValue(
          "total_processed_kg",
          summary.total_processed_kg,
        ),
        tone: "border-line bg-surface-2 text-content-2",
        icon: Factory,
      },
      ...
      },
      {
        key: "cost_of_scrap",
        label: "Scrap cost",
        value: formatMetricValue("cost_of_scrap", summary.cost_of_scrap),
        tone: "border-danger-border bg-danger-bg text-danger-fg",
        icon: DollarSign,
      },
      {
        key: "avg_scrap_per_event_kg",
        label: "Avg per event",
        value: formatMetricValue(
          "avg_scrap_per_event_kg",
          summary.avg_scrap_per_event_kg,
        ),
        tone: "border-line bg-surface-2 text-content-2",
        icon: Gauge,
      },
      {
        key: "jobs_with_scrap",
        label: "Jobs hit",
        value: formatMetricValue("jobs_with_scrap", summary.jobs_with_scrap),
        tone: "border-line bg-surface-2 text-content-2",
        icon: Package,
      },
      {
  ...
  const scrapMassBalance = useMemo(() => {
    ...
    return [
      {
        label: "Good output",
        value: formatMetricValue("total_good_kg", summary.total_good_kg),
        hint: "Net saleable quantity",
      },
      {
        label: "Processed",
        value: formatMetricValue(
      ...
      },
      {
        ...
        value: formatMetricValue(
          "total_consumed_kg",
          summary.total_consumed_kg,
        ),
        hint: "Material drawdown",
      },
      {
        label: "Avg cost / kg",
        value: formatMetricValue("avg_cost_per_kg", summary.avg_cost_per_kg),
        hint: "Used in scrap costing",
      },
    ];
  }, [isScrapTab, payload.summary]);
    ...
    () =>
      isScrapTab
        ? [
            {
              key: "reason-kg",
              title: "Reason kg split",
              rows: scrapBreakdowns.byReason,
              labelKey: "reason",
            },
            {
              key: "process-kg",
              title: "Process kg split",
              rows: scrapBreakdowns.byProcess,
              labelKey: "process",
            },
            {
              key: "machine-kg",
              title: "Machine kg split",
              rows: scrapBreakdowns.byMachine,
              labelKey: "machine",
            },
          ]
        : [],
  ...
  const reportErrorMessage = reportQuery.isError
    ? describeApiError(reportQuery.error, "Report request failed.")
    : "";
  ...
  return (
    <div className="space-y-6 pb-8 animate-in fade-in duration-500">
      <section
        className={cn(
          "overflow-hidden rounded-[2rem] border border-surface-1/10 bg-gradient-to-br px-6 py-6 text-white shadow-[0_32px_90px_-42px_rgba(15,23,42,0.45)]",
          accentStyle.panel,
        )}
      >
        <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
          <div className="space-y-4">
            <div className="inline-flex items-center gap-2 rounded-full border border-surface-1/15 bg-surface-1/10 px-3 py-1 text-[10px] font-black uppercase tracking-[0.26em] text-white/90">
      ...
      <section className="rounded-[1.75rem] border border-line bg-surface-1/90 p-4 shadow-sm">
        <div className="flex flex-col gap-4">
            ...
            {(["daily", "weekly", "custom"] as FilterPreset[]).map((value) => (
              <button
                ...
                className={cn(
                  ...
                  preset === value
                    ? accentStyle.chip
                    : "border-line bg-surface-2 text-content-3 hover:bg-surface-2",
                )}
              >
                {value}
              </button>
            ))}
            <div className="ml-auto inline-flex items-center gap-2 rounded-full bg-surface-2 px-3 py-1.5 text-xs font-semibold text-content-3 ring-1 ring-line">
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-6">
            <div className="space-y-1.5">
              <Label className="text-[11px] font-bold uppercase tracking-[0.18em] text-content-3">
                Date from
              </Label>
              <Input
                type="date"
        ...
        <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          {scrapKpis.map((metric) => {
            ...
            return (
              <Card
                key={metric.key}
                className={cn("overflow-hidden border shadow-sm", metric.tone)}
              >
      ...
      {isOeeTab ? (
        <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {oeeSpotlightCards.map((metric) => (
            <Card
              key={metric.label}
              className="rounded-[1.55rem] border border-line bg-surface-1 shadow-sm"
            >
              <CardContent className="p-4">
                <div className="text-[11px] font-black uppercase tracking-[0.18em] text-content-3">
                  {metric.label}
                </div>
                <div className="mt-3 text-[1.8rem] font-black tracking-[-0.05em] text-content-1">
                  {metric.value}
                </div>
      ...
      {isScrapTab ? (
        <section className="grid gap-5 xl:grid-cols-[1.05fr_0.95fr] xl:items-start">
          <Card className="rounded-[1.75rem] border-line shadow-sm">
            <CardHeader className="pb-0">
              <CardTitle className="flex items-center gap-2 text-xl font-black text-content-1">
                <Scissors
                  className={cn(
                    "h-5 w-5 rounded-full p-1 text-white",
                    accentStyle.badge,
                  )}
                />
                Scrap Hotspots
            ...
            <CardContent className="grid gap-4 pt-4 md:grid-cols-2">
              ...
              <InsightListCard
                title="By operator"
                rows={scrapBreakdowns.byOperator}
                labelKey="operator"
              />
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-3 pt-4">
              {scrapBreakdowns.topJobs.length ? (
                scrapBreakdowns.topJobs.slice(0, 8).map((row, index) => (
                  <div
                    key={`${row.job_id || row.job_number || index}`}
                    className="rounded-[1.1rem] border border-line bg-surface-2 p-4"
                  >
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div>
                        <div className="text-sm font-black text-content-1">
                          {row.job_number || `Job ${index + 1}`}
                        </div>
                        <div className="text-xs text-content-3">
                          {row.template_name || "Template pending"} ·{" "}
                          {row.process_name || "Process pending"}
                        </div>
                      </div>
                    ...
                    <div className="mt-3 grid gap-2 text-sm text-content-3 sm:grid-cols-3">
                      <div className="rounded-xl bg-surface-1 px-3 py-2">
                        ...
                        <div className="mt-1 font-semibold text-content-1">
                          {formatMetricValue("good_kg", row.good_kg)}
                        </div>
                      </div>
                      <div className="rounded-xl bg-surface-1 px-3 py-2">
                        <div className="text-[10px] font-black uppercase tracking-[0.14em] text-content-4">
                      ...
                      <div className="rounded-xl bg-surface-1 px-3 py-2">
                        ...
                        <div className="mt-1 font-semibold text-content-1">
                          {formatMetricValue("events", row.events)}
                        </div>
                      </div>
                    </div>
                    <div className="mt-2 text-xs text-content-3">
                      {row.machine_name || "Unassigned machine"}
                    </div>
                  </div>
                ))
              ) : (
                <div className="rounded-[1.2rem] border border-dashed border-line bg-surface-2 px-4 py-10 text-center text-sm font-semibold text-content-3">
              ...
              {scrapSignalCards.map((metric) => (
                <div
                  ...
                  className={cn(
                    ...
                    scrapSignalTone(metric.label),
                  )}
                >
                  <div className="text-[11px] font-black uppercase tracking-[0.16em]">
                    {metric.label}
                  </div>
                  <div className="mt-3 text-lg font-black text-content-1">
                    {metric.value}
                  </div>
                  <div className="mt-2 text-xs text-content-3">
                    {metric.hint}
      ...
      {isMrpTab ? (
        <>
          <section className="grid gap-5 xl:grid-cols-[1.2fr_0.8fr] xl:items-start">
            <Card className="rounded-[1.85rem] border-line shadow-sm">
              <CardHeader className="pb-0">
                <CardTitle className="flex items-center gap-2 text-xl font-black text-content-1">
                  <LineChartIcon
                    className={cn(
                  ...
                  {mrpFlowRows.some((row) => row.value > 0) ? (
                    <ResponsiveContainer width="100%" height={360}>
                      ...
                      >
                        ...
                        />
                        <Tooltip
                          formatter={(value) => formatMetricValue("kg", value)}
                        />
            ...
            <Card className="rounded-[1.85rem] border-line shadow-sm">
              ...
              <CardContent className="grid gap-3 pt-4 sm:grid-cols-2 xl:grid-cols-1">
                {mrpKpis.map((metric) => (
                  <div
                    key={metric.label}
                    className={cn("rounded-[1.2rem] border p-4", metric.tone)}
                  >
                    <div className="text-[11px] font-black uppercase tracking-[0.16em]">
                      {metric.label}
                    </div>
                    <div className="mt-3 text-[1.65rem] font-black tracking-[-0.05em] text-content-1">
                      {metric.value}
                    </div>
                    <div className="mt-2 text-xs text-content-3">
                      {metric.hint}
              ...
              <CardContent className="space-y-3 pt-4">
                {mrpBreakdowns.waterfall.length ? (
                  mrpBreakdowns.waterfall.map((row, index) => (
                    <div
                      key={`${row.name || index}`}
                      className="flex items-center justify-between gap-4 rounded-[1.15rem] border border-line bg-surface-2 px-4 py-3"
                    >
                      <div>
                        <div className="text-sm font-black text-content-1">
                          {String(row.name || `Step ${index + 1}`)}
                        </div>
                        <div className="text-xs text-content-3">
                          {toLabel(String(row.type || "detail"))}
                        </div>
                      </div>
                      <Badge
                        className={cn(
        ...
        <>
                  ...
                  {chartSeries.rows.length && chartSeries.primaryKey ? (
                    <ResponsiveContainer width="100%" height={360}>
                      <AreaChart data={chartSeries.rows}>
                        ...
                        <Tooltip
                          formatter={(value, name) => [
                            String(name).includes("challan")
                              ? formatMetricValue("count", value)
                              : formatMetricValue("kg", value),
                            String(name || ""),
                          ]}
                        />
                    ...
                    <div className="flex h-full items-center justify-center rounded-[1.35rem] border border-dashed border-line bg-surface-2 text-sm font-semibold text-content-3">
                      No inter-plant movement trend is available for this filter
                      window.
                    </div>
                  )}
                </div>
              ...
              <CardContent className="grid gap-4 pt-4 md:grid-cols-[0.95fr_1.05fr]">
                <div className="h-[220px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      ...
                      <Tooltip
                        formatter={(value) => formatMetricValue("count", value)}
                      />
                    </PieChart>
                  </ResponsiveContainer>
                </div>
                <div className="space-y-3">
                  {interplantStatusRows.map((row) => (
                    <div
                      key={row.label}
                      className="rounded-[1.1rem] border border-line bg-surface-2 px-4 py-3"
                    >
                      <div className="text-[11px] font-black uppercase tracking-[0.16em] text-content-3">
                        {row.label}
                      </div>
                      <div className="mt-2 text-xl font-black text-content-1">
                        {formatMetricValue("count", row.value)}
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <section className="grid gap-5 xl:grid-cols-[0.95fr_1.05fr] xl:items-start">
              ...
              <CardContent className="pt-4">
                <div className="h-[320px] min-w-0">
                  {interplantRouteRows.length ? (
                    ...
                    <div className="flex h-full items-center justify-center rounded-[1.35rem] border border-dashed border-line bg-surface-2 text-sm font-semibold text-content-3">
                      No route concentration is available for this filter
                      window.
                    </div>
                  )}
                </div>
            ...
            <Card className="overflow-hidden rounded-[1.85rem] border-line shadow-sm">
              ...
              <CardContent className="pt-4">
                <div className="max-h-[520px] overflow-auto rounded-[1.2rem] border border-line">
                  <table className="min-w-full text-sm">
                    <thead className="sticky top-0 bg-surface-2">
                      <tr>
                        {[
                          ...
                          <th
                            key={column}
                            className="border-b border-line px-4 py-3 text-left text-[11px] font-black uppercase tracking-[0.16em] text-content-3"
                          >
                            {toLabel(column)}
                          </th>
                        ))}
                    ...
                    <tbody>
                      {interplantRows.slice(0, 25).map((row, index) => (
                        ...
                        >
                          <td className="px-4 py-3 font-semibold text-content-1">
                            {String(row.dc_no || "—")}
                          </td>
                          ...
                          <td className="px-4 py-3 text-content-2">
                            {String(row.from_plant || "—")}
                          </td>
                          <td className="px-4 py-3 text-content-2">
                            {String(row.to_plant || "—")}
                          </td>
                          <td className="px-4 py-3 text-content-2">
                            {formatMetricValue("kg", row.dispatched_kg)}
                          </td>
                          <td className="px-4 py-3 text-content-2">
                            {formatMetricValue("kg", row.received_kg)}
                          </td>
                        </tr>
                      ))}
                    </tbody>
        ...
        <>
                ...
                <div className="h-[380px] min-w-0">
                  {inventoryBreakdowns.byFamily.length ? (
                    <ResponsiveContainer width="100%" height={380}>
                      <BarChart
                        data={inventoryBreakdowns.byFamily.slice(0, 10)}
                        layout="vertical"
                    ...
                    <div className="flex h-full items-center justify-center rounded-[1.35rem] border border-dashed border-line bg-surface-2 text-sm font-semibold text-content-3">
                      Loading inventory family stock...
                    </div>
                  ) : (
                    <div className="flex h-full items-center justify-center rounded-[1.35rem] border border-dashed border-line bg-surface-2 text-sm font-semibold text-content-3">
                      No family stock is visible for this filter window.
                    </div>
                  )}
                </div>
              </CardContent>
              ...
              <CardContent className="grid gap-4 pt-4 md:grid-cols-[0.95fr_1.05fr] xl:grid-cols-1">
                <div className="h-[230px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        data={inventoryBreakdowns.byStage}
                        dataKey="weight_kg"
                        nameKey="stage"
                        innerRadius={58}
                        outerRadius={92}
                ...
                <div className="space-y-3">
                  {inventoryBreakdowns.byStage.slice(0, 5).map((row) => (
                    <div
                      ...
                      className="flex items-center justify-between rounded-[1.1rem] border border-line bg-surface-2 px-4 py-3"
                    >
                      <div>
                        <div className="text-sm font-black text-content-1">
                        ...
                        <div className="text-xs text-content-3">
                          {formatMetricValue("count", row.count)} rolls
                        </div>
                      </div>
                      <Badge className="rounded-full border border-info-border bg-info-bg text-info-fg">
                        {formatMetricValue("kg", row.weight_kg)}
                      </Badge>
                    </div>
                  ))}
                </div>
          ...
          <section className="grid gap-5 xl:grid-cols-[0.95fr_1.05fr] xl:items-start">
              ...
              <CardContent className="pt-4">
                <div className="h-[280px] min-w-0">
                  <ResponsiveContainer width="100%" height={280}>
                    <BarChart
                      data={inventoryBreakdowns.aging}
            ...
            <Card className="overflow-hidden rounded-[1.85rem] border-line shadow-sm">
              ...
              <CardContent className="pt-4">
                <div className="max-h-[360px] overflow-auto rounded-[1.2rem] border border-line">
                  <table className="min-w-full text-sm">
                    <thead className="sticky top-0 bg-surface-2">
                      <tr>
                        {["variant", "weight_kg", "count"].map((column) => (
                          <th
                            key={`inventory-variant-${column}`}
                            className="border-b border-line px-4 py-3 text-left text-[11px] font-black uppercase tracking-[0.16em] text-content-3"
                          >
                            {toLabel(column)}
                          </th>
                      ...
                      {inventoryBreakdowns.byVariant
                        ...
                        .map((row, index) => (
                          ...
                          >
                            <td className="px-4 py-3 font-semibold text-content-1">
                              {String(
                                row.variant || row.material_name || "Unknown",
                              )}
                            </td>
                            <td className="px-4 py-3 text-content-2">
                              {formatMetricValue("kg", row.weight_kg)}
                            ...
                            <td className="px-4 py-3 text-content-2">
                              {formatMetricValue("count", row.count)}
                            </td>
                          </tr>
                        ))}
                    </tbody>
        ...
        <>
          ...
          <section className="grid gap-5 xl:grid-cols-[1.1fr_0.9fr] xl:items-start">
                  ...
                  {chartSeries.rows.length ? (
                    <ResponsiveContainer width="100%" height={360}>
                      <AreaChart data={chartSeries.rows}>
                        ...
                        <Tooltip
                          formatter={(value, name) => [
                            String(name).includes("yield")
                              ? formatMetricValue("yield_pct", value)
                              : formatMetricValue("kg", value),
                            toLabel(String(name || "")),
                          ]}
                        />
            ...
            <Card className="rounded-[1.85rem] border-line shadow-sm">
              <CardHeader className="pb-0">
                <CardTitle className="text-xl font-black text-content-1">
                  Process mix and queue posture
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-3 pt-4">
                {productionBreakdowns.byProcess
                  .slice(0, 5)
                  .map((row, index) => (
                    <div
                      key={`${row.name || index}`}
                      className="rounded-[1.15rem] border border-line bg-surface-2 px-4 py-3"
                    >
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          ...
                          <div className="text-xs text-content-3">
                            Primary throughput contributor
                          </div>
                        </div>
                        <Badge className="rounded-full border border-info-border bg-info-bg text-info-fg">
                          {formatMetricValue("kg", row.value)}
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {productionBreakdowns.byState
                    .slice(0, 4)
                    .map((row, index) => (
                      <div
                        key={`${row.state || index}`}
                        className="rounded-[1rem] border border-line bg-surface-1 px-4 py-3 shadow-sm"
                      >
                        <div className="text-[11px] font-black uppercase tracking-[0.16em] text-content-3">
                          {toLabel(String(row.state || "unknown"))}
                        </div>
                        <div className="mt-2 text-lg font-black text-content-1">
                          {formatMetricValue("count", row.count)}
                        </div>
              ...
              <CardContent className="pt-4">
                <div className="h-[320px] min-w-0">
                  {productionBreakdowns.byWorkCenter.length ? (
                    <ResponsiveContainer width="100%" height={320}>
                      <BarChart
                        data={productionBreakdowns.byWorkCenter.slice(0, 8)}
                        layout="vertical"
                      ...
                      >
                        ...
                        <YAxis
                          ...
                          dataKey="work_center"
                          width={112}
                          tickFormatter={(value) => compactAxisLabel(value, 16)}
                          tick={{ fontSize: 10 }}
                          stroke="#94a3b8"
                          tickLine={false}
                          axisLine={false}
                    ...
                    <div className="flex h-full items-center justify-center rounded-[1.35rem] border border-dashed border-line bg-surface-2 text-sm font-semibold text-content-3">
                      No work-center load data is available for this filter
                      window.
                    </div>
                  )}
                </div>
              ...
              <CardContent className="pt-4">
                <div className="max-h-[360px] overflow-auto rounded-[1.2rem] border border-line">
                  <table className="min-w-full text-sm">
                    ...
                    <tbody>
                      {productionBreakdowns.rows
                        .slice(0, 18)
                        .map((row, index) => (
                          <tr
                            key={`${row.job_number || index}`}
                            className="border-b border-line last:border-b-0 hover:bg-surface-2"
                          >
                            <td className="px-4 py-3 font-semibold text-content-1">
                            ...
                            <td className="px-4 py-3 text-content-2">
                              {String(row.work_center || "—")}
                            </td>
                            <td className="px-4 py-3 text-content-2">
                              {String(row.status || row.job_state || "—")}
                            </td>
                            <td className="px-4 py-3 text-content-2">
                              {formatMetricValue("kg", row.produced_qty_kg)}
                            </td>
                            <td className="px-4 py-3 text-content-2">
                              {formatMetricValue("kg", row.remaining_qty_kg)}
                            </td>
                            <td className="px-4 py-3 text-content-2">
                              {formatMetricValue("yield_pct", row.yield_pct)}
                            </td>
        ...
        <>
          ...
          <section className="grid gap-5 xl:grid-cols-[1.08fr_0.92fr] xl:items-start">
                ...
                <div className="h-[340px] min-w-0">
                  {salesAnalytics.trend.length ? (
                    <ResponsiveContainer width="100%" height={340}>
                      <AreaChart
                        data={salesAnalytics.trend.map((row, index) => ({
                          ...row,
                          label: labelFromRow(row, index),
                        }))}
                      >
                    ...
                    <div className="flex h-full items-center justify-center rounded-[1.35rem] border border-dashed border-line bg-surface-2 text-sm font-semibold text-content-3">
                      No sales creation trend is available for this filter
                      window.
                    </div>
                  )}
                </div>
            ...
            <Card className="rounded-[1.85rem] border-line shadow-sm">
              ...
              <CardContent className="space-y-3 pt-4">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {Object.entries(salesAnalytics.repeatMix).map(
                    ([key, value]) => (
                      <div
                        key={key}
                        className="rounded-[1rem] border border-line bg-surface-1 px-4 py-3 shadow-sm"
          ...
          <section className="grid gap-5 xl:grid-cols-[0.96fr_1.04fr] xl:items-start">
            <Card className="rounded-[1.85rem] border-line shadow-sm">
              <CardHeader className="pb-0">
                <CardTitle className="text-xl font-black text-content-1">
                  Top customers by ordered weight
                </CardTitle>
              </CardHeader>
              <CardContent className="pt-4">
                <div className="h-[320px] min-w-0">
                  {salesAnalytics.topCustomers.length ? (
                    <ResponsiveContainer width="100%" height={320}>
                      <BarChart
                        data={salesAnalytics.topCustomers.slice(0, 8)}
                        layout="vertical"
                        margin={{ top: 8, right: 8, left: 20, bottom: 0 }}
                      >
                        <CartesianGrid
                    ...
                    <div className="flex h-full items-center justify-center rounded-[1.35rem] border border-dashed border-line bg-surface-2 text-sm font-semibold text-content-3">
                      No customer concentration is available for this filter
                      window.
                    </div>
                  )}
                </div>
            ...
            <Card className="overflow-hidden rounded-[1.85rem] border-line shadow-sm">
              <CardHeader className="pb-0">
                <CardTitle className="text-xl font-black text-content-1">
                  SKU pressure and overdue desk
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-4 pt-4">
                <div className="max-h-[260px] overflow-auto rounded-[1.2rem] border border-line">
                  <table className="min-w-full text-sm">
                    <thead className="sticky top-0 bg-surface-2">
                      <tr>
                        {[
                      ...
                      {salesAnalytics.skuBreakdown
                        ...
                        .map((row, index) => (
                          ...
                          >
                            ...
                            <td className="px-4 py-3 text-content-2">
                              {formatMetricValue("count", row.orders)}
                            </td>
                            <td className="px-4 py-3 text-content-2">
                              {formatMetricValue("count", row.repeat_orders)}
                            </td>
                            <td className="px-4 py-3 text-content-2">
                              {formatMetricValue("value", row.value)}
                            </td>
                          </tr>
                        ))}
                    </tbody>
                ...
                <div className="rounded-[1.2rem] border border-line bg-surface-2 p-4">
                  ...
                  <div className="mt-3 space-y-2">
                    {salesAnalytics.overdue.length ? (
                      salesAnalytics.overdue.slice(0, 5).map((row, index) => (
                        <div
                          key={`${row.order_number || index}`}
                          className="flex items-center justify-between gap-3 rounded-[1rem] bg-surface-1 px-3 py-2 shadow-sm"
                        >
                          <div>
                            <div className="text-sm font-black text-content-1">
                              {String(row.order_number || "Order")}
                            </div>
                            <div className="text-xs text-content-3">
                          ...
                          <Badge className="rounded-full border border-danger-border bg-danger-bg text-danger-fg">
                            {formatMetricValue("count", row.days_overdue)} d
                          </Badge>
                        </div>
                      ))
                    ) : (
                      <div className="text-sm font-semibold text-content-3">
                        No overdue orders in the selected window.
                      </div>
                    )}
                  </div>
                </div>
              ...
              <CardContent className="pt-4">
                <div className="h-[340px] min-w-0">
                  {dispatchAnalytics.dailyTrend.length ? (
                    <ResponsiveContainer width="100%" height={340}>
                      <AreaChart
                        data={dispatchAnalytics.dailyTrend.map(
                          (row, index) => ({
                            ...row,
                            label: labelFromRow(row, index),
                          }),
              ...
              <CardContent className="pt-4">
                <div className="h-[320px] min-w-0">
                  {dispatchAnalytics.byCustomer.length ? (
                    <ResponsiveContainer width="100%" height={320}>
                      <BarChart
                        data={dispatchAnalytics.byCustomer.slice(0, 8)}
                        layout="vertical"
                        margin={{ top: 8, right: 8, left: 20, bottom: 0 }}
                      >
                        <CartesianGrid
                    ...
                    <div className="flex h-full items-center justify-center rounded-[1.35rem] border border-dashed border-line bg-surface-2 text-sm font-semibold text-content-3">
                      No customer dispatch split is available for this filter
                      window.
                    </div>
                  )}
                </div>
              ...
              <CardContent className="pt-4">
                <div className="max-h-[360px] overflow-auto rounded-[1.2rem] border border-line">
                  <table className="min-w-full text-sm">
                    <thead className="sticky top-0 bg-surface-2">
                      <tr>
                        {["dc_no", "customer", "status", "date", "vehicle"].map(
                          (column) => (
                            <th
                              key={`dispatch-${column}`}
                              className="border-b border-line px-4 py-3 text-left text-[11px] font-black uppercase tracking-[0.16em] text-content-3"
        ...
        <>
              ...
              <CardContent className="pt-4">
                <div className="h-[320px] min-w-0">
                  {downtimeAnalytics.dailyTrend.length ? (
                    <ResponsiveContainer width="100%" height={320}>
                      <AreaChart
                        data={downtimeAnalytics.dailyTrend.map(
                          (row, index) => ({
                            ...row,
                            label: labelFromRow(row, index),
                          }),
                      ...
                      >
                        ...
                        <Tooltip
                          formatter={(value, name) => [
                            String(name).includes("event")
                              ? formatMetricValue("count", value)
                              : `${formatMaybeNumber(value, 0)} min`,
                            toLabel(String(name || "")),
                          ]}
                        />
                        <Legend />
              ...
              <CardContent className="pt-4">
                <div className="h-[320px] min-w-0">
                  {downtimeAnalytics.pareto.length ? (
                    <ResponsiveContainer width="100%" height={320}>
                      <BarChart
                        data={downtimeAnalytics.pareto.slice(0, 8)}
                        layout="vertical"
                      ...
                      >
                        ...
                        <Tooltip
                          formatter={(value, name) => [
                            String(name).includes("pct")
                              ? formatMetricValue("pct", value)
                              : `${formatMaybeNumber(value, 0)} min`,
                            toLabel(String(name || "")),
                          ]}
                        />
          ...
          <section className="grid gap-5 xl:grid-cols-[0.9fr_1.1fr] xl:items-start">
            <Card className="rounded-[1.85rem] border-line shadow-sm">
              ...
              <CardContent className="space-y-3 pt-4">
                {downtimeAnalytics.topMachines.length ? (
                  downtimeAnalytics.topMachines
                    .slice(0, 8)
                    .map((row, index) => (
                      <div
                      ...
                      >
                        <div>
                          <div className="text-sm font-black text-content-1">
                            {String(row.machine || "Unknown")}
                          </div>
                          <div className="text-xs text-content-3">
                            {formatMetricValue("count", row.count)} stoppages
                          </div>
                        </div>
                        <Badge className="rounded-full border border-danger-border bg-danger-bg text-danger-fg">
                          {formatMaybeNumber(row.minutes, 0)} min
                        </Badge>
                      </div>
                    ))
                ) : (
                  <div className="text-sm font-semibold text-content-3">
                    No machine downtime rows are visible in this window.
                  </div>
                )}
              </CardContent>
            </Card>
                      ...
                      {downtimeAnalytics.pareto
                        ...
                        .map((row, index) => (
                          ...
                          >
                            ...
                            <td className="px-4 py-3 text-content-2">
                              {formatMetricValue("pct", row.pct)}
                            </td>
                            <td className="px-4 py-3 text-content-2">
                              {formatMetricValue("pct", row.cumulative_pct)}
                            </td>
                          </tr>
                        ))}
                    </tbody>
        ...
        <>
          ...
          <section className="grid gap-5 xl:grid-cols-[1fr_0.9fr] xl:items-start">
              ...
              <CardContent className="pt-4">
                <div className="h-[340px] min-w-0">
                  {operatorAnalytics.leaderboard.length ? (
                    <ResponsiveContainer width="100%" height={340}>
                      <BarChart
                        data={operatorAnalytics.leaderboard.slice(0, 8)}
                        layout="vertical"
                        margin={{ top: 8, right: 8, left: 18, bottom: 0 }}
                      >
                        <CartesianGrid
                        ...
                        <Tooltip
                          formatter={(value, name) => [
                            ...
                            String(name).includes("achievement") ||
                            String(name).includes("efficiency")
                              ? formatMetricValue("pct", value)
                              : formatMetricValue("kg", value),
                            toLabel(String(name || "")),
                          ]}
                    ...
                    <div className="flex h-full items-center justify-center rounded-[1.35rem] border border-dashed border-line bg-surface-2 text-sm font-semibold text-content-3">
                      No operator leaderboard is available for this filter
                      window.
                    </div>
                  )}
                </div>
            ...
            <div className="grid gap-5">
              {[operatorAnalytics.best, operatorAnalytics.worst].map(
                (row, index) => (
                  <Card
                    key={index}
                    className="rounded-[1.85rem] border-line shadow-sm"
                  >
                    <CardHeader className="pb-0">
                      <CardTitle className="text-xl font-black text-content-1">
                        {index === 0 ? "Best spotlight" : "Watchpoint"}
                      </CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-3 pt-4">
                      <div className="rounded-[1.1rem] border border-line bg-surface-2 px-4 py-3">
                        <div className="text-[11px] font-black uppercase tracking-[0.16em] text-content-3">
                          Operator
                        </div>
                        <div className="mt-2 text-lg font-black text-content-1">
                      ...
                      <div className="grid gap-3 sm:grid-cols-2">
                        ...
                        <StatInset
                          icon={Package}
                          label="Output"
                          value={formatMetricValue("kg", row?.produced_kg)}
                        />
                        <StatInset
                          icon={Gauge}
                          label="Completion"
          ...
          <Card className="overflow-hidden rounded-[1.85rem] border-line shadow-sm">
            ...
            <CardContent className="pt-4">
              <div className="max-h-[420px] overflow-auto rounded-[1.2rem] border border-line">
                <table className="min-w-full text-sm">
                  <thead className="sticky top-0 bg-surface-2">
                    <tr>
                      {[
                    ...
                    {operatorAnalytics.leaderboard
                      ...
                      .map((row, index) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 text-content-2">
                            {formatMetricValue("count", row.jobs)}
                          </td>
                          <td className="px-4 py-3 text-content-2">
                            {formatMetricValue("count", row.completed)}
                          </td>
                          <td className="px-4 py-3 text-content-2">
                            {formatMetricValue("kg", row.produced_kg)}
                          </td>
                          <td className="px-4 py-3 text-content-2">
                            {formatMetricValue("kg", row.scrap_kg)}
                          </td>
                          <td className="px-4 py-3 text-content-2">
                            {formatMetricValue("efficiency", row.efficiency)}
                          </td>
                          <td className="px-4 py-3 text-content-2">
                            {formatMetricValue(
                              "completion_rate",
              ...
              <CardContent className="pt-4">
                <div className="h-[340px] min-w-0">
                  {costingAnalytics.monthlyTrend.length ? (
                    <ResponsiveContainer width="100%" height={340}>
                      <AreaChart
                        data={costingAnalytics.monthlyTrend.map(
                          (row, index) => ({
                            ...row,
                            label: labelFromRow(row, index),
                          }),
              ...
              <CardContent className="grid gap-4 pt-4 md:grid-cols-[0.92fr_1.08fr] xl:grid-cols-1">
                <div className="h-[220px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        data={costingAnalytics.costSplit}
                        dataKey="value"
                        nameKey="name"
                        innerRadius={56}
                        outerRadius={84}
                      ...
                      <Tooltip
                        formatter={(value) => formatMetricValue("value", value)}
                      />
                    </PieChart>
                  </ResponsiveContainer>
                </div>
              ...
              <CardContent className="pt-4">
                <div className="max-h-[360px] overflow-auto rounded-[1.2rem] border border-line">
                  <table className="min-w-full text-sm">
                    ...
                    <tbody>
                      {costingAnalytics.customerMargin
                        .slice(0, 12)
                        .map((row, index) => (
                          <tr
                          ...
                          >
                            <td className="px-4 py-3 font-semibold text-content-1">
                              {String(row.customer || "Unknown")}
                            </td>
                            <td className="px-4 py-3 text-content-2">
                              {formatMetricValue("value", row.revenue)}
                            </td>
                            <td className="px-4 py-3 text-content-2">
                              {formatMetricValue("value", row.cost)}
                            </td>
                            <td className="px-4 py-3 text-content-2">
                              {formatMetricValue("value", row.margin)}
                            </td>
                            <td className="px-4 py-3 text-content-2">
                              {formatMetricValue("pct", row.margin_pct)}
                            </td>
                            <td className="px-4 py-3 text-content-2">
                              {formatMetricValue("pct", row.coverage_pct)}
                            </td>
                            <td className="px-4 py-3 text-content-2">
                              {formatMetricValue("count", row.orders)}
                            </td>
                ...
                <div className="h-[180px] min-w-0">
                  {costingAnalytics.overheadTrend.length ? (
                    <ResponsiveContainer width="100%" height={180}>
                      <BarChart
                        data={costingAnalytics.overheadTrend.slice().reverse()}
                      >
                        <CartesianGrid
                          strokeDasharray="3 3"
                          stroke="#e2e8f0"
                    ...
                    <div className="flex h-full items-center justify-center rounded-[1.2rem] border border-dashed border-line bg-surface-2 text-sm font-semibold text-content-3">
                      No overhead months are visible in this filter window.
                    </div>
                  )}
                </div>
                <div className="h-[180px] min-w-0">
                  {costingAnalytics.costPerKgTrend.length ? (
                    <ResponsiveContainer width="100%" height={180}>
                      <LineChart
                        data={costingAnalytics.costPerKgTrend.map(
                          (row, index) => ({
                            ...row,
                            label: labelFromRow(row, index),
                          }),
                    ...
                    <div className="flex h-full items-center justify-center rounded-[1.2rem] border border-dashed border-line bg-surface-2 text-sm font-semibold text-content-3">
                      No cost-per-kg series is visible in this filter window.
                    </div>
                  )}
                </div>
              </CardContent>
        ...
        <>
          ...
          <section className="grid gap-5 xl:grid-cols-[1fr_1fr] xl:items-start">
              ...
              <CardContent className="pt-4">
                <div className="h-[320px] min-w-0">
                  {shiftAnalytics.series.length ? (
                    <ResponsiveContainer width="100%" height={320}>
                      <LineChart
                        data={shiftAnalytics.series.map((row, index) => ({
                          ...row,
                          label: labelFromRow(row, index),
                        }))}
                      >
                    ...
                    <div className="flex h-full items-center justify-center rounded-[1.35rem] border border-dashed border-line bg-surface-2 text-sm font-semibold text-content-3">
                      No shift rhythm is available for this filter window.
                    </div>
                  )}
                </div>
              </CardContent>
            ...
            <Card className="overflow-hidden rounded-[1.85rem] border-line shadow-sm">
              ...
              <CardContent className="pt-4">
                <div className="max-h-[320px] overflow-auto rounded-[1.2rem] border border-line">
                  <table className="min-w-full text-sm">
                    <thead className="sticky top-0 bg-surface-2">
                      <tr>
                        {[
                    ...
                    <tbody>
                      {shiftAnalytics.rows.slice(0, 12).map((row, index) => (
                        <tr
                          key={`${row.shift_code || index}`}
                          className="border-b border-line last:border-b-0 hover:bg-surface-2"
                        >
                          <td className="px-4 py-3 font-semibold text-content-1">
                            {String(row.shift_code || "UNASSIGNED")}
                          </td>
                          <td className="px-4 py-3 text-content-2">
                            {formatMetricValue("kg", row.output_kg)}
                          </td>
                          <td className="px-4 py-3 text-content-2">
                            {formatMetricValue("kg", row.scrap_kg)}
                          </td>
                          ...
                          <td className="px-4 py-3 text-content-2">
                            {formatMaybeNumber(row.downtime_minutes, 0)} min
                          </td>
                          <td className="px-4 py-3 text-content-2">
                            {formatMetricValue("count", row.events)}
                          </td>
                        </tr>
                      ))}
                    </tbody>
        ...
        <>
          ...
          <section className="grid gap-5 xl:grid-cols-[1.02fr_0.98fr] xl:items-start">
            ...
            <Card className="rounded-[1.85rem] border-line shadow-sm">
              <CardHeader className="pb-0">
                <CardTitle className="text-xl font-black text-content-1">
                  Color-family variance watch
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-3 pt-4">
                {inkAnalytics.topVariance.length ? (
                  inkAnalytics.topVariance.slice(0, 6).map((row, index) => (
                    <div
                      key={`${row.color_family || index}`}
                      className="rounded-[1.1rem] border border-line bg-surface-2 px-4 py-3"
                    >
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          <div className="text-sm font-black text-content-1">
                            {String(row.color_family || "Custom")}
                          </div>
                          <div className="text-xs text-content-3">
                            {formatMetricValue("count", row.job_count)} jobs ·{" "}
                            {formatMetricValue("kg", row.actual_consumed_qty)}{" "}
                            consumed
                          </div>
                        </div>
                        <Badge className="rounded-full border border-danger-border bg-danger-bg text-danger-fg">
                          {formatMetricValue("kg", row.variance_qty)}
                        </Badge>
                      </div>
                      <div className="mt-3 grid gap-3 sm:grid-cols-3">
                        <StatInset
                          icon={Package}
                          label="Theoretical"
                          value={formatMetricValue("kg", row.theoretical_qty)}
                        />
                        <StatInset
                          icon={Factory}
                          label="Issued"
                          value={formatMetricValue("kg", row.actual_issued_qty)}
                        />
                        <StatInset
                          icon={ArrowRightLeft}
                          label="Returned"
                  ...
                  <div className="text-sm font-semibold text-content-3">
                    No color-family variance rows are visible in this window.
                  </div>
                )}
              </CardContent>
            </Card>
                  ...
                  <tbody>
                    {inkAnalytics.topVariance.slice(0, 12).map((row, index) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-content-2">
                          {formatMetricValue("kg", row.theoretical_qty)}
                        </td>
                        <td className="px-4 py-3 text-content-2">
                          {formatMetricValue("kg", row.planned_issue_qty)}
                        </td>
                        <td className="px-4 py-3 text-content-2">
                          {formatMetricValue("kg", row.actual_issued_qty)}
                        </td>
                        <td className="px-4 py-3 text-content-2">
                          {formatMetricValue("kg", row.actual_returned_qty)}
                        </td>
                        <td className="px-4 py-3 text-content-2">
                          {formatMetricValue("kg", row.actual_consumed_qty)}
                        </td>
                        ...
                        <td className="px-4 py-3 text-content-2">
                          {formatMetricValue("count", row.job_count)}
                        </td>
                      </tr>
                    ))}
                  </tbody>
      ...
      {isScrapTab ? (
        <section className="grid gap-5 xl:grid-cols-3 xl:items-start">
          {scrapKgBoards.map((group) => (
            <Card
              key={group.key}
              className="rounded-[1.75rem] border-line shadow-sm"
            >
              <CardHeader className="pb-0">
                ...
                {chartSeries.rows.length && chartSeries.primaryKey ? (
                  <ResponsiveContainer width="100%" height={360}>
                    <AreaChart data={chartSeries.rows}>
                      ...
                      <Area
                        type="monotone"
                        dataKey={chartSeries.primaryKey}
                        stroke="#2563eb"
                        fill={`url(#${tab}-fill)`}
                        strokeWidth={2.5}
                      />
                      {chartSeries.secondaryKey ? (
                        <Line
                          type="monotone"
                          dataKey={chartSeries.secondaryKey}
                          stroke="#0f766e"
                          strokeWidth={2}
                          dot={false}
                        />
              ...
              <div className="h-[360px] min-w-0">
                {firstBreakdown?.rows?.length ? (
                  <ResponsiveContainer width="100%" height={360}>
                    <PieChart>
                      <Pie
                        data={normalizeChartRows(
                          firstBreakdown.rows,
                        ).rows.slice(0, 8)}
                        dataKey={
                          normalizeChartRows(firstBreakdown.rows).primaryKey ||
                          "value"
                        }
                        nameKey="label"
                        innerRadius={72}
                        ...
                        paddingAngle={2}
                      >
                        {normalizeChartRows(firstBreakdown.rows)
                          .rows.slice(0, 8)
                          .map((_, index) => (
                            <Cell
                              key={`${firstBreakdown.key}-${index}`}
                              fill={PIE_COLORS[index % PIE_COLORS.length]}
                            />
                          ))}
                      </Pie>
            ...
            <CardContent className="space-y-3 pt-4">
              <div className="rounded-[1.25rem] border border-line bg-surface-2 p-4">
                ...
                <div className="mt-2 text-lg font-black text-content-1">
                  {isOeeTab
                    ? "Machine OEE spread"
                    : firstBreakdown?.title ||
                      (chartSeries.primaryKey
                        ? toLabel(chartSeries.primaryKey)
                        : "No dominant signal")}
                </div>
                <div className="mt-2 text-sm text-content-3">
                  {isOeeTab &&
                  breakdownGroups.find((group) => group.key === "by_machine")
                    ?.rows?.length
              ...
              <div className="rounded-[1.25rem] border border-line bg-surface-2 p-4">
                <div className="text-[11px] font-black uppercase tracking-[0.18em] text-content-3">
                  Secondary watchpoint
                </div>
                <div className="mt-2 text-lg font-black text-content-1">
                  {isOeeTab
                    ? "Loss pressure"
                    : secondBreakdown?.title || "Coverage health"}
                </div>
                <div className="mt-2 text-sm text-content-3">
                  {isOeeTab
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <StatInset
                  icon={Activity}
                  label="Trend points"
                  value={String(chartSeries.rows.length)}
                />
                <StatInset
                  icon={AlertTriangle}
                  label="Warnings"
                  value={String(payload.warnings?.length || 0)}
                />
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="max-h-[620px] space-y-4 overflow-y-auto pr-2 pt-4">
              {visibleBreakdowns.map((group) => {
                ...
                return (
                  <div
                    key={group.key}
                    className="rounded-[1.25rem] border border-line bg-surface-2 p-4"
                  >
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <div className="text-sm font-black text-content-1">
                      ...
                      <Badge
                        className={cn("rounded-full border", accentStyle.chip)}
                      >
                        {normalized.primaryKey
                          ? toLabel(normalized.primaryKey)
                          : "detail"}
                      </Badge>
                    </div>
                    <div className="mt-4 h-[180px] min-w-0">
                      {normalized.rows.length && normalized.primaryKey ? (
                        <ResponsiveContainer width="100%" height={180}>
                          <BarChart
                            data={normalized.rows.slice(0, 8)}
                            layout="vertical"
                            margin={{ left: 20 }}
                          >
                            <CartesianGrid
        ...
        <section className="grid gap-5 xl:grid-cols-2 xl:items-start">
          {visibleBreakdowns.map((group) => {
            ...
            return (
              <Card
                key={`table-${group.key}`}
                className="overflow-hidden rounded-[1.85rem] border-line shadow-sm"
              >
                <CardHeader className="pb-0">
                  <CardTitle className="text-lg font-black text-content-1">
                    {group.title}
                ...
                <CardContent className="pt-4">
                  <div className="max-h-[320px] overflow-auto rounded-[1.15rem] border border-line">
                    <table className="min-w-full text-sm">
                      <thead className="sticky top-0 bg-surface-2">
                        <tr>
                          {columns.map((column) => (
                            <th
                              ...
                              className="border-b border-line px-3 py-2 text-left text-[10px] font-black uppercase tracking-[0.14em] text-content-3"
                            >
                              {toLabel(column)}
                            </th>
                          ))}
...
function StatInset({
  ...
  return (
    <div className="rounded-[1rem] border border-line bg-surface-1 px-4 py-3 shadow-sm">
      <div className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.16em] text-content-3">
        <Icon className="h-3.5 w-3.5 text-content-4" />
        {label}
      </div>
      <div className="mt-2 text-lg font-black text-content-1">{value}</div>
    </div>
  );
}
  ...
  return (
    <div className="rounded-[1.2rem] border border-line bg-surface-2 p-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <Badge className="rounded-full border border-line bg-surface-1 text-content-3">
          {rows.length} rows
        </Badge>
      </div>
      <div className="mt-4 space-y-2">
        {rows.length ? (
          rows.slice(0, 6).map((row, index) => (
            <div
              key={`${title}-${row[labelKey] || index}`}
              className="flex items-center justify-between gap-3 rounded-xl bg-surface-1 px-3 py-2"
            >
              <div className="min-w-0">
                <div className="truncate text-sm font-semibold text-content-1">
                  {String(row[labelKey] || `Row ${index + 1}`)}
                </div>
                <div className="text-xs text-content-3">
                  {formatMetricValue("events", row.events)} event(s)
                </div>
              </div>
          ...
          <div className="rounded-xl border border-dashed border-line bg-surface-1 px-3 py-6 text-center text-sm font-semibold text-content-3">
