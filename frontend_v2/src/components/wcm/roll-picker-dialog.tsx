...
const TIER_PRESENTATION: Record<
  Tier,
  { label: string; subtitle: string; tone: string; icon: React.ElementType }
> = {
  ORDER_BOUND: {
    label: "Reserved",
    subtitle: "Already committed to this order",
    tone: "bg-order-bg text-order-fg border-order-border",
    icon: CheckCircle2,
  },
  EXACT: {
    label: "Exact",
  ...
  WIDER_OK_WITH_SLIT: {
    ...
    tone: "bg-warning-bg text-warning-fg border-warning-border",
    icon: Scissors,
  },
  REMAINDER_POOL: {
    label: "Remainder",
    subtitle: "Recycled child from past slit",
    tone: "bg-surface-2 text-content-2 border-line",
    icon: Boxes,
  },
};

function ageLabel(createdAt: string | null | undefined) {
  if (!createdAt) return { text: "?", tone: "bg-surface-2 text-content-3" };
  const ts = new Date(createdAt).getTime();
  if (Number.isNaN(ts))
    return { text: "?", tone: "bg-surface-2 text-content-3" };
  const days = Math.floor((Date.now() - ts) / (24 * 60 * 60 * 1000));
  if (days <= 0)
    return {
      text: "today",
      tone: "bg-success-bg text-success-fg border border-success-border",
    };
  if (days < 7)
    return {
      text: `${days}d`,
  ...
  if (days < 30)
    return {
      text: `${days}d`,
      tone: "bg-warning-bg text-warning-fg border border-warning-border",
    };
  return {
    text: `${days}d`,
    tone: "bg-danger-bg text-danger-fg border border-danger-border",
  };
}
...
function slitPolicyLabel(value: any) {
  const key = String(value || "").toUpperCase();
  return (
    SLIT_POLICY_LABELS[key] ||
    (key ? key.replace(/_/g, " ").toLowerCase() : "policy default")
  );
}
...
function tierTooltip(c: any, target_width_mm: number): string {
  ...
  if (tier === "WIDER_OK_WITH_SLIT") {
    ...
    const childWidths = (sp.child_widths_mm || [])
      .map((w: number) => Math.round(w))
      .join(" + ");
    const rem = Math.round(sp.remainder_mm || 0);
    const disp = sp.remainder_disposition || "";
    const remText =
      disp === "KEEP" ? "kept" : disp === "SCRAP" ? "scrapped" : "as-is";
...
export function WcmRollPickerDialog({
  jobId,
  open,
  onOpenChange,
  onAssigned,
  ...
  const targetWidth =
    tieredQuery.data?.planned_parent_width_mm ||
    tieredQuery.data?.target_width_mm ||
    0;
  const childTargetWidth =
    tieredQuery.data?.child_target_width_mm || targetWidth || 0;
  ...
  React.useEffect(() => {
    ...
    if (
      lastCandidateCountRef.current &&
      count !== lastCandidateCountRef.current
    ) {
      setStaleFlash(count);
  ...
  const filteredCandidates = React.useMemo(() => {
    ...
    return candidates.filter((c: any) => {
      ...
      if (showAged30) {
        ...
        const days =
          ts > 0 ? Math.floor((now - ts) / (24 * 60 * 60 * 1000)) : 0;
  ...
  const totalSelectedWeight = selectedCandidates.reduce(
    (sum: number, c: any) => sum + Number(c.weight_kg || 0),
    0,
  );
  ...
  const pickedWeightKg = React.useMemo(() => {
    ...
    return selectedCandidates.reduce((sum: number, c: any) => {
      ...
      const widths: number[] = (sp.child_widths_mm || []).map((w: number) =>
        Number(w),
      );
  ...
  const stillNeed =
    needed != null ? Math.max(0, needed - pickedWeightKg) : null;
  ...
  const slitMutation = useMutation({
    mutationFn: async () => {
      ...
      const reasonText =
        (reason || "tiered allocate").trim() || "tiered allocate";
      const picksPayload = selectedCandidates.map((c: any) => {
        ...
        if (c.tier === "WIDER_OK_WITH_SLIT") {
          if (c.slit_preview?.gang_group_id) mode = "GANG";
            ...
            mode =
              slitMode === "MAX" ? "MAX" : slitMode === "GANG" ? "GANG" : "ONE";
        }
        return { roll_id: c.roll_id, mode, reason: reasonText };
      });
      return wcmService.allocateWithSlitBatch(
        jobId,
        picksPayload,
        idemKeyRef.current,
      );
    ...
    onSuccess: (result) => {
      ...
      const description = [
        ...
        children
          ? `${children} child${children === 1 ? "" : "ren"} created`
          : null,
        remainders
          ? `${remainders} remainder${remainders === 1 ? "" : "s"} kept`
          : null,
      ...
      toast({
        title:
          result.picks_count > 1
            ? `Allocated ${result.picks_count} rolls`
            : "Roll assigned",
        description,
      });
    ...
    onError: (err: any) => {
      ...
      if (detail) {
        ...
        else if (detail.picks)
          detailText = (detail.picks as string[]).join("; ");
  ...
  const pickedKgLabel =
    pickedWeightKg > 0 ? ` (${pickedWeightKg.toFixed(1)} kg)` : "";
  const confirmLabel =
      ...
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-base">
            <Scissors className="h-4 w-4 text-order-fg" />
            Pick a roll
            {staleFlash !== null ? (
              <span className="ml-2 inline-flex items-center gap-1 rounded-full border border-warning-border bg-warning-bg px-2 py-0.5 text-[10px] font-bold text-warning-fg">
                <Sparkles className="h-3 w-3" /> Stock updated · {staleFlash}{" "}
                candidates now
              </span>
            ) : null}
          </DialogTitle>
          <DialogDescription>
            Planned parent{" "}
            <b>{targetWidth ? `${targetWidth.toFixed(0)} mm` : "—"}</b> ·{" "}
            {policy?.code || "default"} policy · remainders &lt;{" "}
            {minRemainder.toFixed(0)} mm go to scrap.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="-mt-1 mb-2 flex flex-wrap items-center gap-2">
          <div className="relative flex-1 min-w-[200px]">
            <Barcode className="absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-content-4" />
            <Input
              ref={barcodeRef}
              value={scanInput}
        ...
        <div className="-mt-1 mb-2 grid grid-cols-2 gap-1.5 text-[10px] sm:grid-cols-5">
          <div className="rounded-lg bg-success-bg px-2 py-1 ring-1 ring-success-border">
            ...
            <div className="font-mono font-bold text-success-fg">
              {childTargetWidth ? `${childTargetWidth.toFixed(0)} mm` : "—"}
            </div>
          </div>
          ...
          <div className="rounded-lg bg-order-bg px-2 py-1 ring-1 ring-order-border">
            ...
            <div className="font-mono font-bold text-order-fg">
              {targetWidth ? `${targetWidth.toFixed(0)} mm` : "—"}
            </div>
          </div>
            ...
            <div className="grid gap-2 max-h-[55vh] overflow-y-auto pr-2">
              {filteredCandidates.map((c: any) => {
                ...
                const sameWidth =
                  widths.length > 0 &&
                  widths.every(
                    (w: number) => Math.round(w) === Math.round(widths[0] || 0),
                  );
                const childWidthLabel = sameWidth
                  ? `${widths.length}× ${Math.round(widths[0] || 0)} mm`
                  : widths
                      .map((w: number) => `${Math.round(w)} mm`)
                      .join(" + ");
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      selected
                        ? "ring-2 ring-order-border border-order-border bg-surface-1 shadow-sm"
                  ...
                  >
                    <div className="flex flex-wrap items-center gap-2">
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Badge
                            variant="outline"
                            className={cn(
                              "cursor-help text-[10px] font-semibold border",
                              pres?.tone,
                            )}
                          >
                      ...
                      {c.location_code ? (
                        <span className="rounded-md border border-line bg-surface-2 px-1.5 py-0.5 font-mono text-[10px] text-content-3">
                          {c.plant_code ? `${c.plant_code} · ` : ""}
                          {c.location_code}
                        </span>
                      ) : null}
                      {c.parent_roll?.label_id ? (
                        <span className="rounded-md border border-order-border bg-order-bg px-1.5 py-0.5 text-[10px] text-order-fg">
                      ...
                      {selected ? (
                        <CheckCircle2 className="ml-auto h-4 w-4 text-order-fg" />
                      ) : null}
                    </div>
                    <div className="mt-2 grid grid-cols-2 gap-2 text-xs text-content-3 sm:grid-cols-4">
                      <div>
                        <span className="text-content-4">Width</span>
                        <br />
                        <b>{c.width_mm.toFixed(0)} mm</b>
                        <div className="text-[10px] text-content-4">
                          {widthBasisLabel(c.width_basis)}
                        </div>
                      </div>
                      <div>
                        <span className="text-content-4">Thickness</span>
                        <br />
                        <b>{c.thickness_micron.toFixed(0)} µ</b>
                      </div>
                      <div>
                        <span className="text-content-4">Weight</span>
                        <br />
                        <b>{c.weight_kg.toFixed(2)} kg</b>
                      </div>
                      <div>
                        <span className="text-content-4">Form</span>
                        <br />
                        <b>{stockFormLabel(c.stock_form)}</b>
                      </div>
                    </div>
                    {c.tier === "WIDER_OK_WITH_SLIT" && c.slit_preview && (
                      <>
                        <div className="mt-2 rounded-lg border border-warning-border bg-warning-bg p-2 text-[11px] text-warning-fg">
                          <Scissors className="mr-1 inline h-3 w-3" />
                          Will slit → {childWidthLabel || "child rolls"},
                          remainder{" "}
                          <b>{Math.round(c.slit_preview.remainder_mm)} mm</b> ·
                          ...
                          {c.slit_preview.remainder_disposition ? (
                            <span>
                              ...
                              ·{" "}
                              <b>
                                {c.slit_preview.remainder_disposition === "KEEP"
                                  ? "keep remainder"
                                  : c.slit_preview.remainder_disposition ===
                                      "SCRAP"
                                    ? "scrap remainder"
                          ...
                          {c.slit_preview.gang_job_count ? (
                            <span>
                              {" "}
                              · gang <b>{c.slit_preview.gang_job_count}</b> jobs
                            </span>
                          ) : null}
                        </div>
                        <SlitLayoutMini
                          ...
                          childWidthsMm={(
                            c.slit_preview.child_widths_mm || []
                          ).map((w: number) => Number(w))}
                          remainderMm={Number(c.slit_preview.remainder_mm || 0)}
                          trimMm={Number(c.slit_preview.trim_mm || 0)}
                        />
                      </>
                    )}
                    <div className="mt-1 text-[10px] text-content-4">
                      {pres?.subtitle}
                    </div>
                  </button>
                );
        ...
        {primarySelected && (
          <div className="space-y-3 rounded-xl border border-line bg-surface-2 p-3">
              ...
              <div>
                ...
                <div className="mt-1 grid grid-cols-1 gap-2 sm:grid-cols-3">
                  <SlitModeButton
                    active={slitMode === "ONE"}
                    disabled={Boolean(
                      primarySelected.slit_preview?.gang_group_id,
                    )}
                  ...
                  <SlitModeButton
                    active={
                      slitMode === "GANG" ||
                      Boolean(primarySelected.slit_preview?.gang_group_id)
                    }
                    disabled={!primarySelected.slit_preview?.gang_group_id}
                    label="Slit for gang"
                    detail={
                      primarySelected.slit_preview?.gang_group_id
                        ? `${primarySelected.slit_preview.gang_job_count || 0} committed jobs`
                        : "Commit gang first"
                    }
            ...
            {selectedIds.length > 1 ? (
              <div className="rounded-lg border border-order-border bg-order-bg px-3 py-2 text-[11px] text-order-fg">
                <Info className="mr-1 inline h-3 w-3" />
                Multi-select active · {selectedIds.length} rolls totaling{" "}
                <b>{totalSelectedWeight.toFixed(2)} kg</b>.
              </div>
            ) : null}
          </div>
        )}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active
          ? "border-order-border bg-order-bg text-order-fg ring-1 ring-order-border"
...
function SlitLayoutMini({
  ...
  return (
    <div className="mt-2">
      <div className="mb-1 flex items-center justify-between text-[10px] text-content-3">
        <span>Parent {Math.round(parentWidthMm)} mm</span>
        <span>
          · children {Math.round(totalChildren)} mm · remainder{" "}
          {Math.round(remainderMm)} mm · trim {Math.round(trimMm)} mm
        </span>
      </div>
      <div className="flex h-6 w-full overflow-hidden rounded-md ring-1 ring-line">
        {childWidthsMm.map((w, idx) => {
          const tones = [
            "bg-order-bg text-order-fg",
            "bg-order-bg text-order-fg",
...
function ReservationsPreview({
  ...
  const childTargetWidth = isSlit
    ...
    : null;
  return (
    <div className="rounded-lg border border-order-border bg-surface-1 p-2.5 text-[11px]">
      <div className="font-bold text-order-fg">Reservation preview</div>
      <ul className="mt-1 list-disc pl-5 text-content-2">
        <li>
          Will lock:{" "}
          <b>
          ...
          ({primary.label_id}
          {count > 1 ? ` + ${count - 1}` : ""}, {totalKg.toFixed(2)} kg) for
          this job.
        </li>
        {isSlit ? (
          <>
            <li>
              Will spawn: child roll <b>{childTargetWidth}</b> from parent{" "}
              {Math.round(primary.width_mm)} mm.
            </li>
            <li>
              Will scrap: {Math.round(sp.trim_mm || 0)} mm trim{" "}
              {sp.remainder_disposition === "SCRAP"
                ? `+ ${Math.round(sp.remainder_mm || 0)} mm remainder`
                : ""}
              .
            </li>
          </>
...
function CoverageCard({
  ...
  const ringTone = isMet
    ? "ring-success-border bg-success-bg"
    : isHalf
      ? "ring-warning-border bg-warning-bg"
      : "ring-line bg-surface-2";
  ...
  return (
    <div className={cn("mb-2 rounded-xl px-3 py-2 ring-1", ringTone)}>
      ...
      <div className="mt-1.5 h-2 w-full overflow-hidden rounded-full bg-surface-1 ring-1 ring-line">
        <div
      ...
      <div className="mt-2 grid grid-cols-3 gap-2 text-[11px]">
        <div>
          <div className="text-content-3">Needed</div>
          <div className="font-mono font-bold text-content-1">
            {neededKg.toFixed(1)} kg
          </div>
        </div>
        <div>
          <div className="text-content-3">Picked</div>
          <div className="font-mono font-bold text-content-1">
            {pickedKg.toFixed(1)} kg
          </div>
        </div>
        <div>
          {isMet ? (
            <>
              <div className="text-success-fg">Covered</div>
              <div className="font-mono font-bold text-success-fg">
            ...
            <>
              <div className="text-content-3">Still need</div>
              <div className="font-mono font-bold text-warning-fg">
                {stillNeedKg.toFixed(1)} kg
              </div>
            </>
          )}
        </div>
      ...
      {overByKg > 0 && isMet ? (
        <div className="mt-1.5 inline-flex items-center gap-1 rounded-full border border-warning-border bg-warning-bg px-2 py-0.5 text-[10px] font-bold text-warning-fg">
...
function ZeroCandidatesState({ totalRaw }: { totalRaw: number }) {
  return (
    <div className="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-warning-border bg-warning-bg p-8 text-center">
      <AlertTriangle className="h-7 w-7 text-warning-fg" />
      <div>
        <div className="text-sm font-bold text-warning-fg">
          No eligible rolls
        </div>
        <p className="mt-1 max-w-md text-[11px] text-warning-fg">
          {totalRaw > 0
      ...
      <div className="flex flex-wrap items-center justify-center gap-2">
        <Link
          ...
          className="inline-flex items-center gap-1.5 rounded-lg bg-order-fg px-3 py-1.5 text-[11px] font-bold text-white hover:bg-order-fg"
        >
        ...
        <Link
          ...
          className="inline-flex items-center gap-1.5 rounded-lg border border-warning-border bg-surface-1 px-3 py-1.5 text-[11px] font-bold text-warning-fg hover:bg-warning-bg"
        >
