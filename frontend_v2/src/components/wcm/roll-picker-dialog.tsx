...
const TIER_PRESENTATION: Record<Tier, { label: string; subtitle: string; tone: string; icon: React.ElementType }> = {
    ...
    EXACT: {
        ...
        tone: "bg-success-bg text-emerald-800 border-success-border",
        icon: Sparkles,
    },
    WIDER_OK_WITH_SLIT: {
        label: "Wider · will slit",
        subtitle: "Confirm slit children + remainder",
        tone: "bg-warning-bg text-amber-800 border-warning-border",
        icon: Scissors,
    },
    REMAINDER_POOL: {
        label: "Remainder",
        subtitle: "Recycled child from past slit",
        tone: "bg-slate-50 text-content-2 border-slate-200",
        icon: Boxes,
    },
}

function ageLabel(createdAt: string | null | undefined) {
    if (!createdAt) return { text: "?", tone: "bg-slate-100 text-content-3" }
    const ts = new Date(createdAt).getTime()
    if (Number.isNaN(ts)) return { text: "?", tone: "bg-slate-100 text-content-3" }
    const days = Math.floor((Date.now() - ts) / (24 * 60 * 60 * 1000))
    if (days <= 0) return { text: "today", tone: "bg-success-bg text-success-fg border border-success-border" }
    if (days < 7) return { text: `${days}d`, tone: "bg-success-bg text-success-fg border border-success-border" }
    if (days < 30) return { text: `${days}d`, tone: "bg-warning-bg text-amber-800 border border-warning-border" }
    return { text: `${days}d`, tone: "bg-danger-bg text-rose-800 border border-danger-border" }
}
            ...
            <DialogContent className="max-w-3xl">
                <DialogHeader>
                    <DialogTitle className="flex items-center gap-2 text-base">
                        ...
                        Pick a roll
                        {staleFlash !== null ? (
                            <span className="ml-2 inline-flex items-center gap-1 rounded-full border border-warning-border bg-warning-bg px-2 py-0.5 text-[10px] font-bold text-amber-800">
                ...
                <div className="-mt-1 mb-2 grid grid-cols-2 gap-1.5 text-[10px] sm:grid-cols-5">
                    <div className="rounded-lg bg-success-bg px-2 py-1 ring-1 ring-emerald-200">
                        <div className="font-black uppercase tracking-widest text-success-fg">child target</div>
                    ...
                    <div className="rounded-lg bg-warning-bg px-2 py-1 ring-1 ring-amber-200">
                        <div className="font-black uppercase tracking-widest text-warning-fg">policy</div>
                        <div className="font-mono font-bold text-amber-900">{policy?.code || "default"}</div>
                    </div>
                    <div className="rounded-lg bg-info-bg px-2 py-1 ring-1 ring-sky-200">
                        <div className="font-black uppercase tracking-widest text-info-fg">stock form</div>
                                ...
                                return (
                                    <button
                                        ...
                                        className={cn(
                                            ...
                                            selected
                                                ? "ring-2 ring-indigo-400 border-indigo-300 bg-surface-1 shadow-sm"
                                    ...
                                    >
                                        <div className="flex flex-wrap items-center gap-2">
                                            ...
                                            {c.location_code ? (
                                                <span className="rounded-md border border-slate-200 bg-slate-50 px-1.5 py-0.5 font-mono text-[10px] text-content-3">
                                        ...
                                        <div className="mt-2 grid grid-cols-2 gap-2 text-xs text-content-3 sm:grid-cols-4">
                                            <div><span className="text-content-4">Width</span><br /><b>{c.width_mm.toFixed(0)} mm</b><div className="text-[10px] text-content-4">{widthBasisLabel(c.width_basis)}</div></div>
                                            <div><span className="text-content-4">Thickness</span><br /><b>{c.thickness_micron.toFixed(0)} µ</b></div>
                                            <div><span className="text-content-4">Weight</span><br /><b>{c.weight_kg.toFixed(2)} kg</b></div>
                                            <div><span className="text-content-4">Form</span><br /><b>{stockFormLabel(c.stock_form)}</b></div>
                                        </div>
                                        {c.tier === "WIDER_OK_WITH_SLIT" && c.slit_preview && (
                                            <>
                                                <div className="mt-2 rounded-lg border border-warning-border bg-warning-bg p-2 text-[11px] text-amber-900">
                                        ...
                                        <div className="mt-1 text-[10px] text-content-4">{pres?.subtitle}</div>
                                    </button>
                                )
                            })}
                        </div>
        ...
        <button
            ...
            className={cn(
                ...
                active
                    ...
                    : "border-slate-200 bg-surface-1 text-content-3 hover:bg-slate-50",
            )}
        >
    ...
    return (
        <button
            ...
            className={cn(
                ...
                active ? "border-indigo-300 bg-indigo-50 text-indigo-900 ring-1 ring-indigo-200" : "border-slate-200 bg-surface-1 text-slate-700 hover:border-line-strong",
...
function ReservationsPreview({
    ...
    const childTargetWidth = isSlit
        ...
        : null
    return (
        <div className="rounded-lg border border-indigo-200 bg-surface-1 p-2.5 text-[11px]">
...
function CoverageCard({
    ...
    const ringTone = isMet
        ? "ring-emerald-200 bg-success-bg"
        : isHalf
            ? "ring-amber-200 bg-warning-bg"
    ...
    return (
        <div className={cn("mb-2 rounded-xl px-3 py-2 ring-1", ringTone)}>
            ...
            <div className="mt-1.5 h-2 w-full overflow-hidden rounded-full bg-surface-1 ring-1 ring-slate-200">
                <div
            ...
            {overByKg > 0 && isMet ? (
                <div className="mt-1.5 inline-flex items-center gap-1 rounded-full border border-warning-border bg-amber-100 px-2 py-0.5 text-[10px] font-bold text-amber-900">
...
function ZeroCandidatesState({ totalRaw }: { totalRaw: number }) {
    return (
        <div className="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-warning-border bg-amber-50/60 p-8 text-center">
            ...
            <div className="flex flex-wrap items-center justify-center gap-2">
                ...
                <Link
                    ...
                    className="inline-flex items-center gap-1.5 rounded-lg border border-amber-300 bg-surface-1 px-3 py-1.5 text-[11px] font-bold text-amber-800 hover:bg-amber-100"
                >
