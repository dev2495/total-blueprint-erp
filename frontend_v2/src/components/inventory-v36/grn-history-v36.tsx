...
type SourceFilter = "ALL" | "ROLL" | "BULK" | "PACKAGING"
...
const SOURCE_ICON: Record<string, string> = { ROLL: "🌀", BULK: "🧪", PACKAGING: "📦" }
const SOURCE_TONE: Record<string, string> = {
    ROLL: "bg-violet-50 text-violet-800 ring-violet-200",
    BULK: "bg-emerald-50 text-emerald-800 ring-emerald-200",
    PACKAGING: "bg-amber-50 text-amber-800 ring-amber-200",
}
...
export function GrnHistoryV36() {
    ...
    const allRows = React.useMemo(() => Array.isArray(grnQuery.data) ? grnQuery.data : [], [grnQuery.data])
    ...
    const kpi = React.useMemo(() => {
        ...
        return { total, rollCount, bulkCount, packagingCount, totalQty }
    }, [filtered])
            ...
            <GradientHero
                ...
                actions={
                    <div className="flex items-center gap-2">
                        <Link href="/inventory/grn-v36" className="inline-flex items-center gap-1.5 rounded-xl bg-white px-4 py-1.5 text-xs font-bold text-indigo-700 shadow-md hover:bg-indigo-50">
