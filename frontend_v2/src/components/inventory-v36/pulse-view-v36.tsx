...
import { Activity, BarChart3, Boxes, Clock, MapPin, Sparkles, TrendingUp } from "lucide-react"
...
interface PulseViewProps {
    kpis: PulseKpi[]
    primaryBreakdown: { title: string; entries: PulseBreakdownEntry[]; unit?: string }
    secondaryBreakdown?: { title: string; entries: PulseBreakdownEntry[]; unit?: string }
    ageing?: PulseAgeing
    locationBreakdown?: PulseBreakdownEntry[]
    statRow?: PulseStat[]
    matrix?: PulseMatrix
    topList?: { title: string; subtitle?: string; rows: Array<{ label: string; sub?: string; value: string; tone?: "default" | "good" | "warn" | "bad" }> }
...
export function PulseViewV36({ kpis, primaryBreakdown, secondaryBreakdown, ageing, locationBreakdown, statRow, matrix, topList, extra }: PulseViewProps) {
    return (
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
                <PulseCard title={primaryBreakdown.title} subtitle="Top 10 by share" icon={<BarChart3 className="h-4 w-4 text-indigo-500" />} action={<TotalChip total={primaryBreakdown.entries.reduce((s, e) => s + e.value, 0)} unit={primaryBreakdown.unit || ""} />}>
                    <HorizontalBars entries={primaryBreakdown.entries.slice(0, 10)} unit={primaryBreakdown.unit || ""} />
                </PulseCard>

                {secondaryBreakdown && (
                    <PulseCard title={secondaryBreakdown.title} subtitle="Distribution" icon={<Activity className="h-4 w-4 text-teal-500" />} action={<TotalChip total={secondaryBreakdown.entries.reduce((s, e) => s + e.value, 0)} unit={secondaryBreakdown.unit || ""} />}>
                        <Donut entries={secondaryBreakdown.entries} unit={secondaryBreakdown.unit || ""} />
                    </PulseCard>
                )}

                {ageing && (
                    <PulseCard title="Ageing" subtitle="Days since last movement" icon={<Clock className="h-4 w-4 text-amber-500" />}>
            ...
            {locationBreakdown && locationBreakdown.length > 0 && (
                <PulseCard title="Location split" subtitle="On-hand by warehouse / yard" icon={<MapPin className="h-4 w-4 text-rose-500" />} action={<TotalChip total={locationBreakdown.reduce((s, e) => s + e.value, 0)} unit="" />}>
                    <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8">
                        {locationBreakdown.slice(0, 16).map((b, i) => (
                            <LocationTile key={i} entry={b} idx={i} max={Math.max(...locationBreakdown.map((x) => x.value), 1)} />
                        ))}
                    </div>
                </PulseCard>
            )}
            ...
            {(matrix || topList) && (
                <div className={cn("grid grid-cols-1 gap-4", matrix && topList ? "xl:grid-cols-2" : "")}>
...
function PulseMatrixCard({ matrix }: { matrix: PulseMatrix }) {
    ...
    let maxCell = 0
    for (const r of matrix.rows) for (const c of matrix.cols) {
    ...
    const intensity = (v: number): string => {
        ...
        if (ratio < 0.15) return "bg-indigo-50 text-indigo-800 ring-1 ring-indigo-100"
        if (ratio < 0.4) return "bg-indigo-100 text-indigo-900 ring-1 ring-indigo-200"
        if (ratio < 0.7) return "bg-indigo-200 text-indigo-900 ring-1 ring-indigo-300"
    ...
    return (
        <PulseCard
            title={matrix.title}
            subtitle={matrix.subtitle || `${matrix.rows.length} ${matrix.rowLabel} × ${matrix.cols.length} ${matrix.colLabel}`}
            icon={<Boxes className="h-4 w-4 text-indigo-500" />}
            action={<TotalChip total={Object.values(matrix.cells).reduce((s, row) => s + Object.values(row).reduce((a, b) => a + b, 0), 0)} unit={matrix.unit || ""} />}
        >
                                ...
                                {showCols.map((col) => {
                                    ...
                                    return (
                                        <td key={col} className="px-0.5 py-0.5 text-center">
                                            <span className={cn("inline-flex h-6 w-12 items-center justify-center rounded-md font-mono font-bold tabular-nums text-[10px]", intensity(v))} title={`${row} × ${col} · ${fmt(v, 0)} ${matrix.unit || ""}`}>
                                                {v > 0 ? fmt(v, 0) : "—"}
                                            </span>
                                        </td>
                                    )
...
function PulseTopList({ title, subtitle, rows }: { title: string; subtitle?: string; rows: Array<{ label: string; sub?: string; value: string; tone?: "default" | "good" | "warn" | "bad" }> }) {
    if (rows.length === 0) {
        return (
                ...
                {rows.map((r, i) => {
                    ...
                    return (
                        <div key={i} className="flex items-center justify-between gap-2 py-2">
                            ...
                            <div className={cn("font-mono text-sm font-black tabular-nums flex-none", TONE)}>{r.value}</div>
                        </div>
                    )
                })}
            </div>
    ...
    return (
        <div className="relative rounded-2xl border border-slate-200 bg-white p-3.5 shadow-sm">
            ...
            <div className="flex items-start justify-between gap-2 pl-1">
                ...
                {icon && <span className={cn("flex h-7 w-7 items-center justify-center rounded-lg flex-none", TONE.icon)}>{icon}</span>}
            </div>
            ...
            {sub && <div className="mt-0.5 text-[10px] text-slate-500 truncate pl-1">{sub}</div>}
...
function MiniSparkline({ data, color }: { data: number[]; color: string }) {
    ...
    return (
        <svg viewBox={`0 0 ${w} ${h}`} className="w-full" preserveAspectRatio="none" height="22">
            <defs>
...
function MicroStat({ label, value, sub, tone = "default" }: PulseStat) {
    ...
    return (
        <div className="border-l-2 border-slate-200 pl-3">
            ...
            <div className={cn("font-display text-base font-black tabular-nums", TONE)}>{value}</div>
                ...
                return (
                    <div key={i}>
                        <div className="flex items-center justify-between text-[11px]">
                            <span className="flex items-center gap-1.5 font-bold text-slate-700 truncate max-w-[55%]" title={e.label}>
                                <span className="inline-block h-2 w-2 rounded-sm flex-none" style={{ backgroundColor: color }} />
                                {e.label}
                            </span>
        ...
        <div className="flex items-center gap-4">
            <svg viewBox="0 0 110 110" width="120" height="120">
                ...
                {entries.map((e, i) => {
                    ...
                    acc += len
                    return (
                        <circle key={i} cx="55" cy="55" r={r} fill="none" stroke={color} strokeWidth="14" strokeDasharray={`${len} ${C - len}`} strokeDashoffset={off} transform="rotate(-90 55 55)" strokeLinecap="butt" />
                    )
                })}
                <text x="55" y="51" textAnchor="middle" className="fill-slate-900" style={{ fontSize: 14, fontWeight: 800 }}>{fmt(total, 0)}</text>
                <text x="55" y="64" textAnchor="middle" className="fill-slate-500" style={{ fontSize: 8, fontWeight: 700, letterSpacing: 1.2 }}>TOTAL {unit}</text>
            </svg>
                ...
                {entries.map((e, i) => {
                    ...
                    return (
                        <div key={i} className="flex items-center gap-2">
                            <span className="inline-block h-2.5 w-2.5 rounded-sm flex-none" style={{ backgroundColor: color }} />
                            <span className="flex-1 font-semibold text-slate-700 truncate" title={e.label}>{e.label}</span>
                            <span className="font-mono text-slate-700 tabular-nums">{fmt(e.value, 0)}</span>
                            <span className="font-mono font-bold text-slate-500 w-9 text-right tabular-nums">{pct.toFixed(0)}%</span>
                        </div>
                    )
                })}
            </div>
    ...
    return (
        <div className="space-y-3">
            <div className="flex h-2.5 w-full overflow-hidden rounded-full ring-1 ring-slate-200">
                <div className="bg-emerald-400" style={{ width: `${f}%` }} title={`Fresh ${f.toFixed(0)}%`} />
                <div className="bg-amber-400" style={{ width: `${a}%` }} title={`Aged ${a.toFixed(0)}%`} />
                <div className="bg-rose-400" style={{ width: `${o}%` }} title={`Old ${o.toFixed(0)}%`} />
            </div>
...
function AgeBucket({ color, label, n, pct }: { color: "emerald" | "amber" | "rose"; label: string; n: number; pct: number }) {
    ...
    return (
        <div className={cn("rounded-lg px-2 py-1.5 ring-1", TONE)}>
            ...
            <div className="text-[9px] opacity-60 tabular-nums">{pct.toFixed(0)}%</div>
        </div>
    )
}
...
function LocationTile({ entry, idx, max }: { entry: PulseBreakdownEntry; idx: number; max: number }) {
    ...
    return (
        <div className="rounded-xl border border-slate-200 bg-slate-50/40 p-2.5">
            ...
            <div className="mt-1 font-mono text-sm font-bold text-slate-900 tabular-nums">{fmt(entry.value, 0)}</div>
...
export const INVENTORY_CLASS_TABS: ClassTab[] = [
    ...
    { id: "rolls", label: "Roll Explorer", icon: "🌀", href: "/inventory/rolls-v36" },
    { id: "bulk", label: "Bulk Inventory", icon: "🧪", href: "/inventory/bulk-v36" },
    { id: "packaging", label: "Packaging", icon: "📦", href: "/inventory/packaging-v36" },
    { id: "addons", label: "Inks · Adhesives", icon: "🎨", href: "/inventory/addons-v36" },
    { id: "grn", label: "GRN History", icon: "📥", href: "/inventory/grn-history-v36" },
    { id: "period", label: "Stock Lifecycle", icon: "📅", href: "/inventory/period" },
    { id: "trace", label: "Roll Genealogy", icon: "🌳", href: "/inventory/traceability-v36" },
    { id: "transfers", label: "Inter-Plant", icon: "🚚", href: "/inventory/inter-plant-v36" },
]

// ─── Pulse / Browse mode toggle ──────────────────────────────────

export function ModeToggle({ mode, onChange }: { mode: "pulse" | "browse"; onChange: (m: "pulse" | "browse") => void }) {
    return (
...
export function SubtleHero({ eyebrow, title, subtitle, chips, actions }: {
    eyebrow: string
    title: string
    subtitle?: string
    chips?: { icon?: React.ReactNode; label: string; value: string; tone?: "default" | "good" | "warn" }[]
    actions?: React.ReactNode
}) {
    return (
        <section className="relative overflow-hidden rounded-2xl border border-slate-200 bg-gradient-to-br from-white via-slate-50/40 to-indigo-50/30 px-5 py-4 shadow-sm">
