...
function countNodes(node?: GenealogyNode): number {
    if (!node) return 0
    return 1 + (Array.isArray(node.children) ? node.children.reduce((n, c) => n + countNodes(c as GenealogyNode), 0) : 0)
}

function statusTone(status?: string | null) {
    const k = String(status || "").toUpperCase()
    if (k === "AVAILABLE") return "bg-emerald-100 text-emerald-800 ring-emerald-200"
    if (k === "RESERVED" || k === "IN_PROCESS") return "bg-amber-100 text-amber-800 ring-amber-200"
...
export function TraceabilityV36() {
    ...
    const rollOptions = React.useMemo(() => {
        ...
        return rows.slice().sort((a: Roll, b: Roll) => {
    ...
    const traceMutation = useMutation({
        ...
        onSuccess: (data) => { setResult(data); setErrorText("") },
        onError: (err: any) => { setResult(null); setErrorText(err?.response?.data?.error || "Unable to trace this roll.") }
    })
    ...
    const trees = g?.trees || []
    const totalNodes = trees.reduce((n, t) => n + countNodes(t as GenealogyNode), 0)
    ...
    const flow = React.useMemo(() => {
        ...
        const yieldPct = original > 0 ? Math.round((current / original) * 100) : 100
    ...
    return (
        <div className="space-y-4 pb-12">
            ...
            <GradientHero
                ...
                palette="blue"
                chips={[
                    { icon: <GitBranch className="h-3.5 w-3.5" />, label: "Trees", value: `${trees.length}`, tone: "ok" },
                    { icon: <CornerDownRight className="h-3.5 w-3.5" />, label: "Lineage", value: `${childrenCount}`, tone: "violet" },
                    { icon: <History className="h-3.5 w-3.5" />, label: "Events", value: `${timeline.length}`, tone: "ok" },
                ]}
                actions={
                ...
                <div className="flex flex-wrap items-center gap-3">
                    <div className="relative flex-1 min-w-[280px]">
                        ...
                        <Input
                            ...
                            onKeyDown={(e) => { if (e.key === "Enter" && query.trim()) traceMutation.mutate(query.trim()) }}
                    ...
                    >
                        ...
                        {rollOptions.map((roll: any) => (
                            <option key={roll.id} value={roll.id}>{roll.label_id} · {Number(roll.weight_kg || 0).toFixed(2)}kg</option>
                        ))}
                    </select>
                    <Button
            ...
            {r && g && (
                <>
                    ...
                    <div className="grid grid-cols-2 gap-3 md:grid-cols-5">
                        <Kpi tone="blue" icon="⚖️" label="Current weight" value={fmtKg(r.weight_kg)} />
                        <Kpi tone="violet" icon="📦" label="Original weight" value={fmtKg(r.original_weight_kg)} />
                        <Kpi tone="emerald" icon="🌳" label="Lineage children" value={`${childrenCount}`} />
                        <Kpi tone="amber" icon="🕒" label="Timeline events" value={`${timeline.length}`} />
                    ...
                    <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
                        <header className="flex flex-wrap items-start justify-between gap-3 border-b border-slate-100 bg-gradient-to-r from-blue-50/80 via-white to-white px-5 py-4">
                            <div>
                                ...
                                <h2 className="font-display text-lg font-bold text-slate-900">{r.label_id}</h2>
                                <div className="mt-0.5 text-xs text-slate-500">{r.material_name || "—"} · {r.grade_name || "no grade"}</div>
                            </div>
                            <div className="flex flex-wrap items-center gap-2">
                                <span className={cn("rounded-md px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ring-1", roleTone(r.roll_role))}>{r.roll_role || "ROLL"}</span>
                                <span className={cn("rounded-md px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ring-1", statusTone(r.status))}>{r.status || "—"}</span>
                                {r.job_number && <span className="rounded-md bg-slate-100 px-2 py-0.5 font-mono text-[10px] font-bold text-slate-700 ring-1 ring-slate-200">JOB · {r.job_number}</span>}
                            </div>
                        </header>
                        <div className="grid grid-cols-2 gap-2 p-4 md:grid-cols-4">
                            <Field label="Stage" value={r.stage_name || "—"} />
                            <Field label="Plant" value={r.plant_name || "—"} icon={<Factory className="h-3.5 w-3.5 text-blue-500" />} />
                            <Field label="Location" value={r.location_name || "—"} icon={<MapPin className="h-3.5 w-3.5 text-blue-500" />} />
                            <Field label="Width × thickness" value={`${Number(r.width_mm || 0)}mm × ${Number(r.thickness_micron || 0)}μ`} />
                        ...
                        <div className="border-t border-slate-100 px-5 py-4">
                            <div className="flex items-center gap-2 mb-2">
                                ...
                                <span className="text-[10px] font-black uppercase tracking-[0.22em] text-slate-500">Weight flow · original → consumed → current</span>
                            </div>
                            <WeightFlow current={flow.current} consumed={flow.consumed} original={Math.max(flow.original, 1)} />
                            <div className="mt-2 grid grid-cols-3 gap-2 text-[11px]">
                                <div className="rounded-lg bg-blue-50 px-2.5 py-1.5 ring-1 ring-blue-100">
                                ...
                                <div className="rounded-lg bg-violet-50 px-2.5 py-1.5 ring-1 ring-violet-100">
                                    <div className="text-[9px] font-black uppercase text-violet-700">Original</div>
                                    <div className="font-mono font-bold text-violet-900">{fmtKg(flow.original)}</div>
                                </div>
                            </div>
                        </div>
                    </section>
                    ...
                    <div className="grid grid-cols-1 gap-5 xl:grid-cols-2">
                        <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
                            ...
                            <div className="max-h-[640px] overflow-auto p-4 space-y-3">
                                {trees.length === 0 ? (
                                    <div className="text-xs text-slate-500 italic">No descendants yet.</div>
                                ) : (
                                    trees.map((t) => <Node key={t.id} node={t as GenealogyNode} />)
                                )}
                            </div>
                        </section>
                    ...
                    <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
                        ...
                        <div className="grid grid-cols-1 gap-2 p-4 md:grid-cols-2">
                            {(result?.recent_movements || []).length === 0 ? (
                                <div className="col-span-full text-xs text-slate-500 italic">No physical movements yet.</div>
                            ) : (
...
function Kpi({ tone, icon, label, value }: { tone: "blue" | "violet" | "amber" | "emerald" | "rose"; icon: string; label: string; value: string }) {
    const TONE = {
    ...
    return (
        <div className={cn("overflow-hidden rounded-2xl bg-gradient-to-br p-4 text-white shadow-lg ring-1 ring-white/10", TONE)}>
...
function WeightFlow({ current, consumed, original }: { current: number; consumed: number; original: number }) {
    const c = Math.round((current / original) * 100)
    const k = Math.round((consumed / original) * 100)
    return (
        <div className="flex h-8 w-full overflow-hidden rounded-lg ring-1 ring-slate-200 bg-slate-100">
            <div className="flex items-center justify-center bg-blue-500 text-[10px] font-bold text-white" style={{ width: `${c}%` }}>{c}%</div>
            <div className="flex items-center justify-center bg-rose-400 text-[10px] font-bold text-white" style={{ width: `${k}%` }}>{k > 5 ? `${k}% consumed` : ""}</div>
        </div>
    )
}

function Node({ node, isChild = false, isLast = false }: { node: GenealogyNode; isChild?: boolean; isLast?: boolean }) {
        ...
        <div className="relative">
                ...
                <div className="flex items-start justify-between gap-2">
                    <div className="flex items-start gap-2 min-w-0">
                        <span className={cn("flex h-7 w-7 items-center justify-center rounded-full flex-none", !isChild ? "bg-blue-100 text-blue-700" : "bg-slate-100 text-slate-500")}>
                            {!isChild ? <GitCommit className="h-3.5 w-3.5" /> : <CornerDownRight className="h-3.5 w-3.5" />}
                        </span>
                        <div className="min-w-0">
                            <div className="font-mono text-xs font-bold text-slate-900 truncate flex items-center gap-2">
                                {node.label_id}
                                {node.job_number && <span className="rounded-sm bg-blue-50 px-1 text-[9px] font-bold text-blue-700 ring-1 ring-blue-200">JOB · {node.job_number}</span>}
                            </div>
                            <div className="text-[10px] text-slate-500">{node.material_name || "—"} · {node.stage_name || `stage ${node.stage_index ?? "—"}`}</div>
                        </div>
                    </div>
                    <div className="flex flex-col items-end gap-1 flex-none">
                        <span className={cn("rounded-md px-1.5 py-0.5 text-[9px] font-bold uppercase ring-1", roleTone((node as any).roll_role))}>{(node as any).roll_role || "ROLL"}</span>
                        <span className={cn("rounded-md px-1.5 py-0.5 text-[9px] font-bold uppercase ring-1", statusTone(node.status))}>{node.status || "—"}</span>
                    </div>
                </div>
            ...
            {hasKids && (
                <div className="relative mt-3 pl-8">
                    {node.children.map((c, i) => <Node key={c.id} node={c as GenealogyNode} isChild isLast={i === node.children.length - 1} />)}
                </div>
            )}
        </div>
    )
...
function EmptyState() {
    return (
        <div className="rounded-2xl border-2 border-dashed border-slate-200 bg-white p-12 text-center">
            ...
            <div className="mt-4 font-display text-lg font-bold text-slate-900">Trace any roll back to its origin</div>
            <div className="mt-1 text-sm text-slate-500">Search a roll label or pick from recent rolls. We will surface the full tree, ancestor chain, weight flow, and movement timeline.</div>
