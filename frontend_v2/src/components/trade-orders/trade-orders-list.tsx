...
import { Plus, Search, Repeat, ClipboardList, Truck, CheckCircle2, XCircle } from "lucide-react"
    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-rose-50/40 via-white to-slate-50/40 px-4 py-4 sm:px-6">
            <GradientHero
                ...
                chips={[
                    { icon: <ClipboardList className="h-4 w-4" />, label: "Draft", value: String(draftCount), tone: "warn" },
                    { icon: <CheckCircle2 className="h-4 w-4" />, label: "Confirmed", value: String(confirmedCount), tone: "info" },
                    { icon: <Truck className="h-4 w-4" />, label: "Dispatched", value: String(dispatchedCount), tone: "ok" },
                ]}
                actions={
            ...
            <section className="mt-4">
                {isLoading ? (
                    ...
                    <div className="flex flex-col items-center gap-3 rounded-3xl border border-slate-200 bg-surface-1 p-10 text-center">
                        ...
                        {orders.length === 0 ? (
                            <Link href="/sales/trade-orders/new">
                                <Button className="bg-rose-600 text-white hover:bg-rose-700"><Plus className="mr-1.5 h-4 w-4" /> Create first trade order</Button>
                            </Link>
                        ) : null}
                    </div>
                ) : (
                ...
                <header className="flex items-start justify-between gap-2">
                    <div>
                        ...
                        <div className="mt-0.5 text-[11px] text-slate-500">
                            {o.order_date} · {o.plant_name || "no plant"} · {o.items?.length || 0} {(o.items?.length || 0) === 1 ? "line" : "lines"}
                        </div>
                    </div>
                    <Badge variant="outline" className={cn("text-[10px]", STATUS_TONE[o.status])}>
                        {o.status.toLowerCase()}
                    </Badge>
                </header>
                <div className="mt-4 flex items-baseline justify-between rounded-2xl bg-slate-50 px-3 py-2">
                    <div className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Grand total</div>
                    <div className="font-mono text-base font-bold text-slate-900">
                        ₹ {Number(o.grand_total || 0).toLocaleString(undefined, { maximumFractionDigits: 2 })}
                    </div>
                </div>
            </article>
        </Link>
