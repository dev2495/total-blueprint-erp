...
export function TradeOrderDetail({ id }: { id: string }) {
  ...
  const confirmMut = useMutation({
    ...
    onSuccess: (o) => {
      invalidate(o);
      toast({ title: "Order confirmed", description: o.code });
    },
    onError: (err: any) =>
      toast({
  ...
  const dispatchMut = useMutation({
    ...
    onSuccess: (o) => {
      ...
      toast({
        title: "Dispatched",
        description: `${o.code} · stock decremented`,
      });
    },
    onError: (err: any) =>
      toast({
  ...
  const cancelMut = useMutation({
    ...
    onSuccess: (o) => {
      invalidate(o);
      toast({ title: "Cancelled", description: o.code });
    },
    onError: (err: any) =>
      toast({
        title: "Cancel failed",
    ...
    <div className="min-h-screen bg-gradient-to-b from-danger-bg via-white to-surface-2 px-4 py-4 sm:px-6">
      <GradientHero
        ...
        chips={[
          ...
          {
            label: "Grand ₹",
            value: Number(order.grand_total || 0).toLocaleString(undefined, {
              ...
              <tbody className="divide-y divide-line">
                {order.items.map((it) => (
                  <tr key={it.id || it.line_no}>
                    <td className="px-3 py-2 font-mono text-content-3">
                      {it.line_no}
                    </td>
                    <td className="px-3 py-2">
                      <span className="inline-flex items-center gap-1.5">
                        {it.item_type === "TRADING_GOOD" ? (
                          <ShoppingBag className="h-3 w-3 text-success-fg" />
                        ) : (
                          <Package className="h-3 w-3 text-order-fg" />
                        )}
                    ...
                    <td className="px-3 py-2 text-right font-mono font-bold">
                      ₹{" "}
                      {Number(it.line_total || 0).toLocaleString(undefined, {
                        maximumFractionDigits: 2,
                      })}
                    </td>
                  </tr>
        ...
        <section className="space-y-4">
          <div className="rounded-3xl bg-gradient-to-br from-danger-solid via-warm to-warning-fg p-6 text-white shadow-[0_20px_60px_-30px_rgba(244,63,94,0.45)]">
            ...
            <dl className="mt-3 space-y-2">
              <Row
                label="Subtotal"
                value={`₹ ${Number(order.subtotal || 0).toLocaleString(undefined, { maximumFractionDigits: 2 })}`}
              />
              <Row
                label="GST"
                value={`₹ ${Number(order.gst_total || 0).toLocaleString(undefined, { maximumFractionDigits: 2 })}`}
              />
              <div className="my-2 h-px bg-surface-1/30" />
              <Row
                label="Grand total"
                value={`₹ ${Number(order.grand_total || 0).toLocaleString(undefined, { maximumFractionDigits: 2 })}`}
                big
              />
            </dl>
          </div>

          <div className="rounded-3xl bg-surface-1 p-6 shadow-[0_20px_60px_-30px_rgba(15,23,42,0.25)] ring-1 ring-line">
            <h3 className="text-sm font-bold text-content-1">Actions</h3>
            <p className="text-[11px] text-content-3">Workflow controls</p>
            <div className="mt-4 flex flex-col gap-2">
              {canConfirm ? (
                <Button
                  onClick={() => confirmMut.mutate()}
                  disabled={confirmMut.isPending}
                  className="bg-warning-fg text-white hover:bg-warning-fg"
                >
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        This will consume stock at the order&apos;s plant and
                        cannot be undone. Make sure the lines and quantities are
                        correct.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
          ...
          {order.notes ? (
            <div className="rounded-3xl bg-surface-1 p-6 shadow-[0_20px_60px_-30px_rgba(15,23,42,0.25)] ring-1 ring-line">
              ...
              <p className="mt-2 whitespace-pre-wrap text-[12px] text-content-3">
                {order.notes}
              </p>
            </div>
          ) : null}
