...
export function HelpScreenshotImage({
  imageKey,
  compact = false,
}: {
  imageKey: string;
  ...
  return (
    <img
      ...
      className={
        compact
          ? "h-32 w-full rounded-lg border border-line object-cover"
...
export function HelpFlowDiagram({
  flow,
  locale,
}: {
  flow?: DecisionFlow;
  ...
  if (!nodes.length) {
    return (
      <div className="rounded-xl border border-dashed border-line-strong bg-surface-2 p-4 text-sm font-semibold text-content-3">
        {locale === "hi"
          ? "Visual flow उपलब्ध नहीं है।"
          : "Visual flow is not available for this page."}
      </div>
    );
  }

  const nodeTitles = new Map(
    nodes.map((node) => [node.id, localize(node.title, locale)]),
  );
  ...
  return (
    <div className="rounded-2xl border border-line bg-gradient-to-br from-white to-surface-2 p-4">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="rounded-full border border-line bg-surface-1 px-3 py-1 text-[11px] font-bold text-content-3">
          ...
          >
            ...
            <div className="space-y-2">
              {node.outcomes.map((outcome, outcomeIndex) => {
                const target = outcome.nextId
                  ? nodeTitles.get(outcome.nextId)
                  : null;
                return (
                  <div
                    key={`${node.id}-${outcomeIndex}`}
                  ...
                  >
                    ...
                    {target ? (
                      ...
                      <div className="mt-1 text-[11px] leading-4 text-content-3">
                        {localize(outcome.resolution, locale)}
                      </div>
                    ) : null}
                  </div>
                );
