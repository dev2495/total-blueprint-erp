  ...
  const buildPayload = () => ({
    ...
    lines: lines
      ...
      .map((l, idx) => ({
        ...
        inventory_material:
          l.stock_class === "BULK" ||
          l.stock_class === "PACKAGING" ||
          l.stock_class === "ROLL"
            ? l.inventory_material || null
            : null,
        trading_good:
          l.stock_class === "TRADING_GOOD" ? l.trading_good || null : null,
        location:
          l.stock_class === "BULK" || l.stock_class === "PACKAGING"
            ? l.location || null
            : null,
        inventory_roll:
          l.stock_class === "ROLL" ? l.inventory_roll || null : null,
        delta_qty: Number(l.delta_qty || 0),
        uom: l.uom || "KG",
        notes: l.notes || "",
      })) as Array<
      Omit<StockAdjustmentLine, "id" | "before_qty" | "after_qty" | "value_inr">
    >,
  });
        ...
        <section className="rounded-3xl bg-surface-1 p-6 shadow-[0_20px_60px_-30px_rgba(15,23,42,0.25)] ring-1 ring-line lg:col-span-3">
          <header className="mb-4">
            ...
            <p className="text-[11px] text-content-3">
              Plant, reason, and notes apply to all lines.
            </p>
          </header>
        ...
        <section className="rounded-3xl bg-surface-1 p-6 shadow-[0_20px_60px_-30px_rgba(15,23,42,0.25)] ring-1 ring-line lg:col-span-3">
          <header className="mb-4 flex flex-wrap items-center justify-between gap-2">
            <div>
              ...
              <p className="text-[11px] text-content-3">
                Pick a stock class per line, choose the item and enter the
                delta.
              </p>
            </div>
...
function LineRow({
  ...
  const needsLocation =
    line.stock_class === "BULK" || line.stock_class === "PACKAGING";
    ...
    <div className="rounded-2xl border border-line bg-surface-2 p-4">
      ...
      <div className="mt-2 grid gap-3 md:grid-cols-4">
        ...
        {needsLocation ? (
          <div className="md:col-span-3 md:col-start-2">
            ...
            >
              <SelectTrigger className="mt-1">
                <SelectValue
                  placeholder={plantId ? "Pick location" : "Pick a plant first"}
                />
              </SelectTrigger>
              <SelectContent>
                {locations.map((l) => (
        ...
        <div className="md:col-span-2">
          ...
          <div className="mt-1 flex h-9 items-center rounded-md border border-line bg-surface-1 px-3 font-mono text-[11px] text-content-2">
            {before === null ? (
              <span className="text-content-4">
                Pick a roll or material to preview qty
              </span>
            ) : (
              <>
                <span className="text-content-3">Before</span>
                <b className="mx-1">{before.toFixed(3)}</b>
                <span className="mx-1">→</span>
                <span className="text-content-3">After</span>
