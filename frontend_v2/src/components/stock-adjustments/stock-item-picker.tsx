...
export function StockItemPicker({
  ...
  const items = React.useMemo<StockItemPickerValue[]>(() => {
    ...
    const list = ((library.data as any[]) || []).filter(
      CATEGORY_FILTERS[stockClass],
    );
    return list.map((m) => ({
      id: String(m.id),
      code: String(m.code || ""),
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-[420px] p-0" align="start">
        <Command shouldFilter={false}>
          ...
          <CommandList>
            {loading ? (
              <div className="flex items-center justify-center gap-2 p-6 text-xs text-content-3">
