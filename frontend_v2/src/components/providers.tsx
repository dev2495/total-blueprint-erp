...
function shouldSkipGlobalError(meta: unknown) {
  ...
  return (
    flags.silentError === true ||
    flags.suppressGlobalError === true ||
    flags.disableGlobalErrorToast === true
  );
}
...
function reportClientDataError({
  ...
  const looksLikeBareNetworkFailure =
    /^(request failed\.?|failed to fetch\.?|load failed\.?)$/i.test(
      message.trim(),
    );
  if (
    title === "Client runtime error" &&
    !status &&
    looksLikeBareNetworkFailure &&
    operationPointsAtCurrentPage(operation)
  ) {
  ...
  if (typeof console !== "undefined") {
    console.warn("[client-data-error]", {
      title,
      status,
      message,
      operation,
...
function isBrowserResourceFailure(event: ErrorEvent) {
  ...
  const looksLikeBareNetworkFailure =
    /^(request failed\.?|failed to fetch\.?|load failed\.?)$/i.test(message);
  let filenamePath = "";
  try {
    filenamePath = filename
      ? new URL(filename, currentRoute || undefined).pathname
      : "";
  } catch {
    filenamePath = "";
  }
