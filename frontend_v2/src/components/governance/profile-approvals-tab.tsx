...
function getErrorDetail(error: unknown) {
    ...
    return detail || maybe?.response?.data?.message || maybe?.message || "Could not review request.";
}
...
export function ProfileApprovalsTab() {
    ...
    const reviewMutation = useMutation({
        ...
        onSuccess: async () => {
            await queryClient.invalidateQueries({ queryKey: ["governance-profile-change-requests"] });
        ...
        <Card>
            ...
            <CardContent className="space-y-4">
                <div className="flex flex-wrap items-center gap-2">
                    {(["PENDING", "APPROVED", "REJECTED", "ALL"] as FilterState[]).map((value) => (
                        <Button
                            key={value}
                            size="sm"
                                ...
                                <TableCell className="text-right">
                                    {row.status === "PENDING" ? (
                                        <div className="flex justify-end gap-2">
                                            ...
                                            >
                                                {reviewMutation.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : "Approve"}
                                            </Button>
                                            <Button
                                                size="sm"
                                                variant="outline"
                                        ...
                                        <span className="text-xs text-slate-500">{formatDateTime(row.reviewed_at)}</span>
                                    )}
                                </TableCell>
                            </TableRow>
                        ))}
