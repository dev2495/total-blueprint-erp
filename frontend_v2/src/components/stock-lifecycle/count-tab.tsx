...
function snapshotCountRows(
  snapshotRows: Array<Record<string, any>>,
): CountRow[] {
  return snapshotRows
    .map((row, index) => {
      ...
      const qty =
        Number(row.qty ?? row.quantity ?? row.qty_kg ?? row.weight_kg ?? 0) ||
        0;
      ...
      return {
        ...
        code:
          stockClass === "ROLL"
            ? labelId || row.material_code || "ROLL"
        ...
        thicknessMicron:
          row.thickness_micron == null ? null : Number(row.thickness_micron),
...
function foundStockMasterRows(
  catalogRows: StockLifecycleRow[],
  represented: Set<string>,
): CountRow[] {
  return catalogRows
    .filter(
      (row) =>
        row.stock_class !== "ROLL" &&
        String(row.category || "").toUpperCase() !== "FILM_FAMILY",
    )
    .map((row) => {
...
export function CountTab({ plantId, catalog, categoryFilter }: CountTabProps) {
  ...
  const [granuleCodeFilter, setGranuleCodeFilter] =
    React.useState(ALL_GRANULE_CODES);
  ...
  const allRows = React.useMemo(() => {
    const stockRows = Array.isArray((snapshot as any)?.rows)
      ? ((snapshot as any).rows as Array<Record<string, any>>)
      : [];
    const rows = snapshotCountRows(stockRows);
    const represented = new Set(
      rows.map((row) => `${row.stock_class}:${row.materialId}`),
    );
  ...
  const rows = React.useMemo(() => {
    ...
    if (scope !== "ALL")
      filtered = filtered.filter((row) => row.reportingScope === scope);
    if (rollFormFilter !== ALL_ROLL_FORMS) {
      filtered = filtered.filter(
        (row) => row.stock_class === "ROLL" && row.stockForm === rollFormFilter,
      );
    }
    if (granuleCodeFilter !== ALL_GRANULE_CODES) {
      filtered = filtered.filter(
        (row) =>
          row.stock_class === "BULK" && row.granuleCodeId === granuleCodeFilter,
      );
    }
    if (locationFilter !== ALL_LOCATIONS) {
      filtered = filtered.filter(
        (row) => !row.locationId || row.locationId === locationFilter,
      );
    }
    if (categoryFilter)
      filtered = filtered.filter(
        (row) => row.category === String(categoryFilter).toUpperCase(),
      );
    if (search.trim()) {
  ...
  const selectedLocation = React.useCallback(
    (row: CountRow, draft?: CountDraft) =>
      draft?.locationId ||
      row.locationId ||
      (locationFilter !== ALL_LOCATIONS ? locationFilter : ""),
    [locationFilter],
  );
  ...
  const selectedLocationName = React.useMemo(() => {
    ...
    return (
      plantLocations.find((location) => location.id === locationFilter)?.name ||
      "Selected location"
    );
  ...
  const countMode = React.useMemo(() => {
    const scopeLabel =
      SCOPE_OPTIONS.find((item) => item.value === scope)?.label || "All";
    const locationLabel = selectedLocationName || "all locations";
    const rollFormLabel =
      rollFormFilter !== ALL_ROLL_FORMS
        ? STOCK_FORM_LABELS[rollFormFilter] || rollFormFilter.replace(/_/g, " ")
        : "";
    const granuleLabel =
      granuleCodeFilter !== ALL_GRANULE_CODES
        ? granuleCodeOptions.find((item) => item.id === granuleCodeFilter)
            ?.label || "selected granule code"
        : "";
  ...
  const stats = React.useMemo(() => {
    ...
    return {
      ...
      missingReason,
      missingLocation,
      blocked: missingReason + missingLocation,
    };
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      const lines = rows
        .map((row) => {
          ...
          if (
            ...
            (!draft.reason.trim() || !draft.reasonCode)
          ) {
            throw new Error(
              `Row ${row.code} has ${variance.toFixed(1)}% variance; a reason code and note are required.`,
            );
          }
          const locationId = selectedLocation(row, draft);
          if (!locationId) {
            throw new Error(
              `Select a location for ${row.code} before posting the count.`,
            );
          }
          return {
            ...
            counted_at: countedAsOf
              ? new Date(countedAsOf).toISOString()
      ...
      if (lines.length === 0) {
        throw new Error(
          "Enter counted quantity for at least one row. Physical count posts only rows you enter.",
        );
      }
      ...
      const searchLabel = search.trim()
        ? ` · item filter: ${search.trim()}`
        : "";
      const rollFormLabel = countMode.rollFormLabel
        ? ` · ${countMode.rollFormLabel}`
        : "";
      const granuleLabel = countMode.granuleLabel
        ? ` · granule code ${countMode.granuleLabel}`
        : "";
      ...
      const batch: any = await stockLifecycleService.createCountBatch({
        ...
        _v36_workflow: {
          ...
          filters: {
            ...
            location_name:
              locationFilter === ALL_LOCATIONS ? "" : locationLabel,
            item_search: search.trim(),
            category: categoryFilter || "",
            roll_stock_form:
              rollFormFilter === ALL_ROLL_FORMS ? "" : rollFormFilter,
            roll_stock_form_label: countMode.rollFormLabel,
            granule_code_id:
              granuleCodeFilter === ALL_GRANULE_CODES ? "" : granuleCodeFilter,
  ...
  return (
    <div
      data-testid="stock-count-tab"
      className="flex flex-col gap-5 lg:grid lg:grid-cols-[minmax(0,1fr)_280px]"
    >
      <div className="flex flex-col gap-4">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex flex-wrap gap-1 rounded-2xl border border-line bg-surface-1 p-1">
          ...
          <Select value={locationFilter} onValueChange={setLocationFilter}>
            <SelectTrigger
              data-testid="count-location-filter"
              className="h-10 rounded-xl border-line bg-surface-1 text-xs font-bold sm:w-[240px]"
            >
          ...
          <Select value={rollFormFilter} onValueChange={setRollFormFilter}>
            <SelectTrigger
              data-testid="count-roll-form-filter"
          ...
          <Select
            value={granuleCodeFilter}
            onValueChange={setGranuleCodeFilter}
          >
            <SelectTrigger
              data-testid="count-granule-code-filter"
              className="h-10 rounded-xl border-line bg-surface-1 text-xs font-bold sm:w-[230px]"
            >
              <SelectValue placeholder="All granule codes" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value={ALL_GRANULE_CODES}>
                All granule inward codes
              </SelectItem>
        ...
        <div className="grid gap-2 md:grid-cols-4">
          ...
          <div className="rounded-2xl border border-line bg-surface-1 px-4 py-3 md:col-span-4">
            <div className="grid gap-2 md:grid-cols-[220px_minmax(0,1fr)]">
              <Select value={countPolicy} onValueChange={setCountPolicy}>
                ...
                <SelectContent>
                  <SelectItem value="SOFT_FREEZE">
                    Soft freeze · allow reviewed movement
                  </SelectItem>
                  <SelectItem value="HARD_FREEZE">
                    Hard freeze · no movement until post
                  </SelectItem>
                  <SelectItem value="SPOT_COUNT">
                    Spot count · selected rows only
                  </SelectItem>
                </SelectContent>
              </Select>
              <div className="rounded-xl bg-surface-2 px-3 py-2 text-xs font-semibold text-content-3">
                System quantity is loaded live; count cutoff and policy are
                stored with the audit batch so later reconciliation can separate
                count time from posting time.
              </div>
            </div>
          </div>
        </div>

        <div className="overflow-hidden rounded-2xl border border-line bg-surface-1 shadow-[0_8px_30px_-20px_rgba(15,23,42,0.18)]">
          <div className="hidden grid-cols-[132px_minmax(0,1fr)_170px_105px_120px_105px_minmax(0,1fr)] gap-3 bg-surface-2 px-4 py-2 text-[11px] font-semibold uppercase tracking-wide text-content-3 lg:grid">
            <div>Code</div>
            <div>Material</div>
            <div>Location</div>
          ...
          <div className="divide-y divide-line">
            {rows.length === 0 ? (
              ...
              rows.map((row) => {
                const draft = drafts[row.rowKey];
                const counted =
                  draft?.counted === "" || draft?.counted == null
                    ? null
                    : Number(draft.counted);
                const variance =
                  counted == null
                    ? null
                ...
                const isOver =
                  variance != null && absVariance >= VARIANCE_THRESHOLD;
                const isExact = variance != null && absVariance === 0;
                const locationMissing =
                  Boolean(draft?.counted) && !selectedLocation(row, draft);
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      (reasonRequired || locationMissing) && "bg-danger-bg",
                    )}
                  >
                    <div className="min-w-0">
                      <div className="truncate font-mono text-xs text-content-2">
                        {row.code}
                      </div>
                      {row.labelId ? (
                        <div className="mt-1 text-[10px] font-bold text-order-fg">
                          roll label
                        </div>
                      ) : null}
                    </div>
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 flex flex-wrap gap-1 text-[11px] text-content-3">
                        <Badge
                          variant="outline"
                          className="h-5 rounded-full border-line bg-surface-2 px-2 text-[10px]"
                        >
                          {row.category || "STOCK"}
                        </Badge>
                        <Badge
                        ...
                        {row.granuleCodeLabel ? (
                          <Badge
                            variant="outline"
                            className="h-5 rounded-full border-warning-border bg-warning-bg px-2 text-[10px] text-warning-fg"
                          >
                        ...
                        {row.stockForm ? (
                          <Badge
                            variant="outline"
                            className="h-5 rounded-full border-info-border bg-info-bg px-2 text-[10px] text-primary"
                          >
                            {STOCK_FORM_LABELS[row.stockForm] ||
                              row.stockForm.replace(/_/g, " ")}
                          </Badge>
                        ) : null}
                        {row.status && row.stock_class === "ROLL" ? (
                          <Badge
                            variant="outline"
                            className="h-5 rounded-full border-success-border bg-success-bg px-2 text-[10px] text-success-fg"
                          >
                            {row.isFg ? "FG" : "WIP"} ·{" "}
                        ...
                        {row.batchNo ? (
                          <span className="font-mono">{row.batchNo}</span>
                        ) : null}
                      </div>
                    </div>
                    <div>
                      {row.locationLocked ? (
                        <div className="flex min-h-8 items-center gap-1.5 rounded-xl border border-line bg-surface-2 px-2 text-xs font-semibold text-content-2">
                      ...
                      {locationMissing ? (
                        <p className="mt-1 text-[11px] font-semibold text-danger-fg">
                          Location required
                        </p>
                      ) : null}
                    </div>
                    ...
                    <Input
                      ...
                      value={draft?.counted || ""}
                      onChange={(event) =>
                        setDraft(row.rowKey, { counted: event.target.value })
                      }
                      placeholder="0.000"
                      className="h-8"
                    />
                    ...
                    <div>
                      {isOver ? (
                        <Select
                          ...
                          onValueChange={(value) =>
                            setDraft(row.rowKey, { reasonCode: value })
                          }
                        >
                          <SelectTrigger
                            className={cn(
                              "mb-1 h-8 rounded-xl bg-surface-1 text-xs",
                              reasonRequired &&
                                !draft?.reasonCode &&
                                "border-danger-border ring-1 ring-danger-border",
                            )}
                      ...
                      <Textarea
                        ...
                        value={draft?.reason || ""}
                        onChange={(event) =>
                          setDraft(row.rowKey, { reason: event.target.value })
                        }
                        disabled={!isOver}
      ...
      <aside className="space-y-4">
        ...
        <div className="rounded-2xl border border-line bg-gradient-to-br from-white to-surface-2 p-4 shadow-sm">
          ...
          <dl className="mt-3 space-y-2 text-sm">
            <div className="flex items-center justify-between">
              ...
              <dd className="font-display text-base font-semibold text-success-fg">
                {stats.ready}
              </dd>
            </div>
            <div className="flex items-center justify-between">
              <dt className="text-content-3">
                Over {VARIANCE_THRESHOLD}% threshold
              </dt>
              <dd className="font-display text-base font-semibold text-warning-fg">
                {stats.overThreshold}
              </dd>
            </div>
            <div className="flex items-center justify-between">
              ...
              <dd
                className={cn(
                  ...
                  stats.missingReason > 0 ? "text-danger-fg" : "text-content-4",
                )}
              >
                {stats.missingReason}
              </dd>
          ...
          {stats.blocked > 0 ? (
            ...
            <div className="mt-3 text-xs text-content-3">
              Enter counted quantities to build a batch.
            </div>
          )}
          <Button
            data-testid="count-post-batch"
            className="mt-4 w-full bg-gradient-to-r from-order-fg via-order-fg to-order-fg text-white hover:from-order-fg hover:via-order-fg hover:to-order-fg"
            disabled={
              stats.ready === 0 || stats.blocked > 0 || mutation.isPending
            }
        ...
        <div className="rounded-2xl border border-order-border bg-order-bg p-4 text-xs text-order-fg">
          <div className="flex items-center gap-2 font-semibold text-order-fg">
            <PackageCheck className="h-4 w-4" />
            One stock-cycle truth
          </div>
          <p className="mt-1">
            Use plant-wide, location, class, item, roll-form, or granule-code
            counts. The posted audit sheet keeps that label in snapshots and
            history.
          </p>
        </div>
      </aside>
