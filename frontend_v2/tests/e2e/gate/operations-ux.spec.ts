...
test("inventory, logistics, artwork, cylinder, and tooling pages use the upgraded visual shell", async ({ page }, testInfo) => {
  ...
  await expect(page.locator("body")).toContainText("Unified Inventory Workspace")
  await expect(page.locator("body")).toContainText("Variant / Family KG")
  await expect(page.locator("body")).toContainText("Variant Size Heatmap")
  ...
  await expect(page.locator("body")).toContainText("Open / sealed gonnies")
  ...
  await expect(page.locator("body")).toContainText("Dispatch terminal")
